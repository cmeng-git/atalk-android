...
# IF you need to build api-16 min, modify it to 14; and will not build for 64 bit archs.
...
#
# So if you do not need 64 bit arch, then api level 16 is better
#
...
# Observation: need API-23 min on ubuntu; otherwise undefined reference to stdin, stdout, stderr,
# atof, signal, tcsetattr and tcsetattr etc.
# Also Adroid Studio NDK built API must set to the same level API-23
#
# cmeng - set to API:-15 for aTalk mininum support for platform API-15
