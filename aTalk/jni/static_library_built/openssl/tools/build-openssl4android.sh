#!/bin/bash
#
# Copyright 2016 leenjewel / cmeng
# 
...
LIB_GIT="openssl-1.0.2l"
LIB_OPENSSL="openssl"
LIB_DEST_DIR=${TOOLS_ROOT}/libs

#[ -d ${LIB_DEST_DIR} ] && rm -rf ${LIB_DEST_DIR}
[ -f "${LIB_GIT}.tar.gz" ] || wget https://www.openssl.org/source/${LIB_GIT}.tar.gz;

# Unarchive library, then configure and make for specified architectures
configure_make() {
  ARCH=$1; ABI=$2;
# rm -rf "${LIB_OPENSSL}"
# tar xfz "${LIB_GIT}.tar.gz" "${LIB_OPENSSL}"
  pushd "${LIB_OPENSSL}"
  configure $*

  #support openssl-1.0.x
  if [[ $LIB_OPENSSL != openssl-1.2.* ]]; then
