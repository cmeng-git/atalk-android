...
public final class Manager {
	...
	public static Processor createProcessor(DataSource source)
		...
		try {
			logger.info("### Creating Processor for DataSource ###: " + source + " for type: " + contentType);
			...
			logger.info("Failed to create Processor for DataSource#1: " + source + " and type: " + source.getContentType());
			// no need to log, will be logged by call to createProcessor.
		} catch (Exception e) {
			logger.log(Level.FINE, "" + e, e);
		}
		logger.info("### Re-attempt to create Processor for DataSource ###: " + source + " with type: unknown");
	...
	private static Processor createProcessor(DataSource source, String contentType)
		...
		for (String handlerClassName : getProcessorClassList(contentType)) {
			logger.info("### Creating Processor with className: " + handlerClassName + " for type: " + contentType);
			try {
				final Class<?> handlerClass = Class.forName(handlerClassName);
				...
				logger.finer("createProcessor: Class not found: " + e.getMessage()); // no need for call stack
			} catch (IncompatibleSourceException e) {
