...
{
    /**
     * The default number of binds that a <tt>NetworkAddressManagerService</tt> implementation
     ...
     * <tt>NetworkAddressManagerService</tt> implementation should execute in case a port is
     ...
     * and a port in the range specified by the <tt>minPort</tt> and <tt>maxPort</tt> parameters.
     * We first try to bind the newly created socket on the <tt>preferredPort</tt> port number and
     * then proceed incrementally upwards until we succeed or reach the bind retries limit. If we
     * reach the <tt>maxPort</tt> port number before the bind retries limit, we will then start
     * over again at <tt>minPort</tt> and keep going until we run out of retries.
     *
