...
public class DynamicPayloadTypeRegistry
{
	 ...
	 *
	 * @param format
	 *        the <tt>MediaFormat</tt> instance that we'd like to obtain a payload type number for.
	 *
	 ...
	 *         if we have already registered more dynamic formats than allowed for by RTP.
	 */
	 ...
	 *
	 * @param format
	 *        the {@link MediaFormat} whose preferred dynamic PT number we are trying to obtain.
	 *
	 ...
	 *
	 * @param payloadType
	 *        the payload type number that we'd like to allocated to <tt>format</tt>.
	 * @param format
	 *        the <tt>MediaFormat</tt> that we'd like to create a dynamic mapping for.
	 *
	 * @throws IllegalArgumentException
	 *         in case <tt>payloadType</tt> has already been assigned to another format.
	 */
		...
		throws IllegalArgumentException
	{
		...
		if (alreadyMappedFmt != null) {
			...
			logger.warn("Remote party is trying to remap payload type " + payloadType
				+ " and reassign it from " + alreadyMappedFmt + " to " + format
				+ ". We'll go along but there might be issues because"
				+ " of this. We'll also expect to receive " + format + " with PT="
				+ newlyObtainedPT);
	 ...
	 *
	 * @param payloadType
	 *        the number of the payload type that we are trying to get a format for.
	 *
		...
		throws IllegalStateException
	{
		while (true) {
			if (nextDynamicPayloadType < 0) {
				throw new IllegalStateException("Impossible to allocate more than the already 32 "
	 ...
	 *
	 * @param payloadTypePreference
	 *        the dynamic payload type number that we are trying to determine as being claimed as
	 ...
	 *
	 * @param format
	 *        the {@link MediaFormat} whose mapping we are looking for
