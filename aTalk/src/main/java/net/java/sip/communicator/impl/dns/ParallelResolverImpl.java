...
/**
 * The purpose of this class is to help avoid the significant delays that occur in networks where DNS servers
 * would ignore SRV, NAPTR, and sometimes even A/AAAA queries (i.e. without even sending an error response).
 * We also try to handle cases where DNS servers may return empty responses to some records.
 *
 * We achieve this by entering a redundant mode whenever we detect an abnormal delay (longer than <tt>DNS_PATIENCE</tt>)
 * while waiting for a DNS response, or when that response is not considered satisfying.
 *
 * Once we enter redundant mode, we start duplicating all queries and sending them to both our primary and backup
 * resolvers (in case we have any). We then always return the first response we get, regardless of who sent it.
 *
 * We exit redundant mode after receiving <tt>DNS_REDEMPTION</tt> consecutive timely and correct responses from
...
{
    /**
     * The <tt>Logger</tt> used by the <tt>ParallelResolver</tt> class and its instances for logging output.
     */
    private static final Logger logger = Logger.getLogger(ParallelResolverImpl.class);

    /**
     * Indicates whether we are currently in a mode where all DNS queries are sent to both
     * the primary and the backup DNS servers.
     */
    private volatile static boolean redundantMode = false;

    /**
     * The currently configured number of milliseconds that we need to wait before entering redundant mode.
     */
    private static long currentDnsPatience = DNS_PATIENCE;

    /**
     * The currently configured number of times that the primary DNS would have to provide a faster response
     * than the backup resolver before we consider it safe enough to exit redundant mode.
     */
    public static int currentDnsRedemption = DNS_REDEMPTION;

    /**
     * The number of fast responses that we need to get from the primary resolver before we exit
     * redundant mode. <tt>0</tt> indicates that we are no longer in redundant mode
     */
    private static int redemptionStatus = 0;

    /**
     * A lock that we use while determining whether we've completed redemption and can exit redundant mode.
     */
    ...
    {
        String rslvrAddrStr = DnsUtilActivator.getConfigurationService().getString(
                DnsUtilActivator.PNAME_BACKUP_RESOLVER, DnsUtilActivator.DEFAULT_BACKUP_RESOLVER);
        String customResolverIP
                = DnsUtilActivator.getConfigurationService().getString(DnsUtilActivator.PNAME_BACKUP_RESOLVER_FALLBACK_IP,
                DnsUtilActivator.getResources().getSettingsString(DnsUtilActivator.PNAME_BACKUP_RESOLVER_FALLBACK_IP));
        ...
        if (resolverAddress == null) {
            // name resolution failed for backup DNS resolver, try with the IP address of the default backup resolver
            try {
        ...
        int resolverPort = DnsUtilActivator.getConfigurationService().getInt(
                DnsUtilActivator.PNAME_BACKUP_RESOLVER_PORT, SimpleResolver.DEFAULT_PORT);
    ...
    /**
     * Sets the specified array of <tt>backupServers</tt> used if the default DNS doesn't seem to be doing that well.
     *
     * @param backupServers the list of backup DNS servers that we should use if, and only if, the default
     * servers don't seem to work that well.
     */
    private void setBackupServers(InetSocketAddress[] backupServers)
    {
        try {
            // DnsServersDetector dnsDetector = new DnsServersDetector(aTalkApp.getGlobalContext());
            // String[] dnsServers = dnsDetector.getServers();
            // String dnsServer = dnsServers[0];
            // Lookup.setDefaultResolver(new ExtendedResolver(dnsServers));
            ...
            // this shouldn't be thrown since we don't do any DNS querying in here. this is why
            // we take an InetSocketAddress as a param.
            ...
            throws IOException
    {
        ...
        // if we are not in redundant mode we should wait a bit and see how this goes.
        // if we get a reply we could return bravely.
        if (!redundantMode) {
            if (resolution.waitForResponse(currentDnsPatience)) {
                ...
                synchronized (redemptionLock) {
                    ...
                    logger.info("Primary DNS seems laggy: no response for " + query.getQuestion().getName() + "/"
                            + Type.string(query.getQuestion().getType()) + " after "
                            + currentDnsPatience + "ms. Enabling redundant mode.");
                }
            }
        }
    ...
    /**
     * Sets whether TCP connections will be sent by default with the default resolver. Backup servers would
     * always be contacted the same way.
     *
    ...
    /**
     * Sets whether truncated responses will be ignored. If not, a truncated response over UDP will cause
     * a retransmission over TCP. Backup servers would always be contacted the same way.
     *
     ...
     *
     * @param level The EDNS level to use. 0 indicates EDNS0 and -1 indicates no EDNS.
     * @param payloadSize The maximum DNS packet size that this host is capable of receiving over UDP. If
     * 0 is specified, the default (1280) is used.
     * @param flags EDNS extended flags to be set in the OPT record.
     * @param options EDNS options to be set in the OPT record, specified as a List of OPTRecord.Option elements.
     * @throws IllegalArgumentException An invalid field was specified.
     * @see OPTRecord
     */
    @SuppressWarnings("rawtypes")
    // that's the way it is in dnsjava
    ...
    /**
     * Determines if <tt>response</tt> can be considered a satisfactory DNS response and returns accordingly.
     *
     * We consider non-satisfactory responses that may indicate that the local DNS does not work properly and
     * that we may hence need to fall back to the backup resolver.
     *
     * Basically the goal here is to be able to go into redundant mode when we come across DNS servers that
     * send empty responses to SRV and NAPTR requests.
     *
     * @param response the dnsjava {@link Message} that we'd like to inspect.
     * @return <tt>true</tt> if <tt>response</tt> appears as a satisfactory response and <tt>false</tt> otherwise.
     */
    private boolean isResponseSatisfactory(Message response)
    {
        ...
        if ((answerRR != null && answerRR.length > 0) || (authorityRR != null && authorityRR.length > 0)
        ...
        // if we received NODATA (same as NOERROR and no response records) for an AAAA or a NAPTR query
        // then it makes sense since many existing domains come without those two.
    ...
    /**
     * The class that listens for responses to any of the queries we send to our default and backup servers and
     * returns as soon as we get ne or until our default resolver fails.
     */
    private class ParallelResolution implements Runnable
    {
        ...
        /**
         * The field where we would store the first incoming response to our query.
         */
        private volatile Message response;

        /**
         * The field where we would store the first error we receive from a DNS or a backup resolver.
         */
         ...
         *
         * @param query the DNS query that we'd like to send to our primary and backup resolvers.
         */
        ...
        /**
         * Starts this collector which would cause it to send its query to the default resolver.
         */
        ...
        public void run()
        {
            ...
            // if the backup resolvers had already replied we ignore the reply of the primary one whatever it was.
            if (done)
                return;
        ...
        {
            // yes. a second thread in the thread ... it's ugly but it works and i do want to keep code simple
            // to read ... this whole parallel resolving is complicated enough as it is.
            backupQueriesPool.execute(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    Message localResponse = null;
                    try {
                        logger.info("Sending query for " + query.getQuestion().getName() + "/"
                                + Type.string(query.getQuestion().getType()) + " to backup resolvers");
                    ...
                    // if the default resolver has already replied we ignore the reply of the backup ones.
                    if (done) {
                        return;
                    }

                    synchronized (ParallelResolution.this) {
                        // contrary to responses from the primary resolver, in this case we don't care whether
                        // the response is satisfying: if it isn't, there's nothing we can do
                        if (response == null) {
                            response = localResponse;
        ...
        /**
         * Waits for a response or an error to occur during <tt>waitFor</tt> milliseconds.If neither happens, we return false.
         *
         * @param waitFor the number of milliseconds to wait for a response or an error or <tt>0</tt> if we'd
         * like to wait until either of these happen.
         * @return <tt>true</tt> if we returned because we received a response from a resolver or errors from
         * everywhere, and <tt>false</tt> that didn't happen.
         */
        ...
        /**
         * Waits for resolution to complete (if necessary) and then either returns the response we received
         * or throws whatever exception we saw.
         *
         * @return the response {@link Message} we received from the DNS.
         * @throws IOException if this resolution ended badly because of a network IO error
         * @throws RuntimeException if something unexpected happened during resolution.
         * @throws IllegalArgumentException if something unexpected happened during resolution or if there was no response.
         */
