	...
	implements OperationSetBasicTelephony<T>
{
	...
	public void fireCallEvent(CallEvent event)
	{
		...
		for (CallListener listener : listeners) {
			if (logger.isDebugEnabled())
				logger.debug("Dispatching a CallEvent to " + listener.getClass()
					+ " . The event is: " + event);
	 ...
	 *
	 * @param call
	 *        the <tt>Call</tt> whose mute state is to be set
	 * @param mute
	...
	public Recorder createRecorder(Call call)
		throws OperationFailedException
	{
		return (call instanceof MediaAwareCall<?, ?, ?>) ? ((MediaAwareCall<?, ?, ?>) call)
			.createRecorder() : null;
	}
}
