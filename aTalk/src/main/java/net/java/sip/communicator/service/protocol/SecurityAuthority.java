 ...
 * is not available for a server, or once it has changed, or re-demand one after a faulty
 * authentication)
 *
 * @author Emil Ivov
...
public interface SecurityAuthority
{
	...
	/**
	 * Indicates that the reason for obtaining credentials is that the last login has an invalid
	 * authorization
	 */
	 ...
	 * sasl, incompatible authentication mechanism etc
	 */
	public static final int AUTHENTICATION_FAILED = 6;

	/**
	 * Indicates that the reason for obtaining credentials is that the last login has failed
	 * server account registration
	 */
	public static final int AUTHENTICATION_FORBIDDEN = 7;

	/**
	 * Indicates that the reason for obtaining credentials is that the last login is
	 * not authorized - IBR registration required.
	 */
	 ...
	 * @param isShowServerOption
	 *        true is to request user for server option
