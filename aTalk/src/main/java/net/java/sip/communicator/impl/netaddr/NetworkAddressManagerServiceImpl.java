...
public class NetworkAddressManagerServiceImpl implements NetworkAddressManagerService
{
	...
	/**
	 * A thread which periodically scans network interfaces and reports changes in network
	 * configuration.
	 */
	private NetworkConfigurationWatcher networkConfigurationWatcher = null;

	/**
	 * The service name to use when discovering TURN servers through DNS using SRV requests as per
	 * RFC 5766.
	 */
	public static final String TURN_SRV_NAME = "turn";

	/**
	 * The service name to use when discovering STUN servers through DNS using SRV requests as per
	 * RFC 5389.
	 */
	 ...
	 * 		the destination that we'd like to use the localhost address with.
	...
	public synchronized InetAddress getLocalHost(InetAddress intendedDestination)
	{
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Querying for a localhost address" + " for intended destination '"
					+ intendedDestination + "'");
		}

		/*
		 * use native code (JNI) to find source address for a specific destination address on
		 * Windows XP SP1 and over.
		 * 
		 * For other systems, we used method based on DatagramSocket.connect which will returns us
		  * source address. The reason why we cannot use it on Windows is because its socket
		  * implementation returns the any address...
		 */
		String osVersion;

		if (OSUtils.IS_WINDOWS && !(osVersion = System.getProperty("os.version")).startsWith("4")
				/* 95/98/Me/NT */
...
//			}
		...
		// windows socket implementations return the any address so we need to find something
		// else here ... InetAddress.getLocalHost seems to work better on windows so let's hope
		// it'll do the trick.
		if (localHost == null) {
			try {
		...
		if (localHost.isAnyLocalAddress()) {
			...
			try {
				...
				if (intendedDestination instanceof Inet6Address) {
					// return the first globally route-able ipv6 address we find on the machine
					...
					while (!done && ifaces.hasMoreElements()) {
						...
						while (addresses.hasMoreElements()) {
							...
							if ((address instanceof Inet6Address) && !address.isAnyLocalAddress()
									&& !address.isLinkLocalAddress() && !address
									.isLoopbackAddress()
	 ...
	 * @param port
	 * 		the port whose mapping we are interested in.
	...
	private DatagramSocket initRandomPortSocket()
	{
		...
		if (bindRetriesStr != null) {
			try {
				...
				logger.error(bindRetriesStr + " does not appear to be an integer. "
						+ "Defaulting port bind retries to " + bindRetries, ex);
			}
		}
		...
		for (int i = 0; i < bindRetries; i++) {
			try {
				...
				if (exc.getMessage().indexOf("Address already in use") == -1) {
					logger.fatal("An exception occurred while trying to create a local host " +
							"discovery socket.", exc);
					return null;
				}
	 ...
	 * @param minPort
	 * 		the port number where we should first try to bind before moving to the next one (i.e.
	 * 		<tt>minPort + 1</tt>
	 * 		)
	 * @param maxPort
	 * 		the maximum port number where we should try binding before giving up and throwing
	 * 		an exception.
	 * @return the newly created <tt>DatagramSocket</tt>.
	 * @throws IllegalArgumentException
	 * 		if either <tt>minPort</tt> or <tt>maxPort</tt> is not a valid port number or if
	 * 		<tt>minPort > maxPort</tt>.
	 * @throws IOException
	 * 		if an error occurs while the underlying resolver lib is using sockets.
	 * @throws BindException
	 * 		if we couldn't find a free port between <tt>minPort</tt> and <tt>maxPort</tt> before
			...
			throws IllegalArgumentException, IOException, BindException
	{
		...
		for (int i = 0; i < bindRetries; i++) {
			try {
				...
				if (logger.isInfoEnabled()) {
					logger.info("Retrying a bind because of a failure to bind to address "
							+ laddr + " and port " + port);
					if (logger.isTraceEnabled())
		...
		throw new BindException("Could not bind to any port between " + minPort
				+ " and " + (port - 1));
	}
	 ...
	 *
	 * @param domainName
	 * 		the domain name that we are trying to discover a TURN server for.
	 * @param userName
	 * 		the name of the user we'd like to use when connecting to a TURN server (we won't be
	 * 		using credentials in case we only have a STUN server).
	 * @param password
	 * 		the password that we'd like to try when connecting to a TURN server (we won't be using
			...
			byte[] password)
	{
		...
		try {
			SRVRecord srvRecord = NetworkUtils.getSRVRecord(TURN_SRV_NAME,
					Transport.UDP.toString(), domainName);
			if (srvRecord != null) {
			...
			if (srvrAddress != null) {
				...
				// harvesting.
				return new TurnCandidateHarvester(new TransportAddress(srvrAddress,
						srvRecord.getPort(), Transport.UDP),
						new LongTermCredential(userName, password));
			}

			// srvrAddres was null. try for a STUN only server.
			srvRecord = NetworkUtils.getSRVRecord(STUN_SRV_NAME, Transport.UDP.toString(),
					domainName);
			if (srvRecord != null) {
				srvrAddress = srvRecord.getTarget();
		...
		if (srvrAddress != null) {
			return new StunCandidateHarvester(new TransportAddress(srvrAddress, port,
					Transport.UDP));
		}
	 ...
	 *
	 * @param rtpPort
	 * 		the port that we should try to bind the RTP component on (the RTCP one would
	 ...
	 * @throws IllegalArgumentException
	 * 		if <tt>rtpPort</tt> is not a valid port number.
	 * @throws IOException
	 * 		if an error occurs while the underlying resolver is using sockets.
	 * @throws BindException
	 * 		if we couldn't find a free port between within the default number of retries.
	 */
			...
			throws IllegalArgumentException, IOException, BindException
	{
		...
		if (numComponents > 1) {
			agent.createComponent(stream, Transport.UDP, portBase + 1, portBase + 1,
					portBase + 101);
		}
		return stream;
	}
