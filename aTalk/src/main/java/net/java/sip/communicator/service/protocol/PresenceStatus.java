...
public class PresenceStatus implements Comparable<PresenceStatus>
{
	...
	/**
	 * An integer above which all values of the status coefficient indicate that a status with
	 * connectivity (communication is possible).
	 */
	public static final int ONLINE_THRESHOLD = 20;

	/**
	 * An integer above which all values of the status coefficient indicate both connectivity and
	 * availability but the person is away from the computer. This value has special meaning, it is
	 * the border between dnd and away and statuses with this value are normally considered on the
	 * phone.
	 */
	...
	/**
	 * An integer above which all values of the status coefficient indicate both connectivity and
	 * availability but the person is away from the computer.
	 */
	 ...
	 * disability for communication and 100 maximum ability and user willingness. Implementors of
	 * this service should respect the following indications for status values. 0 - complete
	 * disability 1:10 - initializing. 1:20 - trying to enter a state where communication is
	 * possible (Connecting ..) 20:50 - communication is possible but might be unwanted, inefficient
	 * or delayed(e.g. Away state in IM clients) 50:80 - communication is possible (On - line)
	 * 80:100 - communication is possible and user is eager to communicate. (Free for chat! Talk to
	 * me, etc.)
	 */
	 ...
	 *
	 * @param status
	 *        the status variable representing the new instance
	 * @param statusName
	 *        the name of this PresenceStatus
	 */
	 ...
	 *        the name of this PresenceStatus
	 * @param statusIcon
	 *        an image that graphically represents the status or null if no such image is available.
	 */
	...
	/**
	 * Returns a string representation of this provider status. Strings returned by this method have
	 * the following format: PresenceStatus:<STATUS_STRING>: <MESSAGE_STATUS> and are meant to be
	 ...
	 *
	 * @return true if the the status coefficient is higher than the ONLINE_THRESHOLD and false
	 *         otherwise
	 */
	...
	/**
	 * Indicates whether the user is both Online and available (can be reached and is likely to
	 * respond) or not.
	 *
	 * @return true if the the status coefficient is higher than the AVAILABLE_THRESHOLD and false
	 *         otherwise
	 */
	...
	/**
	 * Indicates whether the user is Online, available and eager to communicate (can be reached and
	 * is likely to become annoyingly talkative if contacted).
	 *
	...
	/**
	 * Compares this instance with the specified object for order. Returns a negative integer, zero,
	 * or a positive integer as this status instance is considered to represent less, as much, or
	 * more availability than the one specified by the parameter.
	 * <p>
	 *
	 * @param target
	 *        the <code>PresenceStatus</code> to be compared.
	...
	/**
	 * Indicates whether some other object is "equal to" this one. To PresenceStatus instances are
	...
	public boolean equals(Object obj)
	{
		...
		return status.getStatus() == getStatus() && status.getStatusName().equals(getStatusName());
	}
	 ...
	 * <p>
	 *
	 * @return a hash code value for this object (which is actually the result of the
	 *         getStatusName().hashCode()).
	 */
	@Override
	public int hashCode()
	{
