...
public class CallPeerJabberImpl extends
		AbstractCallPeerJabberGTalkImpl<CallJabberImpl, CallPeerMediaHandlerJabberImpl, JingleIQ> {
	/**
	 ...
	 * @param owningCall
	 * 		the call that contains this call peer.
	 */
	 ...
	 * 		the call that contains this call peer.
	 * @param sessionIQ
	 * 		The session-initiate <tt>JingleIQ</tt> which was received from <tt>peerAddress</tt>
	 ...
	 *
	 * @param failed
	 * 		indicates if the hangup is following to a call failure or simply a disconnect
	 * @param reasonText
	 * 		the text, if any, to be set on the <tt>ReasonPacketExtension</tt> as the value of its
	 * @param reasonOtherExtension
	 * 		the <tt>ExtensionElement</tt>, if any, to be set on the <tt>ReasonPacketExtension</tt>
	...
	public void hangup(boolean failed, String reasonText, ExtensionElement reasonOtherExtension) {
		...
		if (prevPeerState.equals(CallPeerState.CONNECTED)
			...
			responseIQ = JinglePacketFactory.createBye(getProtocolProvider().getOurJID(),
					peerJid, getSID());
		}
			...
			responseIQ = JinglePacketFactory.createCancel(getProtocolProvider().getOurJID(),
					peerJid, getSID());
		}
		else if (prevPeerState.equals(CallPeerState.INCOMING_CALL)) {
			responseIQ = JinglePacketFactory.createBusy(getProtocolProvider().getOurJID(), peerJid,
					getSID());
		}
		...
		if (responseIQ != null) {
			if (reasonOtherExtension != null) {
				ReasonPacketExtension reason = responseIQ.getExtension(
	...
	protected synchronized void initiateSession(Iterable<ExtensionElement> sessionInitiateExtensions)
		...
		synchronized (sidSyncRoot) {
			sessionInitIQ = JinglePacketFactory.createSessionInitiate(protocolProvider.getOurJID(),
	 ...
	 *
	 * @param content
	 * 		The {@link JingleIQ} that contains content that remote peer has accepted
	 */
	public void processContentAccept(JingleIQ content) {
		...
		try {
			...
			JingleIQ errResp = JinglePacketFactory.createSessionTerminate(
					getProtocolProvider().getOurJID(), peerJid, sessionInitIQ.getSID(),
	 ...
	 *
	 * @param content
	 * 		The {@link JingleIQ} that contains content that remote peer wants to be added
	 */
	public void processContentAdd(final JingleIQ content) {
		...
		try {
			...
			answerContents = null;
			contentIQ = JinglePacketFactory.createContentReject(getProtocolProvider().getOurJID(),
					this.peerJid, getSID(), answerContents);
		}

		if (contentIQ == null) {
			/* send content-accept */
			contentIQ = JinglePacketFactory.createContentAccept(getProtocolProvider().getOurJID(),
					this.peerJid, getSID(), answerContents);
	 ...
	 *
	 * @param content
	 * 		The {@link JingleIQ} that contains content that remote peer wants to be modified
	 */
	 ...
	 *
	 * @param content
	 * 		The {@link JingleIQ} that contains content that remote peer wants to be removed
	 */
	 ...
	 *
	 * @param sessionInitIQ
	 * 		The session-accept {@link JingleIQ} to process.
	 */
	 ...
	 *
	 * @param info
	 * 		the {@link SessionInfoPacketExtension} that we just received.
	 */
	 ...
	 *
	 * @param sessionInitIQ
	 * 		The {@link JingleIQ} that created the session that we are handling here.
	 */
	protected synchronized void processSessionInitiate(JingleIQ sessionInitIQ) {
		// Do initiate the session.
		this.sessionInitIQ = sessionInitIQ;
		this.initiator = true;
		XMPPTCPConnection connection = getProtocolProvider().getConnection();
		...
		try {
			connection.sendStanza(JinglePacketFactory.createRinging(sessionInitIQ));
		}
		catch (NotConnectedException | InterruptedException e) {
			e.printStackTrace();
		}
		...
		if ((discoverInfo != null)
			...
			try {
				getProtocolProvider().getConnection().sendStanza(
						JinglePacketFactory.createDescriptionInfo(sessionInitIQ.getTo(),
	 ...
	 *
	 * @param jingleIQ
	 * 		the {@link JingleIQ} that's terminating our session.
	 */
	 ...
	 *
	 * @param transfer
	 * 		the "XEP-0251: Jingle Session Transfer" transfer packet (extension) to process
	 * @throws OperationFailedException
	 * 		if anything goes wrong while processing the specified <tt>transfer</tt> packet
	 * 		(extension)
	 */
	public void processTransfer(TransferPacketExtension transfer)
		...
		OperationSetBasicTelephonyJabberImpl basicTelephony = (OperationSetBasicTelephonyJabberImpl) getProtocolProvider()
	 ...
	 *
	 * @param jingleIQ
	 * 		the <tt>transport-info</tt> {@link JingleIQ} to process
	 */
	 ...
	 *
	 * @param onHold
	 * 		<tt>true</tt> to have the <tt>CallPeer</tt> put on hold; <tt>false</tt>, otherwise
	 * @throws OperationFailedException
	 * 		if we fail to construct or send the INVITE request putting the remote side on/off
	 * 		hold.
	 */
	public void putOnHold(boolean onHold)
		...
		JingleIQ onHoldIQ = JinglePacketFactory.createSessionInfo(
				getProtocolProvider().getOurJID(), peerJid, getSID(), type);
		try {
	...
	private boolean sendAddVideoContent() {
		...
		ProtocolProviderServiceJabberImpl protocolProvider = getProtocolProvider();
		JingleIQ contentIQ = JinglePacketFactory.createContentAdd(protocolProvider.getOurJID(),
	...
	public void sendCoinSessionInfo() {
		JingleIQ sessionInfoIQ = JinglePacketFactory.createSessionInfo(getProtocolProvider()
				.getOurJID(), this.peerJid, getSID());
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> for which to return the <tt>MediaDirection</tt>
	...
	public boolean sendModifyVideoContent() {
		...
		if (newSenders != senders) {
			...
			JingleIQ contentIQ = JinglePacketFactory.createContentModify(
					protocolProvider.getOurJID(), this.peerJid, getSID(), ext);
	...
	public void sendModifyVideoResolutionContent() {
		CallPeerMediaHandlerJabberImpl mediaHandler = getMediaHandler();
		ContentPacketExtension remoteContent = mediaHandler.getRemoteContent(
		...
		ProtocolProviderServiceJabberImpl protocolProvider = getProtocolProvider();
		JingleIQ contentIQ = JinglePacketFactory.createContentModify(protocolProvider.getOurJID(),
	...
	private void sendRemoveVideoContent() {
		...
		ProtocolProviderServiceJabberImpl protocolProvider = getProtocolProvider();
		JingleIQ contentIQ = JinglePacketFactory.createContentRemove(protocolProvider.getOurJID(),
	 ...
	 *
	 * @param contents
	 * 		the local candidate addresses to be sent from the local peer to the remote peer using
	...
	protected void sendTransportInfo(Iterable<ContentPacketExtension> contents) {
		...
		StanzaCollector collector = protocolProvider.getConnection().createStanzaCollector(
	 ...
	 *
	 * @param to
	 * 		the address of the callee to transfer this <tt>CallPeer</tt> to
	 * @param sid
	...
	protected void transfer(Jid to, String sid)
		...
		transferSessionInfo.setTo(getPeerJid());
		...
		if (sid != null) {
			...
			OperationSetBasicTelephonyJabberImpl basicTelephony = (OperationSetBasicTelephonyJabberImpl) protocolProvider
					.getOperationSet(OperationSetBasicTelephony.class);
		...
		StanzaCollector collector = connection.createStanzaCollector(new StanzaIdFilter(
				transferSessionInfo.getStanzaId()));
		try {
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> for which to get the current value of the <tt>senders</tt>
	 * 		field.
	 ...
	 *
	 * @param content
	 * 		the <tt>ContentPacketExtension</tt> for which to get the <tt>MediaType</tt>
