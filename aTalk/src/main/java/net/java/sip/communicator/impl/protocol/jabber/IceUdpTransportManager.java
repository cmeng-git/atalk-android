 ...
 */
public class IceUdpTransportManager extends TransportManagerJabberImpl implements
		PropertyChangeListener {
	/**
	 * The <tt>Logger</tt> used by the <tt>IceUdpTransportManager</tt> class and its instances for
	 * logging output.
	 */
	...
	protected Agent createIceAgent() {
		...
		if (accID.isStunServerDiscoveryEnabled()) {
			...
			String username = provider.getOurJID().asBareJid().toString();
			...
			if (logger.isInfoEnabled())
				logger.info("Auto discovered STUN/TURN-server harvester is " + autoHarvester);
		...
		for (StunServerDescriptor desc : accID.getStunServers()) {
			TransportAddress addr = new TransportAddress(desc.getAddress(), desc.getPort(),
					Transport.UDP);
			...
			if (addr.getAddress() == null) {
				logger.info("Unresolved STUN server address for " + addr);
			}
			else {
		...
		if (!atLeastOneStunServer && accID.isUseDefaultStunServer()) {
			 ...
			 */
			TransportAddress addr = new TransportAddress(DEFAULT_STUN_SERVER_ADDRESS,
					DEFAULT_STUN_SERVER_PORT, Transport.UDP);
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("End gathering harvester within " + gatheringHarvesterTime + " ms; Size: "
					+ agent.getHarvestCount());
		}
		return agent;
	}
	...
	protected StreamConnector doCreateStreamConnector(MediaType mediaType)
		...
		return (streamConnectorSockets == null) ? super.doCreateStreamConnector(mediaType)
				: new DefaultStreamConnector(streamConnectorSockets[0 /* RTP */],
			...
			for (int i = 0; i < COMPONENT_IDS.length; i++) {
				...
				if (component != null) {
					...
					if (selectedPair != null) {
						DatagramSocket streamConnectorSocket = selectedPair.getLocalCandidate()
								.getDatagramSocket();
		...
		if (stream != null) {
			...
			for (int i = 0; i < COMPONENT_IDS.length; i++) {
				...
				if (component != null) {
					...
					if (selectedPair != null) {
						InetSocketAddress streamTargetAddress = selectedPair.getRemoteCandidate()
								.getTransportAddress();
	...
	/**
	 * Creates a {@link CandidatePacketExtension} and initializes it so that it would describe the
	 * state of <tt>candidate</tt>
	 *
	 * @param candidate the ICE4J {@link Candidate} that we'd like to convert into an XMPP packet extension.
	 * @return a new {@link CandidatePacketExtension} corresponding to the state of the
	...
	protected synchronized boolean startConnectivityEstablishment(
		...
		for (Map.Entry<String, IceUdpTransportPacketExtension> e : remote.entrySet()) {
			...
			List<CandidatePacketExtension> candidates
					= transport.getChildExtensionsOfType(CandidatePacketExtension.class);
			...
			for (CandidatePacketExtension candidate : candidates) {
				...
				if (candidate.getGeneration() == generation) {
					...
					if (((relAddr = candidate.getRelAddr()) != null)
						...
						relatedAddress = new TransportAddress(relAddr, relPort,
			...
			for (IceMediaStream stream : iceAgent.getStreams()) {
				for (Component component : stream.getComponents()) {
					if (component.getRemoteCandidateCount() < 1) {
						logger.warn("### Insufficient remote candidates to startConnectivityEstablishment! "
								+ component.toShortString()
								+ ": " + component.getRemoteCandidateCount());
	...
	public void wrapupConnectivityEstablishment()
		...
		if ((delegate == null) || (delegate == this)) {
			...
			if (IceProcessingState.FAILED.equals(iceAgent.getState())) {
				String msg = JabberActivator.getResources().getI18NString(
						"service.protocol.ICE_FAILED");
	 ...
	 */
	public void propertyChange(PropertyChangeEvent evt) {
		getCallPeer().getMediaHandler().firePropertyChange(evt.getPropertyName(),
