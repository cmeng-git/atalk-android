...
public abstract class AbstractOperationSetBasicAutoAnswer implements OperationSetBasicAutoAnswer
{
	 ...
	 *
	 * @param answerWithVideo
	 *        A boolean set to true to activate the auto answer with video when receiving a video
	...
	/**
	 * Waits for peer to switch into INCOMING_CALL state, before auto-answering the call in a new
	 * thread.
	 */
	private class AutoAnswerThread extends CallPeerAdapter implements Runnable
	{
		 ...
		 *        The call peer which has generated the call.
		 * @param isVideoCall
		 *        Indicates if the remote peer which has created this call wish to have a video
		 *        call.
		 */
		 ...
		 *
		 * @param evt
		 *        the <tt>CallPeerChangeEvent</tt> instance containing the
		 */
		@Override
