...
/**
 * This class represents the "mail-thread-info" element the Google use in their mail notifications
 * to deliver detailed thread information.
 *
 * @author Emil Ivov
 */
public class MailThreadInfo
{
	...
	/**
	 * The name of the XML tag element containing a pipe separated list of labels assigned to this
	 * thread.
	 */
	...
	/**
	 * PARTICIPATION_SOLE_RECIPIENT indicates that the user is the sole recipient for messages in
	 * this thread.
	 */
	public static final int PARTICIPATION_SOLE_RECIPIENT = 2;

	/**
	 * A number indicating the local user's participation level in this thread. One of the
	 * PARTICIPATION_XXX constants defined above.
	 */
	...
	/**
	 * The URL linking to this thread (as opposed to the URL delivered in the <tt>MailboxIQ</tt>
	 * that links to the mailbox itself).
	 */
	...
	{
		/**
		 * The name of the XML tag element containing information for an individual sender and
	...
	/**
	 * Returns the participation index for this thread. The participation index is a number
	 * indicating the local user's participation level in this thread: PARTICIPATION_NONE indicates
	 * that the user has not participated; PARTICIPATION_ONE_OF_MANY indicates that the user is one
	 * of many recipients listed in the thread; PARTICIPATION_SOLE_RECIPIENT indicates that the user
	 * is the sole recipient for messages in this thread.
	 *
	 * @return one of the PARTICIPATION_XXX values defines in this class and indicating whether the
	 *         local is the sole, one of many or not a participant of this thread.
	 */
	...
	/**
	 * Specifies the participation index for this thread. The participation index is a number
	 * indicating the local user's participation level in this thread: PARTICIPATION_NONE indicates
	 * that the user has not participated; PARTICIPATION_ONE_OF_MANY indicates that the user is one
	 * of many recipients listed in the thread; PARTICIPATION_SOLE_RECIPIENT indicates that the user
	 * is the sole recipient for messages in this thread.
	 *
	 * @param participation
	 *        one of the PARTICIPATION_XXX values defines in this class and indicating whether the
	 *        local is the sole, one of many or not a participant of this thread.
	 */
	...
	/**
	 * Returns an iterator over a list of one or more sender instances, each of which describes a
	 * participant in this thread.
	 *
	 * @return an iterator over a list of one or more sender instances, each of which describes a
	 *         participant in this thread.
	 */
	...
	/**
	 * Returns the number of people that have been posting in this thread and that we have unread
	 * messages from.
	 *
	 * @return the number of people that have been posting in this thread and that we have unread
	 *         messages from.
	 */
	...
	/**
	 * Returns the sender that initiated the thread or the first sender in the list if for some
	 * reason we couldn't determine the originator.
	 *
	 * @param firstNameOnly
	 *        use only first name
	 * @return the sender that initiated the thread or the first sender in the list if for some
	 *         reason we couldn't determine the originator.
	 */
	 ...
	 *
	 * @return a timestamp of the most recent message, in milliseconds since the UNIX epoch.
	 */
	public long getDate()
	{
		return date;
	...
	private String getFormattedDate()
	{
		...
		if (now.get(Calendar.YEAR) != threadDateCal.get(Calendar.YEAR)
			...
			|| now.get(Calendar.DAY_OF_MONTH) != threadDateCal.get(Calendar.DAY_OF_MONTH)) {
	 ...
	 *
	 * @param date
	 *        a timestamp of the most recent message in this thread.
	 */
	...
	/**
	 * Returns an URL linking to this thread. It is important to distinguish between this URL and
	 * the one returned by the <tt>MailboxIQ</tt> which points to the whole mailbox.
	 *
	...
	/**
	 * Sets an URL linking to this thread. It is important to distinguish between this URL and the
	 * one returned by the <tt>MailboxIQ</tt> which points to the whole mailbox.
	 *
	 * @param url
	 ...
	 *
	 * @param labels
	 *        a pipe ('|') delimited list of labels that apply to this thread.
	 */
	...
	/**
	 * Creates and initializes a <tt>MailThreadInfo</tt> instance according to the details that come
	 * with the parser.
	 *
	 * @param parser
	 *        the parse that we are to read the <tt>MailThreadInfo</tt> from.
	 *
	 * @return the newly created <tt>MailThreadInfo</tt> instance.
	 *
	 * @throws XmlPullParserException
	 *         if something goes wrong while parsing the document.
	 * @throws NumberFormatException
	 *         in case we fail to parse any of the elements that we expect to be numerical.
	 * @throws IOException
	 *         in case reading the input xml fails.
	 */
	 ...
	 *
	 * @param parser
	 *        the parser containing the sender list.
	 *
	 * @throws XmlPullParserException
	 *         if something goes wrong while parsing the document.
	 * @throws NumberFormatException
			...
			while (eventType != XmlPullParser.END_TAG) {
				...
				if (Sender.ELEMENT_NAME.equals(name)) {
					...
					if (originatorStr != null)
						sender.originator = (Integer.parseInt(originatorStr) == 1);
	...
	/**
	 * Creates an html description of all participant names in the thread. We try to do this in a
	 * Gmail-like (although quite simplified) way:<br/>
