...
public class StunServerDescriptor implements Serializable
{
	...
	/**
	 * The password that we need to use when authenticating with the server or <tt>null</tt> if no
	 ...
	 *        the port of the server
	 * @param supportTurn
	 *        indicates if this STUN server supports TURN
	 * @param username
	 *        the user name for authenticating
	 * @param password
	 *        the password
	 */
	public StunServerDescriptor(String address, int port, boolean supportTurn, String username,
		String password)
	{
	 ...
	 *
	 * @param turnSupported
	 *        <tt>true</tt> to indicate that TURN is supported, <tt>false</tt> - otherwise
	 */
	 ...
	 *
	 * @param props
	 *        the account properties {@link Map} that we'd like to store this descriptor in.
	 * @param namePrefix
	 *        the prefix that we should prepend to every property name.
	 */
	 ...
	 *
	 * @param props
	 *        the account properties {@link Map} that we'd like to load this descriptor from.
	 * @param namePrefix
	 *        the prefix that we should prepend to every property name.
	 *
	...
	public static StunServerDescriptor loadDescriptor(Map<String, String> props, String namePrefix)
	{
		...
		boolean stunIsTurnSupported = Boolean.parseBoolean(props.get(namePrefix
			+ STUN_IS_TURN_SUPPORTED));

		StunServerDescriptor stunServer = new StunServerDescriptor(stunAddress, stunPort,
			stunIsTurnSupported, stunUsername, stunPassword);
	...
	{
		return "StunServerDesc: " + getAddress() + "/" + getPort() + " turnSupp="
			+ this.isTurnSupported();
	}
}
