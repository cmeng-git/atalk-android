...
public class AvatarCacheUtils
{
     ...
     * corresponding to the given protocol provider.
     *
     * @param protocolContact the <tt>ProtocolProviderService</tt>, which
     * account avatar image we're looking for
     * @return the bytes of the avatar image stored for the account
     * corresponding to the given protocol provider
     */
    ...
    {
        return AVATAR_DIR + File.separator
                + escapeSpecialCharacters(protocolProvider.getAccountID().getAccountUniqueID())
                + File.separator + escapeSpecialCharacters(protocolProvider.getAccountID().getAccountUniqueID());
    }
    ...
    {
        return AVATAR_DIR + File.separator
                + escapeSpecialCharacters(protocolContact.getProtocolProvider().getAccountID().getAccountUniqueID())
                + File.separator + escapeSpecialCharacters(protocolContact.getAddress());
    }
    ...
    public static String getCachedAvatarPath(ProtocolProviderService protocolProvider, String contactAddress)
    {
        return AVATAR_DIR + File.separator
                + escapeSpecialCharacters(protocolProvider.getAccountID().getAccountUniqueID())
                + File.separator + escapeSpecialCharacters(contactAddress);
    }
    ...
    {
        try {
            File avatarFile = UtilActivator.getFileAccessService()
     ...
     * @return a <tt>String</tt> derived from the specified <tt>id</tt> by escaping characters
     */
    ...
    public static void cacheAvatar(Contact protoContact,
            byte[] avatarBytes)
    {
        String avatarDirPath = AVATAR_DIR + File.separator
                + escapeSpecialCharacters(protoContact.getProtocolProvider().getAccountID().getAccountUniqueID());
        String avatarFileName = escapeSpecialCharacters(protoContact.getAddress());

        cacheAvatar(avatarDirPath, avatarFileName, avatarBytes);
    }

    /**
     * Stores avatar bytes for the account corresponding to the given <tt>protocolProvider</tt>.
     *
     * @param protocolProvider the protocol provider corresponding to the account, which avatar we're storing
     * @param avatarBytes the avatar image bytes
     */
    public static void cacheAvatar(ProtocolProviderService protocolProvider, byte[] avatarBytes)
    {
        ...
        String avatarDirPath = AVATAR_DIR + File.separator + escapeSpecialCharacters(accountUID);
            ...
            byte[] avatarBytes)
    {
        ...
        try {
            ...
            avatarFile = fileAccessService.getPrivatePersistentFile(
                    new File(avatarDirPath, avatarFileName).toString(), FileCategory.CACHE);
