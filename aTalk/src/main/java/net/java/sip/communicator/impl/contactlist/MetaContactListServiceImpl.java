...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>MetaContactListServiceImpl</tt> class and its instances
	 * for logging output.
	 */
	...
	/**
	 * The number of milliseconds to wait for confirmations of account modifications before
	 * deciding to drop.
	 */
	...
	/**
	 * Contains (as keys) <tt>MetaContactGroup</tt> names that are currently being resolved
	 * against a given protocol and that this class's <tt>ContactGroupListener</tt> should ignore
	 ...
	 */
	private final Hashtable<String, List<ProtocolProviderService>> groupEventIgnoreList
			= new Hashtable<>();

	/**
	 * Contains (as keys) <tt>Contact</tt> addresses that are currently being resolved against a
	 * given protocol and that this class's <tt>ContactListener</tt> should ignore as
	 ...
	 * in this table and is issued by one of the providers mapped against this groupName.
	 */
	private final Hashtable<String, List<ProtocolProviderService>> contactEventIgnoreList
			= new Hashtable<>();
	...
	public MetaContactListServiceImpl()
	{
		rootMetaGroup = new MetaContactGroupImpl(this,
				ContactlistActivator.getResources().getI18NString("service.gui.CONTACTS"),
				ContactGroup.ROOT_GROUP_UID);
	}
	...
	public void stop(BundleContext bc)
	{
		...
		for (ProtocolProviderService pps : currentlyInstalledProviders.values()) {
	...
	/**
	 * Adds a listener for <tt>MetaContactListChangeEvent</tt>s posted after the tree changes.
	 *
	 * @param listener
	 * 		the listener to add
	 */
	 ...
	 *
	 * @param provider
	 * 		the ProtocolProviderService that should create the contact indicated by
	 * 		<tt>contactID</tt>.
	 * @param metaContact
	 * 		the meta contact where that the newly created contact should be associated to.
	 * @param contactID
	 ...
	 * 		the identifier of the contact that the specified provider
	 * @param fireEvent
	 * 		specifies whether or not an even is to be fire at the end of the method.Used when this
	 ...
	 * 		the meta contact group which is the parent group of the newly created contact
	 * @param metaContact
	 * 		the meta contact where that the newly created contact should be associated to.
	 * @param contactID
	 * 		the identifier of the contact that the specified provider
	 * @param fireEvent
	 * 		specifies whether or not an even is to be fired at the end of the method.Used when
		...
		if (evtRetriever.evt instanceof SubscriptionEvent
			...
			throw new MetaContactListException(
					"Failed to create a contact with address: " + contactID + " "
							+ ((SubscriptionEvent) evtRetriever.evt).getErrorReason(), null,
	 ...
	 *
	 * @param protoProvider
	 * 		a reference to the protocol provider where the groups should be created.
	 * @param metaGroup
	 * 		a ref to the last group of the path that should be created in the specified
	 * 		<tt>protoProvider</tt>
	...
	{
		Iterator<ContactGroup> contactGroupsForProv
				= metaGroup.getContactGroupsForProvider(protoProvider);
		...
		MetaContactGroupImpl parentMetaGroup
				= (MetaContactGroupImpl) findParentMetaContactGroup(metaGroup);
		if (parentMetaGroup == null) {
			if (logger.isDebugEnabled())
				logger.debug("Resolve failed at group" + metaGroup);
			throw new NullPointerException("Internal Error. Orphan group.");
		}
		OperationSetPersistentPresence opSetPersPresence = protoProvider.getOperationSet(
		...
		try {
			// create the group
			opSetPersPresence.createServerStoredContactGroup(parentProtoGroup,
		...
		fireMetaContactGroupEvent(metaGroup, evtRetriever.evt.getSourceProvider(),
	...
	/**
	 * Returns the meta contact group that is a direct parent of the specified <tt>child</tt>. If
	 * no parent is found <tt>null</tt> is returned.
	 *
	 * @param child
	 * 		the <tt>MetaContactGroup</tt> whose parent group we're looking for. If no parent is
	...
	/**
	 * Returns the meta contact group that is a direct parent of the specified <tt>child</tt>,
	 * beginning the search at the specified root. If no parent is found <tt>null</tt> is returned.
	 *
	 * @param child
	 * 		the <tt>MetaContactGroup</tt> whose parent group we're looking for.
	 * @param root
	 * 		the parent where the search should start.
	 * @return the <tt>MetaContactGroup</tt> that contains <tt>child</tt> or null if no parent was
	 * found.
	 */
	private MetaContactGroup findParentMetaContactGroup(MetaContactGroupImpl root,
			MetaContactGroup child)
	{
	 ...
	 *
	 * @param child
	 * 		the <tt>MetaContact</tt> whose parent group we're looking for.
	 * @return the <tt>MetaContactGroup</tt>
	 * @throws IllegalArgumentException
	 * 		if <tt>child</tt> is not an instance of MetaContactImpl
	 */
	 ...
	 * <tt>contactID</tt>, then creates a new MetaContact which will encapsulate the newly
	 * created protocol specific contact.
	 *
	 * @param provider
	 * 		a ref to <tt>ProtocolProviderService</tt> instance which will create the actual
	 * 		protocol specific contact.
	 * @param metaContactGroup
	 * 		the MetaContactGroup where the newly created meta contact should be stored.
	 * @param contactID
	 * 		a protocol specific string identifier indicating the contact the protocol provider
	 * 		should create.
			...
			throws MetaContactListException
	{
		if (!(metaContactGroup instanceof MetaContactGroupImpl)) {
			throw new IllegalArgumentException(
					metaContactGroup + " is not an instance of MetaContactGroupImpl");
		}

		MetaContactImpl newMetaContact = new MetaContactImpl();
		this.addNewContactToMetaContact(provider, metaContactGroup, newMetaContact, contactID,
				false);
	...
	/**
	 * Creates a <tt>MetaContactGroup</tt> with the specified group name. The meta contact group
	 ...
	 *
	 * @param parent
	 * 		the meta contact group inside which the new child group must be created.
	 * @param groupName
	...
	/**
	 * Renames the specified <tt>MetaContactGroup</tt> as indicated by the <tt>newName</tt> param.
	 ...
	 * 		the group to rename.
	 * @param newGroupName
	 * 		the new name of the <tt>MetaContactGroup</tt> to rename.
	 */
	public void renameMetaContactGroup(MetaContactGroup group, String newGroupName)
	{
		...
		while (groups.hasNext()) {
			...
			OperationSetPersistentPresence opSetPresence
					= protoGroup.getProtocolProvider().getOperationSet(
					OperationSetPersistentPresence.class);
			if (opSetPresence != null) {
				try {
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> that we are renaming
	 * @param newDisplayName
	 * 		a <tt>String</tt> containing the new display name for <tt>metaContact</tt>.
	 * @throws IllegalArgumentException
	 ...
	 * 		implementation.
	 */
	private void renameMetaContact(MetaContact metaContact, String newDisplayName,
			...
			throws IllegalArgumentException
	{
		...
		fireMetaContactEvent(
				new MetaContactRenamedEvent(metaContact, oldDisplayName, newDisplayName));
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> that we are operating on
	 * @throws IllegalArgumentException
	 * 		if <tt>metaContact</tt> is not an instance that belongs to the underlying
	 * 		implementation.
	 */
			...
			throws IllegalArgumentException
	{
		...
		if (metaContact.getContactCount() == 1) {
			renameMetaContact(metaContact, metaContact.getDefaultContact().getDisplayName(),
					false);
		}
		else {
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> that change avatar
	 * @param protoContact
	 * 		the <tt>Contact> that change avatar
	 * @param newAvatar
	 * 		avatar image bytes
	 * @throws IllegalArgumentException
	 * 		if <tt>metaContact</tt> is not an instance that belongs to the underlying
	...
	/**
	 * Makes the specified <tt>contact</tt> a child move to the <tt>newParentMetaGroup</tt>
	 * MetaContactGroup. If <tt>contact</tt> was previously a child of a meta contact, it will be
	 ...
	 * @param newParentMetaGroup
	 * 		the MetaContactGroup where we'd like contact to be moved.
	 * @throws MetaContactListException
	 * 		with an appropriate code if the operation fails for some reason.
	 */
			...
			throws MetaContactListException
	{
		...
		if (opSetMUC != null && opSetMUC.isPrivateMessagingContact(contact.getAddress())) {
			...
			fireMetaContactEvent(metaContactImpl, newParentMetaGroupImpl,
					MetaContactEvent.META_CONTACT_ADDED);
		...
		if (metaContactImpl != null) {
			fireMetaContactEvent(metaContactImpl, newParentMetaGroup,
					MetaContactEvent.META_CONTACT_ADDED);
	...
	/**
	 * Makes the specified <tt>contact</tt> a child of the <tt>newParent</tt> MetaContact.
	 *
	 * @param contact
	 * 		the <tt>Contact</tt> to move to the
			...
			throws MetaContactListException
	{
		...
		MetaContactImpl currentParentMetaContact = (MetaContactImpl) this.findMetaContactByContact(
				contact);
	 ...
	 * @param newMetaGroup
	 * 		the <tt>MetaContactGroup</tt> that should be the new parent of <tt>contact</tt>.
	 * @throws MetaContactListException
	 * 		with an appropriate code if the operation fails for some reason.
	 * @throws IllegalArgumentException
	 * 		if <tt>newMetaGroup</tt> or <tt>metaContact</tt> do not come from this implementation.
	 */
			...
			while (contacts.hasNext()) {
				...
				if (opSetPresence == null) {
					...
					if (newMetaGroup.equals(getRoot()))
						opSetPresence.moveContactToGroup(protoContact,
			...
			throws MetaContactListException
	{
		...
		try {
			...
			throw new MetaContactListException(errorTxt, ex,
					MetaContactListException.CODE_NETWORK_ERROR);
		}
	}
			...
			throws MetaContactListException
	{
		...
		MetaContactGroupImpl parentMetaGroup
				= (MetaContactGroupImpl) findParentMetaContactGroup(groupToRemove);
	 ...
	 *
	 * @param metaContainer
	 * 		the MetaContactGroup that we'd like to remove a contact group from.
	 * @param groupToRemove
	 * 		the ContactGroup that we'd like removed.
	 * @param sourceProvider
	 * 		the ProtocolProvider that the contact group belongs to.
	 */
	 ...
	 * contacts belonging to the specified <tt>groupToRemove</tt>. Note that this method won't
	 ...
	 *
	 * @param parentMetaGroup
	 * 		the MetaContactGroup whose children we should go through
	 * @param groupToRemove
	 * 		the proto group that we want removed together with its children.
	 */
	 ...
	 *
	 * @param contactGroup
	 * 		the protocol specific <tt>contactGroup</tt> that we're looking for.
	 ...
	 * provided a <tt>Contact</tt> as its author.
	 *
	 * @param contact
	 * 		the protocol specific <tt>contact</tt> that we're looking for.
	 ...
	 * <tt>contactAddress</tt> and with a source provider matching <tt>accountID</tt>, or null if
	 ...
	 * ProtocolProviderService has only provided a <tt>Contact</tt> as its author.
	 *
	 * @param contactAddress
	 * 		the address of the protocol specific <tt>contact</tt> that we're looking for.
	 * @param accountID
	 * 		the ID of the account that the contact we're looking for must belong to.
	 * @return the MetaContact containing the specified contact or null if no such contact is
	 * present in this contact list.
	 */
	...
	/**
	 * Returns a list of all <tt>MetaContact</tt>s containing a protocol contact from the given
	 * <tt>ProtocolProviderService</tt>.
	 *
	 * @param protocolProvider
	 * 		the <tt>ProtocolProviderService</tt> whose contacts we're looking for.
	 * @return a list of all <tt>MetaContact</tt>s containing a protocol contact from the given
	 * <tt>ProtocolProviderService</tt>.
	 */
			...
			ProtocolProviderService protocolProvider)
	{
		...
		this.findAllMetaContactsForProvider(protocolProvider, rootMetaGroup, resultList);
		return resultList.iterator();
	}

	/**
	 * Returns a list of all <tt>MetaContact</tt>s contained in the given group and containing a
	 * protocol contact from the given <tt>ProtocolProviderService</tt>.
	 *
	 * @param protocolProvider
	 * 		the <tt>ProtocolProviderService</tt> whose contacts we're looking for.
	 * @param metaContactGroup
	...
	/**
	 * Returns a list of all <tt>MetaContact</tt>s containing a protocol contact corresponding to
	 * the given <tt>contactAddress</tt> string.
	 *
	 * @param contactAddress
	 * 		the contact address for which we're looking for a parent <tt>MetaContact</tt>.
	 * @return a list of all <tt>MetaContact</tt>s containing a protocol contact corresponding to
	 * the given <tt>contactAddress</tt> string.
	 */
			...
			MetaContactGroup metaContactGroup, List<MetaContact> resultList)
	{
		...
		while (childContacts.hasNext()) {
			MetaContact metaContact = childContacts.next();
			Iterator<Contact> protocolContacts = metaContact.getContactsForProvider
					(protocolProvider);
	 ...
	 *
	 * @param presenceOpSet
	 * 		the presence operation set whose contact list we'd like to synchronize with the local
	 * 		contact list.
	 */
	 ...
	 * <tt>contactGroup</tt> and adds them to <tt>metaGroup</tt>
	 *
	 * @param protoGroup
	 * 		the <tt>ContactGroup</tt> to add.
	 * 		<p>
	 * @param metaGroup
	 * 		the <tt>MetaContactGroup</tt> where <tt>ContactGroup</tt> should be added.
	 * @param fireEvents
	 * 		indicates whether or not events are to be fired upon adding subContacts and subgroups.
			...
			metaGroup, boolean fireEvents)
	{
		...
		while (subgroupsIter.hasNext()) {
			...
			if (fireEvents) {
				this.fireMetaContactGroupEvent(newMetaGroup, group.getProtocolProvider(), group,
						MetaContactGroupEvent.META_CONTACT_GROUP_ADDED);
			}
		}
		...
		while (contactsIter.hasNext()) {
			...
			if (fireEvents) {
				this.fireMetaContactEvent(newMetaContact, metaGroup,
						MetaContactEvent.META_CONTACT_ADDED);
			}
		}
	}
	...
	private synchronized void handleProviderAdded(ProtocolProviderService provider)
	{
		...
		if (logger.isDebugEnabled())
			logger.debug("Adding protocol provider " + accountUid);

		this.currentlyInstalledProviders.put(accountUid, provider);
		...
		if (opSetPersPresence != null) {
			...
			if (logger.isDebugEnabled())
				logger.debug("All contacts loaded for account " + accountUid);
			synchronizeOpSetWithLocalContactList(opSetPersPresence);
		}
		else {
			if (logger.isDebugEnabled())
				logger.debug("Service did not have a peer. pres. op. set.");
		}
	...
	{
		if (logger.isDebugEnabled())
			logger.debug("Removing protocol provider " + pps);

		AccountID accountID = pps.getAccountID();
		this.currentlyInstalledProviders.remove(accountID.getAccountUniqueID());
		...
		if (persPresOpSet != null) {
			...
			while (subgroups.hasNext()) {
				...
				// remove the group
				this.removeContactGroupFromMetaContactGroup((MetaContactGroupImpl)
						findMetaContactGroupByContactGroup(group), group, pps);
			}
			// cmeng - not allow to remove the root group
			// this.removeContactGroupFromMetaContactGroup(this.rootMetaGroup, rootGroup, pps);
		}

	}

	/**
	 * Registers <tt>group</tt> to the event ignore list. This would make the method that is
	 ...
	 *
	 * @param group
	 * 		the name of the group that we'd like to register.
	 * @param ownerProvider
	 * 		the protocol provider that we expect the addition to come from.
	 */
	 ...
	 *
	 * @param group
	 * 		the group whose presence in the ignore list we'd like to verify.
	 * @param ownerProvider
	 * 		the provider that <tt>group</tt> belongs to.
	...
	/**
	 * Removes the <tt>group</tt> from the group event ignore list so that events concerning this
	 * group get treated.
	 *
	 * @param group
	 * 		the group whose that we'd want out of the ignore list.
	 * @param ownerProvider
	 * 		the provider that <tt>group</tt> belongs to.
	 */
	...
	/**
	 * Registers <tt>contact</tt> to the event ignore list. This would make the method that is
	 ...
	 *
	 * @param contact
	 * 		the address of the contact that we'd like to ignore.
	 * @param ownerProvider
	 * 		the protocol provider that we expect the addition to come from.
	 */
	 ...
	 *
	 * @param contact
	 * 		the contact whose presence in the ignore list we'd like to verify.
	 * @param ownerProvider
	 ...
	 */
	private boolean isContactInEventIgnoreList(String contact,
	 ...
	 * @return true if the contact is in the contact event ignore list and false otherwise.
	 */
	private boolean isContactInEventIgnoreList(Contact contact, ProtocolProviderService
			ownerProvider)
	{
	 ...
	 *
	 * @param contact
	 * 		the contact whose that we'd want out of the ignore list.
	 * @param ownerProvider
	 * 		the provider that <tt>group</tt> belongs to.
	 */
	...
	public void serviceChanged(ServiceEvent event)
	{
		...
		if (event.getType() == ServiceEvent.REGISTERED) {
			...
			// if not ignore it.
			String providerMask
					= System.getProperty(MetaContactListService.PROVIDER_MASK_PROPERTY);
			if (providerMask != null && providerMask.trim().length() > 0) {
				...
				if ((servRefMask == null) || !servRefMask.equals(providerMask)) {
					if (logger.isDebugEnabled())
						logger.debug("Ignoring masked provider: " + accountID);
					return;
				}
			}

			if (sourceFactory != null && currentlyInstalledProviders.containsKey(
					pps.getAccountID().getAccountUniqueID())) {
				if (logger.isDebugEnabled())
					logger.debug("Modifying an existing installed account: " + accountID);
			...
			logger.warn("Account uninstalled. Removing all its meta contacts: " + accountID);
			this.handleProviderRemoved(pps);
		}
	}

	/**
	 * The class would listen for events delivered to <tt>SubscriptionListener</tt>s.
	 */
	private class ContactListSubscriptionListener implements SubscriptionListener
	{
		 ...
		 * generate the corresponding <tt>MetaContactEvent</tt>.
		 *
		 * @param evt
		 * 		the SubscriptionEvent that we'll be handling.
		 */
		 ...
		 *
		 * @param evt
		 * 		a reference to the SubscriptionMovedEvent containing previous and new parents as
		...
		public void subscriptionMoved(SubscriptionMovedEvent evt)
		{
			...
			MetaContactImpl currentMetaContact
					= (MetaContactImpl) findMetaContactByContact(sourceContact);
			if (currentMetaContact == null) {
				logger.warn("Received a move event for a contact that is not in our contact list.",
						new NullPointerException("Received a move event for a " +
								"contact that is not in our contact list."));
				return;
			}
			...
			if (currentMetaContact.getContactCount() == 1) {
				...
				fireMetaContactEvent(new MetaContactMovedEvent(currentMetaContact, oldParentGroup,
						newParentGroup));
			}
			...
			// move the source contact to it.
				...
				if (newMetaContact != null) {
					...
					fireMetaContactEvent(newMetaContact, newParentGroup,
							MetaContactEvent.META_CONTACT_ADDED);
		 ...
		 *
		 * @param evt
		 * 		the SubscriptionEvent containing the source contact
		 */
			...
			if (mc != null) {
				...
				if (mc.getContactCount() == 1 && !mc.isDisplayNameUserDefined()) {
					...
					if (newDisplayName != null && !newDisplayName.equals(oldDisplayName)) {
						...
						fireMetaContactEvent(new MetaContactRenamedEvent(mc, oldDisplayName,
								newDisplayName));
		 ...
		 *
		 * @param evt
		 * 		the <tt>ContactPropertyChangeEvent</tt> containing the source contact and the old
			...
			if (ContactPropertyChangeEvent.PROPERTY_DISPLAY_NAME.equals(evt.getPropertyName())) {
				if (evt.getOldValue() != null && evt.getOldValue().equals(mc.getDisplayName())) {
					...
					fireProtoContactEvent(evt.getSourceContact(),
							ProtoContactEvent.PROTO_CONTACT_RENAMED, mc, mc);
				}
			}
		 ...
		 *
		 * @param evt
		 * 		the <tt>SubscriptionEvent</tt> containing the contact that has been removed.
		 */
		public void subscriptionRemoved(SubscriptionEvent evt)
		{
			...
			MetaContactImpl metaContact = (MetaContactImpl) findMetaContactByContact(
			...
			if (metaContact.getContactCount() == 0) {
				...
				fireMetaContactEvent(metaContact, metaContactGroup,
						MetaContactEvent.META_CONTACT_REMOVED);
			}
			else {
	...
	/**
	 * The class would listen for events delivered to <tt>ServerStoredGroupListener</tt>s.
	 */
	private class ContactListGroupListener implements ServerStoredGroupListener
	{
		 ...
		 *
		 * @param parent
		 * 		a reference to the <tt>MetaContactGroupImpl</tt> where <tt>group</tt>'s newly
		 ...
		 * <tt>ContactGroup</tt>.
		 */
		private MetaContactGroup handleGroupCreatedEvent(MetaContactGroupImpl parent,
				ContactGroup group)
		{
			...
			if (newMetaGroup == null) {
				newMetaGroup = new MetaContactGroupImpl(MetaContactListServiceImpl.this,
						group.getGroupName());
		 ...
		 *
		 * @param evt
		 * 		the ServerStoredGroupEvent containing the source group.
		 */
		 ...
		 *
		 * @param evt
		 * 		a ServerStoredGroupEvent containing the source group.
		 */
		 ...
		 * group indicated by the delivered <tt>evt</tt>
		 *
		 * @param evt
		 * 		the ServerStoredGroupEvent confining the group that has been removed.
		 */
		public void groupRemoved(ServerStoredGroupEvent evt)
		{
			...
			removeContactGroupFromMetaContactGroup(metaContactGroup, evt.getSourceGroup(),
					evt.getSourceProvider());
		...
		public void groupNameChanged(ServerStoredGroupEvent evt)
		{
			...
			if (metaContactGroup.countContactGroups() == 1) {
				...
				((MetaContactGroupImpl) metaContactGroup).setGroupName(
	 ...
	 * <tt>MetaContactListListener</tt>s that a MetaContact is added or removed from the
	 * MetaContactList.
	 *
	 * @param sourceContact
	 * 		the contact that this event is about.
	 * @param parentGroup
	 * 		the group that the source contact belongs or belonged to.
	 * @param eventID
	...
	/**
	 * Gets a copy of the list of current <code>MetaContactListListener</code> interested in
	 * events fired by this instance.
	 *
	 * @return an array of <code>MetaContactListListener</code>s currently interested in events
	...
	private MetaContactListListener[] getMetaContactListListeners()
	{
		...
		synchronized (metaContactListListeners) {
			listeners = metaContactListListeners.toArray(
					new MetaContactListListener[metaContactListListeners.size()]);
		}
		return listeners;
	}

	/**
	 * Creates the corresponding <tt>MetaContactPropertyChangeEvent</tt> instance and notifies all
	 * <tt>MetaContactListListener</tt>s that a MetaContact has been modified. Synchronized to
	...
	/**
	 * Creates the corresponding <tt>ProtoContactEvent</tt> instance and notifies all
	 * <tt>MetaContactListListener</tt>s that a protocol specific <tt>Contact</tt> has been added
	 ...
	 *
	 * @param source
	 * 		the contact that has caused the event.
	 * @param eventName
	 * 		One of the ProtoContactEvent.PROTO_CONTACT_XXX fields indicating the exact type of the
	 * 		event.
	 * @param oldParent
	 * 		the <tt>MetaContact</tt> that was wrapping the source <tt>Contact</tt> before the
	 * 		event occurred or <tt>null</tt> if the event is caused by adding a new <tt>Contact</tt>
	 * @param newParent
	 * 		the <tt>MetaContact</tt> that is wrapping the source <tt>Contact</tt> after the event
	 * 		occurred or <tt>null</tt> if the event is caused by removing a <tt>Contact</tt>
	 */
	 ...
	 *
	 * @param evt
	 * 		the ContactPresenceStatusChangeEvent describing the status change.
	 */
	 ...
	 *
	 * @param parentGroup
	 * 		the group that contains the meta contact group we're about to load.
	 * @param metaContactGroupUID
	 * 		the unique identifier of the meta contact group.
	 * @param displayName
	 * 		the name of the meta contact group.
	 ...
	 * specified arguments and adds it to <tt>containingMetaContactGroup</tt>
	 *
	 * @param containingMetaGroup
	 * 		the <tt>MetaContactGroupImpl</tt> where the restored contact group should be added.
	 * @param contactGroupUID
	 * 		the unique identifier of the group.
	 * @param parentProtoGroup
	 * 		the identifier of the parent proto group.
	 * @param persistentData
	 * 		the persistent data last returned by the contact group.
	 * @param accountID
	 * 		the ID of the account that the proto group belongs to.
	...
	public ContactGroup loadStoredContactGroup(MetaContactGroupImpl containingMetaGroup,
			String contactGroupUID, ContactGroup parentProtoGroup, String persistentData,
			String accountID)
	{
	 ...
	 *
	 * @param parentGroup
	 * 		the group that contains the meta contact we're about to load.
	 * @param metaUID
	 * 		the unique identifier of the meta contact.
	 * @param displayName
	 * 		the display name of the meta contact.
	 * @param details
	 * 		the details for the contact to create.
	 * @param protoContacts
	 * 		a list containing descriptors of proto contacts encapsulated by the meta contact that
	 * 		we're about to create.
	 * @param accountID
	 * 		the identifier of the account that the contacts originate from.
	 * @return the loaded meta contact.
	 */
	public MetaContactImpl loadStoredMetaContact(MetaContactGroupImpl parentGroup, String metaUID,
			String displayName, JSONObject details,
			List<MclStorageManager.StoredProtoContactDescriptor> protoContacts, String accountID)
	{
		...
		for (MclStorageManager.StoredProtoContactDescriptor contactDescriptor : protoContacts) {
			...
			if (mc != null) {
				logger.warn("Ignoring duplicate proto contact " + contactDescriptor
						+ " accountID = " + accountID + ". The contact was also present in the"
		...
		if (newMetaContact.getContactCount() == 0) {
			logger.error("Found an empty meta contact. Throwing an exception so that the storage"
					+ " manager would remove it.");
	 ...
	 * <tt>MetaContactListListener</tt>s that a MetaContactGroup is added or removed from the
	 ...
	 *
	 * @param source
	 * 		the MetaContactGroup instance that is added to the MetaContactList
	 * @param provider
	 * 		the ProtocolProviderService instance where this event occurred
	 * @param sourceProtoGroup
	 * 		the proto group associated with this event or null if the event does not concern a
	 * 		particular source group.
	 * @param eventID
	 * 		one of the METACONTACT_GROUP_XXX static fields indicating the nature of the event.
	 */
			...
			ProtocolProviderService provider, ContactGroup sourceProtoGroup, int eventID)
	{
		MetaContactGroupEvent evt
				= new MetaContactGroupEvent(source, provider, sourceProtoGroup, eventID);
	...
	private static class BlockingGroupEventRetriever implements ServerStoredGroupListener
	{
		 ...
		 *
		 * @param groupName
		 * 		the name of the group whose birth we're waiting for.
		 */
		 ...
		 *
		 * @param event
		 * 		a ServerStoredGroupChangeEvent containing a reference to the newly created group.
		 */
		 ...
		 *
		 * @param millis
		 * 		the number of ms that we should wait before we determine failure.
		 */
		public synchronized void waitForEvent(long millis)
		{
			...
			if (evt == null) {
				try {
					...
					logger.error("Interrupted while waiting for group creation", ex);
				}
			}
		}
	}
	 ...
	 */
	private static class BlockingSubscriptionEventRetriever implements SubscriptionListener,
			ServerStoredGroupListener
	{
		 ...
		 *
		 * @param event
		 * 		a ServerStoredGroupEvent containing a reference to the newly created group.
		 */
		public synchronized void groupCreated(ServerStoredGroupEvent event)
		{
			Contact contact = event.getSourceGroup().getContact(subscriptionAddress);
			if (contact != null) {
				this.evt = event;
		 ...
		 *
		 * @param event
		 * 		a <tt>SubscriptionEvent</tt> containing a reference to the newly created contact.
		 */
		public synchronized void subscriptionCreated(SubscriptionEvent event)
		{
			if (event.getSourceContact().getAddress().equals(subscriptionAddress)
		 ...
		 *
		 * @param event
		 * 		a <tt>SubscriptionEvent</tt> containing a reference to the contact we are trying
		 * 		to subscribe.
		 */
		 ...
		 *
		 * @param millis
		 * 		the number of ms to wait upon determining a failure.
		 */
		public synchronized void waitForEvent(long millis)
		{
			// no need to wait if an event is already there.
			if (evt == null) {
				try {
					...
					logger.error("Interrupted while waiting for contact creation", ex);
				}
			}
		}
	}
	...
	public void supportedOperationSetsChanged(ContactCapabilitiesEvent event)
	{
		...
		MetaContactImpl metaContactImpl
				= (MetaContactImpl) findMetaContactByContact(event.getSourceContact());
		...
		fireProtoContactEvent(contact, ProtoContactEvent.PROTO_CONTACT_MODIFIED, metaContactImpl,
				metaContactImpl);
	}
}
