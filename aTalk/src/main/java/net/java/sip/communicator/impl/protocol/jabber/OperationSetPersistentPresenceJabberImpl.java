		...
		implements VCardAvatarListener, UserAvatarListener
{
	...
	/**
	 * A map containing bindings between aTalk's jabber presence status instances and Jabber status
	 * codes
	 */
	 ...
	 *
	 * @param listener
	 * 		a ServerStoredGroupChangeListener impl that would receive events upon group changes.
	 */
	@Override
			...
			throws OperationFailedException
	{
		...
		if (!parent.canContainSubgroups())
			throw new IllegalArgumentException(
					"The specified contact group cannot contain child groups. Group:" + parent);
	 ...
	 *
	 * @param id
	 * 		the address of the contact to create.
	 ...
	 * @param id
	 * 		the address of the contact to create.
	 * @param isPrivateMessagingContact
	 * 		indicates whether the contact should be private messaging contact or not.
	 * @return the newly created volatile <tt>ContactImpl</tt>
	 */
	public synchronized ContactJabberImpl createVolatileContact(String id,
			...
			boolean isPrivateMessagingContact, String displayName)
	{
		...
		if ((notInContactListGroup != null) && (sourceContact
				= notInContactListGroup.findContact(XmppStringUtils.parseBareJid(id))) != null)
			return sourceContact;
		else {
			sourceContact = ssContactList.createVolatileContact(id, isPrivateMessagingContact,
					displayName);
	 ...
	 *
	 * @param address
	 * 		an identifier of the contact that we'll be creating.
	 * @param persistentData
	 * 		a String returned Contact's getPersistentData() method during a previous run and that
	 * 		has been persistently stored locally.
	 * @param parentGroup
	 * 		the group where the unresolved contact is supposed to belong to.
			...
			ContactGroup parentGroup)
	{
		if (!(parentGroup instanceof ContactGroupJabberImpl
			...
			throw new IllegalArgumentException("Argument is not an jabber contact group (group="
					+ parentGroup + ")");
	 ...
	 *
	 * @param groupUID
	 * 		an identifier, returned by ContactGroup's getGroupUID, that the protocol provider may
	 * 		use in order to create the group.
	 * @param persistentData
	 * 		a String returned ContactGroups's getPersistentData() method during a previous run and
	 * 		that has been persistently stored locally.
	 * @param parentGroup
	 * 		the group under which the new group is to be created or null if this is group directly
	 * 		underneath the root.
	 * @return the unresolved <tt>ContactGroup</tt> created from the specified <tt>uid</tt> and
	 * <tt>persistentData</tt>
	 */
	public ContactGroup createUnresolvedContactGroup(String groupUID, String persistentData,
			ContactGroup parentGroup)
	{
	 ...
	 *
	 * @param contactID
	 * 		a String identifier of the contact which we're seeking a reference of.
	 ...
	 */
	private ContactResourceJabberImpl createResource(Presence presence, Jid fullJid,
			Contact contact)
	{
	 ...
	 *
	 * @param contactToMove
	 * 		the <tt>Contact</tt> to move
	 * @param newParent
	 * 		the <tt>ContactGroup</tt> where <tt>Contact</tt> would be placed.
	 */
			...
			throws OperationFailedException
	{
		...
		if (!(newParent instanceof AbstractContactGroupJabberImpl))
			throw new IllegalArgumentException(
					"The specified group is not an jabber contact group." + newParent);
	 ...
	 *
	 * @param status
	 * 		the PresenceStatus as returned by getSupportedStatusSet
	 * @param statusMessage
	 * 		the message that should be set as the reason to enter that status
	 * @throws IllegalArgumentException
	 * 		if the status requested is not a valid PresenceStatus supported by this provider.
	 * @throws IllegalStateException
	 * 		if the provider is not currently registered.
	 * @throws OperationFailedException
	 * 		with code NETWORK_FAILURE if publishing the status fails due to a network error.
	 */
			...
			throws IllegalArgumentException, IllegalStateException, OperationFailedException
	{
		...
		for (
				Iterator<PresenceStatus> supportedStatus
	 ...
	 * 		the identifier of the contact whose status we're interested in.
	 * @return the <tt>PresenceStatus</tt> of the contact with the specified
	 * <tt>contactIdentifier</tt>
	 * @throws IllegalArgumentException
	 * 		if the specified <tt>contactIdentifier</tt> does not identify a contact known to the
	 * 		underlying protocol provider
	 * @throws IllegalStateException
	 * 		if the underlying protocol provider is not registered/signed on a public service
	 * @throws OperationFailedException
	 * 		with code NETWORK_FAILURE if retrieving the status fails due to errors experienced
	 ...
	 *
	 * @param listener
	 * 		the ServerStoredGroupChangeListener to remove
	 */
	@Override
	 ...
	 *
	 * @param handler
	 * 		an instance of an AuthorizationHandler for authorization requests coming from other
	 ...
	 *
	 * @param contactIdentifier
	 * 		the identifier of the contact whose status updates we are subscribing for.
	 * 		<p>
	 * @throws IllegalArgumentException
	 ...
	 * 		if the underlying protocol provider is not registered/signed on a public service.
	 * @throws OperationFailedException
	 * 		with code NETWORK_FAILURE if unSubscribing fails due to errors experienced during
	 * 		network communication
	 */
			...
			throws IllegalArgumentException, IllegalStateException, OperationFailedException
	{
		...
		if (!(contact instanceof ContactJabberImpl))
			throw new IllegalArgumentException("Argument is not an jabber contact (contact = "
					+ contact + ")");
	 ...
	 * @throws IllegalArgumentException
	 * 		if <tt>contact</tt> is not an instance that belongs to the underlying implementation.
	 */
	@Override
		...
		public void registrationStateChanged(RegistrationStateChangeEvent evt)
		{
			...
			if (eventNew == RegistrationState.REGISTERING) {
				...
				/* Add avatar change listener to handle contacts' avatar changes via XEP-0153*/
					...
					vCardAvatarManager.addVCardAvatarChangeListener(
							OperationSetPersistentPresenceJabberImpl.this);
				...
				/* Add avatar change listener to handle contacts' avatar changes via XEP-0084 */
					...
					userAvatarManager.addAvatarListener(
							OperationSetPersistentPresenceJabberImpl.this);
					 ...
					 */
					OperationSetServerStoredAccountInfo accountInfoOpSet = parentProvider
				...
				if (xmppConnection != null) {
					...
					if (vCardAvatarManager != null) {
						vCardAvatarManager.removeVCardAvatarChangeListener(
								OperationSetPersistentPresenceJabberImpl.this);
						userAvatarManager.removeAvatarListener(
								OperationSetPersistentPresenceJabberImpl.this);
					}
				}
	 ...
	 *
	 * @param contact
	 * 		the contact which resources to update.
	 * @param removeUnavailable
	 * 		whether to remove unavailable resources.
	...
	private boolean updateResources(ContactJabberImpl contact, boolean removeUnavailable)
	{
		...
		if (xmppConnection == null || !xmppConnection.isConnected()) {
			if (removeUnavailable) {
				Iterator<Map.Entry<Jid, ContactResourceJabberImpl>> iter
	 ...
	 * 		the contact which resources to update.
	 * @param fullJid
	 * 		the full jid to use, if null will use those from the presence stanza
	 * @param presence
	 * 		the presence stanza to use to get info.
	 * @return whether resource has been updated
	 */
	private boolean updateResource(ContactJabberImpl contact, Jid fullJid, Presence presence)
	{
		...
		if (resource != null) {
			...
			if (contactResource == null) {
				...
				boolean newIndicator = mobileIndicator.isMobileResource(resource.toString(),
						fullJid);
				...
				if (oldPriority != contactResource.getPriority()) {
					// priority has been updated so update and the mobile indicator before
					// firing an event
	 ...
	 *
	 * @param contact
	 * 		from its list of resources to remove
	 * @param fullJid
	 * 		the full jid.
	 * @return whether resource has been updated
	 */
	 ...
	 *
	 * @param contact
	 * 		the contact which presence to update if needed.
	 * @param newStatus
	 * 		the new status.
	 */
	...
	class ContactChangesListener implements RosterListener
	{
		...
		void firePresenceStatusChanged(Presence presence)
		{
			...
			try {
						...
						public int compare(Presence o1, Presence o2)
						{
							...
							if (res == 0) {
								res = jabberStatusToPresenceStatus(o2, parentProvider).getStatus()
										- jabberStatusToPresenceStatus(o1,
										parentProvider).getStatus();
				...
				ContactJabberImpl sourceContact
						= ssContactList.findContactById(userJid.toString());
				...
				updateContactStatus(sourceContact,
	...
	private class JabberSubscriptionListener implements StanzaListener
	{
		 ...
		 *
		 * @param stanza
		 * 		stanza received to be processed
		 */
		public void processStanza(Stanza stanza)
		{
			Presence presence = (Presence) stanza;
			...
			new Thread(new Runnable()
			{
				public void run()
				{
					...
					if (responsePresenceType == null) {
						...
						AuthorizationResponse response = handler.processAuthorisationRequest(req,
								srcContact);
...
//	}
	 ...
	 *
	 * @param userID
	 * 		The contact of the sent <presence/> stanza.
	 * @param avatarHash
	 ...
	 * @param vCardInfo
	 * 		The contact VCard info - can contain null.
	 */
	@Override
	 ...
	 */
	public void onAvatarChange(EntityBareJid from, String avatarId,
			List<AvatarMetadata.Info> avatarInfo)
	{
		/*
		 * Retrieves the contact ID that aTalk currently managed concerning the peer that has
		 * send this presence stanza with avatar update.
		 ...
		 * {@link UserAvatarManager#downloadAvatar(EntityBareJid, String,
		 * AvatarMetadata.Info)}
		 */
	 ...
	 *
	 * @param statusName
	 * 		the status to check/create priority
	 * @param availableShift
	 * 		the difference from available resource value to use.
	 */
	 ...
	 *
	 * @param statusName
	 * 		the status name to use
	 * @param value
	 * 		and its priority
	 */
	private static void addPresenceToPriorityMapping(String statusName, int value)
	{
		statusToPriorityMappings.put(statusName.replaceAll(" ", "_").toUpperCase(Locale.US),
				value);
	}
