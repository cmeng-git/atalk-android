 ...
 * user/account that is currently logged in through this provider. This operation set is more or
 * less reciproce to OperationSetServerStoredContactInfo with the most essential difference being
 * the fact that the account info operation set allows you to modify data in addition to reading it
 * (quite natural given that it's your own info that you're dealing with).
 * <p>
 * Examples of account details are your picture, postal or e-mail addresses, work, hobbies,
 * interests, and many many others.
 * <p>
 * Various types of details have been defined in the ServerStoredDetails class and can be used with
 * the get methods of this interface. Implementors may also define their own details by extending or
 * instantiating the ServerStoredDetails.GenericDetail class. Yet, they are encouraged to use
 * existing detail types as fully as possible. Defining your own detail type may lead to limited
 * visualization of its value.
 * <p>
 * As mentioned earlier the operation set supports adding, removing or replaing various details. The
 * exact set of details that can be manipulated through this operation set depends on the
 * implementation and can be retrieved through the getSupportedDetailTypes() method. The maximum
 * number of detail instances supported for a given type of details can be retrieved through the
 * getMaxDetailInstances() method.
 * <p>
 * The OperationSetServerStoredAccountInfo only concerns us (the user currently logged through this
 * provider) and our own details. In order to query details concerning Contacts in our contact list
 * we'd need to use the OperationSetServerStoredContactInfo
 *
 * @author Emil Ivov
 */
public interface OperationSetServerStoredAccountInfo extends OperationSet
{
	 ...
	 * class. If for example an our account has a workaddress and an address detail, a call to this
	 * method with AddressDetail.class would return both of them.
	 * <p>
	 * 
	 * @param detailClass
	 *        one of the detail classes defined in the ServerStoredDetails class, indicating the
	...
	/**
	 * Returns an iterator over all details that are instances of exactly the same class as the one
	 * specified. Not that, contrary to the getDetailsAndDescendants() method this one would only
	 * return details that are instances of the specified class and not only its descendants. If for
	 * example our account has both a work address and an address detail, a call to this method with
	 * AddressDetail.class would return only the AddressDetail instance and not the
	 ...
	 * 
	 * @param detailClass
	 *        the class the support for which we'd like to determine.
	 *        <p>
	 * @return true if the underlying implementation supports setting details of this type and false
	 *         otherwise.
	 */
	 ...
	 * 
	 * @param detailClass
	 *        the class whose edition we'd like to determine if it's possible
	 ...
	 * protocols offer storing mutliple values for a particular detail type. Spoken languages are a
	 * good example.
	 * 
	 * @param detailClass
	 *        the class whose max instance number we'd like to find out.
	 *        <p>
	 ...
	 *         if such a detail already exists and its max instances number has been attained or if
	 *         the underlying implementation does not support setting details of the corresponding
	 *         class.
	 * @throws java.lang.ArrayIndexOutOfBoundsException
	 *         if the number of instances currently registered by the application is already equal
	...
	public void addDetail(ServerStoredDetails.GenericDetail detail)
		throws IllegalArgumentException, OperationFailedException, ArrayIndexOutOfBoundsException;
	 ...
	 * <p>
	 * 
	 * @param currentDetailValue
	 *        the detail value we'd like to replace.
	 * @param newDetailValue
	 *        the value of the detail that we'd like to replace currentDetailValue with.
	 * @throws ClassCastException
	 *         if newDetailValue is not an instance of the same class as currentDetailValue.
	 * @throws OperationFailedException
	 *         with code Network Failure if putting the new value back online has failed
	 */
	 ...
	 * This method performs the actual saving of details online on the server and is supposed to be
	 * invoked after addDetail(), replaceDetail() and/or removeDetail().
	 * <p>
	 * 
	 * @throws OperationFailedException
	 *         with code Network Failure if putting the new values back online has failed.
	 */
	public void save()
		throws OperationFailedException;
	 ...
	 *
	 * @param listener
	 *        the <tt>ServerStoredDetailsChangeListener</tt> to register.
	 */
	 ...
	 *
	 * @param listener
	 *        the <tt>ServerStoredDetailsChangeListener</tt> to unregister.
	 */
