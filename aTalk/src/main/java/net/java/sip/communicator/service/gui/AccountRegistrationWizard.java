...
public abstract class AccountRegistrationWizard
{
	 ...
	 * 		The <tt>ProtocolProviderService</tt> to load data from.
	 */
	 ...
	 * 		if the operation didn't succeed
	 */
	 ...
	 * 		additional account parameter for login
	 * @return the created <tt>ProtocolProviderService</tt> corresponding to the new account
	 * @throws OperationFailedException
	 * 		if the operation didn't succeed
	 */
	public abstract ProtocolProviderService signin(String userName, String password,
			Map<String, String> accountProperties)
			throws OperationFailedException;

	/**
	 * Indicates that the account corresponding to the given <tt>protocolProvider</tt> has been
	 * removed.
	 *
	 * @param protocolProvider
	 * 		the protocol provider that has been removed
	 */
	...
	/**
	 * Returns <code>true</code> if the web sign up is supported by the current implementation,
	 ...
	 * 		if the web sign up operation is not supported by the current implementation.
	 */
	...
	/**
	 * Returns <code>true</code> if the inBand registration is supported by the current
	 ...
	 */
	public void registerAccount(Context context, Jid userJid, String password,
	 ...
	 * Only if the user needs more settings she'll choose to open the advanced wizard, consisted
	 * by all pages.
	 *
	 * @param isCreateAccount
	 * 		indicates if the simple form should be opened as a create account form or as a login
	 * 		form
	...
	/**
	 * Whether the advanced configuration is enabled. Gives an option to disable/hide advanced
	 * config button.
	 *
	...
	public boolean isPreferredProtocol()
	{
		...
		String prefWName = AndroidGUIActivator.getResources().
				getSettingsString("gui.PREFERRED_ACCOUNT_WIZARD");

		return (!StringUtils.isNullOrEmpty(prefWName)
