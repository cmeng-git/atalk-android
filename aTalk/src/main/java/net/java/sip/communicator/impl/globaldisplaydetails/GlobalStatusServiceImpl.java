...
public class GlobalStatusServiceImpl
		implements GlobalStatusService, RegistrationStateChangeListener
{
	/**
	 ...
	 *
	 * @param status
	 * 		the status to which the item should correspond
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider to which the status corresponds
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider to which we change the status.
	 * @param status
	 ...
	 * 		the status to publish.
	 * @param state
	 * 		whether the publish status is invoked after registrationStateChanged for a provider,
	 ...
	 */
	@Override
	public void publishStatus(ProtocolProviderService protocolProvider,
			PresenceStatus status, boolean state)
	{
	 ...
	 * @param status
	 * 		the status to publish.
	 * @param dueToRegistrationStateChanged
	 * 		whether the publish status is invoked after registrationStateChanged for a provider,
	 * 		where the provider is expected to be REGISTERED, if not we do nothing
	 * 		(means it has connection failed soon after firing registered).
	 */
	 ...
	 *
	 * @param globalStatus
	 * 		account status indicator on action bar
	 */
	 ...
	 * @param protocolProvider
	 * 		the protocol provider to which we change the status.
	 * @param floorStatusValue
	 * 		the min status value.
	 * @param ceilStatusValue
	 * 		the max status value.
	 */
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider to save status information for
	 * @param statusName
	 ...
	 *
	 * @param evt
	 * 		a <tt>RegistrationStateChangeEvent</tt> which describes the
	 */
	@Override
	...
	private class PublishPresenceStatusThread extends Thread
	{
		 ...
		 *
		 * @param presence
		 * 		the operation set through which we publish the status
		 * @param status
		 * 		the status to publish
		 */
		...
		public void run()
		{
			try {
				...
				if (e1.getErrorCode() == OperationFailedException.GENERAL_ERROR) {
					String msgText = GlobalDisplayDetailsActivator.getResources().getI18NString(
					...
					GlobalDisplayDetailsActivator.getAlertUIService().showAlertDialog(
							GlobalDisplayDetailsActivator.getResources().getI18NString(
