...
public class LogUploadServiceImpl implements LogUploadService
{
	 ...
	 * email is sent and deleting temp log file, so they are cached and removed on OSGI service
	 * stop action.
	 */
	...
	private static final String storagePath
			= Environment.getExternalStorageDirectory().getPath() + "/atalk-logs/";
	 ...
	 * @param title
	 * 		the title for the action, used any intermediate dialogs that need to be shown, like
	 * 		"Choose action:".
	 */
	public void sendLogs(String[] destinations, String subject, String title)
	{
		try {
			...
			try {
				logcatFile = LoggingUtilsActivator.getFileAccessService()
