		...
		implements UserCapsNodeListener, ContactPresenceStatusListener
{
	 ...
	 * 		the <tt>ProtocolProviderServiceJabberImpl</tt> which will provide the new instance
	 */
		...
		if (supportedOperationSetCount != 0) {
			for (Map.Entry<String, OperationSet> supportedOperationSetEntry
				...
				try {
					...
					opsetClass = null;
					logger.error("Failed to get OperationSet class for name: "
							+ opsetClassName, cnfex);
				}
				if (opsetClass != null) {
	 ...
	 * 		to be checked whether it possesses it as a capability
	 * @param jid
	 * 		the Jabber id for which we're checking supported operation sets
	 * @param opsetClass
	 * 		the <tt>OperationSet</tt> <tt>Class</tt> for which the specified <tt>contact</tt> is
	 * 		to be checked whether it possesses it as a capability
	 * @param online
	 ...
	 * @see AbstractOperationSetContactCapabilities#getOperationSet(Contact, Class)
	 */
	private <U extends OperationSet> U getOperationSet(Jid jid, Class<U> opsetClass,
			boolean online)
	{
	 ...
	 *
	 * @param discManager
	 * 		the <tt>ScServiceDiscoveryManager</tt> which is the <tt>discoveryManager</tt> of
	 * 		{@link #parentProvider}
	 */
		...
		if (opsetPresence != null) {
			...
			if (contact != null) {
				if (online) {
					...
					fireContactCapabilitiesEvent(contact,
	 ...
	 *
	 * @param user
	 * 		the user Jid to search for its contact.
	 */
	public void fireContactCapabilitiesChanged(Jid user)
	{
		...
		if (opsetPresence != null) {
			...
			Presence presence = Roster.getInstanceFor(parentProvider.getConnection())
					.getPresence(user.asBareJid());
			if (presence != null)
