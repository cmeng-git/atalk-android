...
public class CallPeerMediaHandlerJabberImpl extends AbstractCallPeerMediaHandlerJabberGTalkImpl<CallPeerJabberImpl>
{
	 ...
	 * 		the <tt>ScServiceDiscoveryManager</tt> to be checked whether it includes the specified
	 * 		feature
	 * @param discoverInfo
	 * 		the <tt>DiscoveryInfo</tt> which is to be checked whether it contains the specified
	 * 		feature. If <tt>discoverInfo</tt> is <tt>null</tt>, it is considered to contain the
	 * 		specified feature.
	 * @param feature
	 * 		the feature to be determined whether it is supported by both the specified
	...
	/**
	 * Whether other party is able to change video quality settings. Normally its whether we have
	 ...
	 *
	 * @param peer
	 * 		that <tt>CallPeerJabberImpl</tt> instance that we will be managing media for.
	 */
	 ...
	 *
	 * @param stream
	 * 		the {@link MediaStreamTarget} whose post-hold direction we'd like to determine.
	 ...
	 * 		if we fail to create the descriptions for reasons like problems with device
			...
			throws OperationFailedException
	{
		...
		if (call.isConferenceFocus()) {
			for (CallPeerJabberImpl anotherPeer : call.getCallPeerList()) {
				if ((anotherPeer != peer) && anotherPeer.getDirection(
						mediaType).allowsReceiving()) {
	...
	/**
	 * Generates an Jingle {@link ContentPacketExtension} for the specified {@link MediaFormat}
	 * list, direction and RTP extensions taking account the local streaming preference for the
	 * corresponding media type.
	 *
	 * @param supportedFormats
	 * 		the list of <tt>MediaFormats</tt> that we'd like to advertise.
	 * @param direction
	 * 		the <tt>MediaDirection</tt> that we'd like to establish the stream in.
	 * @param supportedExtensions
	 * 		the list of <tt>RTPExtension</tt>s that we'd like to advertise in the
	 * 		<tt>MediaDescription</tt>.
	 * @return a newly created {@link ContentPacketExtension} representing streams that we'd be
	 * able to handle.
	 */
	private ContentPacketExtension createContentForOffer(List<MediaFormat> supportedFormats,
	...
	/**
	 * Creates a <tt>List</tt> containing the {@link ContentPacketExtension}s of the streams that
			...
			throws OperationFailedException
	{
		...
		for (MediaType mediaType : MediaType.values()) {
			...
			if (isDeviceActive(dev)) {
				...
				if (direction != MediaDirection.INACTIVE) {
					ContentPacketExtension content = createContentForOffer(
							getLocallySupportedFormats(dev), direction,
		...
		if (mediaDescs.isEmpty()) {
			ProtocolProviderServiceJabberImpl.throwOperationFailedException(
					AndroidGUIActivator.getResources()
							.getI18NString("service.gui.CALL_NO_ACTIVE_DEVICE"),
	...
	/**
	 * Creates a <tt>List</tt> containing the {@link ContentPacketExtension}s of the streams of a
	 ...
	 *
	 * @param property
	 * 		the name of the property of this <tt>PropertyChangeNotifier</tt> which had its value
	 * 		changed
	 * @param oldValue
	 * 		the value of the property with the specified name before the change
	 * @param newValue
	 ...
	 *
	 * @return the last generated list of {@link ContentPacketExtension}s that the call peer could
	 * use to send a <tt>session-accept</tt>.
	 * @throws OperationFailedException
	 * 		if we fail to configure the media stream
	 */
			...
			throws OperationFailedException
	{
		...
		for (Map.Entry<ContentPacketExtension, RtpDescriptionPacketExtension> en
			...
			// if we answer with video, tell remote peer that video direction is sendrecv, and
			...
			for (PayloadTypePacketExtension payload : theirDescription.getPayloadTypes()) {
				MediaFormat remoteFormat = JingleUtils.payloadTypeToMediaFormat(payload,
						getDynamicPayloadTypes());
				if ((remoteFormat != null)
						&& (format = findMediaFormat(localFormats, remoteFormat)) != null)
					break;
			}
			if (format == null) {
				ProtocolProviderServiceJabberImpl.throwOperationFailedException(
			...
			// extract the extensions that we are advertising: check whether we will be exchanging
			// any RTP extensions.
			List<RTPExtension> rtpExtensions = JingleUtils.extractRTPExtensions(description,
			...
			MediaStream stream = initStream(contentName, connector, dev, format, target, direction,
	 ...
	 *
	 * @param description
	 * 		the <tt>RtpDescriptionPacketExtension</tt> to which a child element will be added.
	 * @param ssrc
	 * 		the SSRC for the <tt>SourcePacketExtension</tt> to use.
	 */
	private void addSourceExtension(RtpDescriptionPacketExtension description, long ssrc)
	{
		MediaType type = MediaType.parseString(description.getMedia());
		...
		sourcePacketExtension.addChildExtension(
				new ParameterPacketExtension("cname", LibJitsi.getMediaService().getRtpCname()));
		sourcePacketExtension.addChildExtension(
	 ...
	 * implementation does not currently provide support for keeping track of multiple remote
	 * SSRCs.
	 * </p>
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the RTP streams the SSRCs of which are to be returned
	 * @return an array of <tt>int</tt> values which represent the SSRCs of RTP streams with the
	 * specified <tt>mediaType</tt> known to be received by a <tt>MediaStream</tt>
			...
			if (peer.isInitiator()) {
				...
				synchronized (supportedTransportsSyncRoot) {
					if (supportedTransports != null && supportedTransports.length > 0) {
						for (String supportedTransport : supportedTransports) {
							...
							else if (ProtocolProviderServiceJabberImpl.URN_XMPP_JINGLE_RAW_UDP_0
									.equals(supportedTransport)) {
						...
						if (transportManager == null) {
							logger.warn("Could not find a supported TransportManager in"
				...
				if (transportManager == null) {
					...
					if ((transportManager == null) && logger.isDebugEnabled()) {
						logger.debug("No known Jingle transport supported by Jabber"
								+ " call peer " + peer);
					}
				}
			}
			...
			if (stream == null)
				...
				if (remoteSSRCs.length == 0)
					...
					for (int remoteSSRC : remoteSSRCs) {
						Component visualComponent = videoStream.getVisualComponent(
	 ...
	 *
	 * @param remote
	 * 		the media descriptions received from the remote peer if any or <tt>null</tt> if
	 * 		<tt>local</tt> represents an offer from the local peer to be sent to the remote peer
	 * @param local
	 * 		the media descriptions sent or to be sent from the local peer to the remote peer. If
	 ...
	 * @throws OperationFailedException
	 * 		if anything goes wrong while starting or wrapping up the gathering of local candidate
	 * 		addresses
	 */
			...
			throws OperationFailedException
	{
		...
		if (remote == null) {
			/*
			 * We'll be harvesting candidates in order to make an offer so it doesn't make sense to
			 * send them in transport-info.
			 */
		...
		if (transportManager.startConnectivityEstablishmentWithJitsiVideobridge) {
			...
			for (MediaType mediaType : MediaType.values()) {
				ColibriConferenceIQ.Channel channel = transportManager.getColibriChannel(mediaType,
						true /* local */);
	 ...
	 *
	 * @param streamName
	 * 		the name of the stream as indicated in the XMPP <tt>content</tt> element.
	 * @param connector
	 * 		the <tt>MediaConnector</tt> that we'd like to bind the newly created stream to.
	 * @param device
	 * 		the <tt>MediaDevice</tt> that we'd like to attach the newly created
	 * 		<tt>MediaStream</tt> to.
	 * @param format
	 * 		the <tt>MediaFormat</tt> that we'd like the new <tt>MediaStream</tt> to be set to
	 * 		transmit in.
	 * @param target
	 * 		the <tt>MediaStreamTarget</tt> containing the RTP and RTCP address:port couples that
	 * 		the new stream would be sending packets to.
	 * @param direction
	 * 		the <tt>MediaDirection</tt> that we'd like the new stream to use (i.e. sendonly,
	 * 		sendrecv, recvonly, or inactive).
	 * @param rtpExtensions
	 * 		the list of <tt>RTPExtension</tt>s that should be enabled for this stream.
	 * @param masterStream
	 * 		whether the stream to be used as master if secured
	 * @return the newly created <tt>MediaStream</tt>.
	 * @throws OperationFailedException
	 * 		if creating the stream fails for any reason (like for example accessing the device or
	 * 		setting the format).
	 */
	protected MediaStream initStream(String streamName, StreamConnector connector,
			...
			throws OperationFailedException
	{
		MediaStream stream = super.initStream(connector, device, format, target, direction,
				rtpExtensions, masterStream);
	...
	/**
	 * Handles the specified <tt>answer</tt> by creating and initializing the corresponding
	 * <tt>MediaStream</tt>s.
	 *
	 * @param answer
	 * 		the Jingle answer
	 * @throws OperationFailedException
	 * 		if we fail to handle <tt>answer</tt> for reasons like failing to initialize media
	 * 		devices or streams.
	 * @throws IllegalArgumentException
	 * 		if there's a problem with the syntax or the semantics of <tt>answer</tt>. Method is
	 ...
	 *
	 * @param conferenceIQ
	 * 		the <tt>ColibriConferenceIQ</tt> which has been received
	 */
	void processColibriConferenceIQ(ColibriConferenceIQ conferenceIQ)
	{
		...
		if (transportManager != null) {
			...
			for (MediaType mediaType : MediaType.values()) {
				ColibriConferenceIQ.Channel dst = transportManager.getColibriChannel(mediaType,
						false /* remote */);

				if (dst != null) {
					ColibriConferenceIQ.Content content = conferenceIQ.getContent(mediaType
							.toString());
	 ...
	 *
	 * @param content
	 * 		a <tt>ContentPacketExtension</tt>
	 * @param modify
	 * 		if it correspond to a content-modify for resolution change
	 * @param masterStream
	 * 		whether the stream to be used as master
	 * @throws OperationFailedException
	 * 		if we fail to handle <tt>content</tt> for reasons like failing to initialize media
	 * 		devices or streams.
	 * @throws IllegalArgumentException
	 * 		if there's a problem with the syntax or the semantics of <tt>content</tt>. The method
			...
			throws OperationFailedException, IllegalArgumentException
	{
		...
		List<MediaFormat> supportedFormats = JingleUtils.extractFormats(description,
				getDynamicPayloadTypes());
		...
		/*
		 * If we are the focus of a conference, we need to take into account the other
		 * participants.
		 */
		...
		List<RTPExtension> remoteRTPExtensions = JingleUtils.extractRTPExtensions(description,
				getRtpExtensionsRegistry());
		List<RTPExtension> supportedExtensions = getExtensionsForType(mediaType);
		List<RTPExtension> rtpExtensions = intersectRTPExtensions(remoteRTPExtensions,
				supportedExtensions);
	 ...
	 *
	 * @param offer
	 * 		the offer that we'd like to parse, handle and get an answer for.
	 * @throws OperationFailedException
	 * 		if we have a problem satisfying the description received in <tt>offer</tt> (e.g.
	 * 		failed to open a device or initialize a stream ...).
	 * @throws IllegalArgumentException
	 * 		if there's a problem with <tt>offer</tt>'s format or semantics.
	 */
			...
			throws OperationFailedException, IllegalArgumentException
	{
		...
		for (ContentPacketExtension content : offer) {
			...
			MediaDirection devDirection = (dev == null)
					? MediaDirection.INACTIVE : dev.getDirection();
			...
			List<RTPExtension> rtpExtensions
					= intersectRTPExtensions(offeredRTPExtensions, supportedExtensions);

			/*
			 * Transport: RawUdpTransportPacketExtension extends IceUdpTransportPacketExtension so
			 * getting IceUdpTransportPacketExtension should suffice.
			 */
			...
			// "MAY instead be empty (with each candidate to be sent as the payload of a
			...
			ContentPacketExtension ourContent = JingleUtils.createDescription(content.getCreator(),
		...
		if (!atLeastOneValidDescription) {
			...
			AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
					...
					AndroidGUIActivator.getResources()
							.getI18NString("service.gui.CALL_NO_MATCHING_FORMAT_H",
		...
		/*
		 * In order to minimize post-pickup delay, start establishing the connectivity prior to
		 * ringing.
		 */
	 ...
	 * specific set of <tt>ContentPacketExtension</tt>s.
	 *
	 * @param contents
	 * 		the <tt>ContentPacketExtenion</tt>s provided by the remote <tt>peer</tt> and
	 * 		containing the transport-related information to be processed
	 * @throws OperationFailedException
	 * 		if anything goes wrong while processing the transport-related information provided by
	 ...
	 * 		devices or streams.
	 * @throws IllegalArgumentException
	 * 		if there's a problem with the syntax or the semantics of <tt>content</tt>. Method is
	 * 		synchronized in order to avoid closing mediaHandler when we are currently in process
	 * 		of initializing, configuring and starting streams and anybody interested in this
	 * 		operation can synchronize to the mediaHandler instance to wait processing to stop
	 * 		(method setState in CallPeer).
	 ...
	 *
	 * @param contentMap
	 * 		the <tt>Map</tt> in which the specified <tt>name</tt> has an association with the
	 ...
	 *
	 * @param onHold
	 * 		<tt>true</tt> if the remote party has put us on hold and <tt>false</tt> if they've
		...
		for (MediaType mediaType : MediaType.values()) {
			...
			if (getPeer().isJitsiVideobridge()) {
				...
				if (remotelyOnHold) {
					 ...
					 */
					MediaDirection newDirection = getPeer().getCall().isConferenceFocus()
	 ...
	 *
	 * @param value
	 * 		whether quality controls is supported..
	 */
	 ...
	 *
	 * @param xmlns
	 * 		the Jingle transport XML namespace specifying the <tt>TransportManager</tt>
	 * 		implementation type to be set on this instance to handle our address management
	 * @throws IllegalArgumentException
	 * 		if the specified <tt>xmlns</tt> does not specify a (supported)
	 ...
	 * 		if no offer or answer has been provided or generated earlier
	 */
	@Override
	public void start()
			throws IllegalStateException
	 ...
	 *
	 * @throws OperationFailedException
	 * 		if anything goes wrong while setting the <tt>connector</tt>s and/or <tt>target</tt>s
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> for which to return a <tt>ColibriConferenceIQ.Channel</tt>
	 ...
	 *
	 * @param locallyOnHold
	 * 		<tt>true</tt> if we are to make our streams stop transmitting and <tt>false</tt> if we
		...
		if (peer.isJitsiVideobridge()) {
			...
			if (locallyOnHold || !CallPeerState.ON_HOLD_MUTUALLY.equals(peer.getState())) {
				for (MediaType mediaType : MediaType.values()) {
					...
					if (channel != null) {
						MediaDirection direction = locallyOnHold ? MediaDirection.INACTIVE
	 ...
	 *
	 * @param isInitiator
	 * 		<tt>true</tt> if the local call instance is the initiator of the call; <tt>false</tt>,
	 * 		otherwise.
	 * @param content
	 * 		The CONTENT element of the JINGLE element which contains the TRANSPORT element
	 * @param mediaType
	 * 		The type of media (AUDIO or VIDEO).
	 */
	private boolean addDtlsAdvertisedEncryptions(boolean isInitiator,
			ContentPacketExtension content, MediaType mediaType)
	{
		if (getPeer().isJitsiVideobridge()) {
	...
	/**
	 * Detects and adds DTLS-SRTP available encryption method present in the transport
	...
	boolean addDtlsAdvertisedEncryptions(boolean isInitiator,
			IceUdpTransportPacketExtension remoteTransport, MediaType mediaType)
	{
		...
		if (remoteTransport != null) {
			...
			if (!remoteFingerpintPEs.isEmpty()) {
				...
				if (accountID.getAccountPropertyBoolean(
					...
					if (isInitiator) {
						dtlsControl = (DtlsControl) srtpControls.get(mediaType,
								SrtpControlType.DTLS_SRTP);
						setup = DtlsControl.Setup.PASSIVE;
					}
					else {
						dtlsControl = (DtlsControl) srtpControls.getOrCreate(mediaType,
								SrtpControlType.DTLS_SRTP);
	 ...
	 * 		The type of media (AUDIO or VIDEO).
	 * @param localContent
	 * 		The element containing the media DESCRIPTION and its encryption.
	 * @param remoteContent
	 * 		The element containing the media DESCRIPTION and its encryption for the remote peer;
	...
	{
		List<SrtpControlType> preferredEncryptionProtocols = getPeer().getProtocolProvider()
				.getAccountID().getSortedEnabledEncryptionProtocolList();
		...
		for (SrtpControlType srtpControlType : preferredEncryptionProtocols) {
			...
			if (srtpControlType == SrtpControlType.DTLS_SRTP) {
				...
				RtpDescriptionPacketExtension localDescription = (localContent == null) ? null
						: JingleUtils.getRtpDescription(localContent);
				RtpDescriptionPacketExtension remoteDescription = (remoteContent == null) ? null
						: JingleUtils.getRtpDescription(remoteContent);

				if (setAndAddPreferredEncryptionProtocol(srtpControlType, mediaType,
	 ...
	 *
	 * @param mediaType
	 * 		The type of media we are modifying the CONTENT to integrate the DTLS-SRTP element(s).
	 * @param localContent
	 * 		The element containing the media CONTENT and its TRANSPORT.
	 * @param remoteContent
	 * 		The element containing the media CONTENT and its TRANSPORT for the remote peer. Null,
	 ...
	 */
	private boolean setDtlsEncryptionOnContent(MediaType mediaType,
			ContentPacketExtension localContent, ContentPacketExtension remoteContent)
	{
		CallPeerJabberImpl peer = getPeer();
		boolean b = false;
		...
		if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION, true)
			...
			if (addFingerprintToLocalTransport) {
				DtlsControl dtlsControl = (DtlsControl) srtpControls.getOrCreate(mediaType,
						SrtpControlType.DTLS_SRTP);
			...
			if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION,
				...
				if ((localFingerprints != null) && !localFingerprints.isEmpty()) {
					if (remoteContent == null) // initiator
					{
						...
						IceUdpTransportPacketExtension transport = remoteContent
					...
					if (localFingerprints != null) {
						List<DtlsFingerprintPacketExtension> fingerprintPEs = localTransport
								.getChildExtensionsOfType(DtlsFingerprintPacketExtension.class);
	 ...
	 *
	 * @param localContents
	 * 		The elements containing the media CONTENT elements and their respective TRANSPORT
	 * 		elements.
	 */
	private void setDtlsEncryptionOnTransports(List<ContentPacketExtension> remoteContents,
			List<ContentPacketExtension> localContents)
	{
		for (ContentPacketExtension localContent : localContents) {
			...
			if (description != null) {
				...
				if (mediaType != null) {
					ContentPacketExtension remoteContent = (remoteContents == null) ? null
							: TransportManagerJabberImpl.findContentByName(remoteContents,
	 ...
	 *
	 * @param transports
	 * 		A <tt>Collection</tt> of XML namespaces of jingle transport elements to be set as the
