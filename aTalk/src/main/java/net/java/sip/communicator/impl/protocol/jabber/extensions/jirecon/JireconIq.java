...
public class JireconIq extends IQ
{
	...
	{
		xml.attribute(RID_ATTR_NAME, rid);
		...
		xml.attribute(MUCJID_ATTR_NAME, mucJid);
		xml.attribute(OUTPUT_ATTR_NAME, output);
	 ...
	 *
	 * @param status
	 *        one of {@link JireconIq.Status} to be set as 'status' attribute value of this IQ.
	 */
	 ...
	 * 
	 * @param mucJid
	 *        the value to be set as 'mucjid' attribute in form of: roomname@muc.server.net/nickname
	 */
	 ...
	 *
	 * @param output
	 *        the output path to be set in 'output' attribute.
	 */
	 ...
	 *
	 * @param rid
	 *        the recording identifier that will be stored in 'rid' attribute of this IQ.
	 */
	...
	public enum Action {
		/**
		 * It can only be set in packet sent from client to component, in order to let Jirecon
		 * component start a new recording session.
		 */
		START("start"),

		/**
		 * It can only be set in packet sent from client to component, in order to let Jirecon
		 * component stop an specified recording session.
		 */
		STOP("stop"),

		/**
		 * It can be set both in packet sent from client to component or packet sent from component
		 * to client, in order to notify the opposite with some information, such as recording
		 * session status.
		 */
		INFO("info"),
		 ...
		 *
		 * @return <tt>Action</tt> value for given string or {@link JireconIq.Action#UNDEFINED} if
		 *         given string does not reflect any of valid values.
		 */
		public static Action parse(String action)
		{
			...
			try {
				return Action.valueOf(action.toUpperCase(Locale.US));
			}
	...
	public enum Status {
		/**
		 * It can only be set in packet sent from component to client, notify the opposite that
		 * "start" command has been received and recording session is starting.
		 */
		INITIATING("initiating"),

		/**
		 * It can only be set in packet sent from component to client, notify the opposite that
		 * recording session has been started successfully.
		 */
		STARTED("started"),

		/**
		 * It can only be set in packet sent from component to client, notify the opposite that
		 * "stop" command has been received and recording session is stopping.
		 */
		STOPPING("stopping"),

		/**
		 * It can only be set in packet sent from component to client, notify the opposite that
		 * recording session has been stopped successfully.
		 */
		STOPPED("stopped"),

		/**
		 * It can only be set in packet sent from component to client, notify the opposite that
		 * recording session has been aborted.
		 */
		ABORTED("aborted"),
		 ...
		 *
		 * @return <tt>Status</tt> value for given string or {@link JireconIq.Status#UNDEFINED} if
		 *         given string does not reflect any of valid values.
		 */
