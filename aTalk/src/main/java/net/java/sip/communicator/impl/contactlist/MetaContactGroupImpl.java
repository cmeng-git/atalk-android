...
public class MetaContactGroupImpl implements MetaContactGroup
{
	 ...
	 * 		the <tt>MetaContactListService</tt> implementation which is to use the new
	 * 		<tt>MetaContactGroup</tt> instance as its root
	 * @param groupName
	 ...
	 *
	 * @param mclServiceImpl
	 * 		the implementation of the <tt>MetaContactListService</tt>, to which this group belongs
	 * @param groupName
	 * 		the name of the group to create
	 * @param mcgUID
	 * 		a metaContact UID that has been stored earlier or null when a new UID needs to be
	 * 		created.
	 */
	MetaContactGroupImpl(MetaContactListServiceImpl mclServiceImpl, String groupName,
			String mcgUID)
	{
		...
		this.groupUID = (mcgUID == null)
				? String.valueOf(System.currentTimeMillis()) + String.valueOf(hashCode())
				: mcgUID;
	}

	/**
	 * Returns a String identifier (the actual contents is left to implementations) that uniquely
	 * represents this <tt>MetaContactGroup</tt> in the containing <tt>MetaContactList</tt>
	 *
	 ...
	 *
	 * @return always <tt>true</tt> since this is the root contact group and in our impl it can
	 * only contain groups.
	 */
	 ...
	 *
	 * @param metaContactID
	 * 		a String identifier obtained through the <tt>MetaContact.getMetaUID()</tt> method.
	 * 		<p>
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> whose index we're looking for.
	 * @return the index of <tt>metaContact</tt> in the list of child contacts or -1 if
	 * <tt>metaContact</tt>.
	 */
	 ...
	 * 		the <tt>MetaContactGroup</tt> whose index we're looking for.
	 * @return the index of <tt>metaContactGroup</tt> in the list of child contacts or -1 if
	 * <tt>metaContact</tt>.
	 */
	 ...
	 * <tt>provider</tt> with the specified identifier.
	 *
	 * @param provider
	 * 		the ProtocolProviderService that the specified <tt>contactID</tt> is pertaining to.
	 * @param contactID
	 * 		a String identifier of the protocol specific contact whose container meta contact
	 * 		we're looking for.
	 * @return the <tt>MetaContact</tt> with the specified identifier.
	 */
	 ...
	 *
	 * @param metaUID
	 * 		the Meta UID of the contact we're looking for.
	 ...
	 *
	 * @param metaUID
	 * 		the Meta UID of the contact group we're looking for.
	 ...
	 * @param ownerProvider
	 * 		a reference to the ProtocolProviderService that the contact we're looking for belongs
	 * 		to.
	 * @return a reference to a <tt>ContactGroup</tt>, encapsulated by this MetaContactGroup,
	 ...
	 *
	 * @param provider
	 * 		a reference to the <tt>ProtocolProviderService</tt> whose ContactGroups we'd like to
	 * 		get.
	 ...
	 *
	 * @param accountID
	 * 		the id of the account whose contact groups we'd like to retrieve.
	 * @return an <tt>Iterator</tt> over all contacts encapsulated in this <tt>MetaContact</tt>
	 ...
	 *
	 * @param protoContact
	 * 		the protocol specific contact whom meta contact we're looking for.
	...
	public MetaContact findMetaContactByContact(Contact protoContact)
	{
		...
		while (contactsIter.hasNext()) {
			...
			Contact storedProtoContact = mContact.getContact(protoContact.getAddress(),
	 ...
	 * <tt>contactAddress</tt> and a source protocol provider with the matching
	 * <tt>accountID</tt>. If no such meta contact exists, the method would return null.
	 *
	 * @param contactAddress
	 * 		the address of the protocol specific contact whose meta contact we're looking for.
	 * @param accountID
	 * 		the ID of the account that the contact we are looking for must belong to.
	 * @return the MetaContactImpl that contains the specified protocol specific contact.
	 */
	 ...
	 * would return null.
	 *
	 * @param protoContactGroup
	 * 		the protocol specific contact group whose meta contact group we're looking for.
	 * @return the MetaContactImpl that contains the specified protocol specific contact.
	 */
	public MetaContactGroupImpl findMetaContactGroupByContactGroup(ContactGroup protoContactGroup)
	{
		...
		while (groupsIter.hasNext()) {
			...
			MetaContactGroupImpl foundMetaContactGroup
					= mGroup.findMetaContactGroupByContactGroup(protoContactGroup);
	 ...
	 *
	 * @param index
	 * 		the index of the meta contact to return.
	 * @return the MetaContact with the specified index,
	 * <p>
	 * @throws IndexOutOfBoundsException
	 * 		in case <tt>index</tt> is not a valid index for this group.
	 */
	...
	/**
	 * Adds the specified <tt>metaContact</tt> to ths local list of child contacts.
	 *
	 * @param metaContact
	...
	/**
	 * Adds the <tt>metaContact</tt> to the local list of child contacts without setting its
	 * parent contact and without any synchronization. This method is meant for use _PRIMARILY_
	 * by the <tt>MetaContact</tt> itself upon change in its encapsulated protocol specific
	 * contacts.
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> to add in the local vector.
	...
	/**
	 * Removes the <tt>metaContact</tt> from the local list of child contacts without unsetting
	 * synchronization. This method is meant for use _PRIMARILY_ by the <tt>MetaContact</tt>
	...
	/**
	 * Removes the specified <tt>metaContact</tt> from the local list of contacts.
	 *
	 * @param metaContact
	 ...
	 * <p>
	 * @throws IndexOutOfBoundsException
	 * 		if <tt>index</tt> is not a valid index.
	 */
	 ...
	 * @return the <tt>MetaContactGroup</tt> with the specified name or null if no such group
	 * exists.
	 */
	 ...
	 *
	 * @param grpUID
	 * 		the uid of the group to return.
	 * @return the <tt>MetaContactGroup</tt> with the specified uid or null if no such group
	 * exists.
	 */
	 ...
	 *
	 * @param contact
	 * 		the <tt>MetaContact</tt> whose relation to this group we'd like to determine.
	 * @return <tt>true</tt> if <tt>contact</tt> is a direct child of this group and
	 ...
	 *
	 * @param group
	 * 		the <tt>MetaContactGroup</tt> whose relation to this group we'd like to determine.
	 * @return <tt>true</tt> if <tt>group</tt> is a direct child of this <tt>MetaContactGroup</tt>
	 ...
	 *
	 * @param protoGroup
	 * 		the root to add to the groups merged in this meta contact group.
	 */
	 ...
	 *
	 * @param protoGroup
	 * 		the group to remove from the groups merged in this meta contact group.
	 */
	 ...
	 *
	 * @param subgroup
	 * 		the MetaContactGroup to register as a subgroup to this root meta contact group.
	 */
	 ...
	 *
	 * @param group
	 * 		the <tt>MetaContactGroup</tt> to remove.
	 ...
	 *
	 * @param target
	 * 		the <code>MetaContactGroup</code> to be compared.
