 ...
 */
public class OperationSetVideoBridgeImpl implements OperationSetVideoBridge, StanzaFilter,
	StanzaListener, RegistrationStateChangeListener
{
	/**
	...
	/**
	 * Implements {@link StanzaFilter}. Determines whether this instance is interested in a specific
	 ...
	 *
	 * @param packet
	 *        the <tt>Packet</tt> to be determined whether this instance is interested in it
	 ...
	 * @throws OperationNotSupportedException
	 *         if allowing additional callees to a pre-established call is not supported.
	 */
	...
	{
		/*
		 * The application is not a Jitsi Videobridge server, it is a client. Consequently, the
		...
		if (IQ.Type.set.equals(conferenceIQ.getType()) && conferenceIQ.getID() != null) {
			OperationSetBasicTelephony<?> basicTelephony = protocolProvider
	...
	/**
	 * Implements {@link StanzaListener}. Notifies this instance that a specific {@link Packet}
	 * (which this instance has already expressed interest into by returning <tt>true</tt> from
	 * {@link #accept(Stanza)}) has been received.
	 *
	 * @param packet
	 *        the <tt>Packet</tt> which has been received and which this instance is given a chance
	 *        to process
	 */
	public void processStanza(Stanza packet)
	{
		/*
		 * As we do elsewhere, acknowledge the receipt of the Packet first and then go about our
		 * business with it.
		 */
		IQ iq = (IQ) packet;
		...
		/*
		 * Now that the acknowledging is out of the way, do go about our business with the Packet.
		 */
		...
		try {
			...
			logger.error("An error occurred during the processing of a "
				+ packet.getClass().getName() + " packet", t);
			...
			if (t instanceof InterruptedException) {
				 ...
				 * swallow any Throwable. Consequently, we should better restore the interrupted
				 * state.
				 */
				interrupted = true;
			}
	...
	public void registrationStateChanged(RegistrationStateChangeEvent ev)
	{
		...
		if (RegistrationState.REGISTERED.equals(registrationState)) {
			protocolProvider.getConnection().addAsyncStanzaListener(this, this);
		}
