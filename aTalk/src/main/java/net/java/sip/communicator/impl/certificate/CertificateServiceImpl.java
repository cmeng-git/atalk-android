...
public class CertificateServiceImpl implements CertificateService, PropertyChangeListener
{
	...
	public List<CertificateConfigEntry> getClientAuthCertificateConfigs()
	{
		...
		for (String propName
				: config.getPropertyNamesByPrefix(PNAME_CLIENTAUTH_CERTCONFIG_BASE, false)) {
	...
	public X509TrustManager getTrustManager(Iterable<String> identitiesToTest)
			throws GeneralSecurityException
	{
		return getTrustManager(identitiesToTest, new EMailAddressMatcher(),
		...
		return new X509TrustManager()
		{
					...
					throws IOException, URISyntaxException, CertificateException
			{
				...
				do {
					...
					byte[] aiaBytes = current.getExtensionValue(
					...
					for (AccessDescription ad : aia.getAccessDescriptions()) {
						...
						if (cache != null && cache.cacheDate.after(new Date())) {
							...
							try {
								InputStream is = HttpUtils.openURLConnection(
			...
			for (String identity : identitiesToTest) {
				try {
					org.apache.http.conn.ssl.SSLSocketFactory
							.BROWSER_COMPATIBLE_HOSTNAME_VERIFIER.verify(identity, cert);
					oneMatched = true;
					break;
				}
				...
				throws CertificateException
		{
			...
			if (!oneMatched)
				throw new CertificateException("The peer provided certificate with Subject <"
						+ cert.getSubjectDN() + "> contains no SAN for <"
						+ identitiesToTest + ">");
		}
	}
	 ...
	 *
	 * @param chain
	 * 		The chain of the certificates to check with user.
	 * @param message
