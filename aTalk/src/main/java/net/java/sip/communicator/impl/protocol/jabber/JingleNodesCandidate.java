...
public class JingleNodesCandidate extends LocalCandidate
{
	...
	/**
	 * Creates a <tt>JingleNodesRelayedCandidate</tt> for the specified transport, address, and
	 * base.
	 *
	...
	private synchronized JingleNodesCandidateDatagramSocket getRelayedCandidateDatagramSocket()
	{
		if (jingleNodesCandidateDatagramSocket == null) {
			try {
				jingleNodesCandidateDatagramSocket = new JingleNodesCandidateDatagramSocket(this,
					localEndPoint);
			}
...
//    protected IceSocketWrapper getCandidateIceSocketWrapper()
	{
		if (socket == null) {
			try {
				socket = new IceUdpSocketWrapper(new MultiplexingDatagramSocket(
