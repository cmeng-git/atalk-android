...
public class JingleNodesCandidateDatagramSocket extends DatagramSocket
{
	...
	{
		return (numOfPacket == 1) || (numOfPacket == 300) || (numOfPacket == 500)
			|| (numOfPacket == 1000) || ((numOfPacket % 5000) == 0);
	}
	 ...
	 *
	 * @param totalNbLost
	 *        The total number of lost packet since the beginning of this stream.
	 * @param totalNbReceived
	 *        The total number of received packet since the beginning of this stream.
	 * @param lastLogTime
	 *        The last time we have logged information about RTP losses.
	 *
	 ...
	 *
	 * @param lastRtpSequenceNumber
	 *        The previous RTP sequence number.
	 * @param newSeq
	 *        The current RTP sequence number.
	 *
	 * @return the number of loss between the 2 last RTP packets received.
	 */
	 ...
	 *
	 * @param jingleNodesCandidate
	 *        the <tt>JingleNodesCandidate</tt> which is to use the new instance as the value of its
	 *        <tt>socket</tt> property
	 * @param localEndPoint
	 *        <tt>TransportAddress</tt> of the Jingle Nodes relay where we will send our packet.
	...
	/**
	 * Sends a datagram packet from this socket. The <tt>DatagramPacket</tt> includes information
	 ...
	 * the received packet into the "pcap" (packet capture) log.
	 *
	 * @param p
	...
	private void updateRtpLosses(DatagramPacket p)
	{
		...
		if (!StunDatagramPacketFilter.isStunPacket(p)) {
			...
			this.lastLostPacketLogTime = logRtpLosses(this.nbLostRtpPackets,
				this.nbReceivedRtpPackets, this.lastLostPacketLogTime);
		}
	}
}
