...
public class JabberAccountRegistration extends JabberAccountID implements Serializable
{
	 ...
	 *
	 * @param userID
	 * 		the identifier of the jabber registration account.
	 */
	...
	/**
	 * Stores Jabber account configuration held by this registration object into given
	 ...
	 * 		the path to protocol icon if used, or <tt>null</tt> otherwise.
	 * @param accountIconPath
	 * 		the path to account icon if used, or <tt>null</tt> otherwise.
	 * @param accountProperties
	 * 		the map used for storing account properties.
	 * @throws OperationFailedException
	 * 		if properties are invalid.
	 */
	public void storeProperties(ProtocolProviderFactory factory, String userName,
			String  passWord, String protocolIconPath, String accountIconPath,
			...
			throws OperationFailedException
	{
		...
		// If it is existing account stored in database - then clean up some old stuffs
		if (accountUuid != null) {
			...
			String[] accKeys = mAccountProperties.keySet().toArray(new String[accountProperties.size()]);
		...
		for (StunServerDescriptor stunServer : stunServers) {
			serverIndex++;
			stunServer.storeDescriptor(mAccountProperties,
	 ...
	 *
	 * @param account
	 * 		the account object that will be used.
	 * @param bundleContext
	 * 		the OSGi bundle context required for some operations.
	 */
	public void loadAccount(AccountID account, BundleContext bundleContext)
	{
		...
		for (int i = 0; i < StunServerDescriptor.MAX_STUN_SERVER_COUNT; i++) {
			...
			String stunPassword = loadStunPassword(bundleContext, account,
		...
		// Encodings
		encodingsRegistration.loadAccount(account,
	...
	protected String getServerFromUserName(String userName)
	{
		...
		if (!StringUtils.isNullOrEmpty(newServerAddr)) {
			return newServerAddr.equals(GOOGLE_USER_SUFFIX)
					? GOOGLE_CONNECT_SRV : newServerAddr;
		}
		return null;
	}
}
