 ...
 *
 * @param <T>
 * 		the peer extension class like for example <tt>CallPeerSipImpl</tt> or
...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>CallPeerMediaHandler</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> instance that we will be managing media for.
	 * @param srtpListener
	 ...
	 *
	 * @param listener
	 * 		the <tt>VideoListener</tt> to be registered with this instance and to start receiving
	 * 		notifications from it about changes in the availability of visual <tt>Component</tt>s
	 * 		displaying video
	 */
	 ...
	 *
	 * @param ev
	 * 		a <tt>PropertyChangeEvent</tt> which specified the property which had its value
		...
		if (callConferenceChange || MediaAwareCall.DEFAULT_DEVICE.equals(propertyName)) {
			...
			for (MediaType mediaType : new MediaType[]{MediaType.AUDIO, MediaType.VIDEO}) {
				...
				if (oldDevice != null) {
					 ...
					 * telephony conference and we have to move its MediaStreams to the
					 * respective mixers.
					 */
					MediaDevice oldValue = (!callConferenceChange
					...
					MediaDevice newValue
							= (!callConferenceChange && (newDevice instanceof MediaDeviceWrapper))
	 ...
	 * collection (or reuse). Synchronized if any other stream operations are in process we won't
	 * interrupt them.
	 */
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> that we'd like to stop a stream for.
	 */
	protected void closeStream(MediaType mediaType)
	{
		...
		/*
		 * This CallPeerMediaHandler releases its reference to the MediaStream it has initialized
		 * via #initStream().
		 */
	 ...
	 * <tt>extensionURN</tt> or <tt>null</tt> if <tt>extList</tt> did not contain such an
	 * extension.
	 *
	 * @param extList
	 * 		the <tt>List</tt> that we will be looking through.
	 * @param extensionURN
	 * 		the URN of the <tt>RTPExtension</tt> that we are looking for.
	 ...
	 *
	 * @param formats
	 * 		the list of <tt>MediaFormat</tt>s to find the specified matching <tt>MediaFormat</tt>
	 * 		into
	 * @param format
	 * 		encoding of the <tt>MediaFormat</tt> to find
	 ...
	 * about a specific type of change in the availability of a specific visual <tt>Component</tt>
	 * depicting video.
	 *
	 * @param type
	 * 		the type of change as defined by <tt>VideoEvent</tt> in the availability of the
	 * 		specified visual <tt>Component</tt> depicting video
	 * @param visualComponent
	 * 		the visual <tt>Component</tt> depicting video which has been added or removed in this
	 * 		<tt>CallPeerMediaHandler</tt>
	 * @param origin
	 * 		{@link VideoEvent#LOCAL} if the origin of the video is local (e.g. it is being locally
	 ...
	 *
	 * @param event
	 * 		the <tt>VideoEvent</tt> to fire to the <tt>VideoListener</tt>s registered with this
	 * 		<tt>CallPeerMediaHandler</tt>
	 */
	...
	{
		return advertisedEncryptionMethods
				.toArray(new SrtpControlType[advertisedEncryptionMethods.size()]);
	}
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> in which the retrieved <tt>MediaDevice</tt> is to capture
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> to retrieve the user preference for
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> to get the SRTP control type for
	 ...
	 *
	 * @param type
	 * 		the <tt>MediaType</tt> of the device whose <tt>RTPExtension</tt>s we are interested in.
	...
	protected List<RTPExtension> getExtensionsForType(MediaType type)
	{
		...
		return (device != null) ? device.getSupportedExtensions() : new ArrayList<RTPExtension>();
	}
	...
	public String getICECandidateExtendedType(String streamName)
	{
		...
		return (transportManager == null)
				? null : transportManager.getICECandidateExtendedType(streamName);
	}
	 ...
	 * @param streamName
	 * 		The stream name (AUDIO, VIDEO);
	 * @return the ICE local host address if this transport manager is using ICE. Otherwise,
	 * returns null.
	 */
	public InetSocketAddress getICELocalHostAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICELocalHostAddress(streamName);
	}
	...
	public InetSocketAddress getICELocalReflexiveAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICELocalReflexiveAddress(streamName);
	}
	 ...
	 * @param streamName
	 * 		The stream name (AUDIO, VIDEO);
	 * @return the ICE local relayed address. May be null if this transport manager is not using
	 * ICE
	...
	public InetSocketAddress getICELocalRelayedAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICELocalRelayedAddress(streamName);
	}
	...
	public InetSocketAddress getICERemoteHostAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICERemoteHostAddress(streamName);
	}
	...
	public InetSocketAddress getICERemoteReflexiveAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICERemoteReflexiveAddress(streamName);
	}
	...
	public InetSocketAddress getICERemoteRelayedAddress(String streamName)
	{
		TransportManager<?> transportManager = queryTransportManager();
		return (transportManager == null)
				? null : transportManager.getICERemoteRelayedAddress(streamName);
	}

	/**
	 * Returns the current state of ICE processing.
	 *
	 * @return the current state of ICE processing if this transport manager is using ICE.
	 * Otherwise, returns null.
	 */
	 ...
	 *
	 * @param mediaDevice
	 * 		the <tt>MediaDevice</tt>.
	 * @return a non-null list of locally supported <tt>MediaFormat</tt>s for <tt>mediaDevice</tt>,
	 * in decreasing order of priority.
	 * @see CallPeerMediaHandler#getLocallySupportedFormats(MediaDevice, QualityPreset,
	 * QualityPreset)
	 */
	 ...
	 * 		the <tt>MediaDevice</tt>.
	 * @param sendPreset
	 * 		the preset used to set some of the format parameters, used for video and settings.
	 * @param receivePreset
			...
			QualityPreset sendPreset, QualityPreset receivePreset)
	{
		...
		if (mediaDevice != null) {
			 ...
			 */
				...
				EncodingConfiguration encodingConfiguration = ProtocolMediaActivator
						.getMediaService().createEmptyEncodingConfiguration();

				encodingConfiguration.loadProperties(accountProperties,
						ProtocolProviderFactory.ENCODING_PROP_PREFIX);
				supportFormats = mediaDevice.getSupportedFormats(sendPreset,
						receivePreset, encodingConfiguration);
			}
			else {
		...
		if (supportFormats.size() == 0) {
			AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
					...
					AndroidGUIActivator.getResources()
							.getI18NString("service.gui.CALL_NO_DEVICE_CODEC_H",
									new String[] {mediaDevice.getMediaType().toString()})
			);
		}
	 ...
	 * <tt>MediaType</tt>. If this instance doesn't have such a <tt>MediaStream</tt>, returns
	 * <tt>null</tt>
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the <tt>MediaStream</tt> to retrieve
	 ...
	 * 		if creating the stream fails for any reason (like, for example, accessing the device
	 * 		or setting the format).
	 */
	protected MediaStream initStream(StreamConnector connector, MediaDevice device,
			MediaFormat format, MediaStreamTarget target, MediaDirection direction,
			...
			throws OperationFailedException
	{
		...
		/*
		 * Do make sure that no unintentional streaming of media generated by the user without
		 * prior consent will happen.
		 */
		direction = direction.and(getDirectionUserPreference(mediaType));
		/*
		 * If the device does not support a direction, there is really nothing to be done at this
		 * point to make it use it.
		 */
	 ...
	 *
	 * @param remoteFormats
	 * 		remote <tt>MediaFormat</tt> found in the SDP message
	 * @param localFormats
	 * 		local supported <tt>MediaFormat</tt> of our device
			...
			List<MediaFormat> localFormats)
	{
		...
		// If we've found some compatible formats, add telephone-event, red and ulpfec back in to
		...
		// the list because there'd be no media.
		if (!ret.isEmpty()) {
	 ...
	 *
	 * @param remoteExtensions
	 * 		the <tt>List</tt> of <tt>RTPExtension</tt>s as advertised by the remote party.
	 * @param supportedExtensions
	 * 		the <tt>List</tt> of <tt>RTPExtension</tt>s that a local <tt>MediaDevice</tt> returned
	 * 		as supported.
			...
			List<RTPExtension> supportedExtensions)
	{
		...
		List<RTPExtension> intersection
				= new ArrayList<>(Math.min(remoteExtensions.size(), supportedExtensions.size()));
		...
		for (RTPExtension remoteExtension : remoteExtensions) {
			RTPExtension localExtension
					= findExtension(supportedExtensions, remoteExtension.getURI().toString());
	...
	{
		return ((dev != null)
				&& !getLocallySupportedFormats(dev, sendPreset, receivePreset).isEmpty());
	}
	 ...
	 *
	 * @return <tt>true</tt> if this handler's streams have been placed on hold and <tt>false</tt>
	 * otherwise.
	 */
	 ...
	 *
	 * @return <tt>true</tt> if local audio transmission is currently on mute and <tt>false</tt>
	 * otherwise.
	 */
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> for which it is to be determined whether RTP translation is
	 ...
	 *
	 * @param ev
	 * 		the <tt>PropertyChangeEvent</tt> fired by the associated <tt>MediaHandler</tt>
	 */
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the <tt>SrtpControl</tt> to be examined
	 * @param srtpControlType
	 * 		the <tt>SrtpControlType</tt> of the <tt>SrtpControl</tt>s to not be removed from this
	 * 		instance and cleaned up. If <tt>null</tt>, all <tt>SrtpControl</tt>s are removed from
	 * 		this instance and cleaned up
	 */
	protected void removeAndCleanupOtherSrtpControls(MediaType mediaType,
	 ...
	 *
	 * @param listener
	 * 		the <tt>VideoListener</tt> to be unregistered from this instance and to stop receiving
	 * 		notifications from it about changes in the availability of visual <tt>Component</tt>s
	 * 		displaying video
	 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>CsrcAudioLevelListener</tt> to set to our audio stream.
	 */
	 ...
	 *
	 * @param enabled
	 * 		<tt>true</tt> if the media handler should transmit local audio and <tt>false</tt>
	 * 		otherwise.
	 */
	...
	public void setLocallyOnHold(boolean locallyOnHold)
	{
		...
		if (locallyOnHold) {
			...
			MediaDirection direction
					= (getPeer().getCall().isConferenceFocus() || audioStream == null)
			...
			MediaStream videoStream = getStream(MediaType.VIDEO);
			if (videoStream != null) {
				direction = getPeer().getCall().isConferenceFocus() ? MediaDirection.INACTIVE
						: videoStream.getDirection().and(MediaDirection.SENDONLY);
				/*
				 * Set the video direction to INACTIVE, because currently we cannot mute video
				 * streams.
				 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>SimpleAudioLevelListener</tt> to add or <tt>null</tt> if we are trying to
	 * 		remove it.
	 */
		...
		synchronized (localUserAudioLevelListenerLock) {
			if (this.localUserAudioLevelListener != listener) {
				...
				if ((mediaHandler != null) && (this.localUserAudioLevelListener != null)) {
					mediaHandler
							.removeLocalUserAudioLevelListener(this.localUserAudioLevelListener);
				}
	 ...
	 *
	 * @param enabled
	 * 		<tt>true</tt> if the media handler should transmit local video and <tt>false</tt>
	 * 		otherwise.
	 */
	 ...
	 *
	 * @param mute
	 * 		<tt>true</tt> if we are to make our audio stream start transmitting silence and
	 ...
	 * 		if this method is called without this handler having first seen a media description
	 * 		or having generated an offer.
	 */
	public void start()
			throws IllegalStateException
	{
	 ...
	 *
	 * @param master
	 * 		the data that we are supposed to pass to our video stream.
	 */
	 ...
	 */
	protected abstract void throwOperationFailedException(String message, int errorCode,
			Throwable cause)
			throws OperationFailedException;
	 ...
	 *
	 * @param mediaType
	 * 		the media type of the stream for which to return the value for 'msid'
	 ...
	 *
	 * @param mediaType
	 * 		the media type of the stream for which to return the value for 'label'
	...
	private class CallPropertyChangeListener implements PropertyChangeListener
	{
		 ...
		 *
		 * @param call
		 * 		the <tt>Call</tt> the new instance is to be added to
		 */
		 ...
		 *
		 * @param event
		 * 		a <tt>PropertyChangeEvent</tt> which specifies the name of the property which had
