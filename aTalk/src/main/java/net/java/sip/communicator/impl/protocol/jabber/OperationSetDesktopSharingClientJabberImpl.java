 ...
 */
public class OperationSetDesktopSharingClientJabberImpl extends
	AbstractOperationSetDesktopSharingClient<ProtocolProviderServiceJabberImpl> implements
	RegistrationStateChangeListener, StanzaListener, StanzaFilter
{
	 ...
	 *        creation of the new instance and for which the new instance is to provide desktop
	 *        sharing.
	 */
	 ...
	 *        <tt>CallPeer</tt> that will be notified
	 * @param payload
	 *        The packet payload containing the key/mouse/videoPanelSize event to send to remote
	 *        peer
	 */
	private void sendRemoteControlExtension(CallPeer callPeer, RemoteControlExtension payload)
	{
		...
		if (this.parentProvider.getDiscoveryManager().includesFeature(InputEvtIQ.NAMESPACE_CLIENT)
			&& discoverInfo != null && discoverInfo.containsFeature(InputEvtIQ.NAMESPACE_SERVER)) {
	...
	public void processStanza(Stanza packet)
	{
		...
		if (inputIQ.getType() == IQ.Type.set && inputIQ.getAction() != InputEvtAction.NOTIFY) {
			...
			if (callPeerID != null) {
				CallPeer callPeer = getListenerCallPeer(callPeerID.toString());
				if (callPeer != null) {
	...
	/**
	 * Returns the callPeer corresponding to the given callPeerAddress given in parameter, if this
	 * callPeer exists in the listener list.
	 *
	 * @param callPeerAddress
	 *        The XMPP address of the call peer to seek.
	 *
	 * @return The callPeer corresponding to the given callPeerAddress given in parameter, if this
	 *         callPeer exists in the listener list. null otherwise.
	 */
