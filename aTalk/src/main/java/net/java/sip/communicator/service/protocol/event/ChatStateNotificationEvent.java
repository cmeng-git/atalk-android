...
public class ChatStateNotificationEvent extends EventObject
{
	...
	private ChatState mChatState = ChatState.inactive;
	 ...
	 *
	 * @param sourceContact
	 * 		the <tt>Contact</tt> that has sent the notification.
	 * @param state
	 * 		the <tt>Contact</tt>'s current chat state
	 * @param msg
	 * 		the message received
	 */
	public ChatStateNotificationEvent(Contact sourceContact, ChatState state, Message msg)
	{
		super(sourceContact);
	 ...
	 *
	 * @return one of the <tt>ChatState</tt>s indicating the chat state that this notification
	 * is about.
	 */
	...
	{
		return new StringBuffer("ChatStateNotificationEvent[from = ")
				.append(getSourceContact().getDisplayName())
				.append(" state = " + getChatState()).toString();
	}
