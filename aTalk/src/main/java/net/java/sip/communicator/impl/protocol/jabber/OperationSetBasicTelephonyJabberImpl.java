...
public class OperationSetBasicTelephonyJabberImpl
        ...
        OperationSetSecureZrtpTelephony, StanzaListener, BasicTelephony,
    ...
    // Jingle RTP session instant.
    private JingleCallSessionImpl mJingleCallSession;

    // Jingle Message instants.
    private JingleMessageManager mJingleMessageManager;
    private JingleMessageSessionImpl mJingleMessageSessionImpl;
    ...
    public void registrationStateChanged(RegistrationStateChangeEvent evt) {
        ...
        if (registrationState == RegistrationState.CONNECTED) {
            ...
            mJingleMessageManager = null;
            mJingleMessageSessionImpl = null;
        }
    }
    ...
    /**
     * Return a new generated random jingle session Id or retrieve it from JingleMessageSessionImpl if call handle via JingleMessage
     *
     * @return Jingle session Id
     */
    public String getSid() {
        return (mSid != null) ? mSid : JingleManager.randomUuid();
    }

    /**
     * init mSid with a randomUuid if callee does not support JingleMessage protocol
     */
    public String initSid() {
        mSid = JingleManager.randomUuid();
        return mSid;
    }
    ...
    AbstractCallPeer<?, ?> createOutgoingCall(CallJabberImpl call, String calleeAddress, Iterable<XmlElement> siChildElement)
        ...
        if (calleeAddress.contains("/")) {
            try {
                ...
                Timber.w("Create outgoing call: %s", e.getMessage());
            }
        }
        ...
        try {
            ...
            if (t instanceof ThreadDeath)
                ...
                if (telephonyDomain != null)
                    ...
                    Timber.e("Call initiateSession: %s", message);
    ...
    public void hangupCallPeer(CallPeer peer, int reasonCode, String reasonText)
        ...
        if (failed && reasonText != null) {
            ...
            reasonText = aTalkApp.getResString(R.string.call_ended);
            jingleReason = new JingleReason(JingleReason.Reason.success, reasonText, null);
        }

        // XXX maybe add answer/hangup abstract method to MediaAwareCallPeer
        if (peer instanceof CallPeerJabberImpl) {
            ((CallPeerJabberImpl) peer).hangup(failed, jingleReason);
        }
    }
    ...
    private static Reason convertReasonCodeToSIPCode(int reasonCode) {
        ...
        case HANGUP_REASON_CALL_RETRACT:
    ...
    @Override
    public void handleJingleCallSession(JingleCallSessionImpl session, Jingle jingle) {
        mJingleCallSession = session;
        handleJingleCallSession(jingle);
    }

    @Override
    public void handleJingleCallSession(Jingle jingle) {
        try {
            /*
    ...
    protected synchronized void processJingleSynchronize(final Jingle jingleIQ) {
        ...
        case session_initiate:
            ...
            // start init new call if not already in a call; use sid from jingleIQ
            if (call == null) {
             ...
             */
            callPeer = new CallPeerJabberImpl(call, jingleIQ, mJingleCallSession);
        ...
        case session_terminate:
            ...
            Timber.w("Received session-terminate after call has disconnected: %s. JingleSession: %s", jingleIQ.getStanzaId(), mJingleCallSession);
            contentMedias = null;
            break;
...
// private Jingle jingleTransports = null;
    ...
    private void processSessionAccept(CallPeerJabberImpl callPeer, Jingle jingle) {
        if (!mTIProcessed) {
            ...
            try {
                ...
                Timber.w("Process session accept: %s", e.getMessage());
            }
        }
    }
    ...
    private void processSessionInitiateInternal(CallJabberImpl call, CallPeerJabberImpl callPeer, Jingle jingle) {
        ...
        if (aTalk.isMediaCallAllowed(false)) {
            jingleSI = jingle;
            new Thread(() -> {
                call.processSessionInitiate(callPeer, jingle, mJingleCallSession);
                jingleSI = null;
            }).start();
        }
        else {
