		...
		ChatRoomMessageListener, AdHocChatRoomMessageListener
{
	...
	private static final String IS_MESSAGE_SUBTYPE_SMS_PROP
			= "msghistory.contactsrc.IS_SMS_ENABLED";
	...
	/**
	 * The maximum number of recent messages to store in the history, but will retrieve just
	 ...
	 */
	public void handleProviderAdded(final ProtocolProviderService provider,
			final boolean isStatusChanged)
	{
		// new Exception().printStackTrace();
		logger.info("Handle new provider added and status changed to online: "
	 ...
	 * 		ProtocolProviderService
	 */
	private void handleProviderAddedInSeparateThread(ProtocolProviderService provider,
	 ...
	 *
	 * @param provider
	 * 		the provider which contact messages we will search e.g Jabber:abc123@atalk.org
	 * @param isStatusChanged
	 * 		is the search because of status changed
	 * @return entries in cached recent messages in history.
	 */
	private List<ComparableEvtObj> getCachedRecentMessages(ProtocolProviderService provider,
			boolean isStatusChanged)
	{
	 ...
	 *
	 * @param res
	 * 		list of lately fetched event
	 * @param cachedRecentMessages
	 * 		list of newly created source contacts or already existed but updated with
	 * 		corresponding event object
	 * @param isStatusChanged
	 * 		whether provider status changed and we are processing
	 */
	 ...
	 * 		Account Uid
	 * @param startDate
	 * 		start date to search; can be null if not applicable
	 ...
	 *
	 * @param evt
	 * 		the <tt>MessageFailedEvent</tt> containing the ID of the
	 */
	@Override
	 ...
	 *
	 * @param evt
	 * 		the <tt>ChatRoomMessageDeliveryFailedEvent</tt> containing
	 */
	@Override
	...
	private void handle(EventObject obj, ProtocolProviderService provider, String id)
	{
		...
		synchronized (recentMessages) {
			...
			MessageSourceContact newSourceContact = new MessageSourceContact(obj,
			...
			List<ComparableEvtObj> removedItems = null;
			if (recentMessages.size() > numberOfMessages) {
				removedItems = new ArrayList<>(recentMessages.subList(numberOfMessages,
						recentMessages.size()));
	 ...
	 *
	 * @param evt
	 * 		the <tt>ContactPropertyChangeEvent</tt> containing the source
	 */
	@Override
	 ...
	 *
	 * @param obj
	 * 		the object that we will try to match.
	 * @param list
	 * 		the list we will search in.
	...
	int getIndex(MessageSourceContact messageSourceContact)
	{
		synchronized (recentMessages) {
			for (int i = 0; i < recentMessages.size(); i++)
				if (recentMessages.get(i).getContact()
						.equals(messageSourceContact.getContact()))
					return i;
			return -1;
		}
	}
	...
	{
		recentQuery
				= (MessageSourceContactQuery) createContactQuery(queryString, numberOfMessages);
		return recentQuery;
	}
	...
	private class ComparableEvtObj implements Comparable<ComparableEvtObj>
	{
		 ...
		 *
		 * @param source
		 * 		used to extract initial values.
		 */
		 ...
		 *
		 * @param source
		 * 		the eventObject to retrieve information
		 */
		...
		{
			return "ComparableEvtObj{" + "address='" + address + '\'' + "," +
					" ppService=" + ppService + '}';
		}
	...
	/**
	 * Permanently removes locally stored message history for the chatRoom, remove any recent
	 * contacts if any.
	 */
