...
public class MetaHistoryServiceImpl implements MetaHistoryService, ServiceListener
{
	 ...
	 * @param descriptor
	 * 		CallPeer address(String), MetaContact or ChatRoom.
	 * @param startDate
	 * 		Date the date of the first record to return
	 ...
	 * 		CallPeer address(String), MetaContact or ChatRoom.
	 * @param endDate
	 * 		Date the date of the last record to return
	 * @return Collection sorted result that consists of records returned from the services we wrap
	 */
	 ...
	 * @return Collection sorted result that consists of records returned from the services we wrap
	 */
	public Collection<Object> findByKeywords(String[] services, Object descriptor,
			String[] keywords)
	{
			...
			String[] keywords, boolean caseSensitive)
	{
		...
		for (int i = 0; i < services.length; i++) {
			...
			if (serv instanceof MessageHistoryService) {
				...
				if (descriptor instanceof MetaContact) {
					result.addAll(mhs.findByKeywords((MetaContact) descriptor, keywords,
							caseSensitive));
				}
				else if (descriptor instanceof ChatRoomWrapper) {
					ChatRoom chatRoom = ((ChatRoomWrapper) descriptor).getChatRoom();
	 ...
	 * @return Collection sorted result that consists of records returned from the services we wrap
	 */
	public Collection<Object> findFirstMessagesAfter(String[] services, Object descriptor,
			Date date, int count)
	{
		MessageProgressWrapper listenWrapper = new MessageProgressWrapper(services.length);
		...
		for (int i = 0; i < services.length; i++) {
			...
			if (serv instanceof MessageHistoryService) {
				...
				if (descriptor instanceof MetaContact) {
					result.addAll(
							mhs.findFirstMessagesAfter((MetaContact) descriptor, date, count));
				}
				else if (descriptor instanceof ChatRoomWrapper) {
					ChatRoom chatRoom = ((ChatRoomWrapper) descriptor).getChatRoom();
	 ...
	 * @return Collection sorted result that consists of records returned from the services we wrap
	 */
	public Collection<Object> findLastMessagesBefore(String[] services, Object descriptor,
			Date date, int count)
	{
		MessageProgressWrapper listenWrapper = new MessageProgressWrapper(services.length);
		...
		for (int i = 0; i < services.length; i++) {
			...
			if (serv instanceof MessageHistoryService) {
				...
				if (descriptor instanceof MetaContact) {
					result.addAll(
							mhs.findLastMessagesBefore((MetaContact) descriptor, date, count));
				}
				else if (descriptor instanceof ChatRoomWrapper) {
					ChatRoom chatRoom = ((ChatRoomWrapper) descriptor).getChatRoom();
	...
	private class MessageProgressWrapper implements MessageHistorySearchProgressListener,
			CallHistorySearchProgressListener
	{
		...
		private void fireProgress(int origProgress, int maxVal, Date startDate, Date endDate,
				String[] keywords)
		{
			ProgressEvent ev = new ProgressEvent(MetaHistoryServiceImpl.this, startDate, endDate,
					keywords);
