...
/**
 * Provider handles parsing of Rayo IQ stanzas and converting objects back to their XML
 * representation.
 * <p>
 ...
 * @author Pawel Domas
 */
public class RayoIqProvider extends IQProvider<IQ>
{
	/**
	 * Rayo namespace.
	 */
	 ...
	 * 		the <tt>ProviderManager</tt> to which this instance wil be bound to.
	 */
	...
	public static abstract class RayoIq extends IQ
	{
		 ...
		 *
		 * @param elementName
		 * 		the name of XML element that will be used.
		 */
		...
		/**
		 * Implementing classes should print their attributes if any in XML format to given
		 * <tt>out</tt> <tt>StringBuilder</tt>.
		 *
		 * @param xml
		 * 		the <tt>IQChildElementXmlStringBuilder</tt> instance used to construct XML
		 * 		representation of this element.
		 */
		protected abstract void printAttributes(IQChildElementXmlStringBuilder xml);
		 ...
		 *
		 * @param name
		 * 		the name of header extension which value we want to retrieve.
		 * @return value of header extension with given <tt>name</tt> if it exists or <tt>null</tt>
		 * otherwise.
		 */
		 ...
		 *
		 * @param name
		 * 		the attribute name of the 'header' extension to be added.
		 * @param value
		 * 		the 'value' attribute of the 'header' extension that will be added to this IQ.
		 */
	...
	public static class DialIq extends RayoIq
	{
		 ...
		 *
		 * @param to
		 * 		the destination address/call URI to be used.
		 * @param from
		 * 		the source address that will be set on new <tt>DialIq</tt> instance.
		 ...
		 *
		 * @param source
		 * 		the new source address value to be set.
		 */
		 ...
		 *
		 * @param destination
		 * 		the new destination address/call URI to set.
		 */
		...
		@Override
		protected void printAttributes(IQChildElementXmlStringBuilder xml)
		{
			String src = getSource();
			xml.optAttribute(SRC_ATTR_NAME, src);

			String dst = getDestination();
			xml.optAttribute(DST_ATTR_NAME, dst);
		}
	}

	/**
	 * Rayo 'ref' IQ sent by the server as a reply to 'dial' request. Holds created call's resource
	 * in 'uri' attribute.
	 */
	public static class RefIq extends RayoIq
	{
		...
		/**
		 * Creates result <tt>RefIq</tt> for given <tt>requestIq</tt> parametrized with given call
		 * <tt>uri</tt>.
		 *
		 * @param requestIq
		 * 		the request IQ which 'from', 'to' and 'id' attributes will be used for
		 * 		constructing result IQ.
		 * @param uri
		 * 		the call URI that will be included in newly created <tt>RefIq</tt>.
		 * @return result <tt>RefIq</tt> for given <tt>requestIq</tt> parametrized with given call
		 * <tt>uri</tt>.
		 */
	 ...
	 * presence indication with {@link EndExtension} is sent through the presence to confirm the
	 * operation.
	 */
	public static class HangUp extends RayoIq
	{
		 ...
		 * 		source JID.
		 * @param to
		 * 		the destination address/call URI to be ended by this IQ.
