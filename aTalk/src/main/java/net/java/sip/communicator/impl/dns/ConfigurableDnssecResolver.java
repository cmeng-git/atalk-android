...
{
    private final static Logger logger = Logger.getLogger(ConfigurableDnssecResolver.class);

    /**
     * Name of the property that defines the default DNSSEC validation behavior.
     */
    public final static String PNAME_DNSSEC_VALIDATION_MODE = "dns.DNSSEC_VALIDATION_MODE";

    /**
     * Default value of {@link #PNAME_DNSSEC_VALIDATION_MODE}
     */
    public final static String PNAME_BASE_DNSSEC_PIN = "dns.pin";
    ...
    private ResourceManagementService RMS = DnsUtilActivator.getResources();
    private Map<String, Date> lastNotifications = new HashMap<>();

    private ExtendedResolver headResolver;

    /**
     * Creates a new instance of this class. Tries to use the system's default forwarders.
     */
    public ConfigurableDnssecResolver(ExtendedResolver headResolver)
    {
        super(headResolver);
        List<String> propNames = config.getPropertyNamesByPrefix("dnssec", false);
        ...
        DnsUtilActivator.getNotificationService().registerDefaultNotificationForEvent(
                ConfigurableDnssecResolver.EVENT_TYPE, NotificationAction.ACTION_POPUP_MESSAGE, null, null);
    }

    /**
     * Inspects a DNS answer message and handles validation results according to the user's preferences.
     *
     * @throws DnssecRuntimeException when the validation failed and the user did not choose to ignore it.
     */
    @Override
            ...
            throws DnssecRuntimeException, IOException
    {
        ...
        SecureResolveMode defaultAction = Enum.valueOf(SecureResolveMode.class,
                config.getString(PNAME_DNSSEC_VALIDATION_MODE, SecureResolveMode.WarnIfBogus.name()));
        ...
        // b2, c2
        if ((pinned == SecureResolveMode.SecureOnly && !msg.isSecure())
            ...
            if (last == null || last.before(new Date(new Date().getTime() - 1000 * 60 * 5))) {
                DnsUtilActivator.getNotificationService().fireNotification(EVENT_TYPE,
                        aTalkApp.getResString(R.string.util_dns_INSECURE_ANSWER_TITLE), text, null);
        ...
        String reason = msg.isBogus()
                ? aTalkApp.getResString(R.string.util_dns_DNSSEC_ADVANCED_REASON_BOGUS, fqdn, msg.getBogusReason())
                : aTalkApp.getResString(R.string.util_dns_DNSSEC_ADVANCED_REASON_UNSIGNED, type, fqdn);
        ...
        switch (result) {
            ...
            case AlwaysAccept:
                if (msg.isBogus())
                    config.setProperty(propName, SecureResolveMode.IgnoreDnssec.name());
                else
                    config.setProperty(propName, SecureResolveMode.WarnIfBogus.name());
                break;
            case AlwaysDeny:
    ...
    private enum DnssecDialogResult
    {
        ...
        AlwaysAccept,
        /**
         * The result shall be rejected automatically unless it is valid according to DNSSEC.
         */
        AlwaysDeny
    }

    /**
     * Dialog to ask and warn the user if he wants to continue to accept an invalid dnssec result.
     */
    private class DnssecDialog extends SIPCommDialog implements ActionListener
    {
        ...
        private void initComponents()
        {
            ...
            setTitle(RMS.getI18NString("util.dns.INSECURE_ANSWER_TITLE"));

            // warning text
            // JLabel imgWarning = new JLabel(RMS.getImage("service.gui.icons.WARNING_ICON"));
            // imgWarning.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
            // add(imgWarning, BorderLayout.WEST);
            JLabel lblWarning = new JLabel(aTalkApp.getResString(R.string.util_dns_DNSSEC_WARNING,
                    aTalkApp.getResString(R.string.service_gui_APPLICATION_NAME), domain));
            // add(lblWarning, BorderLayout.CENTER);

            // standard panel (deny option)
            cmdAck = new JButton(aTalkApp.getResString(R.string.service_gui_OK));
            cmdAck.addActionListener(this);

            cmdShowDetails = new JButton(aTalkApp.getResString(R.string.util_dns_DNSSEC_ADVANCED_OPTIONS));
            ...
            // dd(pnlStandard, BorderLayout.SOUTH);
            ...
            for (DnssecDialogResult r : DnssecDialogResult.values()) {
                JButton cmd = new JButton(RMS.getI18NString("dns.ConfigurableDnssecResolver$DnssecDialogResult."
                        + r.name()));
    ...
    private String getExceptionMessage(SecureMessage msg)
    {
        return msg.getBogusReason() == null
                ? aTalkApp.getResString(R.string.util_dns_INSECURE_ANSWER_MESSAGE_NO_REASON,
                msg.getQuestion().getName().toString())
                : aTalkApp.getResString(R.string.util_dns_INSECURE_ANSWER_MESSAGE_REASON,
                msg.getQuestion().getName().toString(), msg.getBogusReason());
    }
    ...
    public void reset()
    {
            ...
            synchronized (Lookup.class) {
                ...
                for (String fwd : fwds) {
                    try {
                        ...
                        // these properties are normally set by the ValidatingResolver in the constructor
        ...
        for (int i = 1; ; i++) {
            String anchor = DnsUtilActivator.getResources().getSettingsString("dns.DS_ROOT." + i);
            if (anchor == null)
                break;
