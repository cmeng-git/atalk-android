...
public class ConfigurableDnssecResolver extends ValidatingResolver implements CustomResolver {
	...
	public ConfigurableDnssecResolver(ExtendedResolver headResolver) {
		...
		DnsUtilActivator.getNotificationService().registerDefaultNotificationForEvent(
				ConfigurableDnssecResolver.EVENT_TYPE, NotificationAction.ACTION_POPUP_MESSAGE,
				null, null);
	}
	...
	public Message send(Message query)
		...
		SecureResolveMode pinned = Enum.valueOf(SecureResolveMode.class,
				config.getString(propName, defaultAction.name()));
		...
		if (
			...
			if (last == null
				...
				DnsUtilActivator.getNotificationService().fireNotification(
						EVENT_TYPE, R.getI18NString("util.dns.INSECURE_ANSWER_TITLE"), text, null);
	...
	private class DnssecDialog extends SIPCommDialog implements ActionListener {
		...
		private void initComponents() {
			...
			JLabel lblWarning = new JLabel(R.getI18NString("util.dns.DNSSEC_WARNING",
					new String[]{R.getSettingsString("service.gui.APPLICATION_NAME"),
					domain}));
			...
			for (DnssecDialogResult r : DnssecDialogResult.values()) {
				JButton cmd = new JButton(R.getI18NString("dns.ConfigurableDnssecResolver" +
						"$DnssecDialogResult." + r.name()));
	...
	private String getExceptionMessage(SecureMessage msg) {
		return msg.getBogusReason() == null
				? R.getI18NString("util.dns.INSECURE_ANSWER_MESSAGE_NO_REASON",
				new String[]{msg.getQuestion().getName().toString()})
				: R.getI18NString("util.dns.INSECURE_ANSWER_MESSAGE_REASON",
				new String[]{msg.getQuestion().getName().toString(),
	...
	@Override
	public void reset() {
		String forwarders = DnsUtilActivator.getConfigurationService().getString(
