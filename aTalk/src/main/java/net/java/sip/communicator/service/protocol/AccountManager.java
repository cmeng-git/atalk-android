...
/**
 * Represents an implementation of <tt>AccountManager</tt> which loads the accounts in a separate
 * thread.
 *
...
public class AccountManager
{
	...
	/**
	 * The delay in milliseconds the background <tt>Thread</tt> loading the stored accounts should
	 * wait before dying so that it doesn't get recreated for each <tt>ProtocolProviderFactory</tt>
	 * registration.
	 */
	...
	/**
	 * The <tt>AccountManagerListener</tt>s currently interested in the events fired by this
	 * manager.
	 */
	private final List<AccountManagerListener> listeners = new LinkedList<>();

	/**
	 * The queue of <tt>ProtocolProviderFactory</tt> services awaiting their stored accounts to be
	 * loaded.
	 */
	 ...
	 *
	 * @param bundleContext
	 * 		the <tt>BundleContext</tt> in which the new instance is loaded (and in which the
	...
	public AccountManager(BundleContext bundleContext)
	{
		...
		// purge only omemo storage - only when system locked. Wait for account loaded?
		// ServerPersistentStoresRefreshDialog.purgeOmemoStorage();
	 ...
	 *
	 * @param listener
	 * 		the <tt>AccountManagerListener</tt> to add
	 */
	...
	/**
	 * Loads the accounts stored for a specific <tt>ProtocolProviderFactory</tt>.
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt> to load the stored accounts of
	 */
	 ...
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt> which had its stored accounts just loaded
	 */
	 ...
	 *
	 * @param factory
	 * 		the factory which package will be returned.
	 ...
	 *
	 * @param protocolName
	 * 		the protocol name to check for
	 * @param includeHidden
	 * 		whether to include hidden providers
	...
	public boolean hasStoredAccount(String protocolName, boolean includeHidden, String userID)
	{
		...
		try {
			factoryRefs = this.bundleContext.getServiceReferences(
	 ...
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt> which has been registered as a service.
	 */
	 ...
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt> to be queued for loading its stored accounts as
	 * 		soon as possible
	 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>AccountManagerListener</tt> to remove
	 */
				...
				synchronized (loadStoredAccountsQueue) {
					...
					if (factory == null) {
						...
						try {
							...
							logger.warn("The loading of the stored accounts has been interrupted",
									ex);
							interrupted = true;
	 ...
	 *
	 * @param serviceEvent
	 * 		the <tt>ServiceEvent</tt> containing the event data
	 */
	 ...
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt> which created the account to be stored
	 * @param accountID
	 * 		the account in the form of <tt>AccountID</tt> to be stored
	 * @throws OperationFailedException
	 * 		if anything goes wrong while storing the account
	 */
			...
			throws OperationFailedException
	{
		...
		for (Map.Entry<String, String> entry : accountProperties.entrySet()) {
			...
			if (property.equals(ProtocolProviderFactory.PASSWORD)) {
				...
				if (!TextUtils.isEmpty(value) && (credentialsStorage != null)
						&& !credentialsStorage.storePassword(accountUuid, value)) {
		...
		/**
		 * clear the password if missing property, modification can request password delete And
		 * only if it's not stored already in encrypted form. Account registration object clears
		 * also this property in order to forget the password
		 */
		if (!accountProperties.containsKey(ProtocolProviderFactory.PASSWORD)
				&& !configurationProperties.containsKey(accountUuid + ".ENCRYPTED_PASSWORD")) {
			CredentialsStorageService credentialsStorage
					= ServiceUtils.getService(bundleContext, CredentialsStorageService.class);
	 ...
	 *
	 * @param factory
	 * 		account's protocol provider factory
	 * @param accountUID
	 * 		account for which the prefix will be returned
	 * @return configuration prefix for given <tt>accountID</tt> if exists or <tt>null</tt>
	 * otherwise
	 */
	 ...
	 * 		the <tt>ProtocolProviderFactory</tt> which created the account to be stored
	 * @param accountID
	 * 		the AccountID of the account to remove.
	...
	public boolean removeStoredAccount(ProtocolProviderFactory factory, AccountID accountID)
	{
		 ...
		 */
		if (factory == null) {
			factory = ProtocolProviderActivator.getProtocolProviderFactory(
		...
		synchronized (loadStoredAccountsQueue) {
			...
			synchronized (this.storedAccounts) {
				AccountID[] storedAccounts = this.storedAccounts
						.toArray(new AccountID[this.storedAccounts.size()]);

				for (AccountID storedAccount : storedAccounts) {
					ProtocolProviderFactory ppf = ProtocolProviderActivator
							.getProtocolProviderFactory(storedAccount.getProtocolName());
	 ...
	 * @throws OperationFailedException
	 * 		if anything goes wrong while loading the account corresponding to the specified
	 * 		<tt>accountID</tt>
	 */
			...
			throws OperationFailedException
	{
		...
		ProtocolProviderFactory providerFactory = ProtocolProviderActivator
				.getProtocolProviderFactory(accountID.getProtocolName());

		if (providerFactory.loadAccount(accountID)) {
			accountID.putAccountProperty(ProtocolProviderFactory.IS_ACCOUNT_DISABLED,
	 ...
	 * 		the identifier of the account to load
	 * @throws OperationFailedException
	 * 		if anything goes wrong while unloading the account corresponding to the specified
	 * 		<tt>accountID</tt>
	 */
			...
			throws OperationFailedException
	{
		...
		String iconPathProperty
				= accountID.getAccountPropertyString(ProtocolProviderFactory.ACCOUNT_ICON_PATH);
