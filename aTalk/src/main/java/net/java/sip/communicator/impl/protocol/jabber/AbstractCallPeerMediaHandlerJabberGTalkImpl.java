...
/**
 * An implementation of the <tt>CallPeerMediaHandler</tt> abstract class for the common part of
...
public abstract class AbstractCallPeerMediaHandlerJabberGTalkImpl<T extends AbstractCallPeerJabberGTalkImpl<?, ?, ?>>
	...
	/**
	 * Indicates if the <tt>CallPeer</tt> will support </tt>inputevt</tt> extension (i.e. will be
	 ...
	 *
	 * @param peer
	 * 		that <tt>AbstractCallPeerJabberGTalkImpl</tt> instance that we will be managing media
	 * 		for.
	 */
	...
	/**
	 * Detects and adds ZRTP available encryption method present in the description given in
	 * parameter.
	 *
	 * @param isInitiator
	 * 		True if the local call instance is the initiator of the call. False otherwise.
	 * @param description
	 * 		The DESCRIPTION element of the JINGLE element which contains the PAYLOAD-TYPE and
	 * 		(more important here) the ENCRYPTION.
	 * @param mediaType
	 * 		The type of media (AUDIO or VIDEO).
	 */
	protected void addZrtpAdvertisedEncryptions(boolean isInitiator,
			RtpDescriptionPacketExtension description, MediaType mediaType) {
		CallPeer peer = getPeer();
		Call call = peer.getCall();

		/*
		 * ZRTP is not supported in telephony conferences utilizing the server-side technology Jitsi
		 * Videobridge yet.
		 */
	...
	/**
	 * Detects and adds SDES available encryption method present in the description given in
	 * parameter.
	 *
	 * @param isInitiator
	 * 		True if the local call instance is the initiator of the call. False otherwise.
	 ...
	 * 		The type of media (AUDIO or VIDEO).
	 */
	protected void addSDesAdvertisedEncryptions(boolean isInitiator,
			RtpDescriptionPacketExtension description, MediaType mediaType) {
		CallPeer peer = getPeer();

		/*
		 * SDES is not supported in telephony conferences utilizing the server-side technology Jitsi
		 * Videobridge yet.
		 */
		...
		if (encryptionPacketExtension != null) {
			...
			if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION, true)
				...
				SDesControl sdesControl = (SDesControl) srtpControls.getOrCreate(mediaType,
						SrtpControlType.SDES);
				SrtpCryptoAttribute selectedSdes = selectSdesCryptoSuite(isInitiator, sdesControl,
	 ...
	 * 		True if the local call instance is the initiator of the call. False otherwise.
	 * @param sDesControl
	 * 		The SDES based SRTP MediaStream encryption control.
	 * @param encryptionPacketExtension
	 * 		The ENCRYPTION element received from the remote peer. This may contain the SDES crypto
	 * 		suites available for the remote peer.
	 * @return The selected SDES crypto suite supported by both the local and the remote peer. Or
	 * null, if there is no crypto suite supported by both of the peers.
	 */
	protected SrtpCryptoAttribute selectSdesCryptoSuite(boolean isInitiator,
			SDesControl sDesControl, EncryptionPacketExtension encryptionPacketExtension) {
		List<CryptoPacketExtension> cryptoPacketExtensions
				= encryptionPacketExtension.getCryptoList();
		List<SrtpCryptoAttribute> peerAttributes = new ArrayList<SrtpCryptoAttribute>(
				cryptoPacketExtensions.size());
	 ...
	 *
	 * @param encryptionPacketExtension
	 * 		The ENCRYPTION element received from the remote peer. This may contain the ZRTP packet
	 * 		element for the remote peer.
	 ...
	 *
	 * @param mediaType
	 * 		The type of media we are modifying the DESCRIPTION to integrate the ENCRYPTION
	 * 		element.
	 * @param description
	 ...
	 */
	protected boolean setZrtpEncryptionOnDescription(MediaType mediaType,
			RtpDescriptionPacketExtension description, RtpDescriptionPacketExtension remoteDescription) {
		CallPeer peer = getPeer();
		Call call = peer.getCall();
		...
		if (isRemoteZrtpCapable) {
			...
			if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION, true)
					&& accountID.isEncryptionProtocolEnabled(SrtpControlType.ZRTP)
					&& call.isSipZrtpAttribute()) {
				ZrtpControl zrtpControl = (ZrtpControl) getSrtpControls().getOrCreate(mediaType,
	 ...
	 */
	protected boolean setSDesEncryptionOnDescription(MediaType mediaType,
		...
		if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION, true)
			...
			String ciphers = accountID
					.getAccountPropertyString(ProtocolProviderFactory.SDES_CIPHER_SUITES);

			if (ciphers == null) {
				ciphers = JabberActivator.getResources().getSettingsString(
			...
			// act as initiator
			if (remoteDescription == null) {
				EncryptionPacketExtension localEncryption = localDescription
			...
			// act as responder
				...
				if (remoteEncryption != null) {
					SrtpCryptoAttribute selectedSdes = selectSdesCryptoSuite(false, sdesControl,
							remoteEncryption);
	 ...
	 */
	protected void setAndAddPreferredEncryptionProtocol(MediaType mediaType,
			RtpDescriptionPacketExtension localDescription,
			RtpDescriptionPacketExtension remoteDescription) {
	 ...
	 * 		The element containing the media DESCRIPTION and its encryption for the remote peer;
	 * 		<tt>null</tt> if the local peer is the initiator of the call.
	 * @return <tt>true</tt> if the specified encryption protocol has been selected; <tt>false</tt>,
	 * otherwise
	 */
	protected boolean setAndAddPreferredEncryptionProtocol(SrtpControlType srtpControlType,
			MediaType mediaType, RtpDescriptionPacketExtension localDescription,
			RtpDescriptionPacketExtension remoteDescription) {
		/*
