...
public class MediaAwareCallConference extends CallConference
{
	 ...
	 * @param useCase
	 * 		the <tt>MediaUseCase</tt> associated with the intended utilization of the
	...
	public RTPTranslator getRTPTranslator(MediaType mediaType)
	{
		...
		if (MediaType.VIDEO.equals(mediaType) && (!OSUtils.IS_ANDROID || isConferenceFocus())) {
			if (videoRTPTranslator == null) {
				videoRTPTranslator
						= ProtocolMediaActivator.getMediaService().createRTPTranslator();
			}
	...
	private void propertyChange(PropertyChangeEvent ev)
	{
		...
		if (MediaService.DEFAULT_DEVICE.equals(propertyName)) {
			...
			if (source instanceof MediaService) {
				...
				if (newValue == null) {
					newValue = ProtocolMediaActivator.getMediaService().getDefaultDevice(
							MediaType.AUDIO, MediaUseCase.ANY);
				}
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the media which is to be captured and/or played back by the
	 * 		specified <tt>device</tt>
	 * @param device
	 * 		the <tt>MediaDevice</tt> to be used by this telephony conference for capture and/or
	...
	private static class WeakPropertyChangeListener implements PropertyChangeListener
	{
		 ...
		 *
		 * @param notifier
		 * 		the <tt>PropertyChangeNotifier</tt> the new instance is to listen to
		 */
