...
public class SIPAccountRegistration extends SipAccountID implements Serializable
{
	...
	/**
	 * If default domain is set this means we cannot create registerless accounts through this
	 * wizard. And every time we write only the username, will will end up with
	 * username@defaultDomain.
	 *
	 * @param defaultDomain
	 * 		the defaultDomain to set
	 */
	 ...
	 *
	 * @param accountID
	 * 		the account identifier that will be used.
	 * @param bundleContext
	...
	{
		mergeProperties(accountID.getAccountProperties(), mAccountProperties);
		...
		encodingsRegistration.loadAccount(accountID,
	 ...
	 * 		flag indication if it's modification process(has impact on some properties).
	 * @param accountProperties
	 * 		the map that will hold the configuration.
	 */
	public void storeProperties(String userName, String passwd, String protocolIconPath,
			String accountIconPath, Boolean isModification, Map<String, String> accountProperties)
	{
		...
		if (serverAddress != null) {
			...
			if (!userName.contains(serverAddress))
				accountProperties.put(ProtocolProviderFactory.IS_SERVER_OVERRIDDEN,
