...
public class ConfigurationUtils
{
	...
	private static final String SMS_MSG_NOTIFY_TEXT_DISABLED_PROP
			= "gui.contactlist.SMS_MSG_NOTIFY_TEXT_DISABLED_PROP";
	...
	private static String pRouteVideoAndDesktopUsingPhoneNumber
			= "gui.ROUTE_VIDEO_AND_DESKTOP_TO_PNONENUMBER";
	...
	private static String pHideExtendedAwayStatus
			= "protocol.globalstatus.HIDE_EXTENDED_AWAY_STATUS";
	...
	public static void loadGuiConfigurations()
	{
		...
		isApplicationVisible
				= configService.getBoolean("systray.showApplication", isApplicationVisible);
		...
		if (StringUtils.isNullOrEmpty(isSendTypingNotification))
			isSendTypingNotification
					= UtilActivator.getResources().getSettingsString(pTypingNotification);
		...
		String isMultiChatWindowEnabledString
				= configService.getString(pMultiChatWindowEnabled);

		if (StringUtils.isNullOrEmpty(isMultiChatWindowEnabledString))
			isMultiChatWindowEnabledString = UtilActivator.getResources()
					.getSettingsString(pMultiChatWindowEnabled);
		...
		String isLeaveChatRoomOnWindowCloseEnabledString
				= configService.getString(pLeaveChatRoomOnWindowClose);

		if (StringUtils.isNullOrEmpty(isLeaveChatRoomOnWindowCloseEnabledString)) {
			isLeaveChatRoomOnWindowCloseEnabledString = UtilActivator.getResources()
					.getSettingsString(pLeaveChatRoomOnWindowClose);
		}

		if (!StringUtils.isNullOrEmpty(isLeaveChatRoomOnWindowCloseEnabledString)) {
			isLeaveChatRoomOnWindowCloseEnabled
					= Boolean.parseBoolean(isLeaveChatRoomOnWindowCloseEnabledString);
		}
		...
		if (StringUtils.isNullOrEmpty(isHistoryShownString))
			isHistoryShownString
					= UtilActivator.getResources().getSettingsString(pMessageHistoryShown);
		...
		if (StringUtils.isNullOrEmpty(chatHistorySizeString))
			chatHistorySizeString = UtilActivator.getResources()
					.getSettingsString(pChatHistorySize);
		...
		if (StringUtils.isNullOrEmpty(chatWriteAreaSizeString))
			chatWriteAreaSizeString = UtilActivator.getResources()
					.getSettingsString(pChatWriteAreaSize);
		...
		String isTransparentWindowEnabledString
				= configService.getString(pTransparentWindowEnabled);

		if (StringUtils.isNullOrEmpty(isTransparentWindowEnabledString))
			isTransparentWindowEnabledString = UtilActivator.getResources()
					.getSettingsString(pTransparentWindowEnabled);
		...
		if (StringUtils.isNullOrEmpty(windowTransparencyString))
			windowTransparencyString = UtilActivator.getResources()
					.getSettingsString(pWindowTransparency);
		...
		if (StringUtils.isNullOrEmpty(isWindowDecoratedString))
			isWindowDecoratedString
					= UtilActivator.getResources().getSettingsString(pIsWindowDecorated);
		...
		isChatToolbarVisible
				= configService.getBoolean("gui.chat.ChatWindow.showToolbar", true);
		// Load the "isChatToolbarVisible" property
		isChatStyleBarVisible
				= configService.getBoolean("gui.chat.ChatWindow.showStylebar", true);
		...
		lastContactParent
				= configService.getString("gui.addcontact.lastContactParent");

		// Load the "sendFileLastDir" property.
		sendFileLastDir
				= configService.getString("gui.chat.filetransfer.SEND_FILE_LAST_DIR");

		// Load the "ADD_CONTACT_DISABLED" property.
		isAddContactDisabled
				= configService.getBoolean("gui.contactlist.CONTACT_ADD_DISABLED", false);

		// Load the "MERGE_CONTACT_DISABLED" property.
		isMergeContactDisabled
				= configService.getBoolean("gui.contactlist.CONTACT_MERGE_DISABLED", false);

		// Load the "CREATE_GROUP_DISABLED" property.
		isCreateGroupDisabled
				= configService.getBoolean("gui.contactlist.CREATE_GROUP_DISABLED", false);

		// Load the "FLATTEN_GROUP_ENABLED" property.
		isFlattenGroupEnabled
				= configService.getBoolean("gui.contactlist.FLATTEN_GROUP_ENABLED", false);

		// Load the "GO_TO_CHATROOM_DISABLED" property.
		isGoToChatRoomDisabled = configService.getBoolean(
				"gui.chatroomslist.GO_TO_CHATROOM_DISABLED", false);

		// Load the "REMOVE_CONTACT_DISABLED" property.
		isRemoveContactDisabled
				= configService.getBoolean("gui.contactlist.CONTACT_REMOVE_DISABLED", false);

		// Load the "CONTACT_MOVE_DISABLED" property.
		isContactMoveDisabled
				= configService.getBoolean("gui.contactlist.CONTACT_MOVE_DISABLED", false);

		// Load the "CONTACT_RENAME_DISABLED" property.
		isContactRenameDisabled
				= configService.getBoolean("gui.contactlist.CONTACT_RENAME_DISABLED", false);

		// Load the "GROUP_REMOVE_DISABLED" property.
		isGroupRemoveDisabled
				= configService.getBoolean("gui.contactlist.GROUP_REMOVE_DISABLED", false);

		// Load the "GROUP_RENAME_DISABLED" property.
		isGroupRenameDisabled
				= configService.getBoolean("gui.contactlist.GROUP_RENAME_DISABLED", false);

		// Load the "PRESET_STATUS_MESSAGES" property.
		isPresetStatusMessagesEnabled
				= configService.getBoolean("gui.presence.PRESET_STATUS_MESSAGES", true);
		...
		isAdvancedAccountConfigDisabled
				= configService.getBoolean("gui.account.ADVANCED_CONFIG_DISABLED",
				isAdvancedConfigDisabled);

		// Single interface enabled property.
		String singleInterfaceEnabledProp
				= UtilActivator.getResources().getSettingsString(SINGLE_WINDOW_INTERFACE_ENABLED);
		...
		isSingleWindowInterfaceEnabled
				= configService.getBoolean(SINGLE_WINDOW_INTERFACE_ENABLED, isEnabled);
		...
		if (isFontSupportEnabled()) {
			...
			isDefaultFontBold
					= configService.getBoolean("gui.chat.DEFAULT_FONT_BOLD", isDefaultFontBold);

			// Load isItalic chat property.
			isDefaultFontItalic = configService.getBoolean(
					"gui.chat.DEFAULT_FONT_ITALIC", isDefaultFontItalic);

			// Load isUnderline chat property.
			isDefaultFontUnderline = configService.getBoolean(
					"gui.chat.DEFAULT_FONT_UNDERLINE", isDefaultFontUnderline);
		...
		String showStatusChangedInChatDefault
				= UtilActivator.getResources().getSettingsString(pShowStatusChangedInChat);
		...
		showStatusChangedInChat = configService.getBoolean(pShowStatusChangedInChat,
		...
		routeVideoAndDesktopUsingPhoneNumber
				= configService.getBoolean(pRouteVideoAndDesktopUsingPhoneNumber,
				routeVideoAndDesktopUsingPhoneNumber);

		String hideAccountMenuDefaultValue
				= UtilActivator.getResources().getSettingsString(pHideAccountMenu);
		...
		hideAccountSelectionWhenPossible
				= configService.getBoolean(pHideAccountMenu, hideAccountSelectionWhenPossible);
		...
		autoAnswerDisableSubmenu
				= configService.getBoolean(pAutoAnswerDisableSubmenu, autoAnswerDisableSubmenu);

		isChatRoomConfigDisabled = configService.getBoolean(CHAT_ROOM_CONFIG_DISABLED_PROP,
		...
		acceptPhoneNumberWithAlphaChars
				= configService.getBoolean("gui.ACCEPT_PHONE_NUMBER_WITH_ALPHA_CHARS", true);
		...
		String hideExtendedAwayStatusDefaultValue
				= UtilActivator.getResources().getSettingsString(pHideExtendedAwayStatus);
		...
		hideExtendedAwayStatus = configService.getBoolean(pHideExtendedAwayStatus,
				hideExtendedAwayStatus);

		isSmsNotifyTextDisabled = configService.getBoolean(SMS_MSG_NOTIFY_TEXT_DISABLED_PROP,
	 ...
	 *
	 * @param autoPopup
	 * 		indicates to the user interface whether new messages should be opened and bring to
	 * 		front.
	 **/
	 ...
	 *
	 * @param isChatStateNotification
	 * 		<code>true</code> to indicate that chat state notifications are enabled,
	 ...
	 *
	 * @param isEnabled
	 * 		indicates if the chat window could contain multiple chats or only one chat.
	 */
	public static void setMultiChatWindowEnabled(boolean isEnabled)
	{
		isMultiChatWindowEnabled = isEnabled;
		configService.setProperty(pMultiChatWindowEnabled,
	...
	{
		return configService.getBoolean(MessageHistoryService.PNAME_IS_MESSAGE_HISTORY_ENABLED,
				true);
	}
	 ...
	 *
	 * @param isShown
	 * 		indicates if the recent messages is shown
	 */
	 ...
	 *
	 * @param newMessageCommand
	 * 		the command used to send a message ( it could be ENTER_COMMAND or CTRL_ENTER_COMMAND)
	 */
	...
	public static ProtocolProviderService getLastCallConferenceProvider()
	{
		...
		return findProviderFromAccountId(
				configService.getString("gui.call.lastCallConferenceProvider"));
	}

	/**
	 * Returns the protocol provider associated with the given <tt>accountId</tt>.
	 *
	 * @param savedAccountId
	 ...
	 *
	 * @param isNormalize
	 * 		indicates to the user interface whether all dialed phone numbers should be normalized
	 */
	 ...
	 *
	 * @param isEnabled
	 * 		indicates whether to enable or disable alerter.
	 */
	 ...
	 *
	 * @param accept
	 * 		indicates to the user interface whether a string with alphabetical characters might be
	 ...
	 *
	 * @param isEnabled
	 * 		<code>true</code> to indicate that the single window interface is enabled,
	 ...
	 *
	 * @param isShowOffline
	 * 		<code>true</code> to indicate that the offline users should be shown,
	 * 		<code>false</code> otherwise.
	 */
	 ...
	 * 		<code>true</code> to indicate that the call panel should be shown, <code>false</code>
	 * 		otherwise.
	 */
	 ...
	 *
	 * @param isVisible
	 * 		<code>true</code> to indicate that the application should be shown, <code>false</code>
	 * 		otherwise.
	 */
	 ...
	 *
	 * @param isWarningShown
	 * 		indicates if the message warning the user that the application would not be closed if
	 * 		she clicks the X
	 ...
	 *
	 * @param groupName
	 * 		the group name of the selected group when adding last contact
	 */
	 ...
	 *
	 * @param isRequested
	 * 		indicates if a confirmation would be requested from user during the move contact
	 * 		process.
	 */
	 ...
	 *
	 * @param isTransparent
	 * 		indicates if the transparency is enabled in the application.
	 */
	public static void setTransparentWindowEnabled(boolean isTransparent)
	{
		isTransparentWindowEnabled = isTransparent;
		configService.setProperty(pTransparentWindowEnabled,
	 ...
	 *
	 * @param isVisible
	 * 		indicates if the chat toolbar is visible.
	 */
	public static void setChatToolbarVisible(boolean isVisible)
	{
		isChatToolbarVisible = isVisible;
		configService.setProperty("gui.chat.ChatWindow.showToolbar",
	 ...
	 *
	 * @param isEnabled
	 * 		indicates if the chat simple theme is enabled
	 */
	public static void setChatSimpleThemeEnabled(boolean isEnabled)
	{
		isChatSimpleThemeEnabled = isEnabled;
		configService.setProperty(CHAT_SIMPLE_THEME_ENABLED_PROP,
	 ...
	 *
	 * @param isVisible
	 * 		indicates if the chat styleBar is visible.
	 */
	public static void setChatStyleBarVisible(boolean isVisible)
	{
		isChatStyleBarVisible = isVisible;
		configService.setProperty("gui.chat.ChatWindow.showStylebar",
	 ...
	 *
	 * @param isBold
	 * 		indicates if the default chat font is bold
	 */
	 ...
	 *
	 * @param isItalic
	 * 		indicates if the default chat font is italic
	 */
	...
	public static Locale getCurrentLanguage()
	{
		...
		return (localeId != null)
				? ResourceManagementServiceUtils.getLocale(localeId) : Locale.getDefault();
	}
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider to which the chat room belongs
	 * @param oldChatRoomId
	 * 		the old identifier of the chat room
	 * @param newChatRoomId
	 * 		the new identifier of the chat room  = newChatRoomName
	 */
	public static void saveChatRoom(ProtocolProviderService protocolProvider, String oldChatRoomId,
			String newChatRoomId)
	{
		...
		if (cursor.getCount() > 0) {
			if (!oldChatRoomId.equals(newChatRoomId)) {
				...
				mDB.update(ChatSession.TABLE_NAME, contentValues,
	 ...
	 * 		the protocol provider to which the chat room belongs
	 * @param chatRoomId
	 * 		the identifier of the chat room to remove
	 */
	 ...
	 * 		the protocol provider to which the chat room belongs
	 * @param chatRoomId
	 * 		the identifier of the chat room to update
	 * @param chatRoomStatus
	 * 		the new status of the chat room
	 */
	 ...
	 * 		the identifier of the chat room to update
	 * @param property
	 * 		the name of the property of the chat room
	 * @param value
	 * 		the value of the property if null, property will be removed
	 */
	 ...
	 * 		the identifier of the chat room
	 * @param property
	 * 		the property name, saved through the <tt>ConfigurationService</tt>.
	 ...
	 */
	private static JSONObject getChatRoomAttributes(ProtocolProviderService protocolProvider,
			String chatRoomId)
	{
	 ...
	 * 		the protocol provider, to which the chat room belongs
	 * @param chatRoomID
	 * 		the chat room id (cmeng: can contain account serviceName e.g. example.org??)
	 * @return the chatRoom sessionUid saved in <tt>ConfigurationService</tt>.
	 */
	public static String getChatRoomPrefix(ProtocolProviderService protocolProvider,
			String chatRoomID)
	{
	 ...
	 *
	 * @param groupID
	 * 		the identifier of the group (prefixed with group)
	 * @param isCollapsed
	...
	public static void setContactListGroupCollapsed(String groupID, boolean isCollapsed)
	{
		...
		for (String groupRootPropName : groups) {
			...
			if (storedID.equals(groupID)) {
				configService.setProperty(groupRootPropName + ".isClosed",
	...
	public static boolean isContactListGroupCollapsed(String groupID)
	{
		...
		for (String groupRootPropName : groups) {
			...
			if (storedID.equals(groupID)) {
				String status = (String) configService.getProperty(
						groupRootPropName + ".isClosed");
	 ...
	 *
	 * @param factory
	 * 		the <tt>ProtocolProviderFactory</tt>, which package name we're looking for
	...
	{
		return configService.getInt(ProtocolProviderFactory.PREFERRED_CLEAR_PORT_PROPERTY_NAME,
				5060);
	}
	...
	{
		configService.setProperty(ProtocolProviderFactory.PREFERRED_CLEAR_PORT_PROPERTY_NAME,
				port);
	}
	...
	{
		return configService.getInt(ProtocolProviderFactory.PREFERRED_SECURE_PORT_PROPERTY_NAME,
				5061);
	}
	...
	{
		configService.setProperty(ProtocolProviderFactory.PREFERRED_SECURE_PORT_PROPERTY_NAME,
				port);
	}
	...
	public static String[] getEnabledSslProtocols()
	{
		String enabledSslProtocols
				= configService.getString("gov.nist.javax.sip.TLS_CLIENT_PROTOCOLS");
	...
	public static boolean hasEnabledVideoFormat(ProtocolProviderService protocolProvider)
	{
		...
		if (Boolean.parseBoolean(overrideEncodings)) {
			encodingConfiguration
					= UtilActivator.getMediaService().createEmptyEncodingConfiguration();
	 ...
	 *
	 * @param isChatStateEnable
	 * 		indicates whether ChatState feature is enable or disable
	 */
		...
		for (ProtocolProviderService pps : ppServices) {
			...
			if (connection != null) {
				...
				OperationSetContactCapabilitiesJabberImpl
						.setOperationSetChatStateFeatures(isChatStateEnable);
