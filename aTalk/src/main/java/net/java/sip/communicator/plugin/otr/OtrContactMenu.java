 ...
 * Off-the-Record functionality for a specific otrContact.
 *
...
class OtrContactMenu implements ActionListener, ScOtrEngineListener, ScOtrKeyManagerListener
{
    ...
    public OtrContactMenu(OtrContact otrContact, boolean inMacOSXScreenMenuBar, JMenu menu, boolean isSeparateMenu)
    {
        ...
        setSessionStatus(OtrActivator.scOtrEngine.getSessionStatus(this.otrContact));
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if (ACTION_COMMAND_END_OTR.equals(actionCommand)) {
            ...
            // Refresh session.
            OtrActivator.scOtrEngine.refreshSession(otrContact);

        else if (ACTION_COMMAND_AUTHENTICATE_BUDDY.equals(actionCommand))
            // Launch auth buddy dialog.
            SwingOtrActionHandler.openAuthDialog(otrContact);
            ...
            OtrActivator.scOtrEngine.setContactPolicy(otrContact.contact, null);
    }
    ...
    public void contactPolicyChanged(Contact contact)
    {
        ...
        if (OtrContactMenu.this.otrContact != null && contact.equals(OtrContactMenu.this.otrContact.contact))
    ...
    {
        if (otrContact.equals(OtrContactMenu.this.otrContact))
            setSessionStatus(OtrActivator.scOtrEngine.getSessionStatus(otrContact));
    }
     ...
     * and the {@link OtrPolicy} for {@link OtrContactMenu#otrContact}.
     */
    private void buildMenu()
    {
        ...
        cbAlways.setText(String.format(
                OtrActivator.resourceService.getI18NString("plugin.otr.menu.CB_AUTO", new String[]{otrContact.contact.getDisplayName()})));
    ...
    private void setSessionStatus(ScSessionStatus sessionStatus)
    {
        if (sessionStatus != this.sessionStatus) {
            logger.debug("Setting session status of contact " + otrContact.contact + " to " + sessionStatus + ". Was " + this.sessionStatus);
    ...
    private void setOtrPolicy(OtrPolicy otrPolicy)
    {
        if (!otrPolicy.equals(this.otrPolicy)) {
            ...
            if ((separateMenu != null) && (separateMenu.isPopupMenuVisible() || inMacOSXScreenMenuBar)) {
                buildMenu();
            }
        }
    }
    ...
    private void updateIcon()
    {
        ...
        switch (sessionStatus) {
            case ENCRYPTED:
                ...
                imageID = OtrActivator.scOtrKeyManager.isVerified(otrContact.contact, fingerprint)
                        ? "plugin.otr.ENCRYPTED_ICON_16x16" : "plugin.otr.ENCRYPTED_UNVERIFIED_ICON_16x16";
                break;
