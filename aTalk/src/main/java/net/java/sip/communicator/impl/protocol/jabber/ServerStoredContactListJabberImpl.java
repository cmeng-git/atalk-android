...
public class ServerStoredContactListJabberImpl
{
	...
	/**
	 * The operation set that created us and that we could use when dispatching subscription
	 * events.
	 */
	 ...
	 */
	final private Vector<ServerStoredGroupListener> serverStoredGroupListeners = new Vector<>();
	 ...
	 */
	final private Object rosterInitLock = new Object();
	 ...
	 * 		the operation set that created us and that we could use for dispatching subscription
	 * 		events
	 * @param provider
	 ...
	 *
	 * @param userJid
	 * 		the XMPP address of the user (e.g. "jsmith@example.com"). The address could be in any
	 ...
	 */
	RosterEntry getRosterEntry(Jid userJid)
	{
		if ((roster == null) || (userJid == null))
			return null;
		else
			return roster.getEntry(userJid.asBareJid());
	}
	 ...
	 *
	 * @param listener
	 * 		the ServerStoredGroupListener to register for group events
	 */
	 ...
	 *
	 * @param listener
	 * 		the ServerStoredGroupListener to unregister
	 */
	...
	private void fireGroupEvent(ContactGroupJabberImpl group, int eventID)
	{
		...
		ServerStoredGroupEvent evt = new ServerStoredGroupEvent(group, eventID,
				parentOperationSet.getServerStoredContactListRoot(), jabberProvider,
				parentOperationSet);
			...
			while (iter.hasNext()) {
				...
				if (roster != null) {
					parentOperationSet.firePresenceStatusChanged(
							roster.getPresence(c.getJid().asBareJid()));
				}
			}
		}
	 ...
	 *
	 * @param parentGroup
	 * 		the group where that the removed contact belonged to.
	 * @param contact
	 ...
	 *
	 * @param oldParentGroup
	 * 		the group where the source contact was located before being moved
	 * @param newParentGroup
	 * 		the group that the source contact is currently in.
	 * @param contact
	 * 		the contact that was added
	 */
	private void fireContactMoved(ContactGroup oldParentGroup, ContactGroup newParentGroup,
	 ...
	 *
	 * @param name
	 * 		the name of the group we're looking for.
	 ...
	 *
	 * @param id
	 * 		the contactJid of the contact to find.
	 ...
	 *
	 * @param child
	 * 		the contact whose parent group we're looking for.
	 ...
	 * 		OperationFailedException, XmppStringprepException
	 */
	 ...
	 * 		the id of the contact to add.
	 * @param parent
	 * 		the group under which we want the new contact placed.
	 * @throws OperationFailedException
			...
			throws OperationFailedException, XmppStringprepException
	{
		...
		String bareId = parseAddressString(id);
		final BareJid contactJid = JidCreate.bareFrom(bareId);

		// if the contact is already in the contact list and is not volatile, then only broadcast
		// an event - should also check new owner against existing old owner, not just bareJid.
		String accountUuid = parent.getProtocolProvider().getAccountID().getAccountUuid();
		String mcGroupName = parent.getGroupName();
		String[] args = {accountUuid, mcGroupName, bareId};
		...
		if (cursor.getCount() > 0) {
			...
			logger.warn("Contact " + bareId + " already exists in group "
					+ findContactGroup(bareId));
			throw new OperationFailedException("Contact " + bareId + " already exist",
		...
		StanzaListener presenceInterceptor = new StanzaListener()
		{
			...
			public void processStanza(Stanza packet)
			{
				...
				if ((presence.getType() == Presence.Type.subscribe)
						// && bareId.equals(XmppStringUtils.parseBareJid(presence.getTo()))) {
						&& presence.getTo().isParentOf(contactJid)) {
					Nick nicknameExt = new Nick(JabberActivator.getGlobalDisplayDetailsService()
		...
		xmppConnection.addPacketInterceptor(presenceInterceptor,
		...
		try {
			this.roster.createEntry(contactJid, bareId, parentNames);
		}
			...
			if (error != null) {
				if (error.getCondition().equals(XMPPError.Condition.forbidden))
					errorCode = OperationFailedException.FORBIDDEN;
				else if (error.getCondition().equals(XMPPError.Condition.internal_server_error))
		...
		catch (NotLoggedInException | NoResponseException | NotConnectedException
	 ...
	 * 		indicates if the contact should be private messaging contact or not.
	 * @param displayName
		...
		if (theVolatileGroup == null) {
			theVolatileGroup = new VolatileContactGroupJabberImpl(
					JabberActivator.getResources().getI18NString(
							"service.gui.NOT_IN_CONTACT_LIST_GROUP_NAME"), this);
	 ...
	 *
	 * @param parentGroup
	 * 		the group where the unresolved contact is to be created
	 * @param id
	 * 		the Address of the contact to create.
	 * @return the newly created unresolved <tt>ContactImpl</tt>
	 */
	synchronized ContactJabberImpl createUnresolvedContact(ContactGroup parentGroup, String id)
	{
		String bareId = parseAddressString(id);
		ContactJabberImpl existingContact = findContactById(bareId);
	 ...
	 *
	 * @param groupName
	 * 		a String containing the name of the new group.
	 * @throws OperationFailedException
	 * 		with code CONTACT_GROUP_ALREADY_EXISTS if the group we're trying to create is already
	 * 		in our contact list.
	 */
	 ...
	 *
	 * @param groupToRemove
	 * 		the group that we'd like removed.
	 */
	 ...
	 *
	 * @param contact
	 * 		the contact that we'd like moved under the new group.
	 * @param newParent
	 * 		the group where we'd like the parent placed.
	 */
			...
			throws OperationFailedException
	{
		...
		if (!contact.isPersistent()) {
			...
			catch (OperationFailedException | XmppStringprepException ex) {
	 ...
	 * before sending initial presence We extend this and send it after we have dispatched the
	 * roster.
	 * <p>
	...
	private synchronized void initRoster()
	{
		...
		if (roster.getUnfiledEntryCount() > 0) {
			for (RosterEntry item : roster.getUnfiledEntries()) {
				ContactJabberImpl contact = findContactById(item.getJid().toString());
				...
				if (contact == null) {
					...
					// Do not move contact or if request if of the same group.
				...
				parentOperationSet.firePresenceStatusChanged(roster.getPresence(item.getJid()));
			}
		}
		...
		for (RosterGroup item : roster.getGroups()) {
			...
			if (group != null) {
				// the group exist so just resolved. The group will check and create or resolve its
				// entries
		...
		for (RosterGroup item : roster.getGroups()) {
			...
			if (group == null) {
				...
				ContactGroupJabberImpl newGroup = new ContactGroupJabberImpl(item,
						item.getEntries().iterator(), this, true);
				rootGroup.addSubGroup(newGroup);
				...
				if (roster != null) {
					...
					while (cIter.hasNext()) {
						...
						BareJid contactJid = null;
						try {
							contactJid = JidCreate.bareFrom(address);
						}
						catch (XmppStringprepException e) {
							e.printStackTrace();
						}
						parentOperationSet.firePresenceStatusChanged(
								roster.getPresence(contactJid));
					}
				}
			}
		}
	...
	{
		String groupName = JabberActivator.getResources().getI18NString(
	...
	public void fireContactAdded(ContactGroup parentGroup, ContactJabberImpl contact)
	{
		...
		if (roster != null) {
			parentOperationSet.firePresenceStatusChanged(
					roster.getPresence(contact.getJid().asBareJid()));
		}
	 ...
	 *
	 * @param parentGroup
	 * 		the group that the resolved contact belongs to.
	 * @param contact
	...
	public void fireContactResolved(ContactGroup parentGroup, ContactJabberImpl contact)
	{
		...
		// check
		if (roster != null) {
			parentOperationSet.firePresenceStatusChanged(roster.getPresence(
	 ...
	 * contacts from their address book to the roster and those contacts are with subscription
	 * none.
	...
	private void contactDeleted(ContactJabberImpl contact)
	{
		...
		if (group instanceof ContactGroupJabberImpl) {
			...
			// if the group is empty remove it from root group. This group will be removed from
			// server if empty
	...
	private class ChangeListener implements RosterListener
	{
		...
		public void entriesAdded(Collection<Jid> addresses)
		{
			...
			for (Jid id : addresses) {
				addEntryToContactList(id.asBareJid());
			}
		}
		 ...
		 */
		private ContactJabberImpl addEntryToContactList(BareJid rosterEntryID)
		{
			...
			ContactJabberImpl contact = findContactById(entry.getJid().toString());

			if (contact == null) {
				contact = findPrivateContactByRealId(entry.getJid().toString());
			}
			...
			contact = new ContactJabberImpl(entry, ServerStoredContactListJabberImpl.this, true,
					true);
			...
			for (RosterGroup group : entry.getGroups()) {
				...
				if (parentGroup != null) {
					...
					// create the group as it doesn't exist
					ContactGroupJabberImpl newGroup = new ContactGroupJabberImpl(group,
							group.getEntries().iterator(),
		...
		private ContactJabberImpl findPrivateContactByRealId(String id)
		{
			...
			while (it.hasNext()) {
				...
				if (contact.getPersistableAddress() == null)
					continue;
				if (contact.getPersistableAddress().equals(XmppStringUtils.parseBareJid(id))) {
		...
		public void entriesUpdated(Collection<Jid> addresses)
		{
			...
			for (Jid contactJid : addresses) {
				RosterEntry entry = roster.getEntry(contactJid.asBareJid());
				ContactJabberImpl contact = addEntryToContactList(contactJid.asBareJid());
					...
					if (cgr == null) {
						...
						if (group != null) {
							...
							if (currentParentGroup.countContacts() > 1) {
								...
								if (!present
										&& currentParentGroup instanceof ContactGroupJabberImpl) {
									ContactGroupJabberImpl currentGroup
											= (ContactGroupJabberImpl) currentParentGroup;
									currentGroup.setSourceGroup(gr);

									fireGroupEvent(currentGroup,
					...
					if (cgr != null) {
						...
						if (!gr.getName().equals(contactGroup.getGroupName())) {
							...
							if (newParentGroup == null) {
								// create the group as it doesn't exist
								newParentGroup = new ContactGroupJabberImpl(gr,
		...
		public void entriesDeleted(Collection<Jid> addresses)
		{
			for (Jid contactJid : addresses) {
				...
				ContactJabberImpl contact = findContactById(contactJid.toString());
	...
	private class ImageRetriever extends Thread
	{
		...
		private byte[] getAvatar(ContactJabberImpl contact)
		{
			...
			if (result == null) {
				...
				try {
					Iterator<ServerStoredDetails.GenericDetail> iter = infoRetriever.getDetails(
							userJid, ServerStoredDetails.ImageDetail.class);
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Cannot load image for contact " + contact + ": "
								+ ex.getMessage(), ex);
					}
				}
				if (result == null) {
		...
		try {
			...
			for (ServiceReference r : refs) {
				CustomAvatarService avatarService
						= (CustomAvatarService) JabberActivator.bundleContext.getService(r);
			...
			ContactJabberImpl contact)
	{
		...
		if (oldGroup instanceof ContactGroupJabberImpl && oldGroup.countContacts() == 0) {
			...
			fireGroupEvent((ContactGroupJabberImpl) oldGroup,
	 ...
	 *
	 * @param id
	 * 		the initial identifier as added by the user
	 */
