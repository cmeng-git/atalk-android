...
public class ScOtrEngineImpl implements ScOtrEngine, ChatLinkClickedListener, ServiceListener
{
			...
			public void sessionStatusChanged(SessionID sessionID)
			{
				...
				switch (session.getSessionStatus()) {
					case ENCRYPTED:
							...
							String unverifiedSessionWarning = OtrActivator.resourceService
									.getI18NString("plugin.otr.activator.unverifiedsessionwarning",
											new String[]{sender + resourceName,
													this.getClass().getName(), "AUTHENTIFICATION",
													sessionGuid.toString()});
							OtrActivator.uiService.getChat(contact).addMessage(
									sender, new Date(), Chat.SYSTEM_MESSAGE,
									unverifiedSessionWarning, ChatMessage.ENCODE_HTML);
						}
						...
						OtrActivator.uiService.getChat(contact).addMessage(
								sender, new Date(), Chat.SYSTEM_MESSAGE,
								otrAndHistoryMessage, ChatMessage.ENCODE_HTML);
						...
						if (contact.supportResources() && contact.getResources() != null
							...
							OtrActivator.uiService.getChat(contact).addMessage(
									sender, new Date(), Chat.SYSTEM_MESSAGE,
									message, ChatMessage.ENCODE_HTML);
						}
					...
					case PLAINTEXT:
						...
						scSessionStatusMap.put(sessionID, scSessionStatus);
						message = OtrActivator.resourceService.getI18NString(
								"plugin.otr.activator.sessionlost",
								new String[]{sender + resourceName});
						break;
				}
	 ...
	 *
	 * @param guid
	 * 		the <tt>UUID</tt> identifying <tt>ScSessionID</tt>.
	...
	private boolean isHistoryLoggingEnabled(Contact contact)
	{
		MetaContact metaContact
				= OtrActivator.getContactListService().findMetaContactByContact(contact);
		if (metaContact != null)
	...
	public OtrPolicy getGlobalPolicy()
	{
		...
		return new OtrPolicyImpl(this.configurator.getPropertyInt(GLOBAL_POLICY,
				defaultScOtrPolicy));
	}
	...
	{
		org.osgi.framework.ServiceReference ref = OtrActivator.bundleContext
		...
		service.openURL(OtrActivator.resourceService.getI18NString(
	...
	public void serviceChanged(ServiceEvent ev)
	{
		...
		if (ev.getType() == ServiceEvent.UNREGISTERING) {
			if (logger.isDebugEnabled()) {
				logger.debug("Unregistering a ProtocolProviderService," +
						" cleaning OTR's ScSessionID to Contact map.");
				logger.debug("Unregistering a ProtocolProviderService," +
						" cleaning OTR's Contact to SpmProgressDialog map.");
			}
	...
	public void showError(SessionID sessionID, String err)
	{
		...
		OtrActivator.uiService.getChat(contact).addMessage(contact.getDisplayName(),
				new Date(), Chat.ERROR_MESSAGE, err, ChatMessage.ENCODE_PLAIN);
	}
			...
			String secret)
	{
		...
		try {
			...
			logger.error("Error occurred when sending SMP response to contact "
					+ otrContact.contact.getDisplayName(), e);
			showError(session.getSessionID(), e.getMessage());
		}
	}
...
//			}
			...
			int encType = messageText.contains(otrHtmlFallbackMessage)
					? ChatMessage.ENCODE_HTML : ChatMessage.ENCODE_PLAIN;
			Message message = imOpSet.createMessage(messageText, encType, null);
		...
		public void showAlert(SessionID sessionID, String warn)
		{
			...
			Contact contact = otrContact.contact;
			OtrActivator.uiService.getChat(contact).addMessage(contact.getDisplayName(),
					new Date(), Chat.SYSTEM_MESSAGE, warn, ChatMessage.ENCODE_PLAIN);
		}
				...
				throws OtrException
		{
			...
			String error = OtrActivator.resourceService.getI18NString(
					...
					new String[]{contact.getDisplayName()
							+ resourceName});
			OtrActivator.uiService.getChat(contact).addMessage(contact.getDisplayName(),
					new Date(), Chat.ERROR_MESSAGE, error, ChatMessage.ENCODE_PLAIN);
		}
...
//					+ otrContact.resource.getResourceName() : "";
		...
		public void askForSecret(SessionID sessionID, InstanceTag receiverTag, String question)
		{
			...
			Contact contact = otrContact.contact;
			SmpAuthenticateBuddyDialog dialog = new SmpAuthenticateBuddyDialog(otrContact,
					receiverTag, question);
	...
	private class ScSessionStatusScheduler
	{
		...
		public void scheduleScSessionStatusChange(final OtrContact otrContact,
