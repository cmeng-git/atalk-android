...
public class HistoryServiceImpl implements HistoryService
{
			...
			throws Exception
	{
		...
		this.cacheEnabled = getConfigurationService(bundleContext)
				.getBoolean(CACHE_ENABLED_PROPERTY, false);
	 ...
	 *
	 * @param in
	 * 		ByteArrayInputStream the stream to parse
	 * @return Document the result document
	 * @throws SAXException
	 * 		exception
	 * @throws IOException
	 ...
	 *
	 * @param ids
	 * 		Ids - folder names as we are using FileSystem for storing files.
	 */
	...
	private static ConfigurationService getConfigurationService(BundleContext bundleContext)
	{
		...
		return (serviceReference == null) ? null : (ConfigurationService) bundleContext.getService(
				serviceReference);
	}
	 ...
	 *
	 * @param oldId
	 * 		old and existing history
	 * @param newId
	 * 		the place where content of oldId will be moved
	 * @throws java.io.IOException
	 * 		problem moving to newId
	 */
	 ...
	 *
	 * @param rawId
	 * 		the start of the HistoryID of all the histories that will be returned.
	 * @return list of histories which HistoryID starts with <tt>rawId</tt>.
	 * @throws IllegalArgumentException
	 * 		if the <tt>rawId</tt> contains ids which are missing in current history.
	 */
		...
		for (File f : sourceFolder.listFiles()) {
			if (f.isDirectory()) {
				...
				if (f.getName().equals(DATA_FILE))
					continue;
				res.put(f,
						HistoryID.createFromRawStrings(rawID.toArray(new String[rawID.size()])));
			}
		}
	}
}
