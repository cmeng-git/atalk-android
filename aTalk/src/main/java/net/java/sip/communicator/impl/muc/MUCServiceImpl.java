...
public class MUCServiceImpl extends MUCService
{
	...
	public void acceptInvitation(ChatRoomInvitation invitation)
	{
		...
		if (nickName == null) {
			// cmeng - need to add a dialog for Nickname (in invite dialog?
			// String[] joinOptions = ChatRoomJoinOptionsDialog.getJoinOptions(true, chatRoom
			// .getParentProvider(), chatRoom.getIdentifier(),
			// MUCActivator.getGlobalDisplayDetailsService().getDisplayName(chatRoom
			// .getParentProvider()));
			// nickName = joinOptions[0];
			nickName = AndroidGUIActivator.getGlobalDisplayDetailsService()
	 ...
	 * 		is this the first attempt to join room, used to check whether to show some error
	 * 		messages
	 * @param subject
	 * 		the subject which will be set to the room after the user join successful.
	 */
	 ...
	 * 		the chat room to join
	 * @param nickname
	 * 		the nickname we're using to join
	 * @param password
	 * 		the password we're using to join
	 */
	 ...
	 * @param chatRoomProvider
	 * 		the chat room provider to join through.
	 */
	public void joinChatRoom(String chatRoomName, ChatRoomProviderWrapper chatRoomProvider)
	{
		OperationSetMultiUserChat groupChatOpSet = chatRoomProvider.getProtocolProvider()
		...
		if (chatRoom != null) {
			...
			if (chatRoomWrapper == null) {
				...
				fireChatRoomListChangedEvent(chatRoomWrapper,
			...
			MUCActivator.getAlertUIService().showAlertDialog(
					...
					MUCActivator.getResources().getI18NString("service.gui.CHAT_ROOM_NOT_EXIST",
	 ...
	 * @param reason
	 * 		the reason for room creation
	 * @param persistent
	 * 		is the room persistent
	 * @param isPrivate
	 * 		whether the room will be private or public.
	...
	public ChatRoomWrapper createChatRoom(String roomName,
			...
			boolean persistent, boolean isPrivate)
	{
		return createChatRoom(roomName, protocolProvider, contacts, reason, true,
	 ...
	 * @param persistent
	 * 		whether the newly created room will be persistent.
	 * @param isPrivate
	 * 		whether the room will be private or public.
	...
	public ChatRoomWrapper createChatRoom(String roomName,
			ProtocolProviderService protocolProvider,
			Collection<String> contacts, String reason, boolean join,
			boolean persistent, boolean isPrivate)
	{
		...
		try {
			...
			MUCActivator.getAlertUIService().showAlertDialog(
					MUCActivator.getResources().getI18NString("service.gui.ERROR"),
					MUCActivator.getResources().getI18NString("service.gui.CREATE_CHAT_ROOM_ERROR",
							new String[]{protocolProvider.getProtocolDisplayName()}), ex);
		}
	 ...
	 *
	 * @param chatRoomProvider
	 * 		the <tt>ChatRoomProviderWrapper</tt>, which chat rooms we're looking for
	 ...
	 *
	 * @param pps
	 * 		the <tt>ProtocolProviderService</tt>, whom chatRooms we're looking for
	...
	public ChatRoomWrapper leaveChatRoom(ChatRoomWrapper chatRoomWrapper)
	{
		...
		ChatRoomWrapper existChatRoomWrapper = chatRoomList.findChatRoomWrapperFromChatRoom(
				chatRoom);
	...
	private class JoinChatRoomTask extends Thread
	{
		...
		public void run()
		{
			// Must setup up chatRoom and ready to receive incoming messages before joining/sending
			// presence to server
		 ...
		 *
		 * @param returnCode
		 * 		the result code from the chat room join task.
		 */
		private void done(String returnCode)
		{
			...
			switch (returnCode) {
				case JOIN_AUTHENTICATION_FAILED_PROP:
					...
					AuthenticationWindowService authWindowsService
							= ServiceUtils.getService(MUCActivator.bundleContext,
							AuthenticationWindowService.class);

					AuthenticationWindow authWindow = authWindowsService.create(null, null, null,
							false, chatRoomWrapper.isPersistent(),
							// cmeng - icon not implemented in Android
							// AuthenticationWindow.getAuthenticationWindowIcon(chatRoomWrapper
							// .getParentProvider().getProtocolProvider()),
							null,
							resources.getI18NString("service.gui.AUTHENTICATION_WINDOW_TITLE",
									new String[]{chatRoomWrapper.getParentProvider().getName()}),
							resources.getI18NString("service.gui.CHAT_ROOM_REQUIRES_PASSWORD",
									new String[]{chatRoomId}), "", null,
							isFirstAttempt ? null : resources.getI18NString(
									"service.gui.AUTHENTICATION_FAILED",
									new String[]{chatRoomId}), null);

					authWindow.setVisible(true);
					if (!authWindow.isCanceled()) {
						joinChatRoom(chatRoomWrapper, nickName,
				...
				case JOIN_REGISTRATION_REQUIRED_PROP:
					errorMessage = resources.getI18NString(
							"service.gui.CHAT_ROOM_REGISTRATION_REQUIRED",
							new String[]{chatRoomId});
					break;
				case JOIN_PROVIDER_NOT_REGISTERED_PROP:
					errorMessage = resources.getI18NString("service.gui.CHAT_ROOM_NOT_CONNECTED",
							new String[]{chatRoomId});
					break;
				case JOIN_SUBSCRIPTION_ALREADY_EXISTS_PROP:
					errorMessage = resources.getI18NString("service.gui.CHAT_ROOM_ALREADY_JOINED",
							new String[]{chatRoomId});
					break;
				case JOIN_UNKNOWN_ERROR_PROP:
					errorMessage = resources.getI18NString("service.gui.FAILED_TO_JOIN_CHAT_ROOM",
							new String[]{chatRoomId});
					break;
			...
			if (errorMessage != null) {
				MUCActivator.getAlertUIService().showAlertPopup(resources
	 ...
	 * 		the chat room.
	 * @param create
	 * 		if <tt>true</tt> and the chat room wrapper is not found new chatRoomWrapper is created.
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider for the account to synchronize
	 * @param opSet
	 * 		the multi user chat operation set, which give us access to chat room server
	 */
	 ...
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>ChatRoomWrapper</tt> to remove
	 */
	 ...
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>ChatRoomWrapper</tt> to be destroyed.
	 * @param reason
	 * 		the reason for destroying.
	 * @param alternateAddress
	 * 		the entityBareJid of the chatRoom.
	 */
	public void destroyChatRoom(ChatRoomWrapper chatRoomWrapper, String reason,
			EntityBareJid alternateAddress)
	{
	 ...
	 *
	 * @param listener
	 * 		the ChatRoomProviderWrapperListener to be removed
	 */
	 ...
	 *
	 * @param protocolProvider
	 * 		the protocol provider that we're looking for
	 ...
	 *
	 * @param chatRoom
	 * 		the <tt>ChatRoom</tt> that we're looking for
	...
	public void openChatRoom(ChatRoomWrapper room)
	{
		if (room.getChatRoom() == null) {
			room = createChatRoom(room.getChatRoomName(),
		...
		if (!room.getChatRoom().isJoined()) {
			String savedNick = ConfigurationUtils.getChatRoomProperty(
					room.getParentProvider().getProtocolProvider(),
					room.getChatRoomID(), ChatRoom.USER_NICK_NAME);
			String subject = null;

			if (savedNick == null) {
				// String[] joinOptions = ChatRoomJoinOptionsDialog.getJoinOptions(room
				// .getParentProvider().getProtocolProvider(),
				// room.getChatRoomID(),
				// MUCActivator.getGlobalDisplayDetailsService().getDisplayName(room
				// .getParentProvider().getProtocolProvider()));
