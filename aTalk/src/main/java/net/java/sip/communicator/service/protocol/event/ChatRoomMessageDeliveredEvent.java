...
public class ChatRoomMessageDeliveredEvent extends EventObject
{
	 ...
	 *
	 * @param source
	 *        the <tt>ChatRoom</tt> which triggered this event.
	 * @param timestamp
	 *        a date indicating the exact moment when the event occurred
	 * @param message
	 *        the message that triggered this event.
	 * @param eventType
	 *        indicating the type of the delivered event. It's either an ACTION_MESSAGE_DELIVERED or
	 *        a CONVERSATION_MESSAGE_DELIVERED.
	 */
	public ChatRoomMessageDeliveredEvent(ChatRoom source, Date timestamp, Message message,
		int eventType)
	{
		super(source);
