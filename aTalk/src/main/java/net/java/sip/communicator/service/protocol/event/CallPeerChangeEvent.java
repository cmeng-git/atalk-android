...
/**
 * CallPeerChangeEvent-s are triggerred whenever a change occurs in a CallPeer. Dispatched events
 ...
 * <p>
 * CALL_PEER_ADDRESS_CHANGE - means that the transport address of the peer (the one that we use to
...
public class CallPeerChangeEvent extends java.beans.PropertyChangeEvent
{
	...
	/**
	 * An event type indicating that the corresponding event is caused by a change of the CallPeer's
	 * status.
	 */
	public static final String CALL_PEER_STATE_CHANGE = "CallPeerStatusChange";

	/**
	 * An event type indicating that the corresponding event is caused by a change of the peer's
	 * display name.
	 */
	 ...
	 * 
	 * @param source
	 *        the peer that produced the event.
	 * @param type
	 ...
	 *        current value of the changed property.
	 * @param reason
	 *        a string containing a human readable explanation for the reason that triggered this
	 *        event (may be null).
	 * @param reasonCode
	 *        a code for the reason that triggered this event (may be -1 as not specified).
	 */
	public CallPeerChangeEvent(CallPeer source, String type, Object oldValue, Object newValue,
