...
public abstract class AbstractProtocolProviderService implements ProtocolProviderService
{
	 ...
	 * notifications on changes of its state or other properties such as its local address and
	 * display name.
	 *
	 * @param listener
	 ...
	 *
	 * @param <T>
	 * 		the exact type of the <tt>OperationSet</tt> implementation to be added
	 * @param opsetClass
	 * 		the <tt>Class</tt> of <tt>OperationSet</tt> under the name of which the specified
	 ...
	 *
	 * @param oldState
	 * 		the state that the provider had before the change occurred
	 * @param newState
	 * 		the state that the provider is currently in.
	 * @param reasonCode
	 * 		a value corresponding to one of the REASON_XXX fields of the
	 * 		RegistrationStateChangeEvent class, indicating the reason for this state transition.
	 * @param reason
	 * 		a String further explaining the reason code or null if no such explanation is
	 * 		necessary.
	 */
	 ...
	 * 		necessary.
	 * @param userRequest
	 * 		is the event by user request.
	 */
	public void fireRegistrationStateChanged(RegistrationState oldState,
			RegistrationState newState, int reasonCode, String reason, boolean userRequest)
	{
		if (logger.isDebugEnabled())
			logger.debug("The provider changed state from: " + oldState + " to: " + newState
			+ ". Reason: " + reason);
		...
		synchronized (registrationListeners) {
			listeners = registrationListeners
					.toArray(new RegistrationStateChangeListener[registrationListeners.size()]);
		}
	 ...
	 *
	 * @param <T>
	 * 		the exact type of the <tt>OperationSet</tt> that we're looking for
	 * @param opsetClass
	 * 		the <tt>Class</tt> of the operation set that we're looking for.
	...
	{
		String displayName = getAccountID().getAccountPropertyString(
	 ...
	 * the OperationSet-s defined by this service. They MUST ignore any OperationSet-s that they
	 * are
		...
		for (String opSetClassName : getSupportedOperationSets().keySet()) {
			try {
				opSetClasses.add((Class<? extends OperationSet>) getSupportedOperationSets()
						.get(opSetClassName).getClass().getClassLoader().loadClass
								(opSetClassName));
			}
	 ...
	 *
	 * @return <tt>true</tt> if the previous XMPP session's stream was resumed and <tt>false</tt>
	 * otherwise.
	 */
	 ...
	 *
	 * @param listener
	 * 		the listener to register for <tt>RegistrationStateChangeEvent</tt>s.
	 */
	 ...
	 * 		with the corresponding code it the registration fails for some reason (e.g. a
	 * 		networking error or an implementation problem).
	 */
