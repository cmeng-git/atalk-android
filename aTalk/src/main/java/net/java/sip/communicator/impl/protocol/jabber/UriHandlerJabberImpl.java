...
public class UriHandlerJabberImpl implements UriHandler, ServiceListener, AccountManagerListener
{
	 ...
	 * <code>protoFactory</code> is complete, no handling of URIs is to be performed because
	 ...
	 * 		if <tt>protoFactory</tt> is <tt>null</tt>.
	 */
	 ...
	 *
	 * @param uri
	 * 		the xmpp URI that we have to handle.
	 */
	 ...
	 *
	 * @param uri
	 * 		the uri that the user would like us to chat with after registering.
	 * @param provider
	 * 		the provider that we may have to re-register.
	 */
...
//			new ProtocolRegistrationThread(uri, provider).start();
	 ...
	 *
	 * @param event
	 * 		the OSGi <tt>ServiceEvent</tt>
	 */
	public void serviceChanged(ServiceEvent event)
	{
		Object sourceService = JabberActivator.bundleContext
	 ...
	 *
	 * @param message
	 * 		the message that we'd like to show to the user.
	 * @param exc
	 * 		the exception that we'd like to log
	 */
...
//						PopupDialog.ERROR_MESSAGE);
			...
			implements RegistrationStateChangeListener
	{
		 ...
		 *
		 * @param uri
		 * 		the uri that we need to handle.
		 * @param handlerProvider
		 * 		the provider that we are going to make register and that we are going to use to
		 ...
		 *
		 * @param evt
		 * 		the <tt>RegistrationStateChangeEvent</tt> that this thread was initiated with.
		 */
	 ...
	 *
	 * @param uri
	 * 		the uri that we'd like to handle with the provider that we are about to select.
	 * @return the provider that we should handle URIs through.
	 * @throws OperationFailedException
	 * 		with code <tt>OPERATION_CANCELED</tt> if the users.
	 */
			...
			throws OperationFailedException
	{
		...
		if (registeredAccounts.size() == 1) {
			...
			ProtocolProviderService provider = (ProtocolProviderService) JabberActivator
					.bundleContext.getService(providerReference);
		...
		Object result = JabberActivator.getUIService().getPopupDialog()
				.showInputPopupDialog(msg, title, PopupDialog.OK_CANCEL_OPTION,
