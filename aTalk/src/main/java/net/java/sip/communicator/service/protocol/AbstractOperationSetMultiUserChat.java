...
public abstract class AbstractOperationSetMultiUserChat implements OperationSetMultiUserChat
{
	 ...
	 */
	protected void fireInvitationRejectedEvent(ChatRoom sourceChatRoom, EntityBareJid invitee,
		String reason)
	{
		...
		synchronized (invitationRejectionListeners) {
			listeners = invitationRejectionListeners
				.toArray(new ChatRoomInvitationRejectionListener[invitationRejectionListeners.size()]);
		}
	 ...
	 *        address of the new room, if old is destroyed.
	 */
	public void fireLocalUserPresenceEvent(ChatRoom chatRoom, String eventType, String reason,
		String alternateAddress)
	{
		...
		synchronized (presenceListeners) {
			listeners = presenceListeners
				.toArray(new LocalUserChatRoomPresenceListener[presenceListeners.size()]);
		}
