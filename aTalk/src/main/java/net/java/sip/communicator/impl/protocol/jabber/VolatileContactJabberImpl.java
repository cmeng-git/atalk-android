...
public class VolatileContactJabberImpl extends ContactJabberImpl
{
	 ...
	 * 		if <tt>true</tt> this should be private messaging contact.
	 */
	VolatileContactJabberImpl(String id, ServerStoredContactListJabberImpl ssclCallback,
	 ...
	 *
	 * @param id
	 * 		String the user id/address (bareJid from subscription)
			...
			boolean isPrivateMessagingContact, String displayName)
	{
		...
		try {
			if (this.isPrivateMessagingContact) {
				this.displayName = XmppStringUtils.parseResource(id) + " from "
						+ XmppStringUtils.parseBareJid(id);
				this.contactId = id;
			}
			else {
				this.displayName = (displayName == null) ? contactId : displayName;
				this.contactId = XmppStringUtils.parseBareJid(id);
			}
			setJid(JidCreate.from(id));
		}
	...
	public String getPersistableAddress()
	{
		...
		if (mucOpSet != null) {
			chatRoomMember = mucOpSet.getChatRoom(XmppStringUtils.parseBareJid(contactId))
					.findMemberForNickName(XmppStringUtils.parseResource(contactId));
		}
		return ((chatRoomMember == null)
				? null : XmppStringUtils.parseBareJid(chatRoomMember.getJabberID()));
	}
}
