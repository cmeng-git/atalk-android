...
public class MclStorageManager implements MetaContactListListener
{
	 ...
	 *
	 * @param bc
	 * 		a reference to the currently valid OSGI <tt>BundleContext</tt>
	 * @param mclServiceImpl
	 * 		a reference to the currently valid instance of the <tt>MetaContactListServiceImpl</tt>
	 ...
	 *
	 * @param accountUuid
	 * 		the identifier of the account whose contacts we're interested in.
	 */
	 ...
	 *
	 * @param accountUuid
	 * 		a String identifier i.e. prefix with "acc" of the account whose contacts
	 * 		we're interested in..
	 * @param accountUid
	 * 		a String identifier e.g. Jabber:jonh@example.org of the account whose contacts we're
	 * 		interested	in.
	 */
	private void processGroupContact(String accountUuid, String accountUid)
	{
		...
		while (cursor.moveToNext()) {
			String parentProtoGroupUID = cursor.getString(
					cursor.getColumnIndex(MetaContactGroup.PARENT_PROTO_GROUP_UID));
			String groupUID
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.MC_GROUP_UID));
			String groupName
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.MC_GROUP_NAME));
			String protoGroupUID
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.PROTO_GROUP_UID));
			String persistentData
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.PERSISTENT_DATA));

			logger.info("### Fetching contact group: " + parentProtoGroupUID + ": "
					+ protoGroupUID + " for " + accountUuid);
		...
		while (cursor.moveToNext()) {
			...
			String contactAddress
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.CONTACT_JID));
			String displayName
					= cursor.getString(cursor.getColumnIndex(MetaContactGroup.MC_DISPLAY_NAME));
			...
			try {
				details = new JSONObject(
						cursor.getString(cursor.getColumnIndex(MetaContactGroup.MC_DETAILS)));
			}
			...
			if (protoGroupsMap.containsKey(protoGroupUID)) {
				...
				MetaContactImpl metaContactImpl = mclServiceImpl.loadStoredMetaContact(
						metaGroupMap.get(protoGroupUID), metaUID, displayName, details,
						protoContacts, accountUid);
	 ...
	 *
	 * @param mcGroup
	 * 		the MetaContactGroup that the new entry is to be created
	 */
	 ...
	 *
	 * @param protoGroup
	 * 		the <code>ContactGroup</code> which is to be created for
	 * @param metaGroup
	 * 		the parent of the protoGroup
	 */
	 ...
	 *
	 * @param metaContact
	 * 		the MetaContact that the new entry is to be created for
	 */
	private void createMetaContactEntry(MetaContact metaContact)
	{
		...
		JSONObject mcDetails = ((MetaContactImpl) metaContact).getDetails();
	 ...
	 *
	 * @param evt
	 * 		the MetaContactListEvent containing the corresponding contact
	 */
	...
	public void metaContactGroupModified(MetaContactGroupEvent evt)
	{
		...
		switch (evt.getEventID()) {
			...
			case MetaContactGroupEvent.META_CONTACT_GROUP_RENAMED:
				...
				mDB.update(MetaContactGroup.TABLE_NAME, mcValues,
						MetaContactGroup.MC_GROUP_UID + "=?", args);
				break;
		}
	}
	 ...
	 *
	 * @param evt
	 * 		the MetaContactGroupEvent containing the corresponding contact
	 */
	public void metaContactGroupRemoved(MetaContactGroupEvent evt)
	{
		...
		if (findMetaContactGroupEntry(mcGroupUid) == null) {
			logger.warn("Failed to find metaContactGroup for removal (or that has been removed): "
	 ...
	 *
	 * @param evt
	 * 		the MetaContactListEvent containing the corresponding contactGroup and other info.
	...
	private void contactGroupRemovedFromMetaGroup(MetaContactGroupEvent evt)
	{
		...
		if (cursor.getCount() == 0) {  // found no parent
			logger.info("Removing old protoGroup childContacts if any: " + protoGroupUid + ": "
					+ accountUuid);
	...
	public void metaContactRenamed(MetaContactRenamedEvent evt)
	{
		...
		if (!StringUtils.isNullOrEmpty(newDisplayName)
	 ...
	 *
	 * @param evt
	 * 		the MetaContactModifiedEvent containing the corresponding contact
	 */
	 ...
	 *
	 * @param evt
	 * 		the MetaContactMovedEvent containing the reference move information
	 */
...
//		}
		...
		mDB.update(TBL_CHILD_CONTACTS, mcValues,
				MetaContactGroup.MC_UID + "=?", args);
	}
	 ...
	 *
	 * @param evt
	 * 		the MetaContactEvent containing the corresponding metaContact
	 */
	 ...
	 *
	 * @param evt
	 * 		the ProtoContactEvent containing the corresponding contact
	 */
	 ...
	 *
	 * @param evt
	 * 		a reference to the <tt>ProtoContactMovedEvent</tt> instance.
	 */
	 ...
	 * 		the protocol provider of contact whose UUID we are looking for.
	 * @param jid
	 * 		the jid String of the contact whose UUID we are looking for.
	 ...
	 *
	 * @param contactJid
	 * 		the contact Jid we are looking for.
	 ...
	 *
	 * @param metaContactUID
	 * 		the UID String of the metaContact whose contactJid we are looking for.
	 ...
	 * 		the metaContactGroup UID whose groupName we are looking for.
	...
	static class StoredProtoContactDescriptor
	{
		 ...
		 * 		the address of the contact whose descriptor we are looking for.
		 * @param list
		 * 		the <tt>List</tt> of <tt>StoredProtoContactDescriptor</tt> that we are supposed to
	 ...
	 *
	 * @param evt
	 * 		the <tt>MetaContactGroupEvent</tt> containing details of this event.
	 */
	 ...
	 *
	 * @param evt
	 * 		the <tt>MetaContactAvatarUpdateEvent</tt> containing details of this event
	 */
