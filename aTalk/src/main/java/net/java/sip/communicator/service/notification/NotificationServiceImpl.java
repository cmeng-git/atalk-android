...
class NotificationServiceImpl implements NotificationService
{
	...
	/**
	 * Creates an instance of <tt>NotificationServiceImpl</tt> by loading all previously saved
	 * notifications.
	 */
	 ...
	 *
	 * @param handler
	 * 		The handler that executes the action.
	 */
	 ...
	 *
	 * @param listener
	 * 		the listener that we'd like to register to listen for changes in the event
	 ...
	 * of the property. If it is older one we migrate it to new configuration using the default
	 * values.
	 *
	 * @param eventType
	 * 		the event type.
	 * @param defaultAction
	 * 		the default action which values we will use.
	 */
	 ...
	 *
	 * @param data
	 * 		The notification data to act upon.
	 */
	private void fireNotification(NotificationData data)
	{
		...
		for (NotificationAction action : notification.getActions().values()) {
			...
			try {
					...
					case ACTION_SOUND:
						...
						if (soundNotificationAction.isSoundNotificationEnabled()
							...
							((SoundNotificationHandler) handler).start(
									(SoundNotificationAction) action, data);
						}
						break;
					case ACTION_COMMAND:
						...
						((CommandNotificationHandler) handler).execute(
								(CommandNotificationAction) action, cmdargs);
						break;
					case ACTION_VIBRATE:
				...
				logger.error("Error dispatching notification of type" + actionType
						+ " from " + handler, e);
			}
		}
	}
	 ...
	 * notification is currently activated, we go through the list of registered actions and
	 * execute them.
	 *
	 * @param eventType
	 ...
	 * 		the type of the event that we'd like to fire a notification for.
	 * @param title
	 * 		the title of the given message
	 * @param message
	 * 		the message to use if and where appropriate (e.g. with systray or log notification.)
	 * @param icon
	 * 		the icon to show in the notification if and where appropriate
	 * @return An object referencing the notification. It may be used to stop a still running
	 * notification. Can be null if the eventType is unknown or the notification is not active.
	 */
	 ...
	 * 		the icon to show in the notification if and where appropriate
	 * @param extras
	 * 		additional/extra {@link NotificationHandler}-specific data to be provided to the firing
	 * 		of the specified notification(s). The well-known keys are defined by the
	 * 		<tt>NotificationData</tt> <tt>XXX_EXTRA</tt> constants.
	 * @return An object referencing the notification. It may be used to stop a still running
	 * notification. Can be null if the eventType is unknown or the notification is not active.
	 */
	public NotificationData fireNotification(String eventType, String title, String message,
	 ...
	 *
	 * @param eventType
	 * 		the type of the event, which is one of ACTION_XXX constants declared in the
	 * 		<tt>NotificationActionTypeEvent</tt> class.
	 * @param sourceEventType
	 * 		the <tt>eventType</tt>, which is the parent of the action
	 * @param action
	 * 		the notification action
	 */
	private void fireNotificationActionTypeEvent(String eventType, String sourceEventType,
	 ...
	 *
	 * @param eventType
	 * 		the type of the event, which is one of EVENT_TYPE_XXX constants declared in the
	 * 		<tt>NotificationEventTypeEvent</tt> class.
	 * @param sourceEventType
	 * 		the <tt>eventType</tt>, for which this event is about
	 */
	private void fireNotificationEventTypeEvent(String eventType, String sourceEventType)
	{
		...
		NotificationEventTypeEvent event = new NotificationEventTypeEvent(this, eventType,
				sourceEventType);
	 ...
	 *
	 * @param actionType
	 * 		the type for which the list of handlers should be retrieved or <tt>null</tt> if all
	...
	/**
	 * Returns the notification action corresponding to the given <tt>eventType</tt> and
	 * <tt>actionType</tt>.
	 *
	 * @param eventType
	 * 		the type of the event that we'd like to retrieve.
	 * @param actionType
	 * 		the type of the action that we'd like to retrieve a descriptor for.
	 * @return the notification action of the action to be executed when an event of the specified
	 * type has occurred.
	 */
	 ...
	 * have an updated version of already saved/edited notification configurations. Detects old
	 * configurations.
	 *
	 * @param eventType
	 * 		the event type
	 * @param action
	 * 		the action which property to check.
	 * @param property
	 * 		the property name without the action prefix.
	 * @return the property value or null if missing.
	 * @throws IllegalArgumentException
	 * 		when the event ot action is not found.
	 */
	private String getNotificationActionProperty(String eventType, NotificationAction action,
			String property)
			throws IllegalArgumentException
	{
		...
		List<String> eventTypes
				= configService.getPropertyNamesByPrefix(NOTIFICATIONS_PREFIX, true);
		...
		// If we didn't find the given actionType in the configuration there is no need to further
		// check
	...
	/**
	 * Finds the <tt>EventNotification</tt> corresponding to the given <tt>eventType</tt> and
	 * returns its isActive status.
	 *
	 * @param eventType
	 * 		the name of the event (as defined by the plugin that's registered it) that we are
	 * 		checking.
	...
	private boolean isDefault(String eventType, String actionType)
	{
		...
		for (String eventTypeRootPropName : eventTypes) {
			...
			for (String actionPropName : actions) {
				...
				// if setting is missing we accept it is true this way we override old saved
				// settings
				...
				switch (actionType) {
					case ACTION_SOUND:
						String soundFileDescriptor
								= configService.getString(actionPropName + ".soundFileDescriptor");
						String loopInterval
								= configService.getString(actionPropName + ".loopInterval");
						...
						action = new SoundNotificationAction(soundFileDescriptor,
								Integer.parseInt(loopInterval), isSoundNotificationEnabled,
								isSoundPlaybackEnabled, isSoundPCSpeakerEnabled);
						break;
					case ACTION_POPUP_MESSAGE:
						String defaultMessage
								= configService.getString(actionPropName + ".defaultMessage");
						...
						action = new PopupMessageNotificationAction(defaultMessage, timeout,
								groupName);
						break;
					case ACTION_LOG_MESSAGE:
					...
					case ACTION_COMMAND:
						String commandDescriptor
								= configService.getString(actionPropName + ".commandDescriptor");
						action = new CommandNotificationAction(commandDescriptor);
						break;
					case ACTION_VIBRATE:
						String descriptor
								= configService.getString(actionPropName + ".descriptor");
						int patternLen
								= configService.getInt(actionPropName + ".patternLength", -1);
						if (patternLen == -1) {
						...
						for (int pIdx = 0; pIdx < patternLen; pIdx++) {
							pattern[pIdx] = configService.getLong(
	...
	/**
	 * Creates a new default <tt>EventNotification</tt> or obtains the corresponding existing one
	 * and registers a new action in it.
	 *
	 * @param eventType
	 * 		the name of the event (as defined by the plugin that's registering it) that we are
	 * 		setting an action for.
	 * @param action
	 * 		the <tt>NotificationAction</tt> to register
	 */
	public void registerDefaultNotificationForEvent(String eventType, NotificationAction action)
	{
		if (isDefault(eventType, action.getActionType())) {
			...
			fireNotificationActionTypeEvent(
					isNew ? ACTION_ADDED : ACTION_CHANGED, eventType, action);
		}
		else
	 ...
	 * 		setting an action for.
	 * @param actionType
	 * 		the type of the action that is to be executed when the specified event occurs (could
	 * 		be one of the ACTION_XXX fields).
	 * @param actionDescriptor
	 * 		a String containing a description of the action (a URI to the sound file for audio
	 * 		notifications or a command line for exec action types) that should be executed when
	 * 		the action occurs.
	 * @param defaultMessage
	 * 		the default message to use if no specific message has been provided when firing the
	 * 		notification.
	 */
	...
	/**
	 * Creates a new <tt>EventNotification</tt> or obtains the corresponding existing one and
	 ...
	 * 		setting an action for.
	 * @param action
	 * 		the <tt>NotificationAction</tt> responsible for handling the given <tt>actionType</tt>
	 */
	...
	/**
	 * Removes the <tt>EventNotification</tt> corresponding to the given <tt>eventType</tt> from
	 * the table of registered event notifications.
	 *
	 * @param eventType
	 * 		the name of the event (as defined by the plugin that's registering it) to be removed.
	 */
	...
	/**
	 * Removes the given actionType from the list of actions registered for the given
	 * <tt>eventType</tt>.
	 *
	 * @param eventType
	 * 		the name of the event (as defined by the plugin that's registering it) for which we'll
	...
	/**
	 * Deletes all registered events and actions and registers and saves the default events as
	 * current.
	 */
	 ...
	 * @param action
	 * 		the notification action to change
	 * @param isActive
	 * 		is the event active
	 * @param isDefault
	 * 		is it a default one
	 */
	private void saveNotification(String eventType, NotificationAction action, boolean isActive,
			boolean isDefault)
	{
		String eventTypeNodeName = null;
		String actionTypeNodeName = null;
		...
		if (eventTypeNodeName == null) {
			eventTypeNodeName = NOTIFICATIONS_PREFIX + ".eventType"
		...
		if (actionTypeNodeName == null) {
			actionTypeNodeName = actionPrefix + ".actionType"
					+ Long.toString(System.currentTimeMillis());
		...
		if (action instanceof SoundNotificationAction) {
			...
			configProperties.put(actionTypeNodeName + ".soundFileDescriptor",
					soundAction.getDescriptor());
			configProperties.put(actionTypeNodeName + ".loopInterval",
	 ...
	 *
	 * @param eventType
	 * 		the name of the event, which actions should be activated /deactivated.
	 * @param isActive
	 * 		indicates whether to activate or deactivate the actions related to the specified
	 * 		<tt>eventType</tt>.
	 */
