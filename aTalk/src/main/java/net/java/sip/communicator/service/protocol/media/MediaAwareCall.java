 ...
 * @param <U>
 *        the provider extension class like, for example, <tt>OperationSetBasicTelephonySipImpl</tt>
 *        or <tt>OperationSetBasicTelephonySipImpl</tt>
 * @param <V>
 *        the provider extension class like, for example, <tt>ProtocolProviderServiceSipImpl</tt> or
...
public abstract class MediaAwareCall<T extends MediaAwareCallPeer<?, ?, V>, U extends OperationSetBasicTelephony<V>, V extends ProtocolProviderService>
	extends AbstractCall<T, V> implements CallPeerListener, PropertyChangeListener, DTMFListener
{
	/**
	 ...
	 *
	 * @param parentOpSet
	 *        a reference to the operation set that's creating us and that we would be able to use
	 ...
	 *
	 * @param evt
	 *        the event containing the <tt>CallPeer</tt> leaving the call and the reason (if any)
	 *        for the <tt>CallPeerChangeEvent</tt>. Use the event as the cause for the call state
	 *        change event.
	 */
	 ...
	 * created this call.
	 *
	 * @return a reference to the <tt>OperationSetBasicTelephony</tt> instance that created this
	 *         call.
	 */
	 ...
	 *
	 * @param mediaType
	 *        the <tt>MediaType</tt> of the <tt>MediaStream</tt> which RTP and RTCP traffic is to be
	 *        forwarded between
		...
		synchronized (localUserAudioLevelListenersSyncRoot) {
			if ((localUserAudioLevelListeners == null) || localUserAudioLevelListeners.isEmpty()) {
				...
				while (callPeerIter.hasNext()) {
					callPeerIter.next().getMediaHandler()
						.setLocalUserAudioLevelListener(localAudioLevelDelegator);
				}
			}
			 ...
			 */
			localUserAudioLevelListeners = (localUserAudioLevelListeners == null) ? new ArrayList<SoundLevelListener>()
	...
	public void removeLocalUserSoundLevelListener(SoundLevelListener l)
	{
		synchronized (localUserAudioLevelListenersSyncRoot) {
			...
			if (localUserAudioLevelListeners != null) {
				localUserAudioLevelListeners = new ArrayList<SoundLevelListener>(
	 ...
	 *
	 * @param mute
	 *        the new value of the mute property for this call
	 */
	 ...
	 *
	 * @param allowed
	 *        <tt>true</tt> if local video transmission is allowed and <tt>false</tt> otherwise.
	 * @param useCase
	 *        the use case of the video (i.e video call or desktop streaming/sharing session)
	 *
	 ...
	 *
	 * @param listener
	 *        the <tt>PropertyChangeListener</tt> to be notified when the properties associated with
	 ...
	 *
	 * @param listener
	 *        the <tt>PropertyChangeListener</tt> to unregister from member <tt>CallPeer</tt>s
	 *        change their values.
	 */
	 ...
	 *         to be specified when starting the returned <tt>Recorder</tt>)
	 * @throws OperationFailedException
	 *         if anything goes wrong while creating the new <tt>Recorder</tt> for this
	 *         <tt>Call</tt>
	 */
		...
		throws OperationFailedException
	{
		...
		if (recorder != null) {
			...
			final CallChangeListener callChangeListener = new CallChangeListener()
			{
				 ...
				 * 
				 * @param evt
				 *        the <tt>CallChangeEvent</tt> instance containing the source
				 */
				 ...
				 * 
				 * @param evt
				 *        the <tt>CallPeerEvent</tt> containing the source call
				 */
	...
	/**
	 * Sets the <tt>MediaDevice</tt> to be used by this <tt>Call</tt> for audio capture and/or
	 * playback.
	 *
	 * @param audioDevice
	 *        the <tt>MediaDevice</tt> to be used by this <tt>Call</tt> for audio capture and/or
	 *        playback
	 */
	 ...
	 *
	 * @param videoDevice
	 *        the <tt>MediaDevice</tt> to be used by this <tt>Call</tt> for video capture and/or
	 *        playback
	 * @param useCase
	 *        the use case of the video streaming to be set on this <tt>MediaAwareCall</tt> such as
	 ...
	 *
	 * @param newState
	 *        the <tt>CallState</tt> into which this <tt>Call</tt> is to enter
	 * @param cause
	 *        the <tt>CallPeerChangeEvent</tt> which is the cause for the request to have this
	 ...
	 *
	 * @param ev
	 *        a <tt>PropertyChangeEvent</tt> which specifies the name of the property which has its
	 ...
	 *
	 * @param oldValue
	 *        the value of the property <tt>CONFERENCE_FOCUS</tt> before the change
	 * @param newValue
	 *        the value of the property <tt>CONFERENCE_FOCUS</tt> after the change
	 */
