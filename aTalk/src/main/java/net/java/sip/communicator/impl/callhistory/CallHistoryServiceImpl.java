...
public class CallHistoryServiceImpl implements CallHistoryService, CallListener, ServiceListener
{
	...
	final private List<CallHistoryPeerRecordListener> callHistoryRecordListeners
			= new LinkedList<>();
	 ...
	 *
	 * @param metaContact
	 * 		MetaContact which contacts participate in the returned calls
	 * @param startDate
	 * 		Date the start date of the calls
	 ...
	 * 		MetaContact which contacts participate in the returned calls
	 * @param endDate
	 * 		Date the end date of the calls
	 ...
	 *
	 * @param address
	 * 		String the address of the peer (cmeng: may be null?)
	 * @param recordCount
...
//	}
	 ...
	 *
	 * @param mProperties
	 * 		CallRecord properties converted from cursor
	 * @return MessageImpl
	 */
	public static CallRecord createCallRecordFromProperties(Map<String, String> mProperties)
	{
		...
		result.setProtocolProvider(
				getProtocolProvider(mProperties.get(CallHistoryService.ACCOUNT_UID)));
		result.setStartTime(
		...
		for (int i = 0; i < callPeerCount; i++) {
			...
			if (i < callPeerStart.size()) {
				...
				if (logger.isInfoEnabled())
					logger.info("Call history start time list different from ids list.");
			}
			...
			if (i < callPeerEnd.size()) {
				...
				if (logger.isInfoEnabled())
					logger.info("Call history end time list different from ids list.");
			}
			...
			if (callPeerStates != null && i < callPeerStates.size())
				...
				logger.info("Call history state list different from ids list.");
	 ...
	 *
	 * @param str
	 * 		String delimited string states
	...
	private void writeCall(CallRecordImpl callRecord, Contact source, Contact destination)
	{
		...
		String Uuid = String.valueOf(timeStamp) + String.valueOf(Math.abs(date.hashCode()));
	 ...
	 *
	 * @param event
	 * 		the <tt>CallHistoryRecordReceivedEvent</tt> event to be fired
	 */
	 ...
	 * @param peerAddress
	 * 		the address of the peer of the record which will be updated.
	 * @param address
	...
	private class HistoryCallChangeListener implements CallChangeListener
	{
		 ...
		 *
		 * @param evt
		 * 		the <tt>CallChangeEvent</tt> instance containing the source calls and its old
		 * 		and new state.
		 */
