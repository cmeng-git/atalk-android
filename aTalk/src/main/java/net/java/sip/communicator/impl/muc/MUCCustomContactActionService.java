...
public class MUCCustomContactActionService implements CustomContactActionsService<SourceContact>
{
	...
	private static final String OWNER_CANT_REMOVE_CHATROOM_PROPERTY
			= "muc.OWNER_CANT_REMOVE_CHATROOM";
	 ...
	 */
	private String[] actionsLabels = { "service.gui.LEAVE", "service.gui.JOIN",
	 ...
	 */
	private String[] menuActionsNames = { "open", "join", "join_as", "leave", "remove",
			"change_nick", "auto_join", "auto_join_pressed", "open_automatically",
			"destroy_chatroom" };

	/**
	 * Array of labels for the custom menu items.
	 */
	private String[] menuActionsLabels = { "service.gui.OPEN", "service.gui.JOIN",
			"service.gui.JOIN_AS", "service.gui.LEAVE", "service.gui.REMOVE",
			"service.gui.CHANGE_NICK", "service.gui.JOIN_AUTOMATICALLY",
			"service.gui.DONT_JOIN_AUTOMATICALLY", "service.gui.OPEN_AUTOMATICALLY",
			"service.gui.DESTROY_CHATROOM" };

	/**
	 * Array of icons for the custom menu items.
	 */
	private String[] menuActionsIcons = { "chat_room", "join", "join_as", "leave",
			"remove_chat", "rename", "auto_join_on", "auto_join_off",
			"opening_chatroom_settings", "destroy_chat" };
	...
	private MUCCustomActionRunnable joinRunnable = new MUCCustomActionRunnable()
	{
		...
		public void run()
		{
			...
			nickName = ConfigurationUtils.getChatRoomProperty(
	 ...
	 */
	private MUCCustomActionRunnable[] actionsRunnable = { leaveRunnable, joinRunnable, autoJoinRunnable, autoJoinRunnable, destroyActionRunnable };
	...
	}, autoJoinRunnable, autoJoinRunnable, new MUCCustomActionRunnable()
	{
		...
		public void run()
		{
			MUCActivator.getUIService().showChatRoomAutoOpenConfigDialog(chatRoomWrapper.getParentProvider().getProtocolProvider(),
				chatRoomWrapper.getChatRoomID());
		}
	 ...
	 */
	private EnableChecker[] actionsEnabledCheckers = { null, new JoinEnableChecker(), new JoinEnableChecker(), new LeaveEnableChecker(), null, null, null,
		null, null, null };
	...
	public MUCCustomContactActionService()
	{
		...
		for (int i = 0; i < actionsLabels.length; i++) {
			MUCAction item = new MUCAction(actionsNames[i], actionsLabels[i], actionsIcons[i], actionsIconsRollover[i], actionsIconsPressed[i],
				actionsRunnable[i]);
			mucActions.add(item);
		}
	...
	private class MUCAction implements ContactAction<SourceContact>
	{
		 ...
		 *
		 * @param textKey
		 *        the key used to retrieve the label for the action.
		 * @param iconKey
		 *        the key used to retrieve the icon for the action.
		 * @param actionPerformed
		 *        the action executed when the action is pressed.
		 * @param iconRolloverKey
		 *        the key used to retrieve the rollover icon for the action.
		 * @param iconPressedKey
		 *        the key used to retrieve the pressed icon for the action.
		 */
	...
	private class MUCActionMenuItems implements ContactActionMenuItem<SourceContact>
	{
		 ...
		 *
		 * @param enabled
		 *        the <tt>EnabledChecker</tt> instance.
		 */
