...
public class OperationSetWhiteboardingJabberImpl implements OperationSetWhiteboarding
{
	...
	private static final Logger logger = Logger
		.getLogger(OperationSetWhiteboardingJabberImpl.class);
	...
	/**
	 * A list of listeners subscribed for events indicating rejection of a multi user chat
	 * invitation sent by us.
	 */
	...
	/**
	 * Subscribes <tt>listener</tt> so that it would receive events indicating rejection of a
	 * multi-user chat invitation that we've sent earlier.
	 *
	 * @param listener
	 *        the listener that we'll subscribe for invitation rejection events.
	 */
	...
	/**
	 * Adds a listener that will be notified of changes in our status in a chat room such as us
	 * being kicked, banned or dropped.
	 *
	 * @param listener
	...
	/**
	 * Removes a listener that was being notified of changes in our status in a room such as us
	 * being kicked, banned or dropped.
	 *
	 * @param listener
	 *        the <tt>LocalUserChatRoomPresenceListener</tt>.
	...
	/**
	 * Creates a <tt>WhiteboardSession</tt>. For now the session is created locally and neither the
	 * sessionName, nor the sessionProperties are used.
	 * 
	 * @param sessionName
	...
	public List<WhiteboardSession> getCurrentlyJoinedWhiteboards()
	{
		synchronized (whiteboardSessions) {
			List<WhiteboardSession> joinedWhiteboards = new LinkedList<WhiteboardSession>(
				whiteboardSessions);
	...
	private class RegistrationStateListener implements RegistrationStateChangeListener
	{
		...
		public void registrationStateChanged(RegistrationStateChangeEvent evt)
		{
			if (evt.getNewState() == RegistrationState.REGISTERED) {
				presenceOpSet = (OperationSetPersistentPresenceJabberImpl) jabberProvider
	...
	/**
	 * Listens for white-board messages and checks if a white-board session already exists and if
	 * not simulates an invitation to the user.
	 */
	private class WhiteboardSmackMessageListener implements StanzaListener
	{
		public void processStanza(Stanza packet)
		{
			...
			ExtensionElement ext = packet.getExtension(
				WhiteboardObjectPacketExtension.ELEMENT_NAME,
				WhiteboardObjectPacketExtension.NAMESPACE);
			...
			String fromUserID = msg.getFrom().asBareJid().toString();
			...
			for (int i = 0; i < whiteboardSessions.size(); i++) {
				WhiteboardSessionJabberImpl session = (WhiteboardSessionJabberImpl) whiteboardSessions
					.get(i);
			...
			WhiteboardSessionJabberImpl session = new WhiteboardSessionJabberImpl(jabberProvider,
				OperationSetWhiteboardingJabberImpl.this);

			whiteboardSessions.add(session);

			ContactJabberImpl sourceContact = (ContactJabberImpl) presenceOpSet
				.findContactByID(fromUserID);
			...
			session.addWhiteboardParticipant(new WhiteboardParticipantJabberImpl(sourceContact,
				session));
	...
	/**
	 * Delivers a <tt>WhiteboardInvitationEvent</tt> to all registered
	 * <tt>WhiteboardInvitationListener</tt>s.
	 *
	 * @param targetWhiteboard
	 *        the white-board that invitation refers to
	 * @param whiteboardObject
	 *        the white-board object that inviter send with this invitation and which will be shown
	 *        on the white-board if the user accepts the invitation
	 * @param inviter
	...
	public void fireInvitationEvent(WhiteboardSession targetWhiteboard,
		WhiteboardObject whiteboardObject, String inviter, String reason, byte[] password)
	{
		WhiteboardInvitationJabberImpl invitation = new WhiteboardInvitationJabberImpl(
			targetWhiteboard, whiteboardObject, inviter, reason, password);

		WhiteboardInvitationReceivedEvent evt = new WhiteboardInvitationReceivedEvent(this,
			invitation, new Date(System.currentTimeMillis()));

		if (logger.isDebugEnabled())
			logger.debug("Dispatching a WhiteboardInvitation event to "
				+ invitationListeners.size() + " listeners. event is: " + evt.toString());
	...
	/**
	 * Delivers a <tt>WhiteboardSessionPresenceChangeEvent</tt> to all registered
	 * <tt>WhiteboardSessionPresenceChangeEvent</tt>s.
	 *
	 * @param session
	 *        the <tt>WhiteboardSession</tt> which has been joined, left, etc.
	 * @param eventType
	 *        the type of this event; one of LOCAL_USER_JOINED, LOCAL_USER_LEFT, etc.
	 * @param reason
	 *        the reason
	 */
	public void fireWhiteboardSessionPresenceEvent(WhiteboardSession session, String eventType,
		String reason)
	{
		WhiteboardSessionPresenceChangeEvent evt = new WhiteboardSessionPresenceChangeEvent(this,
			session, eventType, reason);
