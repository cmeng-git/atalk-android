...
public class ServerChatRoomQuery extends AsyncContactQuery<ContactSourceService> implements ChatRoomProviderWrapperListener
{
	...
	/**
	 * Creates an instance of <tt>ChatRoomQuery</tt> by specifying the parent contact source, the query string to match and the maximum
	 ...
	 *        indicates whether we should add the chat room to the query results or fire an event without adding it to the results.
	 */
	 ...
	 *
	 * @param pps
	 *        the protocol provider associated with the found chat room.
	 * @param chatRoomName
	 ...
	 *        the protocol provider associated with the found chat room.
	 * @param chatRooms
	 *        The set of chat rooms based on BaseChatRoomSourceContact. This is the full set and it will be filtered according to demands of
	 *        the queryString.
	 * @param addQueryResult
	 *        indicates whether we should add the chat room to the query results or fire an event without adding it to the results.
	 */
	private void addChatRooms(final ProtocolProviderService pps, final Set<BaseChatRoomSourceContact> chatRooms, final boolean addQueryResult)
	{
		...
		while (iterator.hasNext()) {
			...
			if (!((queryString == null || (room.getChatRoomName().contains(queryString) || room.getChatRoomID().contains(queryString))) && isMatching(
				room.getChatRoomID(), pps))) {
				iterator.remove();
			}
		}
	 ...
	 * 
	 * @param status
	 *        {@link ContactQuery#QUERY_CANCELED}, {@link ContactQuery#QUERY_COMPLETED}
	 */
	 ...
	 * 
	 * @param chatRoomID
	 *        the chat room id associated with the contact.
	 * @param pps
	 *        the provider of the chat room contact.
