        ...
        implements ChatRoomProviderWrapperListener
{
    ...
    private void providerAdded(final ChatRoomProviderWrapper provider, final boolean addQueryResult)
    {
        ...
        final Set<BaseChatRoomSourceContact> chatRooms = new HashSet<>(chatRoomNames.size());
            ...
            final boolean addQueryResult)
    {
        ...
        while (iterator.hasNext()) {
            ...
            if (!((queryString == null || (room.getChatRoomName().contains(queryString)
                    || room.getChatRoomID().contains(queryString))) && isMatching(room.getChatRoomID(), pps))) {
                iterator.remove();
            }
        }
