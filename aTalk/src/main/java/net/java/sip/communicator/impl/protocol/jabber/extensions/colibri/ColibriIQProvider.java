...
public class ColibriIQProvider extends IQProvider<ColibriConferenceIQ>
{
	...
	public ColibriIQProvider()
	{
		...
		ProviderManager.addExtensionProvider(
				...
				ColibriConferenceIQ.NAMESPACE, parameterPacketExtension);
		ProviderManager.addExtensionProvider(
				ParameterPacketExtension.ELEMENT_NAME,
				SourcePacketExtension.NAMESPACE, parameterPacketExtension);

		// Shutdown IQ
		ProviderManager.addIQProvider(ShutdownIQ.GRACEFUL_ELEMENT_NAME, ShutdownIQ.NAMESPACE,
				this);
	 ...
	 *
	 * @param parser
	 * 		an <tt>XmlPullParser</tt> which specifies the IQ sub-document to be parsed into a new
	 * 		<tt>IQ</tt> instance
	 *
	 ...
	 * @param name
	 * 		the name of the element at the start tag of which the specified <tt>parser</tt> is
	 * 		presumed to currently be and until the end tag of which XML content is to be thrown
	 * 		away
	 * @throws Exception
	 * 		if an errors occurs while parsing the XML content
	 */
