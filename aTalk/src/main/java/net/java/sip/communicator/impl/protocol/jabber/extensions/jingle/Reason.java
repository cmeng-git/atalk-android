 ...
 * condition that prompted the corresponding jingle action.
 *
 * @author Emil Ivov
 */
public enum Reason {
	/**
	 * A reason indicating that the party prefers to use an existing session with the peer rather
	 * than initiate a new session; the Jingle session ID of the alternative session SHOULD be
	 * provided as the XML character data of the <sid/> child.
	 */
	...
	/**
	 * A reason indicating that the initiator wishes to formally cancel the session initiation
	 * request.
	 */
	CANCEL("cancel"),
	...
	/**
	 * A reason indicating that the session length has exceeded a pre-defined time limit (e.g., a
	 * meeting hosted at a conference service).
	 */
	EXPIRED("expired"),

	/**
	 * A reason indicating that the party has been unable to initialize processing related to the
	 * application type.
	 */
	FAILED_APPLICATION("failed-application"),

	/**
	 * A reason indicating that the party has been unable to establish connectivity for the
	 * transport method.
	 */
	...
	/**
	 * A reason indicating that the party supports the offered application type but does not support
	...
	/**
	 * A reason indicating that the action is generated during the normal course of state management
	 * and does not reflect any error.
	 */
	SUCCESS("success"),

	/**
	 * A reason indicating that a request has not been answered so the sender is timing out the
	 * request.
	 */
	TIMEOUT("timeout"),
	...
	/**
	 * Returns the name of this reason (e.g. "success" or "timeout"). The name returned by this
	 * method is meant for use directly in the XMPP XML string.
	 *
	...
	/**
	 * Returns a <tt>Reason</tt> value corresponding to the specified <tt>reasonValueStr</tt> or in
	 * other words {@link #SUCCESS} for "success" or {@link #TIMEOUT} for "timeout").
	 *
