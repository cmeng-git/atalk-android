...
public class OperationSetFileTransferJabberImpl
		implements OperationSetFileTransfer, NegotiationProgress
{
	/**
	 ...
	 *
	 * @param provider
	 * 		is the provider that created us
	 */
	 ...
	 *
	 * @param toContact
	 * 		the contact that should receive the file
	 * @param file
	 * 		file to send
	 ...
	 * 		file to send
	 * @param gw
	 * 		special gateway to be used for receiver if its jid misses the domain part
	 * @return the transfer object
	 */
			...
			throws IllegalStateException, IllegalArgumentException, OperationNotSupportedException
	{
		...
		if (mucOpSet != null && mucOpSet.isPrivateMessagingContact(toContact.getAddress())) {
			...
			List<Presence> iter = Roster.getInstanceFor(jabberProvider.getConnection())
					.getPresences(toContact.getJid().asBareJid());
			int bestPriority = -1;
		...
		OutgoingFileTransfer transfer
				= manager.createOutgoingFileTransfer((EntityFullJid) fullJid);
	 ...
	 * @return the transfer object
	 */
	public FileTransfer sendFile(Contact toContact, Contact fromContact, String remotePath,
			String localPath)
			throws IllegalStateException, IllegalArgumentException, OperationNotSupportedException
	{
	 ...
	 *
	 * @param listener
	 * 		the <tt>FileTransferListener</tt> to add
	 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>FileTransferListener</tt> to remove
	 */
			...
			throws IllegalStateException
	{
		if (jabberProvider == null)
			...
			if (opSetPersPresence.getPresenceStatus().isOnline()) {
				opSetPersPresence.fireProviderStatusChangeEvent(opSetPersPresence
								.getPresenceStatus(),
		...
		public void registrationStateChanged(RegistrationStateChangeEvent evt)
		{
			...
			if (evt.getNewState() == RegistrationState.REGISTERED) {
				...
				ProviderManager.addIQProvider(StreamInitiation.ELEMENT, StreamInitiation.NAMESPACE,
						new SiThumbProvider());
				ProviderManager.addIQProvider(ThumbnailIQ.ELEMENT, ThumbnailIQ.NAMESPACE,
				...
				ProviderManager.removeIQProvider(StreamInitiation.ELEMENT,
	...
	private class IQRequestHandler extends AbstractIqRequestHandler
	{
		...
		{
			super(StreamInitiation.ELEMENT, StreamInitiation.NAMESPACE, IQ.Type.set,
		...
		public IQ handleIQRequest(IQ packet)
		{
			...
			// cmeng - Send thumbnail request if advertised in streamInitiation packet and the
			// feature is enabled
			org.jivesoftware.smackx.si.packet.StreamInitiation.File file = streamInitiation
					.getFile();
			...
			if ((file instanceof FileElement) && FileTransferConversation.FT_THUMBNAIL_ENABLE) {
				...
				if ((thumbnailElement != null) && (thumbnailElement.getCid() != null)) {
					...
					incomingFileTransferRequest.createThumbnailListeners(
			...
			if (!isThumbnailFile) {
				...
				FileTransferRequestEvent fileTransferRequestEvent = new FileTransferRequestEvent(
						OperationSetFileTransferJabberImpl.this, incomingFileTransferRequest,
						new Date());
	 ...
	 *
	 * @param event
	 * 		the <tt>EventObject</tt> that we'd like delivered to all registered file transfer
	 * 		listeners.
	 */
	 ...
	 *
	 * @param event
	 * 		the <tt>FileTransferEvent</tt> that we'd like delivered to all registered file
	 * 		transfer listeners.
	 */
