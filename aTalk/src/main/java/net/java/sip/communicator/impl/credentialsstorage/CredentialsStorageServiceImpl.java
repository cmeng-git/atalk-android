...
public class CredentialsStorageServiceImpl implements CredentialsStorageService
{
	...
	/**
	 * The property in the configuration that we use to verify master password existence and
	 * correctness.
	 */
	private static final String MASTER_PROP = "credentialsstorage.MASTER";

	/**
	 * This value will be encrypted and saved in MASTER_PROP and will be used to verify the key's
	 * correctness.
	 */
	 ...
	 * 		account UUID
	 * @param password
	 * 		the password to store; remove if password == null
	 ...
	 * properties from the configuration with the oldPassword and encrypts them again with
	 * newPassword.
	 *
	 * @param oldPassword
	 * 		old master password
	...
	private void moveAllPasswordProperties()
	{
		...
		for (String prop : unencryptedProperties) {
			...
			if (idx != -1) {
				...
				else if (!movePasswordProperty(prefix,
	 ...
	 *
	 * @param remove
	 * 		to remove the verification value or just overwrite it.
	 */
	...
	private synchronized boolean createCrypto()
	{
         ...
         * master password prompt shown twice in a row during application startup when
         * unencrypted passwords are to be migrated with the master password already set and the
         * accounts start loading.
         */
		...
		if (crypto == null) {
			...
			if (isUsingMasterPassword()) {
				...
				if (master == null) {
                    ...
                    /*
                     * At this point the master password must be correct, so we set the crypto
                     * instance to use it
                     */
					setMasterPassword(master);
				}
                 ...
                 * encryption/decryption but using some default value, not something specified by
                  * the user.
                 */
	...
	private String showPasswordPrompt()
	{
		...
		// Ask for master password until the input is correct or cancel button is pressed and
		// null returned
	 ...
	 * 		account UUID
	 * @param value
	 * 		the unencrypted account password
	 */
