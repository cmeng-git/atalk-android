...
public class MobileIndicator implements RegistrationStateChangeListener, UserCapsNodeListener
{
	 ...
	 *        the parent provider.
	 * @param ssclCallback
	 *        the callback for the contact list to obtain contacts.
	 */
		...
		ServerStoredContactListJabberImpl ssclCallback)
	{
		...
		String indicatorResource = parentProvider.getAccountID().getAccountProperties()
			.get(MOBILE_INDICATOR_RESOURCE_ACC_PROP);
		if (indicatorResource != null && indicatorResource.length() > 0) {
			isCapsMobileIndicator = false;
			checkStrings = indicatorResource.split(",");
		}
		else {
			String indicatorCaps = parentProvider.getAccountID().getAccountProperties()
				.get(MOBILE_INDICATOR_CAPS_ACC_PROP);
	 ...
	 * @param bareJid
	 *        the bareJid to check.
	 * @return whether resource with that name is mobile or not.
	 */
	boolean isMobileResource(String resourceName, Jid bareJid)
	{
		if (isCapsMobileIndicator) {
			...
			DiscoverInfo caps
					= EntityCapsManager.getDiscoveryInfoByNodeVer(capsManager.getLocalNodeVer());

			if (caps != null && containsStrings(caps.getNode(), checkStrings))
				return true;
			else
				return false;
		}
	 ...
	 * 
	 * @param user
	 *        the contact address with or without resource.
	 */
	private void updateMobileIndicatorUsingCaps(Jid user)
	{
		ContactJabberImpl contact = ssclCallback.findContactById(user.toString());
		...
		for (Map.Entry<Jid, ContactResourceJabberImpl> resEntry
	 ...
	 *        the value to check
	 * @param checkStrs
	 *        an array of strings we are searching for.
