...
public class RegistrationStateChangeEvent extends PropertyChangeEvent
{
	 ...
	 * @param oldValue
	 *        the status the source provider was in before entering the new state.
	 * @param newValue
	 *        the status the source provider is currently in.
	 * @param reasonCode
	 *        a value corresponding to one of the REASON_XXX fields of this class, indicating the
	 *        reason for this state transition.
	 * @param reason
	 *        a String further explaining the reason code or null if no such explanation is
	 *        necessary.
	 */
	public RegistrationStateChangeEvent(ProtocolProviderService source, RegistrationState oldValue,
		RegistrationState newValue, int reasonCode, String reason)
	{
		super(source, RegistrationStateChangeEvent.class.getName(), oldValue, newValue);
	...
	{
		return "RegistrationStateChangeEvent[ oldState=" + getOldState().getStateName()
				+ "; newState=" + getNewState() + "; reasonCode=" + getReasonCode()
				+ "; reason=" + getReason() + "]";
	}
	 ...
	 * 
	 * @param userRequest
	 * <tt>true</tt> if from user request
	 */
