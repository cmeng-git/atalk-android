		...
		implements RegistrationStateChangeListener, StanzaFilter
{
	...
	private static final Logger logger = Logger
			.getLogger(OperationSetTelephonyConferencingJabberImpl.class);
	 ...
	 * 		the Jabber <tt>ProtocolProviderService</tt> implementation which has requested the
	...
	public OperationSetTelephonyConferencingJabberImpl(
		...
		this.isCoinDisabled = JabberActivator.getConfigurationService().getBoolean(
				DISABLE_COIN_PROP_NAME, false);
	}
	...
	private void notify(CallPeer callPeer) {
		...
		Jid to = getBasicTelephony()
				.getFullCalleeURI(((CallPeerJabberImpl) callPeer).getPeerJid());
	 ...
	 *
	 * @param callPeer
	 * 		the <tt>CallPeer</tt> to generate conference-info XML for
	 * @param confInfo
	 * 		the <tt>ConferenceInformationDocument</tt> which is to be included in the IQ
	...
	private CoinIQ getConferenceInfo(CallPeerJabberImpl callPeer, final ConferenceInfoDocument confInfo) {
		...
		CoinIQ iq = new CoinIQ() {
			...
			public IQChildElementXmlStringBuilder getIQChildElementBuilder(IQChildElementXmlStringBuilder xml) {
				...
				String xmlString = confInfo.toString().replace("<conference-info xmlns=\"urn:ietf:params:xml:ns:conference-info\"", "").replace("</conference-info>", "");
			...
			throws OperationFailedException, XmppStringprepException
	{
		return getBasicTelephony()
				.getFullCalleeURI(JidCreate.from(calleeAddressString)).toString();
	}
	...
	private class IQRequestHandler extends AbstractIqRequestHandler {
			...
			if (sid != null) {
				...
				if (callPeer != null) {
					if (type == IQ.Type.error) {
						...
						if (logger.isDebugEnabled())
							logger.debug("Processing COIN from " + coinIQ.getFrom() + " (version="
									+ coinIQ.getVersion() + ")");
					}
				}
			}
		 ...
		 *
		 * @param callPeer
		 * 		the <tt>CallPeer</tt> from which the specified <tt>CoinIQ</tt> was sent
		 * @param coinIQ
		 * 		the <tt>CoinIQ</tt> which was sent from the specified <tt>callPeer</tt>
		 */
		private void handleCoin(CallPeerJabberImpl callPeer, CoinIQ coinIQ) {
			...
			try {
				...
				logger.error("Could not handle received COIN from " + callPeer + "; ResultCoinIQ: "
						+ mCoinIQ);
			}
		}
	}
	...
	protected String getLocalEntity(CallPeer callPeer) {
		...
		if (from != null) {
			...
			logger.info("### RoomName creation (result): " + callPeer.toString() + " ("
					+ chatRoomName + ")");
			if (room != null)
	...
	public ConferenceDescription setupConference(final ChatRoom chatRoom) {
		OperationSetVideoBridge videoBridge = parentProvider
				.getOperationSet(OperationSetVideoBridge.class);
