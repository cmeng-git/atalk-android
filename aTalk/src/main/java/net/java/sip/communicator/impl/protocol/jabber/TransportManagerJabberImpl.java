 ...
 * the harvest. Calling a second wrapup method would deliver the candidate harvest, possibly after
...
public abstract class TransportManagerJabberImpl extends TransportManager<CallPeerJabberImpl>
{
	...
	/**
	 * The indicator which determines whether this <tt>TransportManager</tt> instance is responsible
	 * to establish the connectivity with the associated Jitsi Videobridge (in case it is being
	 * employed at all).
	 */
	 ...
	 *
	 * @param peer
	 *        the CallPeer that we would contact.
	 *
	 ...
	 *         if <tt>destination</tt> is not a valid host/IP/FQDN
	 */
	@Override
	 ...
	 *
	 * @param map
	 *        a <tt>Map</tt> of media-IceUdpTransportPacketExtension pairs which represents the
	 ...
	 *
	 * @param theirContent
	 *        the <tt>ContentPacketExtension</tt> offered by the remote peer to which we are going
	 *        to answer with <tt>ourContent</tt> or <tt>null</tt> if <tt>ourContent</tt> will be an
	 *        offer to the remote peer
	 * @param ourContent
	 *        the <tt>ContentPacketExtension</tt> for which transport candidate harvest is to be
	 *        started
	 * @param transportInfoSender
	 *        a <tt>TransportInfoSender</tt> if the harvested transport candidates are to be sent in
	 *        a <tt>transport-info</tt> rather than in <tt>ourContent</tt>; otherwise, <tt>null</tt>
	 * @param media
	 *        the media of the <tt>RtpDescriptionPacketExtension</tt> child of <tt>ourContent</tt>
	 * @return a <tt>ExtensionElement</tt> to be added as a child to <tt>ourContent</tt>; otherwise,
	 *         <tt>null</tt>
	 * @throws OperationFailedException
	 *         if anything goes wrong while starting transport candidate harvest for the specified
	 *         <tt>ourContent</tt>
	 */
		...
		throws OperationFailedException
	{
		...
		if (isJitsiVideobridge) {
			...
			for (ContentPacketExtension cpe : cpes) {
				...
				if ((colibri == null) || (colibri.getContent(mediaType.toString()) == null)) {
					...
					if (cpes == ourAnswer) {
						local = cpe;
						remote = (theirOffer == null) ? null : findContentByName(theirOffer,
							cpe.getName());
					}
					else {
			...
			if (!contentMap.isEmpty()) {
				...
				for (Map.Entry<ContentPacketExtension, ContentPacketExtension> e : contentMap
					.entrySet()) {
				...
				if (conferenceResult != null) {
					...
					for (ColibriConferenceIQ.Content contentResult : conferenceResult.getContents()) {
						ColibriConferenceIQ.Content content = colibri
							.getOrCreateContent(contentResult.getName());

						for (ColibriConferenceIQ.Channel channelResult : contentResult
							.getChannels()) {
		...
		for (ContentPacketExtension cpe : cpes) {
			...
			if (ourContent != null) {
				ContentPacketExtension theirContent = (theirOffer == null) ? null
				...
				ExtensionElement pe = startCandidateHarvest(theirContent, ourContent,
						transportInfoSender, media);
	 ...
	 *
	 * @param cpExtList
	 *        the list that we will be searching for a specific content.
	 * @param name
	 *        the name of the content element we are looking for.
	...
	public static ContentPacketExtension findContentByName(
		Iterable<ContentPacketExtension> cpExtList, String name)
	{
	 ...
	 *
	 * @param remote
	 *        a <tt>Map</tt> of media-<tt>IceUdpTransportPacketExtension</tt> pairs which represents
	 ...
	 *
	 * @throws OperationFailedException
	 *         if anything goes wrong with connectivity establishment (i.e. ICE failed, ...)
	 */
	 ...
	 *
	 * @param contents
	 *        the collection of contents to remove the content with the specified name from
	 * @param name
	...
	protected ContentPacketExtension removeContent(Iterable<ContentPacketExtension> contents,
		String name)
	{
		for (Iterator<ContentPacketExtension> contentIter = contents.iterator(); contentIter
			.hasNext();) {
	 ...
	 * @throws OperationFailedException
	 *         if an error occurs during the cloing of the specified <tt>src</tt> and its candidates
	 */
	 ...
	 *
	 * @param mediaType
	 *        the <tt>MediaType</tt> associated with the specified <tt>streamConnector</tt>
	 * @param streamConnector
	 *        the <tt>StreamConnector</tt> to be closed
	 */
	@Override
			...
			if (streamConnector instanceof ColibriStreamConnector) {
				...
				if (peer != null) {
					...
					if (call != null) {
						...
						call.closeColibriStreamConnector(peer, mediaType,
			...
			if (colibri != null) {
				...
				if (content != null) {
					...
					if (channels.size() == 2) {
						...
						ColibriConferenceIQ.Content requestContent = requestConferenceIQ
							.getOrCreateContent(content.getName());
	 ...
	 *
	 * @param mediaType
	 *        the <tt>MediaType</tt> of the stream for which a <tt>StreamConnector</tt> is to be
	 *        created
	 * @return a <tt>StreamConnector</tt> for the stream of the specified <tt>mediaType</tt>
	 * @throws OperationFailedException
	 *         if the binding of the sockets fails
	 */
				...
				for (Call aCall : conference.getCalls()) {
					...
					while (callPeerIter.hasNext()) {
						...
						if (aCallPeer instanceof CallPeerJabberImpl) {
							TransportManagerJabberImpl aTransportManager = ((CallPeerJabberImpl) aCallPeer)
	 ...
	 *
	 * @param mediaType
	 *        the <tt>MediaType</tt> associated with the content which contains the
	 *        <tt>ColibriConferenceIQ.Channel</tt> to get
	 * @param local
	 *        <tt>true</tt> if the <tt>ColibriConferenceIQ.Channel</tt> which is to be used locally
