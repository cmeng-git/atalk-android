...
public class ContactGroupJabberImpl extends AbstractContactGroupJabberImpl
{
	...
	/**
	 * a list that would always remain empty. We only use it so that we're able to extract empty
	 * iterators
	 */
	 ...
	 * 
	 * @param rosterGroup
	 *        the Jabber Group corresponding to the group
	 * @param groupMembers
	 *        the group members that we should add to the group.
	 * @param ssclCallback
	 *        a callback to the server stored contact list we're creating.
	 * @param isResolved
	 *        a boolean indicating whether or not the group has been resolved against the server.
	 */
	ContactGroupJabberImpl(RosterGroup rosterGroup, Iterator<RosterEntry> groupMembers,
		ServerStoredContactListJabberImpl ssclCallback, boolean isResolved)
	{
	...
	{
		contacts.put(contact.getAddress().toLowerCase(Locale.US), contact);
	}
	...
	{
		contacts.remove(contact.getAddress().toLowerCase(Locale.US));
	}
	...
	{
		if (isResolved)
			// now we use the id field to store the rosterGroup name for later retrieval from
	...
	public boolean equals(Object obj)
	{
		...
		// since Jabber does not support having two groups with the same name at this point we
		// could bravely state that the groups are the same and not bother to compare contacts.
		// (gotta check that though)
		return true;
	}
	...
	ContactJabberImpl findContact(String id)
	{
		if (id == null)
			return null;
		return (ContactJabberImpl) contacts.get(id.toLowerCase(Locale.US));
	}
