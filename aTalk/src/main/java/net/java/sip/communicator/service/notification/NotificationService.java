...
public interface NotificationService
{
	 ...
	 *
	 * @param eventType
	 *        the name of the event (as defined by the plug-in that's registering it) that we are setting an action for.
	 * @param action
	 *        the <tt>NotificationAction</tt>, which would be used to perform the notification action.
	 */
	void registerNotificationForEvent(String eventType, NotificationAction action);

	/**
	 * Registers a default notification for the given <tt>eventType</tt> by specifying the action to be performed when a
	 * notification is fired for this event.
	 *
	 * Unlike the other <tt>registerDefaultNotificationForEvent</tt> method, this one allows the user to specify its own
	 ...
	 *        the name of the event (as defined by the plug-in that's registering it) that we are setting an action for.
	 * @param handler
	 *        the <tt>NotificationActionHandler</tt>, which would be used to perform the notification action.
	 */
	 ...
	 *
	 * Actions registered by this method would be handled by some default <tt>NotificationHandler</tt>s, declared by the
	 * implementation.
	 * <p>
	 ...
	 *        the name of the event (as defined by the plug-in that's registering it) that we are setting an action for.
	 * @param actionType
	 *        the type of the action that is to be executed when the specified event occurs (could be one of the
	 *        ACTION_XXX fields).
	 * @param actionDescriptor
	 *        a String containing a description of the action (a URI to the sound file for audio notifications or a
	 *        command line for exec action types) that should be executed when the action occurs.
	 * @param defaultMessage
	 *        the default message to use if no specific message has been provided when firing the notification.
	 */
	 ...
	 * <p>
	 * This method does nothing if the given <tt>eventType</tt> or <tt>actionType</tt> are not contained in the list of
	 * registered types.
	 *
	 * @param eventType
	 *        the name of the event (as defined by the plugin that's registering it) for which we'll remove the
	 *        notification.
	 * @param actionType
	 *        the type of the action that is to be executed when the specified event occurs (could be one of the
	 *        ACTION_XXX fields).
	 ...
	 *
	 * @param listener
	 *        the listener that we'd like to register for changes in the event notifications stored by this service.
	 */
	 ...
	 *
	 * @param actionType
	 *        the type for which the list of handlers should be retrieved or <tt>null</tt> if all handlers shall be
	 *        returned.
	...
	/**
	 * Fires all notifications registered for the specified <tt>eventType</tt> using <tt>message</tt> as a notification
	 * message wherever appropriate (e.g. systray notifications, logs, etc.)
	 * <p>
	 * This method does nothing if the given <tt>eventType</tt> is not contained in the list of registered event types.
	 * </p>
	 *
	 ...
	 * @param messageTitle
	 *        the message title to use if and where appropriate (e.g. with systray)
	 * @param message
	 ...
	 * @return An object referencing the notification. It may be used to stop a still running notification. Can be null
	 *         if the eventType is unknown or the notification is not active.
	 */
	 ...
	 *        the icon to show in the notification if and where appropriate
	 * @param extras
	 *        additional/extra {@link NotificationHandler}-specific data to be provided to the firing of the specified
	 *        notification(s). The well-known keys are defined by the <tt>NotificationData</tt> <tt>XXX_EXTRA</tt>
	 *        constants.
	 * @return An object referencing the notification. It may be used to stop a still running notification. Can be null
	 *         if the eventType is unknown or the notification is not active.
	 */
	NotificationData fireNotification(String eventType, String messageTitle, String message, byte[] icon, Map<String, Object> extras);
	...
	/**
	 * Stops a notification if notification is continuous, like playing sounds in loop. Do nothing if there are no such
	 ...
	 * @param isActive
	 *        indicates whether to activate or deactivate the actions related to the specified <tt>eventType</tt>.
	 */
