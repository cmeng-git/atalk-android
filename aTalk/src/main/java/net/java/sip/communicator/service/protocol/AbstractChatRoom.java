...
public abstract class AbstractChatRoom implements ChatRoom
{
	...
	protected final List<ChatRoomConferencePublishedListener> conferencePublishedListeners
			= new ArrayList<>();
	...
	protected void fireConferencePublishedEvent(ChatRoomMember member, ConferenceDescription cd,
		int eventType)
	{
		ChatRoomConferencePublishedEvent evt
				= new ChatRoomConferencePublishedEvent(eventType, this, member, cd);
	...
	protected boolean processConferenceDescription(ConferenceDescription cd, String participantName)
	{
		if (cd.isAvailable()) {
			...
			ConferenceDescription cachedDescription = (ConferenceDescription)
					this.cachedConferenceDescriptions.get(participantName);
