...
public final class AccountManagerUtils
{
	...
	{
		return bundleContext.getService(bundleContext.getServiceReference(AccountManager.class));
	}

	/**
	 * Starts a specific <code>Bundle</code> and waits for the <code>AccountManager</code> available
	 * in a specific <code>BundleContext</code> to load the stored accounts of a
	 * <code>ProtocolProviderFactory</code> with a specific protocol name.
	 *
	 * @param bundleContextWithAccountManager
	 *        the <code>BundleContext</code> in which an <code>AccountManager</code> service is
	 *        registered
	 * @param bundleToStart
	 *        the <code>Bundle</code> to be started
	 * @param protocolNameToWait
	 *        the protocol name of a <code>ProtocolProviderFactory</code> to wait the end of the
	...
	public static void startBundleAndWaitStoredAccountsLoaded(
		BundleContext bundleContextWithAccountManager, final Bundle bundleToStart,
		...
		AccountManagerListener listener = new AccountManagerListener()
		{
			public void handleAccountManagerEvent(AccountManagerEvent event)
			{
				...
				if (factory != null) {
					...
					try {
						factoryRefs = bundleContext.getServiceReferences(
							ProtocolProviderFactory.class, "(" + ProtocolProviderFactory.PROTOCOL
								+ "=" + protocolNameToWait + ")");
					}
