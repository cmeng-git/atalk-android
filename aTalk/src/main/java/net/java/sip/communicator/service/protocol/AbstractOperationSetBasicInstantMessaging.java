 ...
 */
public abstract class AbstractOperationSetBasicInstantMessaging implements
		OperationSetBasicInstantMessaging
{
	 ...
	 *
	 * @param message
	 * 		the <tt>Message</tt> that has been received.
	 * @param from
	 * 		the <tt>Contact</tt> that <tt>message</tt> was received from.
	 */
	...
	private EventObject[] messageTransform(final EventObject evt, final MessageEventType eventType)
	{
		...
		switch (eventType) {
			case MessageDelivered:
				protocolProvider = ((MessageDeliveredEvent) evt).getDestinationContact()
						.getProtocolProvider();
				break;
			case MessageDeliveryFailed:
				protocolProvider = ((MessageDeliveryFailedEvent) evt).getDestinationContact()
						.getProtocolProvider();
				break;
			...
			case MessageReceived:
				protocolProvider
						= ((MessageReceivedEvent) evt).getSourceContact().getProtocolProvider();
				break;
			default:
		...
		// 'current' contains the events that need to be transformed. It should not contain null
		// values.
		...
		for (Map.Entry<Integer, Vector<TransformLayer>> entry
			...
			for (TransformLayer transformLayer : entry.getValue()) {
				...
				while (!current.isEmpty()) {
					...
					switch (eventType) {
						case MessageDelivered:
							MessageDeliveredEvent transformedDelivered = transformLayer
						...
						case MessageDeliveryPending:
							MessageDeliveredEvent[] evts = transformLayer
									.messageDeliveryPending((MessageDeliveredEvent) event);
							...
							break;
						case MessageDeliveryFailed:
							MessageDeliveryFailedEvent transformedDeliveryFailed = transformLayer
