...
public class ChatRoomListImpl implements RegistrationStateChangeListener, ServiceListener
{
	...
	{
		try {
			ServiceReference[] serRefs = MUCActivator.bundleContext
	 ...
	 *
	 * @param pps
	 * 		the <tt>ProtocolProviderService</tt> corresponding to the chat server
	 */
	ChatRoomProviderWrapper addRegisteredChatProvider(ProtocolProviderService pps)
	{
		...
		for (String chatRoomID: chatRoomList) {
			ChatRoomWrapper chatRoomWrapper
					= new ChatRoomWrapperImpl(chatRoomProvider, chatRoomID);
	 ...
	 *
	 * @param pps
	 * 		the <tt>ProtocolProviderService</tt> corresponding to the server to remove
	 */
	 ...
	 * 		the <tt>ChatRoomProviderWrapper</tt> corresponding to the server to remove
	 * @param permanently
	 * 		whether to remove any listener and stored configuration
	 */
	...
	public void removeChatRoom(ChatRoomWrapper chatRoomWrapper)
	{
		...
		if (providersList.contains(chatRoomProvider)) {
			...
			ConfigurationUtils.removeChatRoom(chatRoomProvider.getProtocolProvider(),
	 ...
	 *
	 * @param chatRoomID
	 * 		the id of <tt>ChatRoom</tt> that we're looking for
	 * @param pps
	 * 		the protocol provider associated with the chat room.
	 ...
	 *
	 * @param evt
	 * 		a <tt>RegistrationStateChangeEvent</tt> which describes the event that occurred.
	 */
	@Override
