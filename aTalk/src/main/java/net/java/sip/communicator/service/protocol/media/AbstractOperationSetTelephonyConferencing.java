...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractOperationSetTelephonyConferencing</tt> class and
	 * its instances.
	 */
	private static final Logger logger = Logger
		.getLogger(AbstractOperationSetTelephonyConferencing.class);
	 ...
	 *        the <tt>ProtocolProviderService</tt> implementation which has requested the creation
	 ...
	 *
	 * @param oldValue
	 *        the old value of the <tt>basicTelephony</tt> property
	 * @param newValue
	 *        the new value of the <tt>basicTelephony</tt> property
	 */
	 ...
	 *
	 * @param event
	 *        a <tt>CallEvent</tt> which specified the newly-established <tt>Call</tt>
	 */
	 ...
	 *
	 * @param event
	 *        a <tt>CallEvent</tt> which specified the <tt>Call</tt> which has just ended
	 */
	 ...
	 *
	 * @param event
	 *        a <tt>CallPeerEvent</tt> which specifies the <tt>CallPeer</tt> which has been added to
	 *        a <tt>Call</tt>
	 */
	 ...
	 *
	 * @param event
	 *        a <tt>CallPeerEvent</tt> which specifies the <tt>CallPeer</tt> which has been removed
	 *        from a <tt>Call</tt>
	 */
	 ...
	 *
	 * @param event
	 *        a <tt>CallChangeEvent</tt> which specifies the <tt>Call</tt> which has changed its
	 ...
	 *         if establishing the conference call fails
	 ...
	 *        the list of addresses that we should call
	 * @param conference
	 *        the <tt>CallConference</tt> which represents the state of the telephony conference
	 ...
	 * @throws OperationFailedException
	 *         if establishing the conference <tt>Call</tt> fails
	 */
	 ...
	 *
	 * @param calleeAddress
	 *        the address of the callee to be invited to the specified existing <tt>Call</tt>
	 * @param call
	 *        the existing <tt>Call</tt> to invite the callee with the specified address to
	 ...
	 * @throws OperationFailedException
	 *         if inviting the specified callee to the specified <tt>Call</tt> fails
	 */
	 ...
	 *
	 * @param endpoint
	 *        an XML <tt>Node</tt> which represents the <tt>endpoint</tt> XML element from which to
	 ...
	 *
	 * @param callPeer
	 *        the <tt>CallPeer</tt> whose remote SSRC for the media of the specified
	 *        <tt>mediaType</tt> is to be returned
	 * @param mediaType
	 *        the <tt>MediaType</tt> of the specified <tt>callPeer</tt>'s media whose remote SSRC is
	 *        to be returned
	 ...
	 *
	 * @param event
	 *        a <tt>CallEvent</tt> which specifies the newly-received incoming <tt>Call</tt>
	 */
	 ...
	 * @throws OperationFailedException
	 *         if inviting the specified callee to the specified call fails
	 */
	 ...
	 *
	 * @param call
	 *        the <tt>Call</tt> which specifies the telephony conference the associated
	 ...
	 *
	 * @param call
	 *        the <tt>Call</tt> whose <tt>CallPeer</tt>s are to be notified about changes in the
	 ...
	 *
	 * @param event
	 *        a <tt>CallEvent</tt> which specifies the newly-created outgoing <tt>Call</tt>
	 */
	 ...
	 * <tt>Call</tt>.
	 *
	 * @param calleeAddressString
	 *        a <tt>String</tt> value which represents a callee address to be parsed into an object
	 ...
	 * @throws OperationFailedException
	 *         if parsing the specified <tt>calleeAddressString</tt> fails
	 */
	 ...
	 *
	 * @param ev
	 *        a <tt>PropertyChangeEvent</tt> which describes the source of the event, the name of
	 ...
	 *
	 * @param event
	 *        a <tt>RegistrationStateChangeEvent</tt> which specifies the old and the new value of
	 ...
	 *
	 * @param callPeer
	 *        the <tt>CallPeer</tt> which is a conference focus and has sent the specified
	 *        conference-info XML document
	 * @param confInfo
	 *        the conference-info XML document to use to update the conference-related information
		...
		ConferenceInfoDocument confInfo)
	{
		...
		ConferenceMember[] toRemove = conferenceMembers.toArray(new ConferenceMember[conferenceMembers.size()]);
	 ...
	 *        the <tt>CallPeer</tt> which is a conference focus and has sent the specified
	 *        conference-info XML document
	 * @param conferenceInfoXML
	 *        the conference-info XML document sent by <tt>callPeer</tt> in order to update the
	 ...
	 *
	 * @throws XMLException
	 *         If <tt>conferenceInfoXML</tt> couldn't be parsed as a <tt>ConferenceInfoDocument</tt>
	 */
		...
		throws XMLException
	{
		...
		if (ourVersion == -1) {
			if (documentState == ConferenceInfoDocument.State.FULL) {
				...
				logger.warn("Received a conference-info document with state '" + documentState
					+ "'. Cannot apply it, because we haven't "
					+ "initialized a local document yet. Sending peer: " + callPeer);
				return -1;
			}
		}
			...
			if (logger.isInfoEnabled()) {
				logger.info("Received a stale conference-info document. Local " + "version "
					+ ourVersion + ", document version " + documentVersion + ". Sending peer: "
					+ callPeer);
			}
			return -1;
		}
		else // ourVersion != -1 && ourVersion < documentVersion
		{
			if (documentState == ConferenceInfoDocument.State.FULL)
				...
				if (documentVersion == ourVersion + 1)
					 ...
					 */
					logger.warn("Received a Conference Information document " + "with state '"
						+ documentState + "' and version " + documentVersion
						+ ". Cannon apply it, because local " + "version is " + ourVersion
						+ ". Sending peer: " + callPeer);
					return -1;
				}
			}
			else
	 ...
	 *
	 * @param address
	 *        the <tt>String</tt> value representing an address from which any parameters are to be
	 *        removed
	 ...
	 *
	 * @param confInfo
	 *        the <tt>ConferenceInformationDocument</tt> to which to add a <tt>user</tt> element
	 * @param callPeer
	 *        the <tt>CallPeer</tt> which should be described
	 * @param remote
	 *        <tt>true</tt> to describe <tt>callPeer</tt>, or <tt>false</tt> to describe the local
	 *        peer.
	 */
		...
		MediaAwareCallPeer<?, ?, ?> callPeer, boolean remote)
	{
		...
		logger.info("### Gettting entity (remote): " + entity + " (" + remote + ")");
	 ...
	 *
	 * @param callPeer
	 *        The <tt>CallPeer</tt> for which we are creating a Conference Information document.
	 ...
	 *
	 * @param callPeer
	 *        the <tt>CallPeer</tt> which is to get its state described in a <tt>status</tt> XML
	...
	/**
	 * @param from
	 *        A document with state <tt>full</tt> from which to generate a "diff".
	 * @param to
	 *        A document with state <tt>full</tt> to which to generate a "diff"
	 ...
	 *
	 * @param callPeer
	 *        the <tt>CallPeer</tt> which is a conference focus and has sent the specified partial
	 *        conference-info XML document
	 * @param diff
	 *        the partial conference-info XML document sent by <tt>callPeer</tt> in order to update
		...
		ConferenceInfoDocument diff)
	{
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Applied a partial conference-info notification. " + " Base: "
				+ ourDocument + "\nDiff: " + diff + "\nResult:" + newDocument);
		}
	...
	/**
	 * @param a
	 *        A document with state <tt>full</tt> which to compare to <tt>b</tt>
	 * @param b
	 *        A document with state <tt>full</tt> which to compare to <tt>a</tt>
	 ...
	 *
	 * @param a
	 *        A <tt>ConferenceInfoDocument.User</tt> to compare
	 * @param b
	 *        A <tt>ConferenceInfoDocument.User</tt> to compare
	 ...
	 *
	 * @param a
	 *        A <tt>ConferenceInfoDocument.Endpoint</tt> to compare
	 * @param b
	 *        A <tt>ConferenceInfoDocument.Endpoint</tt> to compare
	 * @return <tt>false</tt> if <tt>a</tt> and <tt>b</tt> are found to be different in a way that
	 *         is significant for our needs, <tt>true</tt> otherwise.
	...
	/**
	 * @param a
	 *        A <tt>String</tt> to compare to <tt>b</tt>
	 * @param b
	 *        A <tt>String</tt> to compare to <tt>a</tt>
