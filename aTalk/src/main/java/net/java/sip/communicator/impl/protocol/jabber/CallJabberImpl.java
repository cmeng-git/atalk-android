...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>CallJabberImpl</tt> class and its instances for logging
	 * output.
	 */
	private static final Logger logger = Logger.getLogger(CallJabberImpl.class);

	/**
	 * The Jitsi Videobridge conference which the local peer represented by this instance is a
	 * focus of.
	 */
	...
	private CallPeerMediaHandlerJabberImpl mediaHandler;

	/**
	 * Initializes a new <tt>CallJabberImpl</tt> instance.
	 *
	 * @param parentOpSet
	 * 		the {@link OperationSetBasicTelephonyJabberImpl} instance in the context of which this
	 ...
	 * <tt>MediaStream</tt> of a specific <tt>MediaType</tt> upon request from a specific
	 * <tt>CallPeer</tt>.
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> which requests the closing of the specified
	 * 		<tt>colibriStreamConnector</tt>
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the <tt>MediaStream</tt> with which the specified
	 * 		<tt>colibriStreamConnector</tt> is associated
	 * @param colibriStreamConnector
	 * 		the <tt>ColibriStreamConnector</tt> to close on behalf of the specified <tt>peer</tt>
	 */
	 ...
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> which is to use the allocated colibri (conference) channels
	 * @param contentMap
	 * 		the local and remote <tt>ContentPacketExtension</tt>s which specify the
			...
			throws OperationFailedException
	{
		...
		Jid jvb = (colibri == null)
				? getJitsiVideobridge() : colibri.getFrom();
		...
		for (Map.Entry<ContentPacketExtension, ContentPacketExtension> e : contentMap.entrySet()) {
			...
			// DTLS-SRTP
			setDtlsEncryptionOnChannel(mediaType, localContent, remoteContent, peer,
		...
		StanzaCollector packetCollector = connection.createStanzaCollector(new StanzaIdFilter(
				conferenceRequest.getStanzaId()));
		...
		try {
			response = packetCollector.nextResult(SmackConfiguration
		...
		if (response == null) {
			logger.error("Failed to allocate colibri channels: response is null. Maybe the "
					+ "response timed out.");
			return null;
		}
			...
			logger.error("Failed to allocate colibri channels: response is not a colibri "
					+ "conference");
			return null;
		}
	...
	/**
	 * Initializes a <tt>ColibriStreamConnector</tt> on behalf of a specific <tt>CallPeer</tt>
	 * to be used in association with a specific <tt>ColibriConferenceIQ.Channel</tt> of a specific
	 * <tt>MediaType</tt>.
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> which requests the initialization of a
	 * 		<tt>ColibriStreamConnector</tt>
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the stream which is to use the initialized
	 * 		<tt>ColibriStreamConnector</tt> for RTP and RTCP traffic
	 * @param channel
	 * 		the <tt>ColibriConferenceIQ.Channel</tt> to which RTP and RTCP traffic is to be sent
	 * 		and from which such traffic is to be received via the initialized
	 * 		<tt>ColibriStreamConnector</tt>
	 * @param factory
	 * 		a <tt>StreamConnectorFactory</tt> implementation which is to allocate the sockets to
		...
		synchronized (colibriStreamConnectors) {
			...
			if (colibriStreamConnector == null) {
				...
				if (streamConnector != null) {
					...
					colibriStreamConnectors.set(index,
	 ...
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> which uses the specified (colibri) conference channels to be
	 * 		expired
	 * @param conference
	 * 		a <tt>ColibriConferenceIQ</tt> which specifies the (colibri) conference channels to be
	 * 		expired
	 */
	 ...
	 * <tt>CallJabberImpl</tt>, in order to request the the direction of the <tt>channel</tt>
	 * with ID <tt>channelID</tt> be set to <tt>direction</tt>
	 *
	 * @param channelID
	 * 		the ID of the <tt>channel</tt> for which to set the direction.
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the channel (we can deduce this by searching the
	 * 		<tt>ColibriConferenceIQ</tt>, but it's more convenient to have it)
	 * @param direction
	 * 		the <tt>MediaDirection</tt> to set.
	 */
	public void setChannelDirection(String channelID, MediaType mediaType,
	...
	/**
	 * Creates a <tt>CallPeerJabberImpl</tt> from <tt>calleeJID</tt> and sends them
	 * <tt>session-initiate</tt> IQ request.
	 *
	 * @param calleeJID
	 * 		the party that we would like to invite to this call.
	 * @param discoverInfo
	 * 		any discovery information that we have for the jid we are trying to reach and that we
	 * 		are passing in order to avoid having to ask for it again.
	 * @param sessionInitiateExtensions
	 * 		a collection of additional and optional <tt>ExtensionElement</tt>s to be added to the
	 * 		<tt>session-initiate</tt> {@link JingleIQ} which is to init this
	 * 		<tt>CallJabberImpl</tt>
	 * @param supportedTransports
	 * 		the XML namespaces of the jingle transports to use.
			...
			throws OperationFailedException
	{
		...
		// If this was the first peer we added in this call, then the call is new and we need to
		// notify everyone of its creation.
	 ...
	 *
	 * @throws OperationFailedException
	 * 		if a problem occurred during message generation or there was a network problem
	 */
	@Override
			...
			throws OperationFailedException
	{
		...
		for (CallPeerJabberImpl peer : getCallPeerList())
			// cmeng (2016/09/14): Never send 'sendModifyVideoContent' before it is connected
			// => Smack Exception
	 ...
	 * the <tt>ColibriConferenceIQ</tt> is processed by the associated
	 ...
	 *
	 * @param jingleIQ
	 * 		the {@link JingleIQ} that created the session.
	 * @return the newly created {@link CallPeerJabberImpl} (the one that sent the INVITE).
	 */
	public CallPeerJabberImpl processSessionInitiate(JingleIQ jingleIQ)
	{
		...
		/*
		 * We've already sent ack to the specified session-initiate so if it has been sent as part
		...
		try {
			...
			if (transfer != null) {
				...
				if (sid != null) {
					ProtocolProviderServiceJabberImpl protocolProvider = getProtocolProvider();
					basicTelephony = (OperationSetBasicTelephonyJabberImpl) protocolProvider
							.getOperationSet(OperationSetBasicTelephony.class);
					CallJabberImpl attendantCall
							= basicTelephony.getActiveCallsRepository().findSID(sid);

					if (attendantCall != null) {
						attendant = attendantCall.getPeer(sid);
						if ((attendant != null)
								&& basicTelephony.getFullCalleeURI(attendant.getPeerJid())
		...
		CoinPacketExtension coin = (CoinPacketExtension) jingleIQ.getExtension(
	 ...
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> associated with the method invocation
	 * @param channel
	 * 		the <tt>ColibriConferenceIQ.Channel</tt> which represents the state of the remote
	 * 		DTLS-SRTP endpoint
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the media to be transmitted over the DTLS-SRTP session
	 */
			...
			ColibriConferenceIQ.Channel channel, MediaType mediaType)
	{
		...
		if (dtlsControl != null) {
			dtlsControl.setSetup(peer.isInitiator() ? DtlsControl.Setup.ACTIVE
	 ...
	 * 		the <tt>MediaType</tt> of the media to be transmitted over the DTLS-SRTP session
	 * @param localContent
	 * 		the <tt>ContentPacketExtension</tt> of the local peer in the negotiation between the
	 ...
	 * @param remoteContent
	 * 		the <tt>ContentPacketExtension</tt>, if any, of the remote peer in the negotiation
	 ...
	 * 		<tt>localContent</tt> represents an answer from the local peer to an offer from the
	 * 		remote peer
	 * @param peer
	 * 		the <tt>CallPeer</tt> which represents the remote peer and which is associated with
	 * 		the specified <tt>channel</tt>
	 * @param channel
	 * 		the <tt>ColibriConferenceIQ.Channel</tt> which represents the state of the remote
	 * 		DTLS-SRTP endpoint.
			...
			if (remoteTransport != null) {
				...
				if (!remoteFingerprints.isEmpty()) {
					...
					if (localTransport != null) {
						List<DtlsFingerprintPacketExtension> localFingerprints = localTransport
								.getChildExtensionsOfType(DtlsFingerprintPacketExtension.class);
						...
						if (localFingerprints.isEmpty()) {
							for (DtlsFingerprintPacketExtension remoteFingerprint
								...
								localFingerprint.setFingerprint(remoteFingerprint
										.getFingerprint());
	 ...
	 *
	 * @param jvb
	 * 		the address/JID of the Jitsi Videobridge
	 * @param peer
	 * 		the <tt>CallPeer</tt> associated with the method invocation
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the media to be transmitted over the DTLS-SRTP session
			...
			MediaType mediaType, ColibriConferenceIQ.Channel channel)
	{
		...
		if (accountID.getAccountPropertyBoolean(ProtocolProviderFactory.DEFAULT_ENCRYPTION, true)
			...
			DtlsControl dtlsControl = (DtlsControl) mediaHandler.getSrtpControls().getOrCreate(
					mediaType, SrtpControlType.DTLS_SRTP);
	 ...
	 *
	 * @param dtlsControl
	 * 		the <tt>DtlsControl</tt> the properties of which are to be set on the specified
	 * 		<tt>localTransport</tt>
	 * @param localTransport
	 * 		the <tt>IceUdpTransportPacketExtension</tt> on which the properties of the specified
	 * 		<tt>dtlsControl</tt> are to be set
	 */
	static void setDtlsEncryptionOnTransport(DtlsControl dtlsControl,
			IceUdpTransportPacketExtension localTransport)
	{
	...
	private void setTransportOnChannel(CallPeerJabberImpl peer, String media,
			ColibriConferenceIQ.Channel channel)
			throws OperationFailedException
	{
		ExtensionElement transport
				= peer.getMediaHandler().getTransportManager().createTransport(media);
			...
			throws OperationFailedException
	{
		if (remoteContent != null) {
			IceUdpTransportPacketExtension transport
					= remoteContent.getFirstChildOfType(IceUdpTransportPacketExtension.class);
			channel.setTransport(TransportManagerJabberImpl.cloneTransportAndCandidates
					(transport));
		}
	}
	 ...
	 *
	 * @param channel
	 * 		the <tt>ColibriConferenceIQ.Channel</tt> to ensure the <tt>transport</tt> on
	 * @param peer
	 * 		the <tt>CallPeerJabberImpl</tt> which is associated with the specified
			...
			ColibriConferenceIQ.Channel channel, CallPeerJabberImpl peer)
	{
		...
		if (transport == null) {
			ExtensionElement pe = peer.getMediaHandler().getTransportManager()
	 ...
	 * <p>
	 * Implements
	 * {@link net.java.sip.communicator.service.protocol.event.DTMFListener
	 * #toneReceived(net.java.sip.communicator.service.protocol.event.DTMFReceivedEvent)}
	 * <p>
	 * Forwards DTMF events to the <tt>IncomingDTMF</tt> operation set, setting this <tt>Call</tt>
	 * as the source.
	 */
	@Override
	public void toneReceived(DTMFReceivedEvent evt)
	{
		OperationSetIncomingDTMF opSet = getProtocolProvider().getOperationSet(
