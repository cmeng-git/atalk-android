 ...
 *
 * @param <U> the peer extension class like for example <tt>CallPeerSipImpl</tt> or
...
public abstract class TransportManager<U extends MediaAwareCallPeer<?, ?, ?>> {
	/**
	 * The <tt>Logger</tt> used by the <tt>TransportManager</tt> class and its instances for logging
	 * output.
	 */
	 ...
	 * <p>
	 * Potentially initialized by {@link #initializePortNumbers()} if the necessary properties are
	 * set.
	 * </p>
	 */
	...
	protected StreamConnector createStreamConnector(MediaType mediaType)
		...
		try {
			rtpSocket = nam.createDatagramSocket(localHostForPeer, portTracker.getPort(),
		...
		try {
			rtcpSocket = nam.createDatagramSocket(localHostForPeer, portTracker.getPort(),
					portTracker.getMinPort(), portTracker.getMaxPort());
		} catch (Exception exc) {
			throw new OperationFailedException(
					"Failed to allocate the network ports necessary for the call.",
	...
	protected synchronized static void initializePortNumbers() {
		...
		if (minPort != null) {
			maxPort = cfg
					.getString(OperationSetBasicTelephony.MAX_DATA_CHANNEL_PORT_NUMBER_PROPERTY_NAME);
			if (maxPort != null) {
				// Try the specified range; otherwise, leave the tracker to null.
	 ...
	 *
	 * @return an <tt>InetAddress</tt> that we use in one of the <tt>StreamConnector</tt>s in this
	 * class.
	 */
	 ...
	 *
	 * @param target <tt>MediaStreamTarget</tt>
	 * @param type the {@link MediaType} of the connector we'd like to send the hole punching packet
	 * through.
			...
			synchronized (connector) {
				...
				for (int i = 0; i < packetCount; i++) {
					...
					if ((socket = connector.getDataSocket()) != null) {
						...
						socket.send(new DatagramPacket(buf, buf.length, dataAddress.getAddress(),
								dataAddress.getPort()));
					}
					...
					if ((socket = connector.getControlSocket()) != null) {
						...
						socket.send(new DatagramPacket(buf, buf.length,
								controlAddress.getAddress(), controlAddress.getPort()));
					}
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Gets the SIP traffic class associated with a specific <tt>MediaType</tt> from the
	 * configuration.
	 *
	...
	/**
	 * Returns the port tracker that we are supposed to use when binding ports for the specified
	 * {@link MediaType}.
	 *
	 ...
	 * @return the port tracker that we are supposed to use when binding ports for the specified
	 * {@link MediaType}.
	 */
	 ...
	 * {@link MediaType} with the specified name or the default tracker in case the name
	 * doesn't ring a bell.
	 */
	protected static PortTracker getPortTracker(String mediaTypeStr) {
		try {
			...
			logger.info("Returning default port tracker for unrecognized media type: "
					+ mediaTypeStr);
