...
public class ChatRoomProviderWrapperImpl implements ChatRoomProviderWrapper
{
	 ...
	 * 		protocol provider, corresponding to the multi user chat account.
	 */
	 ...
	 *
	 * @param chatRoom
	 * 		the chat room we're looking for.
	 ...
	 *
	 * @param chatRoomID
	 * 		the id of the chat room we're looking for.
	 * @return the chat room wrapper contained in this provider that corresponds to the given chat
	 * room id.
	 */
				...
				public void run()
				{
					...
					if (chatRoom != null) {
						...
						if (chatRoomWrapper.isAutoJoin()) {
							...
							if (nickName != null) {
								MUCActivator.getMUCService().joinChatRoom(chatRoom, nickName,
										null);
							}
							else {
						...
						if (chatRoomWrapper.isAutoJoin()) {
							...
							if (nickName != null) {
								MUCActivator.getMUCService()
										.joinChatRoom(roomWrapper.getChatRoom(), nickName, null);
							}
							else {
