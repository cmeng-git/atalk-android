...
import org.atalk.android.util.java.awt.AlphaComposite;
...
import org.atalk.android.util.javax.swing.ButtonGroup;
...
import org.atalk.android.util.javax.swing.JRadioButtonMenuItem;
...
public class OTRv3OutgoingSessionSwitcher extends SIPCommMenuBar
		implements PluginComponent, ActionListener, ScOtrEngineListener, ScOtrKeyManagerListener {
			...
			switch (session.getSessionStatus(session.getReceiverInstanceTag())) {
				case ENCRYPTED:
					...
					imageIcon = OtrActivator.scOtrKeyManager.isVerified(otrContact.contact, fingerprint)
							? verifiedLockedPadlockImage : unverifiedLockedPadlockImage;
					break;
				case FINISHED:
	...
	private static class SelectorMenu extends SIPCommMenu {
		...
		public void fadeAnimation() {
			// alphaChanger.stop(); - cmeng
			alpha = 0.85f;
			SelectorMenu.this.repaint();
			// alphaChanger.start();  - cmeng
		}
	}
}
