...
{
	/**
	 * Account property to enable per account rejecting calls if the account presence is in DND or
	 * OnThePhone status.
	 */
	 ...
	 * of online accounts to &quot;On the phone&quot; when at least one <tt>Call</tt> is in
	 * progress.
	 */
	private static final String PNAME_ON_THE_PHONE_STATUS_ENABLED
			= "protocol.OnThePhoneStatusPolicy.enabled";
	...
	private static final String PNAME_REJECT_IN_CALL_ON_DND
			= "protocol." + ACCOUNT_PROPERTY_REJECT_IN_CALL_ON_DND;
	...
	/**
	 * The listener utilized by this policy to discover new <tt>Call</tt> and track their
	 * in-progress state.
	 */
	private final SingleCallInProgressPolicyListener listener = new SingleCallInProgressPolicyListener();

	/**
	 * The implementation of the policy to have the presence statuses of online accounts (i.e.
	 * registered <tt>ProtocolProviderService</tt>s) set to &quot;On the phone&quot; when at least
	 * one <tt>Call</tt> is in progress.
	 */
	 ...
	 *
	 * @param bundleContext
	 *        the <tt>BundleContext</tt> to the <tt>Call<tt>s of
	...
	/**
	 * Registers a specific <tt>Call</tt> with this policy in order to have the rules of the latter
	 * apply to the former.
	 *
	 * @param call
	 *        the <tt>Call</tt> to register with this policy in order to have the rules of the
	 ...
	 *
	 * @param telephony
	 *        the <tt>OperationSetBasicTelephony</tt> to register with this policy in order to have
	 *        the rules of the latter apply to the <tt>Call</tt>s created by the former
	 */
	 ...
	 *
	 * @param ev
	 *        a <tt>CallChangeEvent</tt> value which describes the <tt>Call</tt> and the change in
	 *        its state
	 */
	 ...
	 *
	 * @param type
	 *        one of {@link CallEvent#CALL_ENDED}, {@link CallEvent#CALL_INITIATED} and
	 *        {@link CallEvent#CALL_RECEIVED} which describes the type of the event to be handled
	 * @param ev
	 *        a <tt>CallEvent</tt> value which describes the change and the <tt>Call</tt> associated
	 *        with it
	 */
	 ...
	 *
	 * @param ev
	 *        a <tt>CallEvent</tt> which describes the received incoming <tt>Call</tt>
	 */
	private void incomingCallReceived(CallEvent ev)
	{
		...
		if (CallState.CALL_INITIALIZATION.equals(call.getCallState())) {
			...
			if (config.getBoolean(PNAME_REJECT_IN_CALL_ON_DND, false)
				...
				// if our provider has no presence op set, lets search for connected provider which
				// will have
				if (presence == null) {
					// There is no presence OpSet. Let's check the connected CUSAX provider if
					// available
					OperationSetCusaxUtils cusaxOpSet = provider
						.getOperationSet(OperationSetCusaxUtils.class);

					if (cusaxOpSet != null) {
						ProtocolProviderService linkedCusaxProvider = cusaxOpSet
							.getLinkedCusaxProvider();

						if (linkedCusaxProvider != null) {
							// we found the provider, let's take its presence
							// opset
							presence = linkedCusaxProvider
								.getOperationSet(OperationSetPresence.class);
						}
				...
				if (presence != null) {
					...
					// between AVAILABLE and EXTENDED AWAY (>20, <= 31) are the busy statuses as DND
					// and On the phone
	 ...
	 *
	 * @param call
	 *        the <tt>Call</tt> the <tt>CallPeer</tt>s of which are to be put on hold
	 */
	 ...
	 *
	 * @param call
	 *        the <tt>Call</tt> to unregister from this policy in order to have the rules of the
	 ...
	 *
	 * @param telephony
	 *        the <tt>OperationSetBasicTelephony</tt> to unregister from this policy in order to
	 ...
	 * order to apply or unapply the rules of this policy to the <tt>Call</tt>s originating from
	 * them.
	 *
	 * @param ev
	 *        a <tt>ServiceEvent</tt> value which described a change in an OSGi service and which is
	...
	private void serviceChanged(ServiceEvent ev)
	{
		...
		if (service instanceof ProtocolProviderService) {
			...
			OperationSetBasicTelephony<?> telephony = ((ProtocolProviderService) service)
	 ...
	 * <tt>ProtocolProviderService</tt>s) set to &quot;On the phone&quot; when at least one
	 * <tt>Call</tt> is in progress.
	 *
	 * @author Lyubomir Marinov
	 */
	private class OnThePhoneStatusPolicy
	{
		 ...
		 */
		private final Map<ProtocolProviderService, PresenceStatus> presenceStatuses = Collections
			.synchronizedMap(new WeakHashMap<ProtocolProviderService, PresenceStatus>());

		/**
		 * Notifies this instance that the <tt>callState</tt> of a specific <tt>Call</tt> has
		 * changed.
		 *
		 * @param ev
		 *        a <tt>CallChangeEvent</tt> which represents the details of the notification such
		...
		public void callStateChanged(CallChangeEvent ev)
		{
			...
			if ((CallState.CALL_INITIALIZATION.equals(oldCallState) && CallState.CALL_IN_PROGRESS
				.equals(newCallState))
				|| (CallState.CALL_IN_PROGRESS.equals(oldCallState) && CallState.CALL_ENDED
					.equals(newCallState))) {
				run();
			}
			else {
		 ...
		 * supported by a specific <tt>OperationSetPresence</tt> which represents &quot;On the
		 * phone&quot;.
		 *
		 * @param presence
		 *        the <tt>OperationSetPresence</tt> which represents the set of supported
		 ...
		 * @param type
		 *        one of {@link CallEvent#CALL_ENDED}, {@link CallEvent#CALL_INITIATED} and
		 *        {@link CallEvent#CALL_RECEIVED} which describes the type of the event to be
		 *        handled
		 * @param ev
		 *        a <tt>CallEvent</tt> value which describes the change and the <tt>Call</tt>
		 *        associated with it
		 */
		 ...
		 *         in progress i.e. if the local user is currently on the phone; otherwise,
		 *         <tt>false</tt>
		 */
		 ...
		 * specific <tt>OperationSetPresence</tt> with a specific <tt>PresenceStatus</tt> and
		 * catches any exceptions.
		 *
		 * @param presence
		 *        the <tt>OperationSetPresence</tt> on which the method is to be invoked
		 * @param presenceStatus
		 *        the <tt>PresenceStatus</tt> to provide as the respective method argument value
		 */
		private void publishPresenceStatus(OperationSetPresence presence,
		...
		private PresenceStatus rememberPresenceStatus(ProtocolProviderService pps,
			PresenceStatus presenceStatus)
		{
		...
		/**
		 * Finds the first <tt>PresenceStatus</tt> among the set of <tt>PresenceStatus</tt>es
		 * supported by a specific <tt>OperationSetPresence</tt> which represents &quot;In
		 * meeting&quot;.
		 *
		 * @param presence
		 *        the <tt>OperationSetPresence</tt> which represents the set of supported
		...
		private void run()
		{
			...
			if ((ppsRefs == null) || (ppsRefs.length == 0)) {
				...
				for (ServiceReference<?> ppsRef : ppsRefs) {
					ProtocolProviderService pps = (ProtocolProviderService) bundleContext
						.getService(ppsRef);
					...
					if (presence == null) {
						...
						if (onThePhonePresenceStatus == null) {
							...
							/*
							 * If do not know how to define "On the phone" for an
							 * OperationSetPresence, then we'd better not mess with it in the first
							 * place.
							 */
							forgetPresenceStatus(pps);
						}
							...
							if (presenceStatus == null) {
								...
								if (presenceStatus.equals(findInMeetingPresenceStatus(presence))
									&& calendar != null) {
									Map<ProtocolProviderService, PresenceStatus> statuses = calendar
								...
								else if (onThePhonePresenceStatus.equals(presence
									.getPresenceStatus())) {
						...
						/*
						 * Offline accounts do not get their PresenceStatus modified for the
						 * purposes of "On the phone".
						 */
						forgetPresenceStatus(pps);
					}
				}
	...
	{
		/**
		 * Stops tracking the state of a specific <tt>Call</tt> and no longer tries to put it on
		 * hold when it ends.
		 *
		...
		/**
		 * Does nothing because removing <tt>CallPeer<tt>s to <tt>Call</tt>s isn't related to the
		 * policy to put existing calls on hold when a new call becomes in-progress and just
		 * implements <tt>CallChangeListener</tt>.
		 *
		 ...
		 *
		 * @param ev
		 *        the <tt>CallChangeEvent</tt> that we are to deliver.
		 *
		 ...
		 * account when putting existing calls on hold upon a new call entering its in-progress
		 * state.
		 *
		 * @param ev
		 *        the <tt>ServiceEvent</tt> event describing a change in the state of a service
