 ...
 */
public class OperationSetDesktopStreamingJabberImpl extends OperationSetVideoTelephonyJabberImpl
	implements OperationSetDesktopStreaming
{
	...
	/**
	 * Initializes a new <tt>OperationSetDesktopStreamingJabberImpl</tt> instance which builds upon
	 ...
	 *         if <tt>callee</tt> is not a valid Jabber address string.
	 */
	 ...
	 * @param mediaDevice
	 *        the media device to use for the desktop streaming. If the device is null, the default
	 *        device is used.
	 * @param allowed
		...
		throws OperationFailedException
	{
		...
		size = (mediaDeviceFormat == null) ? null : ((VideoMediaFormat) mediaDeviceFormat)
			.getSize();
	...
	/**
	 * Implements OperationSetVideoTelephony#isLocalVideoAllowed(Call). Modifies the local media
	 * setup to reflect the requested setting for the streaming of the local video.
	 *
	 * @param call
	...
	public boolean isPartialStreaming(Call call)
	{
		...
		return (device == null) ? false : JabberActivator.getMediaService().isPartialStreaming(
			device);
	}
	...
	public void movePartialDesktopStreaming(Call call, int x, int y)
	{
		...
		VideoMediaStream videoStream = (VideoMediaStream) callPeerImpl.getMediaHandler().getStream(
			MediaType.VIDEO);
