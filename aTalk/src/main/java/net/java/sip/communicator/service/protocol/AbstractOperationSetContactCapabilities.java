...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractOperationSetContactCapabilities</tt> class and
	 ...
	 * 		the <tt>ProtocolProviderService</tt> implementation which will provide the new
	 * 		instance
	 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>ContactCapabilitiesListener</tt> which is to be notified about changes in the
	 ...
	 *
	 * @param sourceContact
	 * 		the <tt>Contact</tt> which is the source/cause of the event to be fired
	 * @param eventID
	 * 		the ID of the event to be fired which indicates the specifics of the change of the
	 * 		list of <tt>OperationSet</tt> capabilities of the specified <tt>sourceContact</tt> and
	 * 		the details of the event
	 * @param opSets
	 * 		the new set of operation sets for the given source contact
	 */
				...
				switch (eventID) {
					...
					default:
						if (logger.isDebugEnabled()) {
							logger.debug("Cannot fire ContactCapabilitiesEvent with"
									+ " unsupported eventID: " + eventID);
						}
	 ...
	 *
	 * @param <U>
	 * 		the type extending <tt>OperationSet</tt> for which the specified <tt>contact</tt> is
	 * 		to be checked whether it possesses it as a capability
	 * @param contact
	 * 		the <tt>Contact</tt> for which the <tt>opsetClass</tt> capability is to be queried
	 * @param opsetClass
	 * 		the <tt>OperationSet</tt> <tt>Class</tt> for which the specified <tt>contact</tt> is
	 * 		to be checked whether it possesses it as a capability
	 ...
	 * 		to be checked whether it possesses it as a capability
	 * @param online
	 * 		<tt>true</tt> if <tt>contact</tt> is online; otherwise, <tt>false</tt>
	 * @return the <tt>OperationSet</tt> corresponding to the specified <tt>opsetClass</tt>
	 * which is considered by the associated protocol provider to be possessed as a capability by
	 * the specified <tt>contact</tt>; otherwise, <tt>null</tt>
	 * @see OperationSetContactCapabilities#getOperationSet(Contact, Class)
	 ...
	 *
	 * @param contact
	 * 		the <tt>Contact</tt> for which the supported <tt>OperationSet</tt> capabilities are to
	 * 		be retrieved
	 ...
	 *
	 * @param contact
	 * 		the <tt>Contact</tt> which is to be determines whether it is online
		...
		if (opsetPresence == null) {
			...
			if (presenceStatus == null) {
				if ((exception != null) && logger.isDebugEnabled()) {
					logger.debug("Failed to query PresenceStatus of Contact " + contact,
							exception);
				}
				/*
	 ...
	 *
	 * @param listener
	 * 		the <tt>ContactCapabilitiesListener</tt> which is to no longer be notified about
