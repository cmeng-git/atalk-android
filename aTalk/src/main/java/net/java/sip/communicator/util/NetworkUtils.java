...
public class NetworkUtils
{
	 ...
	 * @return true if the address contained by <tt>address</tt> is an IPv4 address and false
	 * otherwise.
	 */
	 ...
	 * @return true if the address contained by <tt>address</tt> is an IPv6 address and false
	 * otherwise.
	 */
	 ...
	 * 		if <tt>domain</tt> is not a valid domain name.
	 * @throws DnssecException
	 * 		when a DNSSEC validation failure occurred.
	 */
	 ...
	 *
	 * @param domain
	 * 		the name of the domain we'd like to resolve.
	 * @param service
	 * 		the service that we are trying to get a record for.
	 * @param proto
	 * 		the protocol that we'd like <tt>service</tt> on.
	...
	{
		SRVRecord[] records
				= getSRVRecords("_" + service + "._" + proto + "." + domain, useDNSCache);
	 ...
	 *
	 * @param protocol
	 * 		The protocol name: "TLS", "TCP" or "UDP".
	 ...
	 *
	 * @param hostAddress
	 * 		the <tt>String</tt> representation of the address
	 ...
	 * 		if any of the <tt>InetAddress</tt> methods
			...
			if (records != null) {
				for (Record r : records) {
					try {
						...
						// queries
						addresses.add(new InetSocketAddress(InetAddress.getByAddress(
								domain, v6lookup ? ((AAAARecord) r).getAddress().getAddress()
			...
			try {
				return new InetSocketAddress(
						InetAddress.getByAddress(domain,
								((ARecord) records[0]).getAddress().getAddress()), port);
			}
			catch (UnknownHostException e) {
				return null;
			}
	 ...
	 *
	 * @param domain
	 * 		the domain we will be resolving
	 * @param type
	 * 		the type of the record we will be trying to obtain.
			...
			throws TextParseException
	{
		...
		if (UtilActivator.getConfigurationService().getBoolean(PNAME_DNS_ALWAYS_ABSOLUTE,
	 ...
	 * 		The first DNS name
	 * @param dns2
	 * 		The DNS name that is compared against dns1
	 ...
	 * ordering.
	 * @throws ParseException
	 * 		if the dns1 or dns2 is not a DNS Name
	 */
	...
	private static void sortSrvRecord(SRVRecord[] srvRecords)
	{
		...
		// Sort the SRV RRs by weight (larger weight has a proportionately higher probability of
		// being selected).
	 ...
	 *
	 * @param srvRecords
	 * 		The list of SRV records already sorted by priority.
	 */
	 ...
	 * 		The list of SRV records already sorted by priority.
	 * @param startIndex
	 * 		The first index (included) for the current priority.
	 * @param endIndex
	 * 		The last index (excluded) for the current priority.
	 */
	 ...
	 * 		The list of SRV records already sorted by priority.
	 * @param startIndex
	 * 		The first unsorted index (included) for the current
	 * 		priority.
	 * @param endIndex
	 ...
	 * 		item to move.
	 */
	private static void moveSelectedSRVRecord(SRVRecord[] srvRecords, int startIndex, int
			endIndex, int selectedWeight)
	{
