...
public class AccountRegistrationImpl extends AccountRegistrationWizard
{
     ...
     * 		additional account parameters for setting up new account/modify e.g. server and port
	 ...
	 * 		the ProtocolProviderFactory which will create the account
	 * @param userName
	 * 		the user identifier
	 * @param password
	 * 		the password
			...
			throws OperationFailedException
	{
		...
		if (accountProperties.get(ProtocolProviderFactory.SERVER_ADDRESS) == null) {
			...
			if (!StringUtils.isNullOrEmpty(serverAddress))
				accountProperties.put(ProtocolProviderFactory.SERVER_ADDRESS,
						XmppStringUtils.parseDomain(userName));
			else
		...
		try {
			...
			protocolProvider = (ProtocolProviderService) JabberAccountRegistrationActivator
			...
			logger.warn(exc.getMessage());
			throw new OperationFailedException("Failed to add account.",
