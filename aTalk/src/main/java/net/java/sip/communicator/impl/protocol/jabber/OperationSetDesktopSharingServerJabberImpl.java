 ...
 */
public class OperationSetDesktopSharingServerJabberImpl extends
	OperationSetDesktopStreamingJabberImpl implements OperationSetDesktopSharingServer,
	RegistrationStateChangeListener, StanzaListener, StanzaFilter
{
	/**
	 * Our class logger.
	 */
	...
	/**
	 * HID service that will regenerates keyboard and mouse events received in Jabber messages.
	 */
	private HIDService hidService = null;

	/**
	 * List of callPeers for the desktop sharing session with remote control granted.
	 */
	private List<String> callPeers = new ArrayList<String>();

	/**
	 * Initializes a new <tt>OperationSetDesktopSharingJabberImpl</tt> instance which builds upon
	 * the telephony-related functionality of a specific
	 * <tt>OperationSetBasicTelephonyJabberImpl</tt>.
	 *
	 * @param basicTelephony
	 *        the <tt>OperationSetBasicTelephonyJabberImpl</tt> the new extension should build upon
	 */
	 ...
	 * @param device
	 *        video device that will be used to stream desktop.
		...
		throws OperationFailedException, ParseException
	{
		...
		Dimension size = (((VideoMediaFormat) call.getDefaultDevice(MediaType.VIDEO).getFormat())
			.getSize());
		Point origin = getOriginForMediaDevice(device);
		return call;
	}
	...
	public Call createVideoCall(Contact callee, MediaDevice device)
		throws OperationFailedException
	{
		MediaAwareCall<?, ?, ?> call = (MediaAwareCall<?, ?, ?>) super.createVideoCall(callee,
			device);
	 ...
	 *        Contact address
	 * @param videoDevice
	 *        specific video device to use (null to use default device)
	 * @return true if contact support Jingle video, false otherwise
	 *
	 * @throws OperationFailedException
	 *         with the corresponding code if we fail to create the video call.
		...
		throws OperationFailedException
	{
		...
		if (calleeAddress.indexOf('/') > 0) {
			try {
				fullCalleeURI = JidCreate.fullFrom(calleeAddress);
			}
			...
			try {
				Roster roster = Roster.getInstanceFor(parentProvider.getConnection());
				fullCalleeURI = roster.getPresence(JidCreate.bareFrom(calleeAddress))
						.getFrom();
			}
			catch (XmppStringprepException e) {
				e.printStackTrace();
		...
		try {
			...
			if (di.containsFeature(InputEvtIQ.NAMESPACE_CLIENT)) {
				...
				// TODO fail or not?
				/*
				 * throw new OperationFailedException( "Failed to create a true desktop sharing.\n"
	...
	/**
	 * Enable desktop remote control. Local desktop can now regenerates keyboard and mouse events
	 * received from peer.
	 *
	 * @param callPeer
	 *        call peer that will take control on local computer
	 */
	public void enableRemoteControl(CallPeer callPeer)
	{
		callPeer.addCallPeerListener(callPeerListener);

		size = (((VideoMediaFormat) ((CallJabberImpl) callPeer.getCall()).getDefaultDevice(
	...
	/**
	 * Disable desktop remote control. Local desktop stops regenerate keyboard and mouse events
	 * received from peer.
	 *
	 * @param callPeer
	 *        call peer that will stop controlling on local computer
	 */
	 ...
	 * RegistrationStateChangeListener for setting up (or down) our <tt>InputEvtManager</tt> when an
	...
	{
		OperationSetDesktopSharingServerJabberImpl.registrationStateChanged(evt, this, this,
	 ...
	 *        the packet listener for InputEvtIQ of this connection.
	 *        (OperationSetDesktopSharingServerJabberImpl or
	 ...
	 *        The XMPP connection.
	 */
	public static void registrationStateChanged(RegistrationStateChangeEvent evt,
		StanzaListener packetListener, StanzaFilter packetFilter, XMPPTCPConnection connection)
	{
		if (connection == null)
			return;
	...
	/**
	 * Handles incoming inputevt packets and passes them to the corresponding method based on their
	 * action.
	 *
	 * @param packet
	 ...
	 *
	 * @param event
	 *        <tt>ComponentEvent</tt> that will be regenerated on local computer
	 */
	 ...
	 *
	 * @param event
	 *        <tt>KeyboardEvent</tt> that will be regenerated on local computer
	 */
	...
	/**
	 * Sends IQ InputEvent START or STOP in order to enable/disable the remote peer to remotely
	 * control our PC.
	 *
	 * @param callPeer
	 *        call peer that will stop controlling on local computer.
	 * @param enables
	 *        True to enable remote peer to gain remote control on our PC. False Otherwise.
	 */
		...
		synchronized (callPeers) {
			if (callPeers.contains(callPeer.getAddress()) != enables) {
				if (isRemoteControlAvailable(callPeer)) {
					...
					XMPPTCPConnection connection = parentProvider.getConnection();
					StanzaCollector collector = connection
						.createStanzaCollector(new StanzaIdFilter(inputIQ.getStanzaId()));
					...
					Stanza p = null;
					try {
						p = collector.nextResult(SmackConfiguration
	 ...
	 */
	private void receivedResponseToIqStart(CallPeer callPeer, Stanza p)
	{
		...
		if (p != null && ((IQ) p).getType() == IQ.Type.result) {
			...
			String packetString = (p == null) ? "\n\tPacket is null (IQ request timeout)."
				: "\n\tPacket: " + p.toXML();
			logger.info("Remote peer has not received/accepted the START "
				+ "action to grant the remote desktop control." + packetString);
		}
	}
	 ...
	 *        call peer that will stop controlling on local computer.
	 * @param p
	 *        The response to our IQ InputEvent STOP sent.
	 */
	private void receivedResponseToIqStop(CallPeer callPeer, Stanza p)
	{
		if (p == null || ((IQ) p).getType() == IQ.Type.error) {
			String packetString = (p == null) ? "\n\tPacket is null (IQ request timeout)."
				: "\n\tPacket: " + p.toXML();
			logger.info("Remote peer has not received/accepted the STOP "
				+ "action to grant the remote desktop control." + packetString);
		}
	...
	/**
	 * Tells if the peer provided can be remotely controlled by this peer: - The server is able to
	 * grant/revoke remote access to its desktop. - The client (the call peer) is able to send mouse
	 * and keyboard events.
	 *
	 * @param callPeer
	 *        The call peer which may remotely control the shared desktop.
	 *
	 * @return True if the server and the client are able to respectively grant remote access and
	 *         send mouse/keyboard events. False, if one of the call participant (server or client)
	 *         is not able to deal with remote controls.
	 */
	public boolean isRemoteControlAvailable(CallPeer callPeer)
	{
		DiscoverInfo discoverInfo = ((AbstractCallPeerJabberGTalkImpl<?, ?, ?>) callPeer)
			.getDiscoveryInfo();

		return parentProvider.getDiscoveryManager().includesFeature(InputEvtIQ.NAMESPACE_SERVER)
			&& (discoverInfo != null) && discoverInfo.containsFeature(InputEvtIQ.NAMESPACE_CLIENT);
	}
}
