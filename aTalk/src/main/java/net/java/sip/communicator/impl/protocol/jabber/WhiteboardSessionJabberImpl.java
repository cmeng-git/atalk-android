 ...
 */
public class WhiteboardSessionJabberImpl implements WhiteboardParticipantListener,
	WhiteboardSession
{
	/**
	 ...
	 */
	private Vector<WhiteboardObjectListener> messageListeners = new Vector<WhiteboardObjectListener>();

	/**
	 * A list containing all <tt>WhiteboardParticipant</tt>s of this session.
	 */
	private Hashtable<String, WhiteboardParticipant> wbParticipants = new Hashtable<String, WhiteboardParticipant>();
	...
	/**
	 * A list of all listeners currently registered for <tt>WhiteboardChangeEvent</tt>s
	 */
	private Vector<WhiteboardChangeListener> whiteboardListeners = new Vector<WhiteboardChangeListener>();

	/**
	 * Stores all white board objects contained in this session.
	 */
	private final Vector<WhiteboardObject> whiteboardObjects = new Vector<WhiteboardObject>();
		...
		OperationSetWhiteboardingJabberImpl opSet)
	{
		...
		// create the UID
		this.whiteboardID = String.valueOf(System.currentTimeMillis())
	 ...
	 *
	 * @return an Iterator over all participants currently involved in the white-board.
	 */
	public Iterator<WhiteboardParticipant> getWhiteboardParticipants()
	{
		return new LinkedList<WhiteboardParticipant>(wbParticipants.values()).iterator();
	}
	...
	/**
	 * Joins this white-board with the nickname of the local user so that the user would start
	...
	public void join()
		throws OperationFailedException
	{
		StanzaExtensionFilter filterWhiteboard = new StanzaExtensionFilter(
			WhiteboardObjectPacketExtension.NAMESPACE);
		this.jabberProvider.getConnection().addAsyncStanzaListener(
			new WhiteboardSmackMessageListener(), filterWhiteboard);
		this.whiteboardOpSet.fireWhiteboardSessionPresenceEvent(this,
			WhiteboardSessionPresenceChangeEvent.LOCAL_USER_JOINED, null);
	}

	/**
	 * Joins this white-board so that the user would start receiving events and WhiteboardObject for
	 * it. The method uses the nickname of the local user and the specified password in order to
	 * enter the white-board session.
	 *
	 * @param password
	 *        the password to use when authenticating on the white-board session.
	 * @throws OperationFailedException
	 *         with the corresponding code if an error occurs while joining the room.
	 */
	...
	/**
	 * Returns true if the local user is currently in the white-board session (after white-boarding
	 * one of the {@link #join()} methods).
	 *
	 * @return true if currently we're currently in this white-board and false otherwise.
	 */
	...
	public void leave()
	{
		try {
			...
			WhiteboardSessionPacketExtension extension = new WhiteboardSessionPacketExtension(this,
				jabberProvider.getAccountID().getAccountJid(),
		...
		whiteboardOpSet.fireWhiteboardSessionPresenceEvent(this,
			WhiteboardSessionPresenceChangeEvent.LOCAL_USER_LEFT, null);
	}
	...
	{
		OperationSetPersistentPresenceJabberImpl presenceOpSet = (OperationSetPersistentPresenceJabberImpl) jabberProvider
			.getOperationSet(OperationSetPresence.class);

		// If there's no presence operation set we return, because there's not contact to associate
		// the event with.
		if (presenceOpSet == null)
			return;

		ContactJabberImpl sourceContact = (ContactJabberImpl) presenceOpSet
			.findContactByID(contactAddress);
	 ...
	 *
	 * @param evt
	 *        The <tt>WhiteboardParticipantChangeEvent</tt> instance containing the source event as
	 ...
	 * @param evt
	 *        The <tt>WhiteboardParticipantChangeEvent</tt> instance containing the source event as
	 *        well as its previous and its new address.
	 */
	 ...
	 * @param evt
	 *        The <tt>WhiteboardParticipantChangeEvent</tt> instance containing the source event as
	 *        well as its previous and its new transport address.
	 */
	...
	/**
	 * Adds <tt>wbParticipant</tt> to the list of participants in this white-board. If the
	 * white-board participant is already included in the white-board, the method has no effect.
	 *
	 * @param wbParticipant
	...
	public void addWhiteboardParticipant(WhiteboardParticipant wbParticipant)
	{
		...
		try {
			this.smackChat = ChatManager.getInstanceFor(jabberProvider.getConnection()).createChat(
					JidCreate.entityFullFrom(wbParticipant.getContactAddress()), null);
		}
	...
	/**
	 * Sets the state of this whiteboard and fires a whiteboard change event notifying registered
	...
	/**
	 * Determines wheter the protocol provider (or the protocol itself) support sending and
	 ...
	 * method to return true even when offline messaging is not supported, and then have the
		...
		throws OperationFailedException
	{
		...
		WhiteboardParticipantJabberImpl participant = (WhiteboardParticipantJabberImpl) participants
			.next();
		...
		try {
			assertConnected();
			org.jivesoftware.smack.packet.Message msg = new org.jivesoftware.smack.packet.Message();
			WhiteboardObjectPacketExtension messageJI = new WhiteboardObjectPacketExtension(
				obj.getID(), WhiteboardObjectPacketExtension.ACTION_DELETE);
			...
			// MessageEventManager.addNotificationsRequests(msg, true, false, false, true);
			smackChat.sendMessage(msg);
			WhiteboardObjectDeliveredEvent msgDeliveredEvt = new WhiteboardObjectDeliveredEvent(
		...
		throws OperationFailedException
	{
		...
		try {
			...
			org.jivesoftware.smack.packet.Message msg = new org.jivesoftware.smack.packet.Message();
			WhiteboardObjectPacketExtension messageJI = new WhiteboardObjectPacketExtension(
				(WhiteboardObjectJabberImpl) message, WhiteboardObjectPacketExtension.ACTION_DRAW);
	...
	/**
	 * Compares the specified object with this whiteboard and returns true if it the specified
	 * object is an instance of a Whiteboard object and if the extending telephony protocol
	 * considers the whiteboards represented by both objects to be the same.
	 *
	 * @param obj
	 *        the whiteboard to compare this one with.
	 * @return true in case both objects are pertaining to the same whiteboard and false otherwise.
	 */
	@Override
	...
	/**
	 * Creates a <tt>WhiteboardParticipantEvent</tt> with <tt>sourceWhiteboardParticipant</tt> and
	 * <tt>eventID</tt> and dispatches it on all currently registered listeners.
	 *
	 * @param sourceWhiteboardParticipant
	 *        the source <tt>WhiteboardParticipant</tt> for the newly created event.
	 * @param eventID
	 *        the ID of the event to create (see CPE member ints)
	 */
	public void fireWhiteboardParticipantEvent(WhiteboardParticipant sourceWhiteboardParticipant,
		int eventID)
	{
		WhiteboardParticipantEvent cpEvent = new WhiteboardParticipantEvent(this,
			sourceWhiteboardParticipant, eventID);

		if (logger.isDebugEnabled())
			logger.debug("Dispatching a WhiteboardParticipant event to "
				+ whiteboardListeners.size() + " listeners. event is: " + cpEvent.toString());
	...
	/**
	 * Creates a <tt>WhiteboardChangeEvent</tt> with this class as <tt>sourceWhiteboard</tt>, and
	 * the specified <tt>eventID</tt> and old and new values and dispatches it on all currently
	 * registered listeners.
	 *
	 * @param type
	 *        the type of the event to create (see WhiteboardChangeEvent member ints)
	 * @param oldValue
	 *        the value of the Whiteboard property that changed, before the event had occurred.
	 * @param newValue
	 *        the value of the Whiteboard property that changed, after the event has occurred.
	 */
	public void fireWhiteboardChangeEvent(String type, Object oldValue, Object newValue)
	{
		...
		if (logger.isDebugEnabled())
			logger.debug("Dispatching a WhiteboardChange event to " + whiteboardListeners.size()
	...
	{
		if (logger.isDebugEnabled())
			logger.debug("Dispatching a WhiteboardMessageEvent event to " + messageListeners.size()
		...
		for (WhiteboardObjectListener listener : listeners) {
			if (evt instanceof WhiteboardObjectDeliveredEvent) {
				...
				WhiteboardObjectJabberImpl wbObj = (WhiteboardObjectJabberImpl) ((WhiteboardObjectReceivedEvent) evt)
				...
				WhiteboardObjectJabberImpl wbObj = (WhiteboardObjectJabberImpl) womevt
					.getSourceWhiteboardObject();
	...
	/**
	 * Listens for white-board messages and fires the appropriate events to notify all interested
	 * listeners.
	 */
	private class WhiteboardSmackMessageListener implements StanzaListener
	{
		public void processStanza(Stanza packet)
		{
			...
			ExtensionElement objectExt = packet.getExtension(
				WhiteboardObjectPacketExtension.ELEMENT_NAME,
				WhiteboardObjectPacketExtension.NAMESPACE);
			ExtensionElement sessionExt = packet.getExtension(
			...
			if (sessionExt != null) {
				...
				if (sessionMessage.getAction()
					.equals(WhiteboardSessionPacketExtension.ACTION_LEAVE)) {
					fireWhiteboardParticipantEvent(
						findWhiteboardParticipantFromContactAddress(sessionMessage
							.getContactAddress()),
						WhiteboardParticipantEvent.WHITEBOARD_PARTICIPANT_REMOVED);
				}
			}
			...
			String fromUserID = msg.getFrom().toString();
			if (logger.isDebugEnabled()) {
			...
			// If there's no presence operation set we return, because there's not contact to
			...
			if (msg.getType() == org.jivesoftware.smack.packet.Message.Type.error) {
				...
				if (errorCode == Condition.service_unavailable) {
					MessageEvent msgEvent = packet.getExtension("x", "jabber:x:event");
					if (msgEvent != null && msgEvent.isOffline()) {
						errorResultCode = WhiteboardObjectDeliveryFailedEvent.OFFLINE_MESSAGES_NOT_SUPPORTED;
					}
				}

				WhiteboardObjectDeliveryFailedEvent evt = new WhiteboardObjectDeliveryFailedEvent(
					WhiteboardSessionJabberImpl.this, newMessage.getWhiteboardObject(),
					sourceContact, errorResultCode, new Date());
				fireMessageEvent(evt);
				return;
			}

			if (newMessage.getAction().equals(WhiteboardObjectPacketExtension.ACTION_DELETE)) {
				WhiteboardObjectDeletedEvent msgDeletedEvt = new WhiteboardObjectDeletedEvent(
					WhiteboardSessionJabberImpl.this, newMessage.getWhiteboardObjectID(),
					sourceContact, new Date());

				fireMessageEvent(msgDeletedEvt);
			}
			else if (newMessage.getAction().equals(WhiteboardObjectPacketExtension.ACTION_DRAW)) {
				WhiteboardObjectReceivedEvent msgReceivedEvt = new WhiteboardObjectReceivedEvent(
					WhiteboardSessionJabberImpl.this, newMessage.getWhiteboardObject(),
					sourceContact, new Date());
	...
	/**
	 * Checks if the participant given by <tt>participantName</tt> is contained in this white-board
	 * session.
	 *
	 * @param participantName
	 *        the name of the participant to search for
	 * @return <code>true</code> if a participant with the given name is contained in this session,
	...
	/**
	 * Searches all participants contained in this white board and returns the one that corresponds
	 * to the given contact address.
	 *
	 * @param contactAddress
	 *        the address of the contact to search for.
	 * @return the <tt>WhiteboardParticipant</tt>, contained in this white board session and
	 *         corresponding to the given contact address
	 */
