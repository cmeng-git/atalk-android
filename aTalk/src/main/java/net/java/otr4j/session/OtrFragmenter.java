...
public class OtrFragmenter
{
	 ...
	 *
	 * @param session
	 * 		session instance (cannot be null)
	 * @param host
	 * 		OTR engine host calling upon OTR session
	 */
	...
	/**
	 * Calculate the number of fragments that are required for the message to be sent fragmented
	 * completely.
	 *
	 * @param message
	 ...
	 * @throws IOException
	 * 		throws an IOException in case fragment size is too small to store any content or when
	 * 		the provided policy does not support fragmentation, for example if only OTRv1 is
	 * 		allowed.
	 */
	 ...
	 * @return returns number of fragments required.
	 * @throws IOException
	 * 		throws an IOException if fragment size is too small.
	 */
	private int computeFragmentNumber(final String message,
			final FragmenterInstructions instructions)
			throws IOException
	{
	 ...
	 * @throws IOException
	 * 		throws an IOException if the fragment size is too small or if the maximum number of
	 * 		fragments is
	 * 		exceeded.
	 */
	 ...
	 * @throws IOException
	 * 		Exception in the case when it is impossible to fragment the message according to the
	...
	private String createV2MessageFragment(final int count, final int total,
			final String partialContent)
	{
		final String msg = String.format(Locale.US, OTR_V2_MESSAGE_FRAGMENT_FORMAT, count + 1,
				total, partialContent);
		return msg;
	}
