...
public class OtrSm
{
	 ...
	 * @param engineHost
	 * 		The host where we can present messages or ask for the shared secret.
	 */
	 ...
	 * 		The secret.
	 * @param initiating
	 * 		Whether we are initiating or responding to an initial request.
			...
			throws OtrException
	{
		...
		if (tlvType == TLV.SMP1Q && nextMsg == SM.EXPECT1) {
			/*
			 * We can only do the verification half now. We must wait for the secret to be entered
			  * to continue.
			 */
			...
			if (smstate.smProgState != SM.PROG_CHEATED) {
				...
				engineHost.askForSecret(session.getSessionID(), session.getReceiverInstanceTag(),
						questionUTF);
			}
			else {
