...
public class SerializationUtils
{
			...
			case AbstractEncodedMessage.MESSAGE_DATA:
				...
				switch (m.messageType) {
					...
					case AbstractEncodedMessage.MESSAGE_DATA:
						...
						break;
					default:
						// NOTE We should probably move at least part of this method into
						// individual
			...
			throws IOException
	{
		...
		if (idxHead > -1) {
			// Message **contains** the string "?OTR". Check to see if it is an error message, a
			// query message or a data
			// message.
			...
			if (contentType == SerializationConstants.HEAD_ERROR
				...
				// Error tag found.
				content = content.substring(idxHead
						+ SerializationConstants.ERROR_PREFIX.length());
				...
				switch (messageType) {
					case AbstractEncodedMessage.MESSAGE_DATA:
						...
						DataMessage dataMessage = new DataMessage(protocolVersion, flags,
								senderKeyID, recipientKeyID, nextDH, ctr, encryptedMessage, mac,
								oldMacKeys);
					...
					case AbstractEncodedMessage.MESSAGE_SIGNATURE: {
						...
						byte[] xEncryptedMac = otr.readMac();
						SignatureMessage signatureMessage = new SignatureMessage(protocolVersion,
					...
					default:
						// NOTE by gp: aren't we being a little too harsh here? Passing the
						// message as a plaintext message to the host application shouldn't hurt
						// anybody.
						otr.close();
						throw new IOException("Illegal message type.");
				}
