...
public class AvatarManager extends Manager
{
	 ...
	 *
	 * @param autoDownload
	 * 		true to enable auto download of avatars (photo image)
	 */
	 ...
	 *
	 * @param userAvatarMode
	 * 		true (default) to enable XEP-0054 User Avatar Mode
	 */
	 ...
	 *
	 * @param storeDir
	 * 		the file directory which will store the avatars
	 */
	 ...
	 *
	 * @param avatarId
	 * 		The avatarId, i.e SHA-1 Hash of the avatarImage.
	 * @param avatarImage
	 * 		Byte[] value of avatar image data.
	 */
	protected static boolean addAvatarImageByAvatarId(String avatarId, byte[] avatarImage)
	{
		if (!TextUtils.isEmpty(avatarId) && isAvatarNew(avatarId)) {
	 ...
	 * 		the bareJid
	 * @param avatarHash
	 * 		the avatar Hash cannot be empty
	 */
	...
	/**
	 * Check if we have the user avatar in store
	 ...
	 */
	protected static boolean isAvatarNew(String avatarId)
	{
		boolean isFound;
		if (persistentAvatarCache != null) {
	 ...
	 *
	 * @param userId
	 * 		the known owner of the avatarHash
	 * @param avatarHash
	 * 		the id of the avatar (Hash)
	 ...
	 *
	 * @param account
	 * 		the roster information belong to this account
	 ...
	 *
	 * @param account
	 * 		all the avatar information belong to this account are to be purged
	 */
	 ...
	 *
	 * @param imageData
	 * 		The image to getHashForJid the hexadecimal representation of the SHA-1 hash.
	 ...
	 *
	 * @param imageBytes
	 * 		The avatar image and must not be null
	 * @param updateVcardTemp
	 * 		<tt>true</tt> to setAvatarHash() if new
	...
	public boolean updateVCardAvatarHash(byte[] imageBytes, boolean updateVcardTemp)
	{
		...
		if (imageBytes.length != 0) {
			...
			if (isImageUpdated || isAvatarNew(avatarHash)) {
