...
public class MclStorageMigrate
{
	 ...
	 * Our logger.
	 */
	private static final Logger logger = Logger.getLogger(MclStorageMigrate.class);
	 ...
	 * @throws XMLException
	 * 		if a problem occurs while parsing contact list contents.
	 */
	...
	public static void processGroupXmlNode(Node groupNode)
	{
		...
		for (int i = 0; i < protoGroups.getLength(); i++) {
			...
			if (!StringUtils.isNullOrEmpty(parentProtoGroupUID)) {
				logger.info("Process meta Group: " + parentProtoGroupUID + ": " + protoGroupUID);
			...
			// update root MetaContactRootGroup if <proto-groups/> child is specified
				...
				logger.info("Process root metaContactGroup: " + ContactGroup.ROOT_NAME);
		...
		for (int i = 0; (childContacts != null) && (i < childContacts.getLength()); i++) {
			...
			try {
				...
				logger.info("Process child contact: " + parentPgUid + ": " + address);
				...
				logger.warn("Failed to parse meta contact " + currentMetaContactNode
						+ ". Exception: ", thr);
			}
		}
	 ...
	 * 		the UID String of the meta contact whose node we are looking for.
	...
	private static void processOtrProperties()
	{
		...
		for (Map.Entry<String, String> entry : otrValues.entrySet()) {
			...
			logger.info("Process account otr properties: " + key + ": " + value);
			...
			if (key.startsWith("Jabber")) {
				if (key.contains("privateKey") || key.contains("publicKey")) {
					...
					try {
						...
						logger.error("Extract account OTR keypair failed! Exception: " + e);
					}
				}
			}
	...
	{
		logger.info("Store account OTR keypair for: " + accountID);
		...
		try {
			...
			logger.error("Saving account private & public keys failed! Exception: " + e);
		}
	}
	...
	private static void putJsonFingerprint(String contact, String print)
	{
		...
		try {
			...
			logger.error("Error in processing FingerPrint: " + ex);
		}
	}
	...
	private static void putOtrPolicy(String contact, String policy)
	{
		...
		try {
			...
			logger.error("Error in processing OTR Policy: " + ex);
		}
	}
