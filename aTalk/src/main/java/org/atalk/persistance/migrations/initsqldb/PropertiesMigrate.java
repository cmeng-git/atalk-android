...
public class PropertiesMigrate
{
	 ...
	 */
	private final static Logger logger = Logger.getLogger(PropertiesMigrate.class);
	...
	public static void migrateProperties(SQLiteDatabase db, SQLiteDatabase propertiesDB)
	{
		...
		mDB = db;
		String otrGlobal = "GLOBAL_POLICY,AUTO_INIT_PRIVATE_MESSAGING,"
				+ "PRIVATE_MESSAGING_MANDATORY";
...
//					accountValues.put(accUuid + property, value);
	...
	private static void processOtr(String property, String value)
	{
		...
		String otrGlobal
				= "GLOBAL_POLICY,AUTO_INIT_PRIVATE_MESSAGING,PRIVATE_MESSAGING_MANDATORY";

		logger.info("Process OTR properties: " + property + ": " + value);
	...
	private static void accountUpdate(String accountUuid, String name, String value)
	{
		...
		logger.info("Account Update: " + accountUuid + ": " + name + ": " + value);
		values.clear();
	...
	{
		logger.info("System Properties: " + name + ": " + value);
		values.clear();
		values.put(AccountID.COLUMN_NAME, name);
		values.put(AccountID.COLUMN_VALUE, value);
	...
	private static void cleanAccProperties()
	{
		...
		while (cursor.moveToNext()) {
			...
			if (!accounts.contains(uuid)) {
				...
				logger.info("Deleting property for non-existence account: " + uuid + ": " + para);
				String[] args = {uuid};
