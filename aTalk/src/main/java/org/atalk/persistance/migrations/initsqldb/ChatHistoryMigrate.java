...
import static org.atalk.android.gui.chat.ChatMessage.*;
...
public class ChatHistoryMigrate extends Activity
{
	 ...
	 */
	private final static Logger logger = Logger.getLogger(ChatHistoryMigrate.class);
				...
				for (File sessionFN : filesList) {
					...
					if (idx != -1) {
						...
						if (StringUtils.isNullOrEmpty(accountUuid)) {
							logger.warn("Skip processing obsolete xml message file: " + fpName);
							continue;
						}
					...
					for (File messageFp : messages) {
						...
						for (File xmlFP : xmlFPs) {
							if ((builder != null) && xmlFP.getName().contains(".xml")
									&& (xmlFP.length() != 0)) {
								logger.info("Processing record xml file: " + userId + ": "
										+ xmlFP);
								xmlDocument = builder.parse(xmlFP);
								processXmlNode(chatType);
							}
							else if (!xmlFP.getName().contains(".dat")) {
								logger.warn("Error parsing chat history file: " + xmlFP);
							}
						}
					}
				}
