...
public abstract class EncodingConfiguration
{
	...
	{
		/*
		 * If they need updating, their caches are invalid and need rebuilding next time they are
		 * requested.
		 */
	 ...
	 *
	 * @param encoding
	 *        the <tt>MediaFormat</tt> specifying the encoding to set the priority of
	 * @param priority
	 *        a positive <tt>int</tt> indicating the priority of <tt>encoding</tt> to set
	 */
	 ...
	 *
	 * @param type
	 *        the <tt>MediaType</tt> we would like to know the available encodings of
	 ...
	 *
	 * @param type
	 *        the <tt>MediaType</tt> of the supported <tt>MediaFormat</tt>s to get
	 ...
	 *
	 * @param encoding
	 *        the <tt>MediaFormat</tt> to get the key in {@link #encodingPreferences} of
		...
		for (Map.Entry<String, String> entry : properties.entrySet()) {
			...
			try {
				...
				if (encodingClockRateSeparator > -1) {
					...
					clockRate = Double.parseDouble(
						fmtName.substring(encodingClockRateSeparator + 1));
				}
				else {
					encoding = fmtName;
