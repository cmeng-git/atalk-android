...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>BasicVolumeControl</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 *        The power level wished for the signal (corresponds to the measured power level).
	 ...
	 *        the name of the configuration property which specifies the value of the volume level
	 *        of the new instance
	 */
	...
	/**
	 * Register for gain change update events. A <code>GainChangeEvent</code> is posted when the
	 * state of the <code>GainControl</code> changes.
	 *
	 * @param listener
	...
	public void addVolumeChangeListener(VolumeChangeListener listener)
	{
		synchronized (volumeChangeListeners) {
			...
			if (!contains)
				volumeChangeListeners.add(new WeakReference<>(listener));
		}
	}
	...
	private void fireVolumeChange()
	{
		...
		synchronized (volumeChangeListeners) {
			...
			ls = new ArrayList<>(volumeChangeListeners.size());
			while (i.hasNext()) {
				VolumeChangeListener l = i.next().get();
	 ...
	 * 1.0 is the loudest useful level that this <code>GainControl</code> supports.
	 *
	 * @param level
	 ...
	 *
	 * @param value
	 *        the new value, changed if different from current volume settings.
