...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>RTCPReports</tt> class and its instances to print debug
	 * information.
	 */
	 ...
	 */
	private final Map<Integer, RTCPExtendedReport.VoIPMetricsReportBlock> receivedVoIPMetrics
			= new HashMap<>();
	 ...
	 * source (SSRC of source defined by RFC 3611).
	 */
	private final Map<Integer, RTCPExtendedReport.VoIPMetricsReportBlock> sentVoIPMetrics
			= new HashMap<>();
	...
	/**
	 * Gets the RTCP sender report (SR) and/or receiver report (RR) blocks sent by the local
	 * endpoint.
	 *
	 * @return the RTCP sender report (SR) and/or receiver report (RR) blocks sent by the local
	 * endpoint
	 */
	...
	public void rtcpExtendedReportReceived(RTCPExtendedReport extendedReport)
	{
		...
		synchronized (receivedExtendedReports) {
			Object oldValue
					= receivedExtendedReports.put((int) extendedReport.getSSRC(), extendedReport);
	...
	public void rtcpExtendedReportSent(RTCPExtendedReport extendedReport)
	{
		...
		synchronized (sentExtendedReports) {
			Object oldValue
					= sentExtendedReports.put((int) extendedReport.getSSRC(), extendedReport);
