...
public class DefaultStreamConnector implements StreamConnector
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>DefaultStreamConnector</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 *
	 * @param bindAddr
	 *        the local <tt>InetAddress</tt> the new <tt>DatagramSocket</tt> is to bind to
	...
	private static synchronized DatagramSocket createDatagramSocket(InetAddress bindAddr)
	{
		...
		for (int i = 0; i < bindRetries; i++) {
			...
			try {
				...
				logger.warn("Retrying a bind because of a failure to bind to address " + bindAddr
					+ " and port " + port, se);
			}
		}
		return null;
	 ...
	 *
	 * @param bindAddr
	 *        the local <tt>InetAddress</tt> the new instance is to attempt to bind to
	 */
	 ...
	 *
	 * @param dataSocket
	 *        the <tt>DatagramSocket</tt> to be used for data (e.g. RTP) traffic
	 * @param controlSocket
	 *        the <tt>DatagramSocket</tt> to be used for control data (e.g. RTCP) traffic
	 */
