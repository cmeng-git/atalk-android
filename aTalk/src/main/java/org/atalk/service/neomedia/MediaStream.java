...
/**
 * The <tt>MediaStream</tt> class represents a (generally) bidirectional RTP stream between exactly
 * two parties. The class reflects one media stream, in the SDP sense of the word.
 * <tt>MediaStream</tt> instances are created through the <tt>openMediaStream()</tt> method of the
 * <tt>MediaService</tt>.
 *
 * @author Emil Ivov
...
public interface MediaStream
{
	...
	/**
	 * Clears the dynamic RTP payload type associations in this <tt>MediaStream</tt>.
	 */
	 ...
	 *
	 * @param listener
	 * 		the listener that we'd like to register for <tt>PropertyChangeEvent</tt>s
	 */
	void addPropertyChangeListener(PropertyChangeListener listener);

	/**
	 * Adds or updates an association in this <tt>MediaStream</tt> mapping the specified
	 * <tt>extensionID</tt> to <tt>rtpExtension</tt> and enabling or disabling its use according to
	 * the direction attribute of <tt>rtpExtension</tt>.
	 *
	 * @param extensionID
	 * 		the ID that is mapped to <tt>rtpExtension</tt> for the lifetime of this
	 * 		<tt>MediaStream</tt>.
	 * @param rtpExtension
	 * 		the <tt>RTPExtension</tt> that we are mapping to <tt>extensionID</tt>.
	 */
	 ...
	 * <tt>encoding</tt> or <tt>-1</tt> if no payload type has been negotiated for it. If
	 * multiple formats match the specified <tt>encoding</tt>, then this method would return the
	 * first one it encounters while iterating through the map.
	 *
	 * @param codec
	 * 		the encoding whose payload type we are trying to obtain.
	 *
	 * @return the payload type number that has been negotiated for the specified <tt>codec</tt>
	 * or <tt>-1</tt> if no payload type has been negotiated for it.
	 */
	byte getDynamicRTPPayloadType(String codec);

	/**
	 * Returns the <tt>MediaFormat</tt> that this stream is currently transmitting in.
	 *
	...
	/**
	 * Returns the <tt>MediaFormat</tt> that is associated to the payload type passed in as a
	 * parameter.
	 *
	 * @param payloadType
	 * 		the payload type of the <tt>MediaFormat</tt> to get.
	 * @return the <tt>MediaFormat</tt> that is associated to the payload type passed in as a
	 * parameter.
	 */
	MediaFormat getFormat(byte payloadType);

	/**
	 * Returns the synchronization source (SSRC) identifier of the local participant or <tt>-1</tt>
	 * if that identifier is not yet known at this point.
	 *
	 * @return the synchronization source (SSRC) identifier of the local participant or <tt>-1</tt>
	 * if that identifier is not yet known at this point.
	 */
	long getLocalSourceID();

	/**
	 * Returns a <tt>MediaStreamStats</tt> object used to get statistics about this
	 * <tt>MediaStream</tt>.
	 *
	 * @return the <tt>MediaStreamStats</tt> object used to get statistics about this
	 * <tt>MediaStream</tt>.
	 */
	 ...
	 *
	 * @param propertyName
	 * 		the name of the opaque property of this <tt>MediaStream</tt> the value of which is to
	 * 		be returned
	 * @return the value of the opaque property of this <tt>MediaStream</tt> specified by
	 * <tt>propertyName</tt>
	 */
	 ...
	 *
	 * @return an <tt>InetSocketAddress</tt> instance indicating the address that we are sending
	 * RTCP packets to.
	 */
	...
	/**
	 * Gets the synchronization source (SSRC) identifier of the remote peer or <tt>-1</tt> if that
	 * identifier is not yet known at this point in the execution.
	 * <p>
	 * <b>Warning</b>: A <tt>MediaStream</tt> may receive multiple RTP streams and may thus have
	 * multiple remote SSRCs. Since it is not clear how this <tt>MediaStream</tt> instance chooses
	 ...
	 * </p>
	 *
	 * @return the synchronization source (SSRC) identifier of the remote peer or <tt>-1</tt> if
	...
	/**
	 * Removes the specified property change <tt>listener</tt> from this stream so that it won't
	...
	/**
	 * Removes the <tt>ReceiveStream</tt> with SSRC <tt>ssrc</tt>, if there is such a
	 * <tt>ReceiveStream</tt>, from the receive streams of this <tt>MediaStream</tt>
	 *
	 * @param ssrc
	...
	/**
	 * Sets the <tt>StreamConnector</tt> to be used by this <tt>MediaStream</tt> for sending and
	 * receiving media.
	 *
	 * @param connector
	 * 		the <tt>StreamConnector</tt> to be used by this <tt>MediaStream</tt> for sending and
	 * 		receiving media
	 */
	 ...
	 *
	 * @param mute
	 * 		<tt>true</tt> if we are to start transmitting "silence" and <tt>false</tt> if we are
	 * 		to use media from this stream's <tt>MediaDevice</tt> again.
	 */
	void setMute(boolean mute);

	/**
	 * Sets the name of this stream. Stream names are used by some protocols, for diagnostic
	 * purposes mostly. In XMPP for example this is the name of the content element that
	 * describes a stream.
	 *
	 * @param name
	 * 		the name of this stream or <tt>null</tt> if no name has been set.
	 */
	 ...
	 * @param propertyName
	 * 		the name of the opaque property of this <tt>MediaStream</tt> the value of which is to
	 * 		be set to the specified <tt>value</tt>
	 * @param value
	 * 		the value of the opaque property of this <tt>MediaStream</tt> specified by
	 * 		<tt>propertyName</tt> to be set
	 */
	void setProperty(String propertyName, Object value);

	/**
	 * Sets the <tt>RTPTranslator</tt> which is to forward RTP and RTCP traffic between this and
	 * other <tt>MediaStream</tt>s.
	 *
	 * @param rtpTranslator
	 * 		the <tt>RTPTranslator</tt> which is to forward RTP and RTCP traffic between this and
	 * 		other <tt>MediaStream</tt>s
	 */
	 ...
	 *
	 * @param ssrcFactory
	 * 		the <tt>SSRCFactory</tt> which is to generate new synchronization source (SSRC)
	 ...
	 * the local <tt>StreamConnector</tt> toward the stream's target address and port. The method
	 * also puts the <tt>MediaStream</tt> in a listening state that would make it play all media
	 * received from the <tt>StreamConnector</tt> on the stream's <tt>MediaDevice</tt>.
	 */
	void start();
	...
	/**
	 * Sets the external (application-provided) <tt>TransformEngine</tt> of this
	 * <tt>MediaStream</tt>.
	 *
	 * @param transformEngine
	 * 		the <tt>TransformerEngine</tt> to use.
	 */
	 ...
	 * 		the packet to send.
	 * @param data
	 * 		{@code true} to send an RTP packet or {@code false} to send an RTCP packet.
	 * @param after
	 * 		the {@code TransformEngine} in the {@code TransformEngine} chain of this
	 ...
	 * 		unless the injection is being performed by a {@code TransformEngine} itself (while
	 * 		executing {@code transform} or {@code reverseTransform} of a {@code PacketTransformer}
	 * 		of its own even).
	 ...
	 *
	 * @param buf
	 * 		the buffer that holds the RTP payload.
	 * @param off
	 * 		the offset in the buff where the RTP payload is found.
	 * @param len
	 * 		then length of the RTP payload in the buffer.
	 ...
	 *
	 * @param baf
	 * 		the {@link ByteArrayBuffer} that holds the RTP payload.
	 ...
	 *
	 * @param pkt
	 * 		the {@link RawPacket} that holds the RTP payload.
	 * @return the primary {@link REDBlock} that contains the payload of the RTP
	 * packet passed in as a parameter, or null if the buffer is invalid.
	 */
