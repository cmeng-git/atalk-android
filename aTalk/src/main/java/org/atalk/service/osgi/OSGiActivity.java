...
public class OSGiActivity extends FragmentActivity
{
	...
	/**
	 * Checks if the crash has occurred since the Jitsi was last started. If it's true asks the
	 * user about eventual logs report.
	 */
	private void checkForSendLogsDialog()
	{
		// Checks if Jitsi has previously crashed and asks the user user about log reporting
		...
		question.setTitle(R.string.service_gui_WARNING).setMessage(
				getString(R.string.service_gui_SEND_LOGS_QUESTION))
	 ...
	 *
	 * @param fragment
	 * 		child <tt>OSGiUiPart</tt> contained in this <tt>Activity</tt>.
	 */
	public void registerOSGiFragment(OSGiUiPart fragment)
	{
		...
		if (bundleContext != null) {
			// If context exists it means we have started already, so start the fragment
			// immediately
			try {
	 ...
	 *
	 * @param fragment
	 * 		the <tt>OSGiUiPart</tt> that will be unregistered.
	 */
	...
	/**
	 * Broadcast listener that listens for {@link aTalkApp#ACTION_EXIT} and then finishes
	 * this <tt>Activity</tt>
	 * .
	 */
