...
public abstract class AbstractSCAudioClip implements SCAudioClip
{
	...
	/**
	 * The indicator which determines whether this instance plays the audio it represents in a
	 * loop.
	 */
	 ...
	 * <tt>AbstractSCAudioClip</tt> does not use it and just remembers it in order to make it
	 * available to extenders.
	 */
	 ...
	 * extenders/implementers to perform per-loop iteraction cleanup. The
	 * <tt>AbstractSCAudioClip</tt> implementation does nothing.
	 */
	...
	protected void internalStop()
	{
		...
		synchronized (sync) {
			...
			while (command != null) {
				try {
					/*
					 * Technically, we do not need a timeout. If a notifyAll() is not called to
					 * wake us up, then we will likely already be in trouble. Anyway, use a
	 ...
	 *
	 * @return <tt>true</tt> if this instance plays the audio it represents in a loop;
				...
				command = new Runnable()
				{
					public void run()
					{
						try {
							synchronized (sync) {
								/*
								 * We have to wait for play(int,Callable<Boolean>) to let go of
								 * sync i.e. be ready with setting up the whole
								 * AbstractSCAudioClip state; otherwise, this Runnable will most
								 * likely prematurely seize to exist.
								 */
								if (!equals(command))
									return;
							}
	 ...
	 *
	 * @param loopCondition
	 * 		a <tt>Callback&lt;Boolean&gt;</tt> which represents the condition on which this audio
	 * 		will play more than once. If <tt>null</tt>, this audio will play once only. If an
	 * 		invocation of <tt>loopCondition</tt> throws a <tt>Throwable</tt>, this audio will
	 * 		discontinue playing.
	 */
		...
		try {
			...
			while (isStarted()) {
				...
				if (loopCondition == null) {
					/*
					 * The interface contract is that this audio plays once only if the
					 * loopCondition is null.
					 */
					break;
				}
				 ...
				 * the loop will continue only if loopCondition successfully and explicitly
				 * evaluates to true.
				 */
				boolean loop = false;
	 ...
	 *
	 * @param invalid
	 * 		<tt>true</tt> to mark this instance invalid or <tt>false</tt> to mark it valid
	 */
	 ...
	 *
	 * @param looping
	 * 		<tt>true</tt> to mark this instance that it should play the audio it represents in a
	 ...
	 *
	 * @param loopInterval
	 * 		the interval of time in milliseconds between consecutive plays of this audio in a loop
