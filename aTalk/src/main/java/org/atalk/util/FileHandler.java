...
public class FileHandler extends java.util.logging.FileHandler
{
	...
	/**
	 * Construct a default <tt>FileHandler</tt>.  This will be configured entirely from
	 * <tt>LogManager</tt> properties (or their default values). Will change
	 * <p>
	 *
	 * @throws IOException
	 * 		if there are IO problems opening the files.
	...
	{
		if (limit == -1) {
			String limitStr = LogManager.getLogManager()
					.getProperty(FileHandler.class.getName() + ".limit");
	...
	private static String getPattern()
	{
		if (pattern == null) {
			...
			if (homeLocation != null && dirName != null) {
				if (pattern == null)
					pattern = homeLocation + "/" + dirName + "/log/atalk%u.log";
				else
	...
	{
		if (count == -1) {
			String countStr = LogManager.getLogManager()
					.getProperty(FileHandler.class.getName() + ".count");
