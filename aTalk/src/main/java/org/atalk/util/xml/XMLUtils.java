...
import static org.atalk.util.StringUtils.*;
...
public class XMLUtils
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>XMLUtils</tt> class for logging output.
	 */
	 ...
	 * 		in case a TransformerException is thrown by the underlying Transformer.
	 */
			...
			throws java.io.IOException
	{
		...
		writeXML(document, new StreamResult(new OutputStreamWriter(fos, "UTF-8")), null, null);
		fos.close();
	}
	 ...
	 * 		the document to write
	 * @param doctypeSystem
	 * 		the doctype system of the xml document that we should record in the file or null if
	 * 		none is specified.
	 * @param doctypePublic
	 * 		the public identifier to be used in the document type declaration.
	 * @throws java.io.IOException
	 * 		in case a TransformerException is thrown by the underlying Transformer.
	 */
			...
			throws java.io.IOException
	{
		try {
			...
			throw new java.io.IOException("Failed to write the configuration file: "
	 ...
	 * 		String
	 */
	public static void printChildElements(Element root, PrintStream out, boolean recurse,
			String prefix)
	{
			...
			String keyAttributeName, String keyAttributeValue)
	{
		...
		for (int i = 0; i < len; i++) {
			...
			// look inside.
			List<Element> childs = locateElements((Element) node, tagName, keyAttributeName,
					keyAttributeValue);
	 ...
	 */
	public static boolean isStandardXmlNamespace(String namespace)
	{
		...
		return normalizeNamespace(XML_NS_URI).equals(namespace)
				...
				|| normalizeNamespace(W3C_XML_SCHEMA_INSTANCE_NS_URI).equals(namespace);
	}
	 ...
	 * @param allowExternalEntities
	 * 		whether parsing of XML external entities and DOCTYPE declarations should be allowed.
	 * @return the W3C Document.
	 * @throws Exception
	 * 		is there is some error during operation.
	 */
	public static Document createDocument(String xml, boolean allowExternalEntities)
			throws Exception
	{
		DocumentBuilderFactory builderFactory = newDocumentBuilderFactory(allowExternalEntities);
	 ...
	 * 		if setting any of the features fails.
	 */
			...
			throws ParserConfigurationException
	{
		...
		try {
			...
			if (OSUtils.IS_ANDROID)
				logger.warn("Failed to set feature: " + FEATURE_EXTERNAL_GENERAL_ENTITIES);
			else
				throw pce;
		}
		...
		try {
			...
			if (OSUtils.IS_ANDROID)
				logger.warn("Failed to set feature: " + FEATURE_EXTERNAL_PARAMETER_ENTITIES);
			else
				throw pce;
		}
		...
		try {
			...
			if (OSUtils.IS_ANDROID)
				logger.warn("Failed to set feature: " + FEATURE_DISSALLOW_DOCTYPE);
			else
				throw pce;
		}
	}
