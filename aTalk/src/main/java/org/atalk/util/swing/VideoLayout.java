		...
		extends FitLayout
{
	 ...
	 *
	 * @param conference
	 * 		<tt>true</tt> if the new instance will be dedicated to
	 ...
	 *
	 * @param size
	 * 		the <tt>Dimension</tt> whose aspect ratio is to be compared
	 ...
	 *
	 * @param remotes
	 * 		the remote visual/video <tt>Component</tt>s to be
	 * 		displayed in a grid
				...
				if (column == 0) {
					...
					if (row == rowsMinus1) {
						...
						if (available < columns) {
							bounds.x = (parentSize.width - available * bounds.width
									- (available - 1) * HGAP) / 2;
						}
					}
				}
