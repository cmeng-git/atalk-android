...
public class OSGiServiceImpl
{
	 ...
	 *
	 * @param service
	 * 		the Android <tt>OSGiService</tt> which is to use the new instance as its very
	 * 		implementation
	 */
	 ...
	 *
	 * @param intent
	 * 		the <tt>Intent</tt> which was used to bind to <tt>service</tt>
	 * @return an <tt>IBinder</tt> through which clients may call on to the public API of
	 * <tt>OSGiService</tt>
	 ...
	 *
	 * @param intent
	 * 		the <tt>Intent</tt> supplied to <tt>Context.startService(Intent}</tt>
	 * @param flags
	 * 		additional data about the start request
	 * @param startId
	 * 		a unique integer which represents this specific request to start
	...
	private void setScHomeDir()
	{
		...
		if (System.getProperty(ConfigurationService.PNAME_SC_HOME_DIR_NAME) == null) {
			if (TextUtils.isEmpty(name)) {
				...
				name = info.name;
				if (TextUtils.isEmpty(name))
					name = aTalkApp.getGlobalContext().getResources().getString(R.string.app_name);
			}
		...
		if (System.getProperty(ConfigurationService.PNAME_SC_LOG_DIR_LOCATION) == null) {
			String homeDir = System.getProperty(ConfigurationService.PNAME_SC_HOME_DIR_LOCATION,
					null);
	...
	private class OnCreateCommand implements Runnable
	{
		public void run()
		{
			...
			configuration.put(Constants.FRAMEWORK_BEGINNING_STARTLEVEL,
			...
			try {
				...
				bundleContext.registerService(BundleContextHolder.class, bundleContextHolder,
						null);
