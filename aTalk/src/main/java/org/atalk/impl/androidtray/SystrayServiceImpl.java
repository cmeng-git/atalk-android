...
public class SystrayServiceImpl extends AbstractSystrayService
{
	 ...
	 * 
	 * @param newHandler
	 *        the handler to set. providing a null handler is like disabling popup.
	...
	private class PopupListenerImpl implements SystrayPopupMessageListener
	{
		...
		public void popupMessageClicked(SystrayPopupMessageEvent evt)
		{
			 ...
			 * if(metaContact == null) { logger.error("Meta contact not found for "+contact); return; }
			 * 
			 ...
			 * 
			 * if(targetIntent == null) { logger.error( "Failed to create chat with " + metaContact); } } else { //
