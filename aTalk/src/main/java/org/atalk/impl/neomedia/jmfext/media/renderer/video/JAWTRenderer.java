...
/**
 * Implements a <tt>VideoRenderer</tt> which uses JAWT to perform native painting in an AWT or Swing
 * <tt>Component</tt>.
 *
		...
		implements VideoRenderer
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>JAWTRenderer</tt> class and its instances for logging
	 * output.
	 */
	 ...
	 *
	 * @param handle
	 * 		the handle to the native counterpart of a <tt>JAWTRenderer</tt> as returned by
	 * 		{@link #open(Component)} which is to be closed
	 * @param component
	 * 		the AWT <tt>Component</tt> into which the <tt>JAWTRenderer</tt> and its native
	 * 		counterpart are drawing. The platform-specific info of <tt>component</tt> is not
	 ...
	 *
	 * @param component
	 * 		the AWT <tt>Component</tt> into which a <tt>JAWTRenderer</tt> and the native
	 ...
	 *
	 * @param handle
	 * 		the handle to the native counterpart of a <tt>JAWTRenderer</tt> which is to draw into
	 ...
	 * 		<tt>paint</tt>.
	 * @param g
	 * 		the <tt>Graphics</tt> context into which the drawing is to be performed
	 * @param zOrder
	 ...
	 *
	 * @param handle
	 * 		the handle to the native counterpart of a <tt>JAWTRenderer</tt> to process the
	 * 		specified data and render it
	 * @param component
	 * 		the <tt>AWT</tt> component into which the specified <tt>JAWTRenderer</tt> and its
	 * 		native counterpart draw
	 * @param data
	 * 		an <tt>int</tt> array which contains the data to be processed and rendered
	 * @param offset
	 * 		the index in <tt>data</tt> at which the data to be processed and rendered starts
	 * @param length
	 * 		the number of elements in <tt>data</tt> starting at <tt>offset</tt> which represent
	 ...
	 * should
	 * not be passed into this <tt>PlugIn</tt> without first calling <tt>open</tt>.
	 *
	 * @throws ResourceUnavailableException
	 * 		if there is a problem during opening
	 */
	 ...
	 *
	 * @param buffer
	 * 		a <tt>Buffer</tt> containing the data to be processed and rendered
	 ...
	 * procedure is executed on the AWT event dispatching thread because an AWT
	 * <tt>Component</tt>'s
	 * properties (such as <tt>preferredSize</tt>) should be accessed in the AWT event dispatching
	 * thread.
	 */
	 ...
	 *
	 * @param bounds
	 * 		the region in the component of this <tt>VideoRenderer</tt> where the video is to be
	 ...
	 *
	 * @param component
	 * 		the AWT <tt>Component</tt> into which this <tt>VideoRenderer</tt> is to draw
	 * @return <tt>true</tt> if this <tt>VideoRenderer</tt> accepted the specified
	 * <tt>component</tt> as the AWT <tt>Component</tt> into which it is to draw;
	 ...
	 *
	 * @param format
	 * 		the <tt>Format</tt> to be set as the <tt>Format</tt> of the input to be processed by
	 * 		this <tt>Renderer</tt>
	 ...
	 * supported input <tt>Format</tt> which most closely matches the specified
	 * <tt>Format</tt>.
	 */
	@Override
	public synchronized Format setInputFormat(Format format)
	{
		...
		if (logger.isDebugEnabled()) {
			logger.debug(getClass().getName() + " 0x" + Integer.toHexString(hashCode())
	...
	/**
	 * Starts the rendering process. Begins rendering any data available in the internal buffers of
	 * this <tt>Renderer</tt>.
	 */
	@Override
	public void start()
