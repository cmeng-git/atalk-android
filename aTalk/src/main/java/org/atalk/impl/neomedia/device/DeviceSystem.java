 ...
 * {@link #createRenderer(boolean)} method.
 *
...
public abstract class DeviceSystem extends PropertyChangeNotifier
{
	 ...
	 *
	 * @param deviceList
	 *        the <tt>List</tt> of <tt>CaptureDeviceInfo</tt> which are to be filtered based on the
	 *        specified <tt>MediaLocator</tt> protocol
	 * @param locatorProtocol
	 *        the protocol of the <tt>MediaLocator</tt>s of the <tt>CaptureDeviceInfo</tt>s which
	 *        are to be returned
	 ...
	 *
	 * @param mediaType
	 *        the <tt>MediaType</tt> of the <tt>DeviceSystem</tt>s to be initialized
	 */
	 ...
	 *
	 * @param classNames
	 *        the names of the classes which extend the <tt>DeviceSystem</tt> class and instances of
	 ...
	 * @throws Exception
	 *         if an error occurs during the initialization of <tt>initialize()</tt> on the
	 ...
	 *        the <tt>DeviceSystem</tt> to invoke <tt>initialize()</tt> on
	 * @param asynchronous
	 *        <tt>true</tt> if the invocation is to be performed in a separate thread and the method
	 ...
	 *
	 * @throws Exception
	 *         if an error occurs during the initialization of this instance. The initialization of
	 ...
	 * initialization procedure (which includes {@link #doInitialize()}) executes once at any given
	 * time.
	 * </p>
	 *
	 * @throws Exception
	 *         if an error occurs during the initialization of this <tt>DeviceSystem</tt>
	 */
		...
		try {
			...
			if (format != null) {
				...
				if (preInitializeDevices != null) {
					for (Iterator<CaptureDeviceInfo> preIter = preInitializeDevices.iterator(); preIter
						.hasNext();) {
		...
		throws Exception
	{
		...
		if (format != null) {
			...
			if ((cdis != null) && (cdis.size() > 0)) {
				...
				for (CaptureDeviceInfo cdi : filterDeviceListByLocatorProtocol(cdis,
					getLocatorProtocol())) {
	 ...
	 *
	 * @return a <tt>String</tt> which represents this <tt>DeviceSystem</tt> in a human-readable
	 *         form
	 */
	@Override
