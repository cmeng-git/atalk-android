...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>RTCPPacketParserEx</tt> class and its instances to print
	 * debug information.
	 */
	 ...
	 *
	 * @param rtcp
	 * 		the {@code RTCPPacket} to represent as a {@code RawPacket}
	 ...
	 * @param base
	 * @param firstbyte
	 * 		the first byte of the RTCP packet
	 * @param type
	 * 		the packet type of the RTCP packet
	 * @param length
	 * 		the length in bytes of the RTCP packet, including all
 ...
 */
			...
			throws IOException
	{
		...
		if (logger.isTraceEnabled()) {
			...
			if (detailStr == null)
				detailStr = "";
			logger.trace("SSRC of packet sender: 0x" + Long.toHexString(senderSSRC)
					+ " (" + senderSSRC + "), SSRC of media source: 0x"
					+ Long.toHexString(sourceSSRC)
					+ " (" + sourceSSRC + "), Payload type (PT): " + ptStr
					+ ", Feedback message type (FMT): " + fmtStr + detailStr);
		}
		return fb;
	}
}
