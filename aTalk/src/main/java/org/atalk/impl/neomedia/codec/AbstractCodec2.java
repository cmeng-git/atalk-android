...
public abstract class AbstractCodec2 extends AbstractCodec
{
	 ...
	 *
	 * @param lastSeqNo
	 * 		the last received sequence number (prior to the current sequence number represented by
	 ...
	 * @param seqNo
	 * 		the current sequence number. May be equal to <tt>lastSeqNo</tt> for the purposes of
	 * 		Codec implementations which repeatedly process one and the same input Buffer multiple
	 * 		times.
	 ...
	 *
	 * @param buffer
	 * 		the <tt>Buffer</tt> whose <tt>data</tt> property value is to be validated
	 * @param newSize
	 * 		the minimum length of the array of <tt>byte</tt> which is to be the value of the
	 * 		<tt>data</tt> property of <tt>buffer</tt>
	 * @param arraycopy
	 * 		<tt>true</tt> to copy the bytes which are in the value of the <tt>data</tt> property
	 ...
	 * @param formatClass
	 * 		the <tt>Class</tt> of input and output <tt>Format</tt>s supported by the new instance
	 ...
	 *
	 * @param inputFormat
	 * 		the <tt>Format</tt> of the input for which the supported output <tt>Format</tt>s are
	 * 		to be returned
	 ...
	 *
	 * @throws ResourceUnavailableException
	 * 		if any of the resources that this <tt>PlugIn</tt> needs to operate cannot be acquired
	 ...
	 * processed or <tt>BUFFER_PROCESSED_FAILED</tt> if the specified was not successfully
	 * processed
