...
class Decoder extends Ld8k
{
	...
	/*
	 * File : DECODER.C Used for the floating point version of G.729 main body (not for G.729A)
	 */
	 ...
	 *
	 * Format for bitstream_file: One (2-byte) synchronization word One (2-byte) size word, 80 words
	 * (2-byte) containing 80 bits.
	 *
	 * Format for outputspeech_file: Synthesis is written to a binary file of 16 bits data.
	 *
	 * @param args
	...
	void process(short[] serial, short[] sp16)
	{
		...
		for (int i = 0; i < L_FRAME; i += L_SUBFR) {
			...
			sf_voic = postfil.post(t0_first, synth, synth_offset + i, ptr_Az, ptr_Az_offset,
				pst_out, i);
			if (sf_voic != 0) {
				voicing = sf_voic;
			}
