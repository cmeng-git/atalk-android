 ...
 * {@link MediaStreamStats2Impl} instead, where we can manage the complexity and consistency better.
 *
...
public class MediaStreamStatsImpl implements MediaStreamStats
{
	 ...
	 * 		The number of step which has not been computed since last update. In our case the
	 * 		number of packets received since the last computation.
	 * @param lastValue
	 ...
	 */
	private static double computeEWMA(long nbStepSinceLastUpdate, double lastValue,
			double newValue)
	{
	 ...
	 * @param intervalMs
	 * 		The number of milliseconds during which <tt>nbBytes</tt> bytes were sent or received.
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> to get the <tt>JitterBufferControl</tt> of
	...
	public static JitterBufferControl getJitterBufferControl(ReceiveStream receiveStream)
	{
		...
		if (ds instanceof PushBufferDataSource) {
			for (PushBufferStream pbs : ((PushBufferDataSource) ds).getStreams()) {
				JitterBufferControl pqc = (JitterBufferControl)
	...
	/**
	 * The detailed statistics about the RTCP reports sent and received by the associated local
	 * peer.
	 */
	...
	private int computeRTTInMs(RTCPFeedback feedback)
	{
		...
		// The RTCPFeedback may represents a Sender Report without any report blocks (and so
		// without LSR and DLSR)
		if (lsr > 0 && dlsr > 0) {
			...
			/*
			 * Even if ntprtd is negative we compute delayLong as it might round to zero.
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) lost in download since the beginning of the
	 * session.
	 *
	 ...
	 * 		The stream direction (DOWNLOAD or UPLOAD) of the stream from which this function
	 * 		retrieve the jitter.
	 * @return the last jitter average computed (in ms).
	 */
	...
	/**
	 * Returns the total number of Protocol Data Units (PDU) discarded by the FMJ packet queue
	 * since the beginning of the session. It's the sum over all <tt>ReceiveStream</tt>s of the
	 * <tt>MediaStream</tt>
	 *
	...
	/**
	 * Returns the total number of packets that are send or receive for this stream since the
	 * stream is created.
	 *
	...
	{
		return getNbPDU(StreamDirection.DOWNLOAD) + getDownloadNbPacketLost()
	...
	/**
	 * Returns the number of packets in the first <tt>JitterBufferControl</tt> found via
	 * <tt>getJitterBufferControls</tt>.
	 *
	 * @return the number of packets in the first <tt>JitterBufferControl</tt> found via
	 * <tt>getJitterBufferControls</tt>.
	 */
	 ...
	 * @param streamDirection
	 * 		The stream direction (DOWNLOAD or UPLOAD) of the
	 ...
	 * @param nbSteps
	 * 		The number of elapsed steps since the last number of loss update.
	 */
	private void updateNbDiscarded(long newNbDiscarded, long nbSteps)
	{
		...
		percentDiscarded
				= MediaStreamStatsImpl.computeEWMA(nbSteps, percentDiscarded, newPercentDiscarded);
	 ...
	 *
	 * @param report
	 * 		the received RTCP RR or SR report
	 */
