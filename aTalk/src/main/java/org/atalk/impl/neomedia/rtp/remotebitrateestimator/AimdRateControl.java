 ...
 * has changes or is unknown, we will switch to a "slow-start mode" where we increase
 * multiplicatively.
 * <p>
...
class AimdRateControl
{
			...
			case kRcIncrease: {
				...
				if (rateControlRegion == RateControlRegion.kRcNearMax) {
					...
					long multiplicativeIncreaseBps
                            = multiplicativeRateIncrease(nowMs, timeLastBitrateChange, currentBitrateBps);
	...
	private void changeRegion(RateControlRegion region, long nowMs)
	{
		...
		if (logger.isTraceEnabled()) {
			logger.trace("region_changed," + hashCode() + "," + nowMs + "," + region);
		}
	}
	...
	private void changeState(RateControlState newState, long nowMs)
	{
		...
		if (logger.isTraceEnabled()) {
			logger.trace("state_changed," + hashCode() + "," + nowMs + "," + rateControlState);
		}
	}
