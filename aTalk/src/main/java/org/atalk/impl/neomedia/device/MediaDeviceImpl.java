...
public class MediaDeviceImpl extends AbstractMediaDevice
{
	...
	/**
	 * The <tt>Logger</tt> used by <tt>MediaDeviceImpl</tt> and its instances for logging output.
	 */
	 ...
	 *
	 * @param captureDevice
	 *        the <tt>CaptureDevice</tt> which is to have its calls traced for debugging output
	 * @param logger
		...
		if (captureDevice instanceof PushBufferDataSource)
			captureDevice = new CaptureDeviceDelegatePushBufferDataSource(captureDevice)
			{
				...
				public void disconnect()
				{
					...
					if (logger.isTraceEnabled())
						logger
							.trace("Disconnected " + MediaDeviceImpl.toString(this.captureDevice));
				}
	 ...
	 *
	 * @param captureDevice
	 *        the <tt>CaptureDevice</tt> to get a human-readable representation of
	 ...
	 *        the <tt>CaptureDeviceInfo</tt> of the JMF <tt>CaptureDevice</tt> the new instance is
	...
	protected CaptureDevice createCaptureDevice()
	{
		...
		if (getDirection().allowsSending()) {
			...
			if (exception != null) {
				logger.error("Failed to create CaptureDevice" + " from CaptureDeviceInfo "
					+ captureDeviceInfo, exception);
			}
			else {
