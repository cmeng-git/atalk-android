...
class QuaGain
{
	...
	/*
	 * File : QUA_GAIN.C Used for the floating point version of both G.729 main body and G.729A
	 */
	 ...
	 */
	int qua_gain(float code[], float[] g_coeff, int l_subfr, FloatReference gain_pit,
		FloatReference gain_code, int tameflag)
	{
		...
		/*
		 * MA prediction is performed on the innovation energy (in dB with mean * removed). * An
		 * initial predicted gain, g_0, is first determined and the correction * factor alpha = gain
		 * / g_0 is quantized. * The pitch gain and the correction factor are vector quantized and
		 * the * mean-squared weighted error criterion is used in the quantizer search. * CS
		 * Codebook , fast pre-selection version *
		 */
		...
		if (tameflag == 1) {
			for (i = 0; i < NCAN1; i++) {
				for (j = 0; j < NCAN2; j++) {
					...
					if (g_pitch < GP0999) {
						...
						dist = g_pitch * g_pitch * g_coeff[0] + g_pitch * g_coeff[1] + g_code
							* g_code * g_coeff[2] + g_code * g_coeff[3] + g_pitch * g_code
							* g_coeff[4];
						if (dist < dist_min) {
							dist_min = dist;
							index1 = cand1 + i;
			...
			for (i = 0; i < NCAN1; i++) {
				for (j = 0; j < NCAN2; j++) {
					g_pitch = gbk1[cand1 + i][0] + gbk2[cand2 + j][0];
					g_code = gcode0 * (gbk1[cand1 + i][1] + gbk2[cand2 + j][1]);
					dist = g_pitch * g_pitch * g_coeff[0] + g_pitch * g_coeff[1] + g_code * g_code
						* g_coeff[2] + g_code * g_coeff[3] + g_pitch * g_code * g_coeff[4];
					if (dist < dist_min) {
						dist_min = dist;
						index1 = cand1 + i;
						index2 = cand2 + j;
