...
public abstract class Devices
{
	 ...
	 * whether the automatic selection of USB devices must be disabled. The default value is
	 * <tt>false</tt>.
	 */
	private static final String PROP_DISABLE_USB_DEVICE_AUTO_SELECTION
			= "neomedia.device.disableUsbDeviceAutoSelection";
	...
	/**
	 * The list of device ID/names saved by the configuration service and previously saved given
	...
	/**
	 * Adds a new device in the preferences (at the first active position if the isSelected argument
	 * is true).
	 *
	 * @param newsDeviceIdentifier
	 *        The identifier of the device to add int first active position of the preferences.
	 * @param isSelected
	 ...
	 *
	 * @param property
	 *        the name of the <tt>ConfigurationService</tt> property which specifies the user's
	 *        preference.
	 */
		...
		if (cfg != null) {
			...
			synchronized (devicePreferences) {
				if (deviceIdentifiersString != null) {
					...
					String[] deviceIdentifiers = deviceIdentifiersString.substring(2,
						deviceIdentifiersString.length() - 2).split("\", \"");
	...
	/**
	 * Renames the old fashioned identifier (name only), into new fashioned one (UID, or name +
	 * transport type).
	 *
	 * @param activeDevices
	 *        The list of the active devices.
	 ...
	 *
	 * @param property
	 *        the name of the <tt>ConfigurationService</tt> property into which the user's
	 ...
	 *        the selected active device
	 * @param save
	 *        <tt>true</tt> to save the choice in the configuration; <tt>false</tt>, otherwise
	 */
