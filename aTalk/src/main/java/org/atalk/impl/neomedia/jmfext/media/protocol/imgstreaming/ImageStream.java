...
public class ImageStream extends AbstractVideoPullBufferStream<DataSource>
{
	 ...
	 * 		its <tt>streams</tt>
	 * @param formatControl
	 * 		the <tt>FormatControl</tt> of the new instance which is to specify the format in which
			...
			throws IOException
	{
		...
		if (format instanceof AVFrameFormat) {
			...
			if (data != null) {
				...
				/*
				 * This can happen when we disconnect a monitor from computer before or during
				 * grabbing.
				 */
	...
	public byte[] readScreen(byte[] output, Dimension dim)
	{
		...
		/*
		 * OK native grabber failed or is not available, try with AWT Robot and convert it to the
		 * right format
		 * 
		 ...
		 * 
		 * Normally not of our supported platform (Windows (x86, x64), Linux (x86, x86-64), Mac
		 * OS X
		...
		if (screen != null) {
			...
			scaledScreen = ImgStreamingUtils.getScaledImage(screen, width, height,
	 ...
	 *
	 * @param displayIndex
	 * 		the index of the display to be used by this <tt>ImageStream</tt>
	 */
