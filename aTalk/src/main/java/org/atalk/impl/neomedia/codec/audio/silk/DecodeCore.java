...
public class DecodeCore
{
	 ...
	 *        Pulse signal.
	 */
	static void SKP_Silk_decode_core(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			if (sigtype == SIG_TYPE_VOICED) {
				...
				if ((k & (3 - (NLSF_interpolation_flag << 1))) == 0) {
					...
					MA.SKP_Silk_MA_Prediction(psDec.outBuf, start_idx + k
						* (psDec.frame_length >> 2), A_Q12, 0, FiltState, sLTP, start_idx,
					...
					for (i = 0; i < (lag + LTP_ORDER / 2); i++) {
						psDec.sLTP_Q16[sLTP_buf_idx - i - 1] = SKP_SMULWB(inv_gain_Q32,
					...
					if (gain_adj_Q16 != 1 << 16) {
						for (i = 0; i < (lag + LTP_ORDER / 2); i++) {
							psDec.sLTP_Q16[sLTP_buf_idx - i - 1] = SKP_SMULWW(gain_adj_Q16,
			...
			if (sigtype == SIG_TYPE_VOICED) {
				...
				for (i = 0; i < psDec.subfr_length; i++) {
					...
					pres_Q10[pres_Q10_offset + i] = (pexc_Q10[pexc_Q10_offset + i] + SigProcFIX
						.SKP_RSHIFT_ROUND(LTP_pred_Q14, 4));
			...
			/* Scale with Gain */
			for (i = 0; i < psDec.subfr_length; i++) {
				pxq[pxq_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(
	...
	private static void SKP_Silk_decode_short_term_prediction(int[] vec_Q10, int[] pres_Q10,
		int pres_Q10_offset, int[] sLPC_Q14, short[] A_Q12_tmp, int LPC_order, int subfr_length)
	{
		int i;
		int LPC_pred_Q10;
		int j;
		for (i = 0; i < subfr_length; i++) {
			...
			for (j = 10; j < LPC_order; j++) {
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, sLPC_Q14[MAX_LPC_ORDER + i - j - 1],
					A_Q12_tmp[j]);
			}
