...
public class SecurityProviderBlockCipherFactory implements BlockCipherFactory
{
	 ...
	 *        the name of the transformation
	 * @param providerName
	 *        the name of the <tt>java.security.Provider</tt> which provides the implementations of
		...
		throws Exception
	{
        return new BlockCipherAdapter(
            Cipher.getInstance(
                transformation.replaceFirst("<size>", Integer.toString(keySize * 8)), provider)
		);
	}
}
