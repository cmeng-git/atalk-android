...
class OveruseEstimator
{
	 ...
	 *
	 * @param matrix
	 *        the <tt>double</tt> two-dimensional matrix to create and return a deep copy of
	...
	/**
	 * Reduces the effects of allocations and garbage collection of the method {@code update}.
	 */
	 ...
	 * groups as defined by the InterArrival class. {@code currentHypothesis} should be the
	 ...
	 */
	public void update(long tDelta, double tsDelta, int sizeDelta, BandwidthUsage currentHypothesis,
			long systemTimeMs)
	{
		...
		if ((currentHypothesis == BandwidthUsage.kBwOverusing && offset < prevOffset)
			|| (currentHypothesis == BandwidthUsage.kBwUnderusing && offset > prevOffset)) {
		...
		double residualForUpdateNoiseEstimate = (Math.abs(residual) < maxResidual) ? residual
		...
		boolean positiveSemiDefinite = E[0][0] + E[1][1] >= 0
			&& E[0][0] * E[1][1] - E[0][1] * E[1][0] >= 0 && E[0][0] >= 0;
