...
public class DeviceConfiguration extends PropertyChangeNotifier implements PropertyChangeListener
{
	...
	/**
	 * The list of class names of custom <tt>Renderer</tt> implementations to be registered with
	 * JMF.
	 */
	...
	/**
	 * The default value to be used for the {@link #PROP_AUDIO_DENOISE} property when it does not
	 * have a value.
	 */
	public static final boolean DEFAULT_AUDIO_DENOISE = true;

	/**
	 * The default value to be used for the {@link #PROP_AUDIO_ECHOCANCEL} property when it does not
	 * have a value.
	 */
	 ...
	 * the order of 300 ms, so a filter length of 100 ms is a good choice (800 samples at 8000 Hz
	 * sampling rate).
	 */
...
//		new Dimension(352, 288),
	...
	/**
	 * Fixes the list of <tt>Renderer</tt>s registered with FMJ in order to resolve operating
	...
	private static void fixRenderers()
	{
		 ...
		 */
		PlugInManager.removePlugIn("com.sun.media.renderer.audio.JavaSoundRenderer",
	 ...
	 * {@link MediaServiceImpl#DISABLE_SET_AUDIO_SYSTEM_PNAME} at the time of the initialization of
	 * this instance.
	 */
				...
				if (cfg != null) {
					...
					if (locatorProtocol != null) {
						for (AudioSystem availableAudioSystem : availableAudioSystems) {
							if (locatorProtocol.equalsIgnoreCase(availableAudioSystem
								.getLocatorProtocol())) {
	...
	/**
	 * Detects capture devices configured through JMF and disable audio and/or video transmission if
	 * none were found.
	 */
	...
	private void extractConfiguredVideoCaptureDevices()
	{
		...
		if (NoneAudioSystem.LOCATOR_PROTOCOL.equalsIgnoreCase(videoCaptureDeviceString)) {
			...
			Format[] formats = new Format[] { new AVFrameFormat(),
				new VideoFormat(Constants.ANDROID_SURFACE), new VideoFormat(VideoFormat.RGB),
				new VideoFormat(VideoFormat.YUV), new VideoFormat(Constants.H264) };
	...
	public AudioSystem[] getAvailableAudioSystems()
	{
		...
		if ((audioSystems == null) || (audioSystems.length == 0))
			...
			for (AudioSystem audioSystem : audioSystems) {
				if (!NoneAudioSystem.LOCATOR_PROTOCOL.equalsIgnoreCase(audioSystem
					.getLocatorProtocol())) {
	 ...
	 * and represent acceptable values for
	 * {@link #setVideoCaptureDevice(CaptureDeviceInfo, boolean)}
	 *
	 * @param useCase
	 *        extract video capture devices that correspond to this <tt>MediaUseCase</tt>
	 ...
	 *
	 * @param ev
	 *        the <tt>PropertyChangeEvent</tt> to notify this <tt>PropertyChangeListener</tt> about
	...
	private void registerCustomRenderers()
	{
		...
		boolean audioSupportIsDisabled = !MediaServiceImpl
			.isMediaTypeSupportEnabled(MediaType.AUDIO);
		boolean videoSupportIsDisabled = !MediaServiceImpl
			.isMediaTypeSupportEnabled(MediaType.VIDEO);
		...
		for (String customRenderer : CUSTOM_RENDERERS) {
			...
			if (customRenderer.startsWith(".")) {
				customRenderer = "org.atalk.impl.neomedia" + ".jmfext.media.renderer"
					+ customRenderer;
			}

			/*
			 * Respect the MediaServiceImpl properties DISABLE_AUDIO_SUPPORT_PNAME and
			...
			if ((renderers == null) || !renderers.contains(customRenderer)) {
				try {
					Renderer customRendererInstance = (Renderer) Class.forName(customRenderer)
						.newInstance();

					PlugInManager.addPlugIn(customRenderer,
						customRendererInstance.getSupportedInputFormats(), null,
						PlugInManager.RENDERER);
					commit = true;
				}
				catch (Throwable t) {
					logger.error("Failed to register custom Renderer " + customRenderer
						+ " with JMF.", t);
				}
			}
		}
		...
		/*
		 * Just in case, bubble our JMF contributions at the top so that they are considered
		 * preferred.
		 */
	 ...
	 *
	 * @param device
	 *        a <tt>CaptureDeviceInfo</tt> describing device to be used by this
	 *        <tt>DeviceConfiguration</tt> for video capture.
	 * @param save
	...
	public void setVideoCaptureDevice(CaptureDeviceInfo device, boolean save)
	{
		if (videoCaptureDevice != device) {
			...
			if (save) {
				...
				if (cfg != null) {
					cfg.setProperty(PROP_VIDEO_DEVICE,
