...
{
	/**
	 * The number of milliseconds to be read from or written to a native CoreAudio stream in a
	 * single transfer of data.
	 */
	public static final int DEFAULT_MILLIS_PER_BUFFER = 20;

	/**
	 * The default value for the sample rate of the input and the output MacCoreaudio streams with
	 * which they are to be opened if no other specific sample rate is specified to the MacCoreaudio
	 * <tt>DataSource</tt> or <tt>MacCoreaudioRenderer</tt> that they represent.
	 */
	...
	public static native float getNominalSampleRate(String deviceUID, boolean isOutputStream,
		boolean isEchoCancel);

	public static native float getMinimalNominalSampleRate(String deviceUID,
		boolean isOutputStream, boolean isEchoCancel);
	...
	public static String getDefaultInputDeviceUID()
	{
		...
		String defaultInputDeviceUID = StringUtils.newString(defaultInputDeviceUIDBytes);
	...
	public static String getDefaultOutputDeviceUID()
	{
		...
		String defaultOutputDeviceUID = StringUtils.newString(defaultOutputDeviceUIDBytes);
	...
	public static native long startStream(String deviceUID, Object callback, float sampleRate,
		int nbChannels, int bitsPerChannel, boolean isFloat, boolean isBigEndian,
		boolean isNonInterleaved, boolean isInput, boolean isEchoCancel);
