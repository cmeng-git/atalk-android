...
{
	/**
	 * The frame rate to be assumed by <tt>JNIEncoder</tt> instance in the absence of any other
			...
			throws ResourceUnavailableException
	{
		...
		if (FFmpeg.avcodec_open2(avcontext, avcodec) < 0) {
			throw new ResourceUnavailableException("Could not open codec. (size= "
					+ width + "x" + height + ")");
		}
	...
	public synchronized int process(Buffer inBuffer, Buffer outBuffer)
	{
		...
		/*
		 * Do not always allocate a new data array for outBuffer, try to reuse the existing one if
		  * it is suitable.
		 */
