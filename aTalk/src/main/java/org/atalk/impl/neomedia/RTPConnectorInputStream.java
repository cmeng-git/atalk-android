...
public abstract class RTPConnectorInputStream<T> implements PushSourceStream
{
	 ...
	 *
	 * @param thread
	 * 		the <tt>Thread</tt> to set the specified <tt>priority</tt> on
	 * @param priority
	 * 		the priority to set on the specified <tt>thread</tt>
	 */
	...
	protected RTPConnectorInputStream(T socket)
	{
		...
		{
			/**
			 * Used for debugging. As we don't log every packet, we must count them and decide
			 * which to log.
			 */
	 ...
	 *
	 * @param p
	 * 		the <tt>DatagramPacket</tt> to be considered for acceptance by all
	...
	private boolean accept(DatagramPacket p)
	{
		...
		if (enabled) {
			...
			if (logger.isTraceEnabled() && !closed) {
				logger.trace("Will drop received packet because this is disabled: "
						+ p.getLength() + " bytes.");
			}
		}
		return accept;
	 ...
	 * 		the <tt>DatagramPacketFilter</tt> which allows dropping <tt>DatagramPacket</tt>s
	...
	{
		datagramPacketFilters = ArrayUtils.add(datagramPacketFilters, DatagramPacketFilter.class,
	 ...
	 * 		<tt>flags</tt>
	 * @param data
	 * 		the <tt>byte[]</tt> that we'd like to copy the content of the packet to.
	 * @param offset
	 * 		the position where we are supposed to start writing in <tt>data</tt>.
	 * @param length
	 * 		the number of <tt>byte</tt>s available for writing in <tt>data</tt>.
			...
			try {
				...
				if (length < pktLength) {
					...
					if (pktBuffer == null) {
						...
						System.arraycopy(pkt.getBuffer(), pkt.getOffset(), data, offset,
								pktLength);
						if (buffer != null)
	 ...
	 * 		the <tt>byte[]</tt> that we'd like to copy the content of the packet to.
	 * @param offset
	 * 		the position where we are supposed to start writing in <tt>buffer</tt>.
	 * @param length
	 * 		the number of <tt>byte</tt>s available for writing in <tt>buffer</tt>.
	 * @return the number of bytes read
	 * @throws IOException
	 * 		if <tt>length</tt> is less than the size of the packet.
	 */
	 ...
	 * 		the <tt>transferHandler</tt> that this connector should be notifying when new data is
	 * 		available for reading.
	 */
	@Override
	 ...
	 *
	 * @param pkts
	 * 		the set of <tt>RawPacket</tt>s to push out of this <tt>PushSourceStream</tt>
	 */
