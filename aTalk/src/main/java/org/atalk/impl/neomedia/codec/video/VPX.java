...
public class VPX
{
	 ...
	 * @param iface
	 * 		Interface to be used. Has to be one of the <tt>VPX.INTERFACE_*</tt> constants.
	 * @param cfg
	 * 		Pointer to a pre-allocated <tt>vpx_codec_dec_cfg_t</tt>, may be 0.
	 * @param flags
	 * 		Flags.
	 ...
	 * @param iter
	 * 		Iterator storage, initialized by setting its first element to 0.
	 * @return Pointer to a <tt>vpx_image_t</tt> describing the decoded frame, or 0 if no more
	 ...
	 * 		Interface to be used. Has to be one of the <tt>VPX.INTERFACE_*</tt> constants.
	 * @param cfg
	 * 		Pointer to a pre-allocated <tt>vpx_codec_enc_cfg_t</tt>, may be 0.
	 * @param flags
	 * 		Flags.
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The error code can be
	 * converted to a <tt>String</tt> with {@link VPX#codec_err_to_string(int)}
	...
	/**
	 * @param context
	 * 		Pointer to the codec context on which to set the configuration
	 * @param cfg
	 ...
	 * <tt>img</tt> has to have all of its other parameters (format, dimensions, strides) already
	 * set.
	 * <p>
	 ...
	 *
	 * @param pkt
	 * 		Pointer to the <tt>vpx_codec_cx_pkt_t</tt> to return the <tt>kind</tt> of.
	 ...
	 * @param si_ptr
	 * 		Pointer to a <tt>vpx_codec_stream_info_t</tt> which will
