...
import org.atalk.service.neomedia.control.*;
...
public class MediaDeviceSession extends PropertyChangeNotifier
{
	 ...
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt> the use of which by a <tt>MediaStream</tt> is to be
	 ...
	 * 		<tt>true</tt> to have this instance dispose of its associated player upon closing;
	 ...
	 *
	 * @param ssrc
	 * 		the new SSRC identifier that we'd like to add to the array of <tt>ssrc</tt>
...
//            }
		...
		VideoFormat result = new VideoFormat(null, new Dimension(width, height),
	 ...
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt> to be checked for suitability to become the
	...
	protected DataSource createCaptureDevice()
	{
		...
		if (captureDevice != null) {
			MuteDataSource muteDataSource
					= AbstractControls.queryInterface(captureDevice, MuteDataSource.class);
			if (muteDataSource == null) {
				...
				muteDataSource = AbstractControls.queryInterface(captureDevice,
	 ...
	 * media to be sent to the remote peer. Allows extenders to override. The default
	 * implementation
	 * returns {@link ContentDescriptor#RAW_RTP}.
	 *
	 * @param processor
	 * 		the <tt>Processor</tt> of captured media to be sent to the remote peer which is to
	 ...
	 *
	 * @param player
	 * 		the <tt>Player</tt> which is to utilize the initialized/returned <tt>Renderer</tt>
	 * @param trackControl
	 * 		the <tt>TrackControl</tt> which represents the media to be played back (and,
	...
	private void disconnectCaptureDevice()
	{
		if (captureDevice != null) {
			/*
			 * As reported by Carlos Alexandre, stopping before disconnecting resolves a slow
			 * disconnect on Linux.
			 */
			try {
				captureDevice.stop();
	 ...
	 *
	 * @param formats
	 * 		the array of <tt>Format</tt>s to be searched for a match to the specified
	 * 		<tt>format</tt>
	 * @param format
	 * 		the <tt>Format</tt> to search for a match in the specified <tt>formats</tt>
	 * @return the first element of <tt>formats</tt> which matches <tt>format</tt> i.e. is of the
	 * same encoding
	 */
		...
		if ((captureDevice != null) && !captureDeviceIsConnected) {
			...
			try {
				...
				logger.warn("Failed to setup an optimized media codec chain"
						+ " by setting the output Format" + " on the input CaptureDevice", t);
			}
			...
			if (exception == null)
				...
				logger.error("Failed to connect to "
						+ MediaStreamImpl.toString(captureDevice), exception);
				captureDevice = null;
			}
		}
		return captureDevice;
	...
	/**
	 * Gets the <tt>MediaFormat</tt> in which this instance captures media from its associated
	 * <tt>MediaDevice</tt>.
	 *
	 * @return the <tt>MediaFormat</tt> in which this instance captures media from its associated
	 * <tt>MediaDevice</tt>
	 */
	...
	public DataSource getOutputDataSource()
	{
		...
		if ((processor == null)
				|| ((processor.getState() < Processor.Realized) && !waitForState(processor,
				Processor.Realized)))
	 ...
	 *
	 * @param dataSource
	 * 		the <tt>DataSource</tt> to get the information related to the playback of
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> to get the information related to the playback of
	 ...
	 *
	 * @param ssrc
	 * 		the SSRC of the <tt>ReceiveStream</tt> to get the rendering the <tt>Player</tt> of
	 * @return the <tt>Player</tt> rendering the <tt>ReceiveStream</tt> with the specified
	 * <tt>ssrc</tt>
	 */
	 ...
	 *
	 * @return a <tt>long[]</tt> array of SSRC identifiers that this device session is handling
	 * streams from.
	 */
	...
	public boolean isMute()
	{
		...
		MuteDataSource muteDataSource = AbstractControls.queryInterface(captureDevice,
				MuteDataSource.class);
	 ...
	 * 		the <tt>DataSource</tt> which has been added for playback on the represented
	 * 		<tt>MediaDevice</tt>
	 */
	 ...
	 *
	 * @param playbackDataSource
	 * 		the <tt>DataSource</tt> which has been removed from playback on the represented
	 * 		<tt>MediaDevice</tt>
	 */
	 ...
	 * <tt>ConfigureCompleteEvent</tt>. Allows extenders to carry out additional processing on the
	 * <tt>Player</tt>.
	 *
	 * @param player
	 * 		the <tt>Player</tt> which is the source of a <tt>ConfigureCompleteEvent</tt>
	 */
	...
	/**
	 * Gets notified about <tt>ControllerEvent</tt>s generated by a specific <tt>Player</tt> of
	 * remote content.
	 * <p>
	 ...
	 *
	 * @param ev
	 * 		the <tt>ControllerEvent</tt> specifying the <tt>Controller</tt> which is the source of
	...
	/**
	 * Notifies this instance that a specific <tt>Player</tt> of remote content has generated a
	 * <tt>RealizeCompleteEvent</tt>. Allows extenders to carry out additional processing on the
	 * <tt>Player</tt>.
	 *
	 * @param player
	 * 		the <tt>Player</tt> which is the source of a <tt>RealizeCompleteEvent</tt>
	 */
	...
	protected void processorControllerUpdate(ControllerEvent ev)
	{
		if (ev instanceof ConfigureCompleteEvent) {
			...
			for (FormatParametersAwareCodec fpac : getAllTrackControls(
				...
				Map<String, String> formatParameters = format == null
						? null : format.getFormatParameters();
	 ...
	 *
	 * @param ssrc
	 * 		the SSRC identifier that we'd like to remove from the array of <tt>ssrc</tt>
	 * 		identifiers stored by this session.
	 */
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> which has been added to the list of playbacks of
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> which has been removed from the list of playbacks of
	 * 		<tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s performed by respective
	 * 		<tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance
	 */
			...
			MediaFormatImpl<? extends Format> mediaFormat)
	{
		...
		if (format instanceof AudioFormat) {
			...
			double sampleRate = OSUtils.IS_ANDROID ? audioFormat.getSampleRate()
					: Format.NOT_SPECIFIED;

			if (sampleRate != Format.NOT_SPECIFIED) {
				FormatControl formatControl
						= (FormatControl) captureDevice.getControl(FormatControl.class.getName());
	 ...
	 *
	 * @param format
	 * 		the <tt>MediaFormat</tt> in which this <tt>MediaDeviceSession</tt> is to output the
	...
	public void setFormat(MediaFormat format)
	{
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Set format " + this.format + " on " + getClass().getSimpleName() + " "
					+ hashCode());
		}
	 ...
	 *
	 * @param processor
	 * 		the <tt>Processor</tt> to set the output <tt>MediaFormatImpl</tt> of
	 * @param mediaFormat
	 ...
	 * <tt>MediaDeviceSession</tt> to the remote peer. Allows extenders to override the set
	 * procedure and to detect when the JMF <tt>Format</tt> of the specified <tt>TrackControl</tt>
	 * changes.
	 *
	 * @param trackControl
	 * 		the <tt>TrackControl</tt> to set the JMF <tt>Format</tt> of
	 * @param mediaFormat
	 * 		the <tt>MediaFormatImpl</tt> to be set on the specified <tt>TrackControl</tt>. Though
	 ...
	 * @param format
	 * 		the JMF <tt>Format</tt> to be set on the specified <tt>TrackControl</tt>. Though
	 ...
	 *
	 * @param mute
	 * 		<tt>true</tt> to set this <tt>MediaDeviceSession</tt> to output "silence" instead of
	...
	{
		InbandDTMFDataSource inbandDTMFDataSource = AbstractControls.queryInterface(captureDevice,
	 ...
	 *
	 * @param playbackDataSource
	 * 		the <tt>DataSource</tt> which to be added to the list of playbacks of
	 * 		<tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s performed by respective
	 * 		<tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance
	 */
		...
		try {
			...
			if (playback == null) {
				if (playbackDataSource instanceof ReceiveStreamPushBufferDataSource) {
					ReceiveStream receiveStream = ((ReceiveStreamPushBufferDataSource)
							playbackDataSource).getReceiveStream();
	 ...
	 *
	 * @param playbackDataSource
	 * 		the <tt>DataSource</tt> which to be removed from the list of playbacks of
	 * 		<tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s performed by respective
	 * 		<tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 *
	 * @param processor
	 * 		the JMF <tt>Processor</tt> which is to transcode {@link #captureDevice} into the
	 * 		format of this instance
	 */
	private void setProcessor(Processor processor)
	{
		if (this.processor != processor) {
			...
			/*
			 * Since the processor has changed, its output DataSource known to the public has also
			 * changed.
			 */
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> which to be added to the list of playbacks of
	 * 		<tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s performed by respective
	 * 		<tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance
	 */
			...
			if (getPlayback(receiveStream) == null) {
				...
				if (receiveStreamDataSource != null) {
					if (receiveStreamDataSource instanceof PushBufferDataSource) {
						...
						logger.warn("Adding ReceiveStream with DataSource not of"
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> which to be removed from the list of playbacks of
	 * 		<tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s performed by respective
	 * 		<tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 *
	 * @param newSsrcList
	 * 		that SSRC array that we'd like to replace the existing SSRC list with.
	 */
	 ...
	 * call this implementation in order to ensure the proper execution of this
	 ...
	 * @param state
	 * 		the state as defined by the respective <tt>Processor</tt> state constants to wait
	 ...
	 *
	 * @param deviceSession
	 * 		the <tt>MediaDeviceSession</tt> to copy the playback part of into this instance
	 */
	...
	private static class Playback
	{
		 ...
		 *
		 * @param dataSource
		 * 		the <tt>DataSource</tt> the information related to the playback of which is to be
		 * 		represented by the new instance
		 */
		 ...
		 *
		 * @param receiveStream
		 * 		the <tt>ReceiveStream</tt> the information related to the playback of which is to
	 ...
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> whose <tt>Processor</tt>'s <tt>TrackControl</tt>s are to be
	 * 		searched.
	 * @param controlType
	 * 		the name of the class to search for.
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 ...
	 * 		the name of the class to search for.
	 * @param processor
	 * 		the <tt>Processor</tt> whose <tt>TrackControls</tt>s will be searched.
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 */
