...
{
	static void SKP_Silk_NSQ_del_dec(SKP_Silk_encoder_state psEncC, /* I/O Encoder State */
	...
	)
	{
		...
		if (psEncCtrlC.sigtype == SIG_TYPE_VOICED) {
			for (k = 0; k < NB_SUBFR; k++) {
				decisionDelay = (decisionDelay < (psEncCtrlC.pitchL[k] - LTP_ORDER / 2 - 1) ? decisionDelay
					: (psEncCtrlC.pitchL[k] - LTP_ORDER / 2 - 1));
			}
		}
		...
		for (i = 0; i < decisionDelay; i++) {
			last_smple_idx = (last_smple_idx - 1) & DECISION_DELAY_MASK;
			q_tmp[q_tmp_offset + i - decisionDelay] = (byte) (psDD.Q_Q10[last_smple_idx] >> 10);
			pxq[pxq_offset + i - decisionDelay] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
				.SKP_RSHIFT_ROUND(
	 ...
	 * @param decisionDelay
	 */
	static void SKP_Silk_noise_shape_quantizer_del_dec(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		...
		int a_Q12_offset, final short b_Q14[], /* I Long term prediction coefs */
	...
	)
	{
		...
		for (i = 0; i < length; i++) {
			...
			if (sigtype == SIG_TYPE_VOICED) {
				/* Unrolled loop */
				LTP_pred_Q14 = SKP_SMULWB(pred_lag_ptr[pred_lag_ptr_offset + 0],
					b_Q14[b_Q14_offset + 0]);
				LTP_pred_Q14 = SKP_SMLAWB(LTP_pred_Q14, pred_lag_ptr[pred_lag_ptr_offset - 1],
					b_Q14[b_Q14_offset + 1]);
				LTP_pred_Q14 = SKP_SMLAWB(LTP_pred_Q14, pred_lag_ptr[pred_lag_ptr_offset - 2],
					b_Q14[b_Q14_offset + 2]);
				LTP_pred_Q14 = SKP_SMLAWB(LTP_pred_Q14, pred_lag_ptr[pred_lag_ptr_offset - 3],
					b_Q14[b_Q14_offset + 3]);
				LTP_pred_Q14 = SKP_SMLAWB(LTP_pred_Q14, pred_lag_ptr[pred_lag_ptr_offset - 4],
			...
			if (lag > 0) {
				...
				n_LTP_Q14 = SKP_SMLAWT(n_LTP_Q14, shp_lag_ptr[shp_lag_ptr_offset - 1],
					HarmShapeFIRPacked_Q14);
			...
			for (k = 0; k < nStatesDelayedDecision; k++) {
				...
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 1],
					a_Q12[a_Q12_offset + 1]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 2],
					a_Q12[a_Q12_offset + 2]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 3],
					a_Q12[a_Q12_offset + 3]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 4],
					a_Q12[a_Q12_offset + 4]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 5],
					a_Q12[a_Q12_offset + 5]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 6],
					a_Q12[a_Q12_offset + 6]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 7],
					a_Q12[a_Q12_offset + 7]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 8],
					a_Q12[a_Q12_offset + 8]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - 9],
					a_Q12[a_Q12_offset + 9]);

				for (j = 10; j < predictLPCOrder; j++) {
					LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psLPC_Q14[psLPC_Q14_offset - j],
						a_Q12[a_Q12_offset + j]);
				}

				/* Noise shape feedback */
				assert ((shapingLPCOrder & 1) == 0); /* check that order is even */
				// assert( ( (SKP_int64)AR_shp_Q13 & 3 ) == 0 ); /* check that array starts at
				// 4-byte aligned address */
				assert (shapingLPCOrder >= 12); /* check that unrolling works */
				/* Partially unrolled */
				n_AR_Q10 = SKP_SMULWB(psLPC_Q14[psLPC_Q14_offset + 0],
					AR_shp_Q13[AR_shp_Q13_offset + 0]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 1],
					AR_shp_Q13[AR_shp_Q13_offset + 1]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 2],
					AR_shp_Q13[AR_shp_Q13_offset + 2]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 3],
					AR_shp_Q13[AR_shp_Q13_offset + 3]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 4],
					AR_shp_Q13[AR_shp_Q13_offset + 4]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 5],
					AR_shp_Q13[AR_shp_Q13_offset + 5]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 6],
					AR_shp_Q13[AR_shp_Q13_offset + 6]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 7],
					AR_shp_Q13[AR_shp_Q13_offset + 7]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 8],
					AR_shp_Q13[AR_shp_Q13_offset + 8]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 9],
					AR_shp_Q13[AR_shp_Q13_offset + 9]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 10],
					AR_shp_Q13[AR_shp_Q13_offset + 10]);
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - 11],
					AR_shp_Q13[AR_shp_Q13_offset + 11]);

				for (j = 12; j < shapingLPCOrder; j++) {
					n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, psLPC_Q14[psLPC_Q14_offset - j],
			...
			smpl_buf_idx[0] = (smpl_buf_idx[0] - 1) & DECISION_DELAY_MASK; /*
																			 * Index to newest
																			 * samples
																			 */
			last_smple_idx = (smpl_buf_idx[0] + decisionDelay) & DECISION_DELAY_MASK; /*
																					 * Index to
																					 * decisionDelay
																					 * old samples
																					 */
	 ...
	 *        Pitch lag
	 */
	static void SKP_Silk_nsq_del_dec_scale_states(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		NSQDelDecStruct psDelDec[], /* I/O Delayed decision states */
	...
	)
	{
		...
		if (inv_gain_Q16 != NSQ.prev_inv_gain_Q16) {
			...
			scale_length = scale_length - SKP_SMULBB(NB_SUBFR - (subfr + 1), length); /*
																					 ...
																					 * frame
																					 */
			scale_length = Math.max(scale_length, lag + LTP_ORDER); /*
	 ...
	 *        Index to LPC buffer
	 */
	static void SKP_Silk_copy_del_dec_state(NSQDelDecStruct DD_dst, /* I Dst del dec state */
