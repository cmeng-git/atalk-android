...
public class DtmfRawPacket extends RawPacket
{
	 ...
	 *
	 * @param buffer
	 * 		the <tt>byte</tt> array that we should use to store packet content
	 * @param offset
	 * 		the index where we should start using the <tt>buffer</tt>.
	 * @param length
	 * 		Length of the packet's data.
	 * @param payload
	 * 		the payload that has been negotiated for telephone events by our signaling modules.
	 */
			...
			volume)
	{
		if (logger.isTraceEnabled()) {
			logger.trace("DTMF send on RTP, code : " + code + " duration = " + duration
					+ " timestamps = " + timestamp + " Marker = " + marker + " End = " + end);
		}
	 ...
	 * @param duration
	 * 		int increments for each dtmf sending updates, stay unchanged at the end for the 2 last
	 * 		packets.
	 * @param volume
