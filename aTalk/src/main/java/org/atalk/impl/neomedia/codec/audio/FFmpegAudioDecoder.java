...
public class FFmpegAudioDecoder extends AbstractFFmpegAudioCodec
{
	...
	public static void assertFindAVCodec(int codecID)
	{
		if (FFmpeg.avcodec_find_decoder(codecID) == 0) {
			throw new RuntimeException("Could not find FFmpeg decoder " + codecIDToString(codecID)
				+ "!");
		}
	}
	...
	protected synchronized int doProcess(Buffer inBuf, Buffer outBuf)
	{
		...
		int consumedInLen = FFmpeg.avcodec_decode_audio4(avctx, frame, got_frame, avpkt);
		...
		if ((consumedInLen < 0) || (consumedInLen > inLen)) {
			...
			if (got_frame[0]) {
				...
				if (data0 == 0) {
					...
					java.nio.FloatBuffer floats = java.nio.ByteBuffer.wrap(bytes)
						.order(java.nio.ByteOrder.nativeOrder()).asFloatBuffer();
					int outLen = floats.limit() * 2;
					byte[] out = validateByteArraySize(outBuf, outLen, false);

					outLen = 0;
					for (int floatI = 0, floatEnd = floats.limit(); floatI < floatEnd; ++floatI) {
						int s16 = Math.round(floats.get(floatI) * Short.MAX_VALUE);
					...
					outBuf.setDuration(20L /* milliseconds */
