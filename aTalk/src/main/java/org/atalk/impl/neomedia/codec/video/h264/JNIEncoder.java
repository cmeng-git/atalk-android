...
public class JNIEncoder extends AbstractCodec implements RTCPFeedbackMessageListener
{
	...
	/**
	 * The default value of the {@link #DEFAULT_INTRA_REFRESH_PNAME} <tt>ConfigurationService</tt>
	 * property.
	 */
	...
	/**
	 * The name of the boolean <tt>ConfigurationService</tt> property which specifies whether
	 * Periodic Intra Refresh is
	 * to be used by default. The default value is <tt>true</tt>. The value may be overridden by
	 * {@link #setAdditionalCodecSettings(Map)}.
	 */
	public static final String DEFAULT_INTRA_REFRESH_PNAME
			= "neomedia.codec.video.h264.defaultIntraRefresh";

	/**
	 * The default maximum GOP (group of pictures) size i.e. the maximum interval between
	 * keyframes. The x264 library
	 * defaults to 250.
	 */
	 ...
	 * encoding is naturally a keyframe and, for the sake of clarity, the initial value is
	 * <tt>true</tt>.
	 */
	private boolean forceKeyFrame = true;

	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>JNIEncoder</tt> to control its key
	 * frame-related logic.
	 */
	...
	/**
	 * The time in milliseconds of the last request for a key frame from the remote peer to this
	 * local peer.
	 */
	private long lastKeyFrameRequestTime = System.currentTimeMillis();

	/**
	 * The packetization mode to be used for the H.264 RTP payload output by this
	 * <tt>JNIEncoder</tt> and the associated packetizer. RFC 3984 "RTP Payload Format for H.264
	 * Video" says that "[w]hen the value of packetization-mode is equal to 0 or
	 * packetization-mode is not present, the single NAL mode, as defined in section
	 * 6.2 of RFC 3984, MUST be used."
	 */
	...
	private Format[] getMatchingOutputFormats(Format inputFormat)
	{
		...
		String[] packetizationModes = (this.packetizationMode == null) ? new String[]{"0", "1"} :
	 ...
	 *
	 * @return <tt>true</tt> if the encoding of <tt>avFrame</tt> is to produce a keyframe;
	...
	/**
	 * Notifies this <tt>JNIEncoder</tt> that the remote peer has requested a key frame from this
	 * local peer.
	 *
	 * @return <tt>true</tt> if this <tt>JNIEncoder</tt> has honored the request for a key frame;
	 * otherwise, <tt>false</tt>
	 */
			...
			throws ResourceUnavailableException
	{
		...
		if (FFmpeg.avcodec_open2(avctx, avcodec,
				...
				"tune", "zerolatency") < 0) {
			throw new ResourceUnavailableException(
					"Could not open H.264 encoder. (size= " + width + "x" + height + ")");
		}
			...
			switch (ev.getFeedbackMessageType()) {
				...
				case RTCPFeedbackMessageEvent.FMT_FIR:
					if (logger.isTraceEnabled()) {
						logger.trace(
								"Scheduling a key-frame, because we received an RTCP PLI or FIR.");
					}
					keyFrameRequest();
					break;
				default:
	 ...
	 *
	 * @param format
	 * 		the <tt>Format</tt> of media data to set on this <tt>Codec</tt>
	 ...
	 * 		the <tt>KeyFrameControl</tt> to be used by this <tt>JNIEncoder</tt> as a means of
	 * 		control over its key frame-related logic
	 */
