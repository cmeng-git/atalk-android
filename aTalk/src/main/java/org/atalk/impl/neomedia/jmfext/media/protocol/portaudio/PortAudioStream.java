...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>PortAudioStream</tt> class and its instances for logging
	 * output.
	 */
	 ...
	 * {@link #readIsMalfunctioningSince}.
	 */
		...
		boolean audioQualityImprovement)
	{
		...
		AudioSystem2 audioSystem = (AudioSystem2) AudioSystem
			.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_PORTAUDIO);
		...
		throws IOException
	{
		...
		try {
			...
			stream = Pa
				.OpenStream(inputParameters, 0 /* outputParameters */, sampleRate,
					framesPerBuffer, Pa.STREAM_FLAGS_CLIP_OFF | Pa.STREAM_FLAGS_DITHER_OFF, null /* streamCallback */);
		}
	 ...
	 *
	 * @param deviceID
	 *        The ID of the device used to be read trough this PortAudioStream. This String contains
			...
			if (stream != 0) {
				...
				try {
					...
					if ((errorCode == Pa.paTimedOut)
						|| (Pa.HostApiTypeId.paMME.equals(pae.getHostApiType()) && (errorCode == Pa.MMSYSERR_NODRIVER))) {
						closed = true;
					}
	 ...
	 *        <tt>true</tt> if <tt>Pa_ReadStream</tt> is malfunctioning; otherwise, <tt>false</tt>
	 */
