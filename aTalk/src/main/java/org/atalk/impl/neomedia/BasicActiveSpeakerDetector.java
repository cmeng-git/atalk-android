 ...
 * {@inheritDoc}
 * <p>
 * For each stream, it keeps the last {@link History#SIZE} levels and uses them to compute a score.
 * </p>
 * <p>
 * At all times one stream (indicated in {@link #active}) is considered active, while the rest are
 * considered 'competing'.
 * </p>
 * <p>
 * When a new audio level is received for a stream, its score is recomputed, and the scores of all
 * streams are examined in order to determine if one of the competing streams should replace the
 * then active stream.
 * </p>
 * <p>
 * In order to be eligible to replace the active stream, a competing stream has to: 1. Have a score
 * at least {@link #ACTIVE_COEF} times as much as the score of the currently active stream. 2. Have
 * score at least {@link #MIN_NEW_ACTIVE_SCORE}. 3. Have had its audio level updated in the last
 * {@link #MAX_NEW_ACTIVE_SILENT_INTERVAL} milliseconds. 4. Have updated its audio level at least
 * {@link #MIN_NEW_ACTIVE_SIZE} times.
 * </p>
 * <p>
 * In order to actually replace the active, a competing stream has to have the highest score amongst
 * all eligible streams.
 * </p>
 * <p>
 * These rules and the constant values were chosen based on a few not very thorough tests in a
 * conference. Some justification for the rules: 1. Helps to avoid often changing the active when
 * there are two streams with similar levels. 2. This is to prevent switching the active stream away
 * during times of &quot;silence&quot;. Without this threshold we observed the following: someone's
 * microphone generates noise with levels above the levels of the active speaker. When the active
 * speaker pauses speaking, the one with the higher noise becomes active. 3. This is for the case
 * when someone quits the conference shouting ;) 4. This is because of the way we compute scores.
