...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>JNIEncoder</tt> class and its instances for logging
	 * output.
	 */
		...
		throws ResourceUnavailableException
	{
		...
		* cfg.getInt(Constants.PROP_OPUS_BITRATE, 32);
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Encoder settings: audio bandwidth " + bw + ", bitrate "
				+ Opus.encoder_get_bitrate(encoder) + ", DTX " + Opus.encoder_get_dtx(encoder)
	 ...
	 *
	 * @param inBuffer
	 *        the <tt>Buffer</tt> from which the media to be encoded is to be read
	 * @param outBuffer
	 *        the <tt>Buffer</tt> into which the encoded media is to be written
	...
	{
		if (opened) {
			Opus.encoder_set_packet_loss_perc(encoder, (percentage > minPacketLoss) ? percentage
				: minPacketLoss);
			if (logger.isTraceEnabled()) {
				logger.trace("Updating expected packet loss: " + percentage + " (minimum "
					+ minPacketLoss + ")");
			}
		}
	}
	...
	public void setFormatParameters(Map<String, String> fmtps)
	{
		...
		if (maxaveragebitrate > 0) {
			Opus.encoder_set_bitrate(encoder, (maxaveragebitrate < bitrate) ? maxaveragebitrate
				: bitrate);
		}
