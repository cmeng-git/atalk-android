...
public class NeomediaActivator implements BundleActivator
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>NeomediaActivator</tt> class and its instances for
	 * logging output.
	 */
	...
	/**
	 * The name of the notification pop-up event displayed when the device configuration has
	 * changed.
	 */
	 ...
	 *
	 * @param bundleContext
	 *        the context in which the neomedia bundle is to start executing
	 * @throws Exception
	 *         if an error occurs while starting the execution of the neomedia bundle in the
	 *         specified context
	 */
		...
		throws Exception
	{
		...
		// if ((cfg == null) || !cfg.getBoolean(AUDIO_CONFIG_DISABLED_PROP,
		// false))
		// {
		...
		// if ((cfg == null) || !cfg.getBoolean(VIDEO_CONFIG_DISABLED_PROP,
		// false))
		// {
		// bundleContext.registerService(
		// ConfigurationForm.class.getName(),
		...
		// if ((cfg == null) || !cfg.getBoolean(H264_CONFIG_DISABLED_PROP,
		// false))
		// {
		...
		// if ((cfg == null) || !cfg.getBoolean(ZRTP_CONFIG_DISABLED_PROP,
		// false))
		// {
		...
		bundleContext.registerService(AudioNotifierService.class.getName(), audioNotifierService,
			null);
	 ...
	 *
	 * @param bundleContext
	 *        the context in which the neomedia bundle is to stop executing
	 * @throws Exception
	 *         if an error occurs while stopping the execution of the neomedia bundle in the
	 *         specified context
	 */
		...
		throws Exception
	{
		try {
			if (deviceConfigurationPropertyChangeListener != null) {
				mediaServiceImpl.getDeviceConfiguration().removePropertyChangeListener(
					deviceConfigurationPropertyChangeListener);
				if (deviceConfigurationPropertyChangeListener != null) {
					deviceConfigurationPropertyChangeListener
	...
	public static PacketLoggingService getPacketLogging()
	{
		if (packetLoggingService == null) {
			packetLoggingService = ServiceUtils.getService(bundleContext,
																					 ...
																					 */
	{
		 ...
		 *
		 * @param enable
		 *        True to register to the popup message notifcation handler. False to unregister.
		 */
		public void managePopupMessageListenerRegistration(boolean enable)
		{
			Iterator<NotificationHandler> notificationHandlers = notificationService
				.getActionHandlers(
					net.java.sip.communicator.service.notification.NotificationAction.ACTION_POPUP_MESSAGE)
				.iterator();
		 ...
		 *
		 * @param event
		 *        The property change event which may concern the audio device.
		 */
				...
				if (notificationService != null) {
					...
					notificationService.fireNotification(DEVICE_CONFIGURATION_HAS_CHANGED,
						resources.getI18NString("impl.media.configform"
							+ ".AUDIO_DEVICE_CONFIG_CHANGED"), resources
							.getI18NString("impl.media.configform"
								+ ".AUDIO_DEVICE_CONFIG_MANAGMENT_CLICK"), null, extras);
				}
			}
		}
		 ...
		 * 
		 * @param evt
		 *        the event triggered when user clicks on the systray popup message
		 */
