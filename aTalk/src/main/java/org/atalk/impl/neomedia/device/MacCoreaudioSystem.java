...
import org.atalk.impl.neomedia.jmfext.media.renderer.audio.MacCoreaudioRenderer;
...
{
	/**
	 * The protocol of the <tt>MediaLocator</tt>s identifying MacCoreaudio <tt>CaptureDevice</tt>s.
	 */
	private static final String LOCATOR_PROTOCOL = LOCATOR_PROTOCOL_MACCOREAUDIO;

	/**
	 * The <tt>Logger</tt> used by the <tt>MacCoreaudioSystem</tt> class and its instances for
	 * logging output.
	 */
	private static final Logger logger = Logger.getLogger(MacCoreaudioSystem.class);

	/**
	 * Gets a sample rate supported by a MacCoreaudio device with a specific device index with which
	 * it is to be registered with JMF.
	 *
	 * @param input
	 *        <tt>true</tt> if the supported sample rate is to be retrieved for the MacCoreaudio
	 *        device with the specified device index as an input device or <tt>false</tt> for an
	 *        output device
	 * @param deviceUID
	 ...
	 *        sample format
	 * @param isEchoCancel
	 *        True if the echo canceller is activated.
	 *
	 * @return a sample rate supported by the MacCoreaudio device with the specified device index
	 *         with which it is to be registered with JMF
	 */
	private static double getSupportedSampleRate(boolean input, String deviceUID,
		boolean isEchoCancel)
	{
		double supportedSampleRate = MacCoreAudioDevice.getNominalSampleRate(deviceUID, false,
			isEchoCancel);
	...
	/**
	 * Initializes a new <tt>MacCoreaudioSystem</tt> instance which creates MacCoreaudio capture and
	 * playback devices by enumerating all host devices with input channels.
	 *
	 * @throws Exception
	 *         if anything wrong happens while creating the MacCoreaudio capture and playback
	 *         devices
	 */
	MacCoreaudioSystem()
		throws Exception
	{
		super(LOCATOR_PROTOCOL, FEATURE_NOTIFY_AND_PLAYBACK_DEVICES | FEATURE_REINITIALIZE
		...
		throws Exception
	{
		...
		for (int i = 0; i < deviceUIDList.length; ++i) {
			...
			if (existingCdis != null) {
				for (CaptureDeviceInfo2 existingCdi : existingCdis) {
					...
					if (id.equals(deviceUID) || id.equals(name)) {
						double rate = ((AudioFormat) existingCdi.getFormats()[0]).getSampleRate();

						if (rate == getSupportedSampleRate(true, deviceUID, isEchoCancel())) {
							cdi = existingCdi;
							break;
						}
					}
			...
			if (cdi == null) {
				cdi = new CaptureDeviceInfo2(name, new MediaLocator(LOCATOR_PROTOCOL + ":#"
					...
					isInputDevice ? getSupportedSampleRate(true, deviceUID, isEchoCancel())
						: MacCoreAudioDevice.DEFAULT_SAMPLE_RATE, sampleSizeInBits, channels,
		...
		if (devicesChangedCallback == null) {
			...
			CoreAudioDevice.setDevicesChangedCallback(devicesChangedCallback);
		}
	}
	 ...
	 *
	 * @param echoCancel
	 *        <tt>true</tt> if echo cancellation is to be performed for captured audio; otherwise,
	 *        <tt>false</tt>
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * The implementation of <tt>MacCoreaudioSystem</tt> always returns &quot;MacCoreaudio&quot;.
	 */
	@Override
