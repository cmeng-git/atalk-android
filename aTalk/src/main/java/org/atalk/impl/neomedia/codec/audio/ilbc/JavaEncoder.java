...
public class JavaEncoder extends AbstractCodec2 implements FormatParametersAwareCodec
{
	...
	/**
	 * The input from previous calls to {@link #doProcess(Buffer, Buffer)} which has not been
	 * consumed yet.
	 */
	private byte[] prevIn;
	...
	public JavaEncoder()
	{
		super("iLBC Encoder", AudioFormat.class, new Format[] { new AudioFormat(Constants.ILBC_RTP,
			8000, 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED) });

		inputFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1,
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer)
	{
		...
		if (inLen >= this.inLen) {
			...
			int outOff = 0;
			byte[] out = validateByteArraySize(outBuffer, outOff + outLen, true);
