...
public class AES
{
	...
	/**
	 * The <tt>BlockCipherFactory</tt> implementations known to the <tt>AES</tt> class among which
	 * the fastest is to be elected as {@link #factory}.
	 */
	...
	/**
	 * The name of the class to instantiate as a <tt>BlockCipherFactory</tt> implementation to be
	 * used by the class <tt>AES</tt> to initialize <tt>BlockCipher</tt>s.
	 */
	private static final String FACTORY_CLASS_NAME;

	/**
	 * The name of the <tt>ConfigurationService</tt> and/or <tt>System</tt> property which specifies
	 * the name of the class to instantiate as a <tt>BlockCipherFactory</tt> implementation to be
	 * used by the class <tt>AES</tt> to initialize <tt>BlockCipher</tt>s.
	 */
	...
	/**
	 * The time in milliseconds at which {@link #factories} were benchMarked and {@link #factory}
	 * was elected.
	 */
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AES</tt> class to print out debug information.
	 */
	...
	static {
        ...
        FACTORY_CLASS_NAME = ConfigUtils.getString(cfg, FACTORY_CLASS_NAME_PNAME, null);
	}

	/**
	 * Benchmarks a specific array/list of <tt>BlockCipherFactory</tt> instances and returns the
	 * fastest-performing element.
	 *
	 * @param factories
	 *        the <tt>BlockCipherFactory</tt> instances to benchmark
	 * @return the fastest-performing <tt>BlockCipherFactory</tt> among the specified
	 *         <tt>factories</tt>
	 */
    private static BlockCipherFactory benchmark(BlockCipherFactory[] factories, int keySize)
	{
        ...
        if (log.length() != 0)
        {
            logger.info("AES benchmark (of execution times expressed in nanoseconds): " + log);
		}
	...
	/**
	 * Initializes a new <tt>BlockCipher</tt> instance which implements Advanced Encryption Standard
	 * (AES).
	 *
	 * @return a new <tt>BlockCipher</tt> instance which implements Advanced Encryption Standard
	 *         (AES)
	 */
		...
		if (factoryClass == null) {
			...
			if ((factoryClassName != null) && (factoryClassName.length() != 0)) {
				...
				if (Character.isUpperCase(factoryClassName.charAt(0))
					...
					factoryClassName = AES.class.getName() + "$" + factoryClassName
				...
				if (add) {
					try {
						...
						if (t instanceof InterruptedException) {
							...
							logger.warn("Failed to employ class " + factoryClassName
	 ...
	 *
	 * @param classes
	 *        the runtime <tt>Class</tt>es to instantiate
	 * @return the <tt>BlockCipherFactory</tt> instances initialized by the specified
	 *         <tt>classes</tt>
	 */
	 ...
	 *
	 * <p>
	 * Benchmarks the well-known <tt>BlockCipherFactory</tt> implementations and returns the fastest
	 * one.
	 * </p>
	 *
	 * @return a <tt>BlockCipherFactory</tt> instance to be used by the <tt>AES</tt> class to
	 *         initialize <tt>BlockCipher</tt>s
	 */
	 ...
	 *
	 * @param factory
	 *        the <tt>BlockCipherFactory</tt> for which a simple class name is to be returned
	...
	{
		/**
		 * The <tt>java.security.Provider</tt> instance (to be) employed for an (optimized) AES
		 * implementation.
		 */
		...
		/**
		 * Gets the <tt>java.security.Provider</tt> instance (to be) employed for an (optimized) AES
		 * implementation.
		 *
		 * @return the <tt>java.security.Provider</tt> instance (to be) employed for an (optimized)
		 *         AES implementation
		 */
