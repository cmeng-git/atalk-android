...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>WebmDataSink</tt> class and its instances for logging
	 * output.
	 */
	...
	/**
	 * The PTS (presentation timestamp) of the last frame written to the output file. In
	 * milliseconds.
	 */
	...
	public WebmDataSink(String filename, DataSource dataSource)
	{
		...
		if (this.autoKeyframeRequestInterval > 0 && logger.isInfoEnabled()) {
			logger.info("Auto keyframe request is initialized for every "
					+ this.autoKeyframeRequestInterval + " frames.");
		}
		...
		synchronized (openCloseSyncRoot) {
			...
			if (waitingForKeyframe && key) {
				if (valid) {
					...
					if (logger.isInfoEnabled())
						logger.info("Received the first keyframe (width=" + width
			...
			if (!waitingForKeyframe) {
				if (key) {
					...
					if (width != oldWidth || height != oldHeight) {
						if (logger.isInfoEnabled()) {
							logger.info("VP8 stream width/height changed. Old: " + oldWidth + "/"
									+ oldHeight + ". New: " + width + "/" + height + ".");
						}
					}
				}
				fd.buffer = data;
	 ...
	 *
	 * @param buf
	 * 		the buffer containing a compressed VP8 frame.
	 * @param offset
	 * 		the offset in <tt>buf</tt> where the VP8 compressed frame starts.
