...
public class DtmfTransformEngine extends SinglePacketTransformer implements TransformEngine
{
	 ...
	 */
	private static final DTMFRtpTone[] supportedTones = new DTMFRtpTone[]{DTMFRtpTone.DTMF_0,
	 ...
	 *
	 * @param stream
	 * 		the <tt>AudioMediaStream</tt> whose RTP packets we are going to be replacing with DTMF.
	 */
	 ...
	 *
	 * @param pkt
	 * 		an incoming packet that we need to parse and handle in case we determine it to be DTMF.
	...
	{
		if (currentTone.isEmpty() || pkt == null || pkt.getVersion() != RTPHeader.VERSION) {
			return pkt;
		}
		...
		DtmfRawPacket dtmfPkt = new DtmfRawPacket(pkt.getBuffer(), pkt.getOffset(),
				pkt.getLength(), currentDtmfPayload);
	 ...
	 * that it
	 * would start replacing packets with dtmf codes.
	 *
	 * @param tone
	 ...
	 * 		The DTMF tone volume.
	 */
	public void startSending(DTMFRtpTone tone, int minimalToneDuration, int maximalToneDuration,
			int volume)
	{
		synchronized (startStopToneMutex) {
	...
	public void stopSendingDTMF()
	{
		synchronized (startStopToneMutex) {
			...
			int stoppingTone = (toneTransmissionState == ToneTransmissionState.END_REQUESTED
					|| toneTransmissionState == ToneTransmissionState.END_SEQUENCE_INITIATED)
					? 1 : 0;
	...
	private class DTMFDispatcher implements Runnable
	{
		...
		/**
		 * Waits for new tone to be reported via the <tt>addTonePacket()</tt> method and then
		 * delivers them to the <tt>AudioMediaStream</tt> that we are associated with.
		 */
		public void run()
		{
			isRunning = true;
