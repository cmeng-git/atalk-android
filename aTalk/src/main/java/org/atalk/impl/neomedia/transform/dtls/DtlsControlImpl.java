 ...
 */
public class DtlsControlImpl extends AbstractSrtpControl<DtlsTransformEngine>
		implements DtlsControl
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>DtlsControlImpl</tt> class and its instances to print
	 * debug information.
	 */
	 ...
	 * @param hashFunction
	 * 		the hash function to be used in order to compute the fingerprint of the specified
	 * 		<tt>certificate</tt>
	...
	private static String findHashFunction(Certificate certificate)
	{
		try {
			...
			if (t instanceof ThreadDeath) {
				...
				logger.warn("Failed to find the hash function of the signature"
						+ " algorithm of a certificate!", t);
				if (t instanceof RuntimeException)
					throw (RuntimeException) t;
				else
					throw new RuntimeException(t);
	 ...
	 *
	 * @param hashFunction
	 * 		the hash function which is not associated with a
	 ...
	 *
	 * @param subject
	 * 		the subject (and issuer) of the new certificate to be generated
	 * @param keyPair
	 * 		the pair of private and public keys of the certificate to be generated
	 ...
	 *
	 * @param fingerprint
	 * 		an array of <tt>bytes</tt> which represents a fingerprint the <tt>String</tt>
	 ...
	 *
	 * @param certificate
	 * 		the certificate to be verified and validated against the fingerprints presented by the
	 * 		remote endpoint via the signaling path
	 * @throws Exception
	 * 		if the specified <tt>certificate</tt> failed to verify and validate against the
			...
			throws Exception
	{
		...
		if (remoteFingerprint.equals(fingerprint)) {
			if (logger.isTraceEnabled()) {
				logger.trace("Fingerprint " + remoteFingerprint + " matches the "
