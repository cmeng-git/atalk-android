...
public class DataSource extends AbstractPullBufferCaptureDevice
{
	...
	protected Format setFormat(int streamIndex, Format oldValue, Format newValue)
	{
		 ...
		 * Afterwards, AudioRecordStream will decide whether to accept further format
		 * specifications.
		 */
		return newValue;
	}
		...
		AudioEffect.OnEnableStatusChangeListener
	{
		...
		/**
		 * The length in bytes of the media data read into a <tt>Buffer</tt> via a call to
		 * {@link #read(Buffer)}.
		 */
		private int length;
			...
			throws IOException
		{
			...
			setThreadPriority();
			try {
				int minBufferSize = AudioRecord.getMinBufferSize((int) sampleRate, channelConfig,
					audioFormat);
		...
		private void configureEffects()
		{
			...
			AudioSystem audioSystem = AudioSystem
				.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_AUDIORECORD);

			// Creates echo canceler if available
			if (AcousticEchoCanceler.isAvailable()) {
				AcousticEchoCanceler echoCanceller = AcousticEchoCanceler.create(audioRecord
					.getAudioSessionId());
			...
			if (AutomaticGainControl.isAvailable()) {
				AutomaticGainControl agc = AutomaticGainControl.create(audioRecord
					.getAudioSessionId());
				if (agc != null) {
			...
			if (NoiseSuppressor.isAvailable()) {
				NoiseSuppressor noiseSuppressor = NoiseSuppressor.create(audioRecord
					.getAudioSessionId());
		 ...
		 *         attempt to set the <tt>Format</tt> did not succeed and any last-known
			...
			throws IOException
		{
			...
			while (toRead > 0) {
				...
				if (read < 0) {
					throw new IOException(AudioRecord.class.getName()
						+ "#read(byte[], int, int) returned " + read);
				}
				else {
			...
			// Apply software gain.
			if (gainControl != null) {
				BasicVolumeControl.applyGain(gainControl, bytes, buffer.getOffset(),
					buffer.getLength());
			}
		}
