...
public class JavaDecoder extends AbstractCodec2
{
	...
	/**
	 * The <tt>Logger</tt> used by this <tt>JavaDecoder</tt> instance for logging output.
	 */
	...
	protected void doClose()
	{
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Packets lost (no FEC in subsequent): " + nbFECNotDecoded);
		}
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf)
	{
		...
		if (decodeFEC) /* Decode with FEC. */
		{
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Packet loss detected. Last seen " + lastSeqNo + ", current " + seqNo);
				logger.trace("Looking for FEC data, found " + lbrrBytes[0] + " bytes");
			}
