...
/**
 * Provides an abstract implementation of <tt>FormatControl</tt> which facilitates implementers by
 * requiring them to implement just {@link FormatControl#getSupportedFormats()} and
...
public abstract class AbstractFormatControl implements FormatControl
{
	 ...
	 *
	 * @param controlsImpl
	 *        the <tt>Controls</tt> implementation from which the <tt>FormatControl</tt> instances
	 *        are to be retrieved
	 ...
	 *
	 * @param enabled
	 *        <tt>true</tt> if this track is to be enabled; otherwise, <tt>false</tt>
	 */
	 ...
	 * currently set format if the specified <tt>Format</tt> is supported and <tt>null</tt> if it is
	 * not supported.
	 *
	 * @param format
	 ...
	 * {@link FormatControl#setFormat(Format)} in the case of not supporting <tt>Format</tt>
	 * setting.
	 *
	 * @param formatControl
	 *        the <tt>FormatControl</tt> for which the functionality is implemented
	 * @param format
	 *        the <tt>Format</tt> specified to be set to <tt>formatControl</tt> and which will be
	 *        ignored in accord with the documentation of <tt>FormatControl#setFormat(Format)</tt>
	 * @return the currently set <tt>Format</tt> of <tt>formatControl</tt> if <tt>format</tt> is
	 *         supported by <tt>formatControl</tt>; <tt>null</tt> if <tt>format</tt> is not
	 *         supported by <tt>formatControl</tt>
	 */
	public static Format setFormat(FormatControl formatControl, Format format)
	{
		/*
		 * Determine whether the specified format is supported by this instance because we have to
		 * return null if it is not supported. Or at least that is what I gather from the respective
		 * javadoc.
		 */
		...
		/*
		 * We do not actually support setFormat so we have to return the currently set format if the
