...
public class DecAPI
{
	...
	static int SKP_Silk_SDK_Decode(Object decState, /* I/O: State */
		...
		int samplesOut_offset, short[] nSamplesOut /* I/O: Number of samples (vector/decoded) */
	)
	{
		...
		{
			if (psDec.nBytesLeft > 0 && psDec.FrameTermination == Define.SKP_SILK_MORE_FRAMES
		...
		if (psDec.fs_kHz * 1000 != decControl.API_sampleRate) {
			...
			ret += Resampler.SKP_Silk_resampler(psDec.resampler_state, samplesOut,
				samplesOut_offset, samplesOut_tmp, 0, nSamplesOut[0]);
	 ...
	 */
	static void SKP_Silk_SDK_search_for_LBRR(byte[] inData, /* I: Encoded input vector */
	...
	{
		SKP_Silk_decoder_state sDec = new SKP_Silk_decoder_state(); // Local decoder state to avoid
																	// interfering with
																	// running decoder */
		...
		while (true) {
			...
			// };
			if (((sDec.FrameTermination - 1) & lost_offset) != 0 && sDec.FrameTermination > 0
				&& sDec.nBytesLeft >= 0) {
