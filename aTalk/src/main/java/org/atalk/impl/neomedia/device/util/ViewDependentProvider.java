...
public abstract class ViewDependentProvider<T>
{
	 ...
	 * 
	 * @param activity
	 *        parent <tt>Activity</tt> that manages the <tt>container</tt>.
	 * @param container
	 *        the container that will hold maintained <tt>View</tt>.
	 */
		...
		if (providedObject != null) {
			try {
				...
				if (providedObject != null) {
                    ...
                    logger.warn("Timeout waiting for preview surface removal!");
				}
			}
			catch (InterruptedException e) {
				throw new RuntimeException(e);
