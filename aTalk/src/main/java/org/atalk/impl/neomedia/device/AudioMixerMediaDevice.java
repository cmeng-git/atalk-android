...
import org.atalk.impl.neomedia.audiolevel.*;
import org.atalk.impl.neomedia.conference.*;
		...
		implements MediaDeviceWrapper
{
	...
	/**
	 * The levels map that we use to cache last measured audio levels for all streams associated
	 * with this mixer.
	 */
	 ...
	 *
	 * @param device
	 * 		the <tt>AudioMediaDeviceImpl</tt> which the new instance is to enable audio mixing on
	 */
	 ...
	 * @throws IOException
	 * 		if anything wrong happens while connecting to the specified <tt>captureDevice</tt>
	...
	/**
	 * Gets the <tt>AudioMixer</tt> which performs audio mixing in this <tt>MediaDevice</tt> (and
	 * rather the session it represents). If it still does not exist, it is created.
	 *
	 ...
	 *
	 * @param listener
	 * 		the <tt>ReceiveStreamBufferListener</tt> which gets notified
	 */
	 ...
	 * 		the <tt>DataSourceFilter</tt> which selects the <tt>DataSource</tt>s to be removed
	 */
	...
	private class AudioMixerMediaDeviceSession extends MediaDeviceSession
	{
		 ...
		 * levels as they change. If <tt>l</tt> is added multiple times it would only be registered
		 * once.
		 *
		 * @param l
		 ...
		 * 		the <tt>MediaStreamMediaDeviceSession</tt> to be added to the mix represented by
		 * 		this instance
		 */
		 ...
		 * 		the <tt>DataSource</tt> providing remote audio to be added to the mix produced by
		 ...
		 *
		 * @param receiveStream
		 * 		the <tt>ReceiveStream</tt> to be added to the list of <tt>ReceiveStream</tt>s
		 ...
		 * 		the <tt>VolumeControl</tt> which is to be control the volume (level) of the audio
		 ...
		 *
		 * @param stream
		 * 		the stream that <tt>l</tt> would like to register as an audio level listener for.
		 * @param listener
		 * 		the listener we'd like to register for notifications from <tt>stream</tt>.
		 */
		 ...
		 *
		 * @param mediaStreamMediaDeviceSession
		 * 		the <tt>MediaStreamMediaDeviceSession</tt> to be removed from the mix represented
		 * 		by this instance
		 */
		 ...
		 *
		 * @param playbackDataSource
		 * 		the <tt>DataSource</tt> providing remote audio to be removed from the mix produced
		 ...
		 *
		 * @param receiveStream
		 * 		the <tt>ReceiveStream</tt> to be removed from the list of <tt>ReceiveStream</tt>s
		 * 		known to this instance to be contributing audio to the mix produced by its
		 * 		associated <tt>AudioMixer</tt>
		 */
		@Override
			...
			implements PropertyChangeListener
	{
		 ...
		 * the
		 * time we get it from the <tt>MediaStream</tt> it might be too early to register it with
		 ...
		 * 		the <tt>MediaDeviceSession</tt> of the <tt>AudioMixer</tt> with its
		 ...
		 *
		 * @param evt
		 * 		that <tt>PropertyChangeEvent</tt> whose old and new value we will be relaying to
		 * 		the stream.
		 */
		 ...
		 *
		 * @param listener
		 * 		the <tt>SimpleAudioLevelListener</tt> that we are to pass to the mixer device
		 ...
		 *
		 * @param csrc
		 * 		the CSRC ID whose last measured audio level we'd like to retrieve.
		 ...
		 *
		 * @param mute
		 * 		<tt>true</tt> to set this <tt>MediaDeviceSession</tt> to output "silence" instead
