...
public class DtlsPacketTransformer implements PacketTransformer
{
	 ...
	 * {@link #dtlsTransport} in order to receive a packet.
	 */
	private static final int DTLS_TRANSPORT_RECEIVE_WAITMILLIS = -1;

	/**
	 * The <tt>Logger</tt> used by the <tt>DtlsPacketTransformer</tt> class and its instances to
	 ...
	 * @param off
	 * 		the offset within <tt>buf</tt> at which the analysis is to start
	 * @param len
	 * 		the number of bytes within <tt>buf</tt> starting at <tt>off</tt> to be analyzed
	...
	public static boolean isDtlsRecord(byte[] buf, int off, int len)
	{
		...
		if (len >= DTLS_RECORD_HEADER_LENGTH) {
			...
			switch (type) {
				...
				default:
					/*
					 * Unless a new ContentType has been defined by the Bouncy Castle Crypto APIs,
					 * the specified buf does not represent a DTLS record.
					 */
					break;
			}
		}
	...
	/**
	 * The <tt>DTLSTransport</tt> through which the actual packet transformations are being
	 ...
	 * 		the <tt>TransformEngine</tt> which is initializing the new instance
	 * @param componentID
	...
	/**
	 * Determines whether {@link #runInConnectThread(DTLSProtocol, TlsPeer, DatagramTransport)} is
	 ...
	 * {@link #runInConnectThread(DTLSProtocol, TlsPeer, DatagramTransport)} while trying to
	 * establish a DTLS connection
	 *
	 * @param ioe
	 * 		the <tt>IOException</tt> to handle
	 * @param msg
	 * 		the human-readable message to log about the specified <tt>ioe</tt>
	 * @param i
	 * 		the number of tries remaining after the current one
	 * @return <tt>true</tt> if the specified <tt>ioe</tt> was successfully handled;
	 ...
	 * 		the (negotiated) <tt>SRTPProtectionProfile</tt> to initialize the new instance with
	 * @param tlsContext
	 ...
	 * @param message
	 * 		a human-readable message explaining what caused the alert. May be <tt>null</tt>.
	 * @param cause
	 * 		the exception that caused the alert to be raised. May be <tt>null</tt>.
	 */
	void notifyAlertRaised(TlsPeer tlsPeer, short alertLevel, short alertDescription,
	...
	private void propertyChange(String propertyName)
	{
		...
		if (propertyName == null) {
			...
			setConnector((AbstractRTPConnector) getProperties().get(propertyName));
		}
	 ...
	 *
	 * @param pkts
	 * 		the {@code RawPacket}s to queue
	 * @param transform
	 * 		{@code true} if {@code pkts} are to be sent to the
	...
	{
		if (pkts != null) {
			Queue<RawPacket> q = transform ? _transformSrtpQueue : _reverseTransformSrtpQueue;
	 ...
	 *
	 * @param pkt
	 * 		the DTLS {@code RawPacket} received from the remote peer to
	 * 		process.
	 * @param outPkts
	 * 		a list of packets, to which application data read from
	...
	private void reverseTransformDtls(RawPacket pkt, List<RawPacket> outPkts)
	{
		...
		synchronized (this) {
			if (datagramTransport == null) {
				logger.warn("Dropping a DTLS packet. This DtlsPacketTransformer has"
						+ " not been started successfully or has been closed.");
			}
			else {
		...
		if (dtlsTransport == null) {
			...
			try {
				do {
					...
					int received = dtlsTransport.receive(
							buf, 0, buf.length, DTLS_TRANSPORT_RECEIVE_WAITMILLIS);
			...
			for (int i = CONNECT_TRIES - 1; i >= 0; i--) {
				...
				try {
					...
					if (handleRunInConnectThreadException(ioe,
							"Failed to connect this DTLS client to a DTLS" + " server!", i)) {
						break;
					}
				}
			}
	 ...
	 *
	 * @param connector
	 * 		the <tt>RTPConnector</tt> which is to use or uses this <tt>PacketTransformer</tt>
	 */
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the stream which this instance is to work for/be associated
	 * 		with
	 */
	 ...
	 * 		the {@code SinglePacketTransformer} to set on
	...
	private synchronized void start()
	{
		if (this.datagramTransport != null) {
			if ((this.connectThread == null) && (dtlsTransport == null)) {
				logger.warn(getClass().getName() + " has been started but has failed to establish"
						+ " the DTLS connection!");
			}
			return;
		}
	...
	private synchronized void stop()
	{
		...
		try {
			/*
			 * The dtlsTransport and srtpTransformer SHOULD be closed, of course. The
			 * datagramTransport MUST be closed.
			 */
			if (dtlsTransport != null) {
				try {
					dtlsTransport.close();
	 ...
	 *
	 * @param inPkts
	 * 		the {@code RawPacket}s to be sent to or received from
	 * 		(depending on {@code transform} the remote peer
	 * @param transform
	 * 		{@code true} of {@code inPkts} are to be sent to the
	 ...
	 * 		the remote peer
	 * @param outPkts
	 * 		the {@code List} of {@code RawPacket}s into which the
	 ...
	 *
	 * @param inPkts
	 * 		the {@code RawPacket}s to be sent to the remote peer
	 * @param outPkts
	 * 		the {@code List} of {@code RawPacket}s into which the
	 * 		results of the processing of {@code inPkts} are to be written
	 * @return the {@code List} of {@code RawPacket}s which are the result of
	 ...
	 *
	 * @param inPkts
	 * 		the SRTP {@code RawPacket}s to be sent to or received from
	 * 		(depending on {@code transform}) the remote peer
	 * @param transform
	 * 		{@code true} of {@code inPkts} are to be sent to the
	 * 		remote peer or {@code false} if {@code inPkts} have been received from
		...
		if (srtpTransformer == null) {
			...
			if (q.size() > 0) {
				synchronized (q) {
					...
					RawPacket template = (inPkts != null && inPkts.length > 0)
							? inPkts[0] : null;
	 ...
	 *
	 * @param srtpTransformer
	 * 		the {@code SinglePacketTransformer} to perform the
	 * 		actual processing
	 * @param inPkts
	 * 		the SRTP {@code RawPacket}s to be sent to or received from
	 * 		(depending on {@code transform}) the remote peer
	 ...
	 *
	 * @param q
	 * 		the queue to remove packets from.
	 * @param template
	 * 		the template
	 */
