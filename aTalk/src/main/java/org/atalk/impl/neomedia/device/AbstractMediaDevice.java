...
public abstract class AbstractMediaDevice implements MediaDevice
{
		...
		throws IOException
	{
		...
		try {
			...
			/*
			 * The old media says it happens when the operating system does not support the
			 * operation.
			 */
	 ...
	 *
	 * @param dataSource
	 *        the <tt>DataSource</tt> which is to be played back by the new <tt>Processor</tt>
	 *        instance
	 * @return a new <tt>Processor</tt> instance which is to be used to play back the media provided
	 *         by the specified <tt>dataSource</tt> or <tt>null</tt> if the specified
	 *         <tt>dataSource</tt> is to not be played back
	 * @throws Exception
