		...
		implements CaptureDevice, MuteDataSource, InbandDTMFDataSource
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMixingPushBufferDataSource</tt> class and its
	 ...
	 *
	 * @param audioMixer
	 * 		the <tt>AudioMixer</tt> performing audio mixing, managing the input
	 * 		<tt>DataSource</tt>s and pushing the data of the new output
	 ...
	 *
	 * @param inDataSource
	 * 		a <tt>DataSource</tt> to be added for mixing to the <tt>AudioMixer</tt> associate with
	 * 		this instance and to not have its audio contributions included in the mixing output
	 * 		represented by this <tt>DataSource</tt>
	 */
	 ...
	 * output <tt>PushBufferDataSources</tt> has been connected and marks this <tt>DataSource</tt>
	 * as connected.
	 *
	 * @throws IOException
	 ...
	 *
	 * @return the <tt>CaptureDeviceInfo</tt> of the <tt>CaptureDevice</tt> of the
	 * <tt>AudioMixer</tt>
	 */
	 ...
	 *
	 * @param controlType
	 * 		a <tt>String</tt> value which names the type of the control of this instance to be
	 * 		retrieved
	 ...
	 * <tt>AudioMixer</tt> because it knows which <tt>CaptureDevice</tt> is being wrapped.
	 *
	 * @return an array of <tt>FormatControl</tt>s of the <tt>CaptureDevice</tt> of the associated
	 * <tt>AudioMixer</tt>
	 */
			...
			if (audioMixerOutStream != null) {
				...
				if (started)
					try {
						...
						logger.error("Failed to start " + outStream.getClass().getSimpleName()
								+ " with hashCode " + outStream.hashCode(), ioex);
					}
			}
		}
	 ...
	 *
	 * @throws IOException
	 * 		if anything wrong happens while starting the output <tt>PushBufferStream</tt> of this
	 * 		<tt>DataSource</tt>
	 */
	@Override
	 ...
	 *
	 * @throws IOException
	 * 		if anything wrong happens while stopping the output <tt>PushBufferStream</tt> of this
	 * 		<tt>DataSource</tt>
	 */
	@Override
