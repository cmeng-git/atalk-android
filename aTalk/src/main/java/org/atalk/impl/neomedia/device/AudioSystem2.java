...
public abstract class AudioSystem2 extends AudioSystem
{
	 ...
	 *
	 * @param listener
	 *        the <tt>UpdateAvailableDeviceListListener</tt> to be notified before and after this
		...
		synchronized (updateAvailableDeviceListListeners) {
			...
			if (add) {
				updateAvailableDeviceListListeners
					.add(new WeakReference<UpdateAvailableDeviceListListener>(listener));
			}
		}
	}
	 ...
	 *
	 * @param devices
	 *        the list of <tt>CaptureDeviceInfo2</tt>s to be sorted so that the ones representing
	 ...
	 *        <tt>updateAvailableDeviceList()</tt> will be invoked or <tt>false</tt> if it was
	 *        invoked
	 */
	private void fireUpdateAvailableDeviceListEvent(boolean will)
	{
		try {
			...
			synchronized (updateAvailableDeviceListListeners) {
				ls = new ArrayList<WeakReference<UpdateAvailableDeviceListListener>>(
					updateAvailableDeviceListListeners);
			}
	 ...
	 *
	 * @throws Exception
	 *         if there was an error during the invocation of <tt>updateAvailableDeviceList()</tt>
