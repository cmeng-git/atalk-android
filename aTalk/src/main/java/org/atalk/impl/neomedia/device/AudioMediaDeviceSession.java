...
public class AudioMediaDeviceSession extends MediaDeviceSession
{
	...
	/**
	 * The <tt>Effect</tt> that we will register with our output data source in order to measure the
	 * outgoing audio levels.
	 */
	private AudioLevelEffect2 outputAudioLevelEffect = null;

	/**
	 * The <tt>VolumeControl</tt> which is to control the volume (level) of the audio (to be) played
	 * back by this instance.
	 */
	private VolumeControl outputVolumeControl;

	/**
	 * The effect that we will register with our stream in order to measure audio levels of the
	 * remote user audio.
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation in order to configure the <tt>VolumeControl</tt> of the
	 * returned <tt>Renderer</tt> for the purposes of having call/telephony conference-specific
	 * volume (levels).
	 */
	@Override
	...
	/**
	 * Returns the last audio level that was measured by this device session for the specified
	 * <tt>ssrc</tt>.
	 *
	 * @param ssrc
	 *        the SSRC ID whose last measured audio level we'd like to retrieve.
	 *
	 * @return the audio level that was last measured for the specified <tt>ssrc</tt> or <tt>-1</tt>
	 ...
	 * player associated with this session's <tt>ReceiveStream</tt> moves enters the
	 * <tt>Configured</tt> state, so we use the occasion to add our audio level effect.
	 *
	 * @param player
	...
	/**
	 * Gets notified about <tt>ControllerEvent</tt>s generated by the processor reading our capture
	...
	/**
	 * Creates an audio level effect and add its to the codec chain of the <tt>TrackControl</tt>
	 * assuming that it only contains a single track.
	 *
	 * @param processor
	 *        the processor on which track control we need to register a level effect with.
	 */
	...
	/**
	 * Adds an audio level effect to the tracks of the specified <tt>trackControl</tt> and so that
	 * we would notify interested listeners of audio level changes.
	 *
	 * @param trackControl
	 *        the <tt>TrackControl</tt> where we need to register a level effect that would measure
	 *        the audio levels of the <tt>ReceiveStream</tt> associated with this class.
	 *
	 * @throws UnsupportedPlugInException
	 *         if we fail to add our sound level effect to the track control of <tt>mediaStream</tt>
	 *         's processor.
	 */
	...
	/**
	 * Sets the <tt>SimpleAudioLevelListener</tt> that this session should be notifying about
	 ...
	 * objects associated with the process (such as event instances listener list iterators and sync
	 * copies).
	 *
	 * @param listener
	 ...
	 *        the <tt>VolumeControl</tt> which is to be control the volume (level) of the audio (to
	...
	/**
	 * Sets <tt>listener</tt> as the <tt>SimpleAudioLevelListener</tt> that we are going to notify
	 ...
	 *
	 * @param listener
	 *        the <tt>SimpleAudioLevelListener</tt> that we want notified for audio level changes in
	...
	/**
	 * Implements a utility which facilitates setting a specific <tt>VolumeControl</tt> on a
	 * specific <tt>Renderer</tt> for the purposes of control over the volume (level) of the audio
	 * (to be) played back by the specified <tt>Renderer</tt>.
	 *
	 * @param renderer
	 *        the <tt>Renderer</tt> on which the specified <tt>volumeControl</tt> is to be set
	 * @param volumeControl
	 *        the <tt>VolumeControl</tt> to be set on the specified <tt>renderer</tt>
	 */
	...
	/**
	 * Performs additional configuration on the <tt>Processor</tt>, after it is <tt>configure</tt>d,
	 * but before it is <tt>realize</tt>d. Adds the <tt>AudioLevelEffect2</tt> instance to the codec
	...
	/**
	 * Enables or disables measuring audio levels for the output <tt>DataSource</tt> of this
	 * <tt>AudioMediaDeviceSession</tt>.
	 *
	 * Note that if audio levels are to be enabled, this method needs to be called (with
	 * <tt>enabled</tt> set to <tt>true</tt>) before the output <tt>DataSource</tt>, or the
	 * <tt>Processor</tt> are accessed (via {@link #getOutputDataSource()} and
	 * {@link #getProcessor()}). This limitation allows to not insert an <tt>Effect</tt> in the
	 * codec chain when measuring audio levels is not required (since we can only do this before the
	 * <tt>Processor</tt> is realized).
	 *
	 * @param enabled
