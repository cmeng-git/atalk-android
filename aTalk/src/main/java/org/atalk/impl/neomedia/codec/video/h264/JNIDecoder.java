...
public class JNIDecoder extends AbstractCodec
{
	...
	/**
	 * Decodes H.264 media data read from a specific input <tt>Buffer</tt> into a specific output
	 * <tt>Buffer</tt>.
	 *
	 * @param in
	 * 		input <tt>Buffer</tt>
	...
	public synchronized int process(Buffer in, Buffer out)
	{
		...
		FFmpeg.avcodec_decode_video(avctx, avframe.getPtr(), got_picture, (byte[]) in.getData(),
				in.getLength());
	 ...
	 * 		the <tt>KeyFrameControl</tt> to be used by this <tt>DePacketizer</tt> as a means of
	 * 		control over its key frame-related logic
	 */
