...
abstract class AndroidCodec extends AbstractCodec2
{
	 ...
	 *
	 * @param codec
	 * 		<tt>MediaCodec</tt> instance to be configured.
	 * @param codecType
	 * 		string codec media type.
			...
			throws ResourceUnavailableException
	{
		...
		logger.info("Opened " + codecType + " " + getStrName() + " for name: "
				+ codecInfo.getName() + " use surface ? " + useSurface());
	}
	...
	private int doProcessImpl(Buffer inputBuffer, Buffer outputBuffer)
	{
		...
		if (mediaCodecOutputIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Reading output: " + info.offset + ":"
						+ info.size + " flag: " + info.flags);
			}
