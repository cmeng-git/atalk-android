		...
		implements TransformEngine
{
	 ...
	 *
	 * @param pkt
	 * 		the <tt>RawPacket</tt> to be examined
	 ...
	 *
	 * @param pkt
	 * 		the <tt>RawPacket</tt> into which <tt>extendedReport</tt> is to be added
	 * @param extendedReport
	 * 		the RTCP XR packet to add into <tt>pkt</tt>
	 ...
	 *
	 * @param pkt
	 * 		the <tt>RawPacket</tt> to which RTCP XR packets are to be added
	 * @param sdpParams
			...
			if ((pt == RTCPPacket.RR) || (pt == RTCPPacket.SR)) {
				...
				if (rc >= 0) {
					...
					RTCPExtendedReport rtcpXR
							= createRTCPExtendedReport(senderSSRC, sourceSSRCs, sdpParams);
	 ...
	 *
	 * @param senderSSRC
	 * 		the synchronization source identifier (SSRC) of the originator of the new RTCP XR
	 * 		packet
	 * @param sourceSSRCs
	 * 		the SSRCs of the RTP data packet sources to be reported upon by the new RTCP XR packet
	 * @param sdpParams
	 ...
	 * @param senderSSRC
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> to be reported upon by the new instance
	 * @return a new <tt>RTCPExtendedReport.VoIPMetricsReportBlock</tt> instance
		...
		if (mediaFormat != null) {
			...
			if (encoding != null) {
				...
				if (Constants.OPUS_RTP.toLowerCase().contains(encoding)
					...
					packetLossConcealment = RTCPExtendedReport
							.VoIPMetricsReportBlock.STANDARD_PACKET_LOSS_CONCEALMENT;
				}
			}
		}
	...
	/**
	 * Gets the number of packets in a <tt>ReceiveStream</tt> which have been decoded by means of
	 * FEC.
	 *
	 * @param receiveStream
	 * 		the <tt>ReceiveStream</tt> of which the number of packets decoded by means of FEC is
	 * 		to be returned
	 * @return the number of packets in <tt>receiveStream</tt> which have been decoded by means
	 * of FEC
	 */
	private long getFECDecodedPacketCount(ReceiveStream receiveStream)
	{
		...
		if (devSession != null) {
			Iterable<FECDecoderControl> decoderControls = devSession.getDecoderControls(
					receiveStream, FECDecoderControl.class);
	 ...
	 * @param buf
	 * 		the byte array from which the RR or SR instance will be initialized.
	 * @param off
	 * 		the offset into <tt>buf</tt>.
	 * @param len
	 * 		the length of the data in <tt>buf</tt>.
	 * @return a new SR or RR <tt>RTCPReport</tt> instance initialized from the specified byte
	 * array.
	 * @throws IOException
	 * 		if an I/O error occurs while parsing the specified
	 ...
	 *
	 * @param pkt
	 * 		the <tt>RawPacket</tt> to parse into a new SR or RR <tt>RTCPReport</tt> instance
	...
	private RTCPReport parseRTCPReport(RawPacket pkt)
			throws IOException
	{
		return parseRTCPReport(pkt.getRTCPPacketType(),
	 ...
	 * about the reception of packets with known types (currently these are RR, SR, XR, REMB,
	 * NACK).
	 *
	 * @param pkt
	 * 		the packet to reverse-transform
			...
			switch (rtcp.type) {
				...
				case RTCPExtendedReport.XR:
					if (rtcp instanceof RTCPExtendedReport) {
						streamStats.getRTCPReports()
								.rtcpExtendedReportReceived((RTCPExtendedReport) rtcp);
					}
					break;
		...
		if (isRTCP(pkt)) {
			...
			try {
				...
				if (t instanceof InterruptedException) {
					...
					logger.error("Failed to analyze an outgoing RTCP packet for the"
							+ " purposes of statistics.", t);
				}
			}
	 ...
	 *
	 * @param map
	 * 		the {@code Map} with {@code Long} values to sum up. Note that
	 ...
	 *
	 * @param map
	 * 		the map to get the value. Note that we synchronize on that object!
	 * @param ssrc
	 * 		the key
	 ...
	 *
	 * @param map
	 * 		the map holding the values. Note that we synchronize on that
	 * 		object!
	 * @param ssrc
