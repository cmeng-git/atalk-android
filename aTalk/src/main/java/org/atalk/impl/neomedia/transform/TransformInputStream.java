...
public abstract class TransformInputStream<T> extends RTPConnectorInputStream<T>
{
	 ...
	 *
	 * @param socket
	 *        the (network) socket from which packets are to be received and transformed by the new
	 *        instance
	 */
	...
	protected RawPacket[] createRawPacket(DatagramPacket datagramPacket)
	{
		...
		return (transformer == null)
				? pkts : transformer.reverseTransform(pkts);
	}
	 ...
	 *
	 * @param transformer
	 *        the <tt>PacketTransformer</tt> which is to be used to reverse-transform packets.
	 */
