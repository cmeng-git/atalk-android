...
public class TransformEngineChain implements TransformEngine {
     ...
     *
     * @param engineChain an array containing <code>TransformEngine</code>s in the order that they are to be applied
     * on outgoing packets.
     */
    ...
    /**
     * Adds a {@link TransformEngine} to this chain, at the position after the {@code after} instance.
     *
     * @param engine the engine to add.
     * @param after the {@link TransformEngine} instance from this chain, after which {@code engine} should be inserted.
     *
     * @return {@code true} if the engine was added, and {@code false} if the
     * engine was not added because it is already a member of the chain.
     */
    ...
    /**
     * Sets the sequence of {@code TransformEngine}s whose {@code PacketTransformer}s this engine
     * chain will be applying to RTP and RTCP packets.
     *
     * @param engineChain the sequence of {@code TransformEngine}s whose {@code PacketTransformer}s this engine
     * chain will be applying to RTP and RTCP packets
     */
    ...
    public class PacketTransformerChain implements PacketTransformer {
         ...
         * it will transform packets via the RTP transformers in this chain rather than the RTCP
         * ones.
         */
         ...
         *
         * @param rtp <code>true</code> if this transformer will be dealing with RTP (i.e. will transform
         ...
         * {@inheritDoc}
         *
         * Transforms the specified {@code pkts} using each of the {@code TransformEngine}s in the chain in order.
         */
        @Override
        ...
        /**
         * Transforms the specified {@code pkts} using the {@code TransformEngine}s in the chain in
         * order starting after a specific {@code TransformEngine}.
         *
         * @param pkts the array of {@code RawPacket}s to transform
         * @param after the {@code TransformEngine} in the chain after which the transformation is to
         * begin. If {@code after} is not in the chain, the transformation executes through
         * the whole chain.
         *
         * @return the array of {@code RawPacket}s that is the result of the transformation of
         * {@code pkts} using the {@code TransformEngine}s in the chain
         */
        public RawPacket[] transform(RawPacket[] pkts, TransformEngine after) {
            ...
            boolean lookForAfter = after != null && engineChainAsList.contains(after);
