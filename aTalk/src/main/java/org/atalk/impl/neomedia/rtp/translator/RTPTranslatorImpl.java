...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>RTPTranslatorImpl</tt> class and its instances for
	 * logging output.
	 */
	private static final Logger LOGGER = Logger.getLogger(RTPTranslatorImpl.class);

	/**
	 * Logs information about an RTCP packet using {@link #LOGGER} for debugging purposes.
	 *
	 * @param obj
	 * 		the object which is the source of the log request
	 * @param methodName
	 * 		the name of the method on <tt>obj</tt> which is the source of the log request
	 * @param buf
	 * 		the <tt>byte</tt>s which (possibly) represent an RTCP packet to be logged for
	 * 		debugging purposes
	 * @param off
	 * 		the position within <tt>buf</tt> at which the valid data begins
	 * @param len
	 * 		the number of bytes in <tt>buf</tt> which constitute the valid data
	 */
					...
					if (rtcpLength <= len) {
						...
						for (int i = 0, end = off + len; i < sc && o + 4 <= end; ++i, o += 4) {
							...
							LOGGER.trace(obj.getClass().getName() + '.' + methodName
									+ ": RTCP BYE SSRC/CSRC " + Long.toString(ssrc & 0xffffffffL));
						}
					}
				}
			}
	 ...
	 * <tt>RTPManager</tt> and it does not deal with SSRC collisions.
	 ...
	 * being added by a specific <tt>StreamRTPManager</tt> but effects the
	 * <tt>RTPTranslatorImpl</tt> globally.
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> that is requesting the association of <tt>format</tt> to
	 * 		<tt>payloadType</tt>
	 * @param format
	 * 		the <tt>Format</tt> which is to be associated with the specified RTP payload type
	 * 		(number)
	 * @param payloadType
	 * 		the RTP payload type (number) to be associated with the specified <tt>format</tt>
	 */
	 ...
	 * related to a specific neomedia <tt>MediaStream</tt> (expressed as a
	 ...
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> which specifies the neomedia <tt>MediaStream</tt> with
	 ...
	 * @param listener
	 * 		the <tt>ReceiveStreamListener</tt> to be notified about <tt>ReceiveStreamEvent</tt>s
	 ...
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> which is requesting the addition of the specified
	 * 		<tt>RemoteListener</tt>
	 * @param listener
	 * 		the <tt>RemoteListener</tt> to be notified about <tt>RemoteEvent</tt>s received by
	 ...
	 * 		a <tt>SendStreamDesc</tt> instance that specifies the <tt>SendStream</tt> to be closed
	 */
	 ...
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> which is requesting the creation of a
	 ...
	 * 		<tt>SendStream</tt>.
	 * @param dataSource
	 * 		the <tt>DataSource</tt> which provides the stream from which a <tt>SendStream</tt> is
	 * 		to be created
	 * @param streamIndex
	 * 		the zero-based position within the array/list of streams of the specified
	 ...
	 */
	public SendStream createSendStream(StreamRTPManager streamRTPManager, DataSource dataSource,
			int streamIndex)
	 ...
	 *
	 * @param streamDesc
	 * 		a <tt>PushSourceStreamDesc</tt> which identifies the peer from which an RTP or RTCP
	 * 		packet has been received
	 * @param buf
	 * 		the buffer which contains the bytes of the received RTP or RTCP packet
	 * @param off
	 * 		the zero-based index in <tt>buf</tt> at which the bytes of the received RTP or RTCP
	 * 		packet begin
	 * @param len
	 * 		the number of bytes in <tt>buf</tt> beginning at <tt>off</tt> which represent the
	 * 		received RTP or RTCP packet
	 * @param flags
	 * 		<tt>Buffer.FLAG_XXX</tt>
	 * @return the number of bytes in <tt>buf</tt> beginning at <tt>off</tt> which represent the
	 * received RTP or RTCP packet
	 * @throws IOException
	 * 		if an I/O error occurs while the method processes the specified RTP or RTCP packet
	 */
	 ...
	 * <tt>StreamRTPManager</tt> to be garbage collected (as far as this <tt>RTPTranslatorImpl</tt>
	 * is concerned).
	 */
	...
	{
		StreamRTPManagerDesc desc = findStreamRTPManagerDescByReceiveSSRC(receiveSSRC,
		/* exclusion */null);
	...
	/**
	 * Finds the first <tt>StreamRTPManager</tt> which is related to a specific receive/remote
	 * SSRC.
	 *
	 * @param receiveSSRC
	 * 		the receive/remote SSRC to which the returned <tt>StreamRTPManager</tt> is to be
	 * 		related
	 * @param exclusion
	 * 		the <tt>StreamRTPManager</tt>, if any, to be excluded from the search
	 * @return the first <tt>StreamRTPManager</tt> which is related to the specified
	 * <tt>receiveSSRC</tt>
	 */
	private StreamRTPManagerDesc findStreamRTPManagerDescByReceiveSSRC(int receiveSSRC,
	...
	/**
	 * Exposes {@link RTPManager#getGlobalReceptionStats()} on the internal/underlying
	 * <tt>RTPManager</tt>.
	 *
	 * @param streamRTPManager
	 * 		ignored
	...
	/**
	 * Exposes {@link RTPManager#getGlobalTransmissionStats()} on the internal/underlying
	 * <tt>RTPManager</tt>.
	 *
	 * @param streamRTPManager
	 * 		ignored
	 * @return the return value of the invocation of
	 * <tt>RTPManager.getGlobalTransmissionStats()</tt> on the internal/underlying
	 * <tt>RTPManager</tt>
	 */
	 ...
	 * the
	 * terms of <tt>RTPManagerImpl</tt>).
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> to which the returned <tt>ReceiveStream</tt>s are to be
	 * 		related
	 * @return the <tt>ReceiveStream</tt>s related to/associated with the specified
	 * <tt>streamRTPManager</tt>
	 */
	 ...
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> to which the returned <tt>SendStream</tt>s are to be
	 * 		related
	...
	private StreamRTPManagerDesc getStreamRTPManagerDesc(StreamRTPManager streamRTPManager,
			boolean create)
	{
	 ...
	 * as a <tt>StreamRTPManager</tt> for the purposes of and in the terms of
	 * <tt>RTPTranslator</tt>
	 ...
	 * 		are to be related
	 * @param listener
	 * 		the <tt>ReceiveStreamListener</tt> to no longer be notified about
	 ...
	 *
	 * @param streamRTPManager
	 * 		the <tt>StreamRTPManager</tt> which is requesting the removal of the specified
	 * 		<tt>RemoteListener</tt>
	 * @param listener
	 * 		the <tt>RemoteListener</tt> to no longer be notified about <tt>RemoteEvent</tt>s
	 ...
	 *
	 * @param event
	 * 		a <tt>ReceiveStreamEvent</tt> which contains the specifics of the event this
