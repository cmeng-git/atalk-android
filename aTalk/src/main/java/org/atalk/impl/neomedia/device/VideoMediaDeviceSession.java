		...
		implements RTCPFeedbackMessageCreateListener
{
	...
	/**
	 * Gets the visual <tt>Component</tt> of a specific <tt>Player</tt> if it has one and ignores
	 * the failure to access it if the specified <tt>Player</tt> is unrealized.
	 *
	 * @param player
	 * 		the <tt>Player</tt> to get the visual <tt>Component</tt> of if it has one
		...
		if (player.getState() >= Player.Realized) {
			try {
				...
				if (logger.isDebugEnabled())
					logger.debug("Called Player#getVisualComponent() on unrealized player "
							+ player, nre);
			}
		}
	 ...
	 */
	final private List<RTCPFeedbackMessageCreateListener> rtcpFeedbackMessageCreateListeners
			= new LinkedList<>();
	 ...
	 *
	 * @param device
	 * 		the video <tt>MediaDevice</tt> the use of which by a <tt>MediaStream</tt> is to be
	 ...
	 *
	 * @param listener
	 * 		the <tt>VideoListener</tt> to be notified when visual/video <tt>Component</tt>s are
	 ...
	 * @param hflip
	 * 		<tt>true</tt> if the image displayed in the local visual <tt>Component</tt> is to be
	...
	private void controllerUpdateForCreateLocalVisualComponent(ControllerEvent ev, boolean hflip)
	{
		if (ev instanceof ConfigureCompleteEvent) {
			...
			playerSizeChange(scev.getSourceController(), VideoEvent.LOCAL, scev.getWidth(),
					scev.getHeight());
		}
	}
	...
	protected DataSource createCaptureDevice()
	{
		...
		if (captureDevice != null) {
			...
			float frameRate;
			DeviceConfiguration deviceConfig = NeomediaServiceUtils.getMediaServiceImpl()
			...
			if (DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equals(protocol)) {
				...
				Dimension dim = VideoMediaStreamImpl.selectVideoSize(captureDevice,
			...
			if (frameRateControl != null) {
				...
				if (logger.isInfoEnabled()) {
					logger.info("video send FPS: "
							+ (frameRate == -1 ? "default(no restriction)" : frameRate));
				}
			}
	 ...
	 *
	 * @param captureDevice
	 * 		the <tt>DataSource</tt> which is to have its media rendered by the new instance as the
		...
		if (dataSource != null) {
			...
			if (exception == null) {
				if (localPlayer != null) {
					 ...
					 * sharing/streaming, do not flip it because it does not
					 * seem natural.
					 */
					final boolean hflip = (captureDevice.getControl(ImgStreamingControl.class
							.getName()) == null);
				...
				logger.error("Failed to connect to " + MediaStreamImpl.toString(dataSource),
						exception);
			}
		}
		return localPlayer;
	 ...
	 *
	 * @param component
	 * 		the local visual <tt>Component</tt> of the local peer to dispose of
	 */
	...
	/**
	 * Notifies the <tt>VideoListener</tt>s registered with this instance about a specific type of
	 ...
	 * @param wait
	 * 		<tt>true</tt> if the call is to wait till the specified <tt>VideoEvent</tt> has been
	...
	protected boolean fireVideoEvent(int type, Component visualComponent, int origin, boolean wait)
	{
		if (logger.isTraceEnabled()) {
			logger.trace("Firing VideoEvent with type " + VideoEvent.typeToString(type)
					+ ", originated from " + VideoEvent.originToString(origin) + " and Wait is "
					+ wait);
		}
	 ...
	 *
	 * @param videoEvent
	 * 		the <tt>VideoEvent</tt> to be fired to the <tt>VideoListener</tt>s registered with
	 * 		this instance
	 * @param wait
	 * 		<tt>true</tt> if the call is to wait till the specified <tt>VideoEvent</tt> has been
	 * 		delivered to the <tt>VideoListener</tt>s; otherwise, <tt>false</tt>
	 ...
	 *
	 * @param ssrc
	 * 		the src-id of the receive stream, which visual <tt>Component</tt> we're looking for
	 ...
	 * 		the <tt>KeyFrameControl.KeyFrameRequester</tt> on which the method is invoked
			...
			try {
				for (TrackControl trackControl : trackControls) {
					...
					if ("h264/rtp".equalsIgnoreCase(fmjEncoding)) {
						...
						trackControl.setCodecChain(new Codec[]{depacketizer, decoder,
								playerScaler});
					}
					else {
				...
				logger.error("Failed to add SwScale or H.264 DePacketizer to codec chain",
						upiex);
				playerScaler = null;
			}
		}
	...
	protected void playerControllerUpdate(ControllerEvent ev)
	{
		...
		if ((ev instanceof SizeChangeEvent)
			...
			SizeChangeEvent scev = (SizeChangeEvent) ev;
			playerSizeChange(scev.getSourceController(), VideoEvent.REMOTE, scev.getWidth(),
					scev.getHeight());
		}
	}
	...
	/**
	 * Notifies this instance that a specific <tt>Player</tt> of local or remote content/video has
	 * generated a <tt>SizeChangeEvent</tt>.
	 *
	 * @param sourceController
	 * 		the <tt>Player</tt> which is the source of the event
	 * @param origin
	 * 		{@link VideoEvent#LOCAL} or {@link VideoEvent#REMOTE} which specifies the origin of
			...
			final int width, final int height)
	{
		...
		if (visualComponent != null) {
			/*
			 * The Player will notice the new size and will notify about it before it reaches the
			 ...
			 * size is reflected on the preferredSize of the Component before the
			 * notification/event
			...
			try {
				...
				fireVideoEvent(
						new SizeChangeVideoEvent(this, visualComponent, origin, width, height),
						false);
			}
		}
	}
	...
	/**
	 * Notifies this instance that the visual <tt>Component</tt> of a <tt>Player</tt> rendering
	 * remote content has been resized.
	 *
	 * @param player
	 * 		the <tt>Player</tt> rendering remote content the visual <tt>Component</tt> of which
	 * 		has been resized
	 * @param ev
	 * 		a <tt>ComponentEvent</tt> which specifies the resized <tt>Component</tt>
	 */
	 ...
	 *
	 * @param listener
	 * 		the <tt>VideoListener</tt> to no longer be notified when visual/video
	...
	public void setFormat(MediaFormat format)
	{
		if (format instanceof VideoMediaFormat
			...
			FrameRateControl frameRateControl = (FrameRateControl) getCaptureDevice().getControl(
	 ...
	 * 		the <tt>KeyFrameControl</tt> to be used by this <tt>VideoMediaDeviceSession</tt> as a
	 * 		means of control over its key frame-related logic
	 */
	...
	protected void startedDirectionChanged(MediaDirection oldValue, MediaDirection newValue)
	{
		...
		/*
		 * Translate the starting and stopping of the playback into respective VideoEvents for the
		 * REMOTE origin.
		 */
		for (Player player : getPlayers()) {
	...
	private class PlayerScaler extends SwScale
	{
		 ...
		 *
		 * @param player
		 * 		the <tt>Player</tt> of remote video into the codec chain of which the new instance
		 * 		is to be set
		 */
		...
		/**
		 * Ensures that this <tt>SwScale</tt> preserves the aspect ratio of its input video when
		 * scaling.
		 *
		 * @param inputFormat
		 * 		format to set
			...
			if (inputFormat instanceof VideoFormat) {
				...
				if ((inputSize != null) && (inputSize.width > 0)) {
					...
					if ((outputSize != null) && ((outputWidth = outputSize.width) > 0)) {
						int outputHeight = (int) (outputWidth * inputSize.height
