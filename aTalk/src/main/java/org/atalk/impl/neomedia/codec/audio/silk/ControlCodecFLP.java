...
public class ControlCodecFLP
{
	...
	)
	{
		...
		if (Define.SWITCH_TRANSITION_FILTERING != 0) {
			...
			if ((psEnc.sCmn.sLP.mode == 1)
				...
				&& (psEnc.speech_activity < 0.5f) && (psEnc.sCmn.nFramesInPayloadBuf == 0)) {
		...
		/* Set packet size */
		if ((PacketSize_ms != 20) && (PacketSize_ms != 40) && (PacketSize_ms != 60)
		...
		if (Define.USE_LBRR != 0) {
			...
			if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
				...
				if (psEnc.sCmn.TargetRate_bps >= LBRRRate_thres_bps) {
					...
					psEnc.sCmn.LBRR_GainIncreases = Math.max(8 - (psEnc.sCmn.PacketLoss_perc >> 1),
						0);
	 ...
	 *        Encoder control.
	 */
	static void SKP_Silk_LBRR_ctrl_FLP(SKP_Silk_encoder_state_FLP psEnc, /* I/O Encoder state FLP */
...
class NSQImplNSQ implements NoiseShapingQuantizerFP
{
	public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc,
		SKP_Silk_encoder_control psEncCtrl, SKP_Silk_nsq_state nsq, short[] x, byte[] q, int arg6,
		short[] arg7, short[] arg8, short[] arg9, int[] arg10, int[] arg11, int[] arg12,
