 ...
 */
package org.atalk.impl.neomedia.jmfext.media.protocol.directshow;
...
public class DirectShowStream extends AbstractPushBufferStream
		<org.atalk.impl.neomedia.jmfext.media.protocol.directshow.DataSource>
{
	/**
	 * The <tt>Logger</tt> used by the <tt>DirectShowStream</tt> class and its instances to print
	 * out debugging information.
	 */
	private static final Logger logger = Logger.getLogger(DirectShowStream.class);

	/**
	 * Determines whether a specific <tt>Format</tt> appears to be suitable for attempts to be set
	 * on <tt>DirectShowStream</tt> instances.
	 * <p>
	 * <b>Note</b>: If the method returns <tt>true</tt>, an actual attempt to set the specified
	 * <tt>format</tt> on an specific <tt>DirectShowStream</tt> instance may still fail but that
	 * will be because the finer-grained properties of the <tt>format</tt> are not supported by
	 * that <tt>DirectShowStream</tt> instance.
	 * </p>
	 *
	 * @param format
	 * 		the <tt>Format</tt> to be checked whether it appears to be suitable for attempts to be
	 * 		set on <tt>DirectShowStream</tt> instances
	 * @return <tt>true</tt> if the specified <tt>format</tt> appears to be suitable for
	 * attempts to be set on <tt>DirectShowStream</tt> instance; otherwise, <tt>false</tt>
	 */
	static boolean isSupportedFormat(Format format)
	{
		if (format instanceof AVFrameFormat) {
			...
			if (pixFmt != -1) {
				...
				/*
				 * We will set the native format in doStart() because a connect-disconnect-connect
				 * sequence of the native capture device may reorder its formats in a different
				 * way. Consequently, in the absence of further calls to setFormat() by JMF, a
				 * crash may occur later (typically, during scaling) because of a wrong format.
				 */
				if (size != null)
					return true;
			}
	...
	/**
	 * The indicator which determines whether {@link #delegate} automatically drops late frames. If
	 * <tt>false</tt>, we have to drop them ourselves because DirectShow will buffer them all and
	 * the video will be late.
	 */
	 ...
	 * by {@link #delegate} out of this instance through the <tt>Buffer</tt>s specified in its
	 * {@link #read(Buffer)}.
	 */
	...
	/**
	 * The <tt>Object</tt> which synchronizes the access to the {@link #data}-related fields of
	 * this instance.
	 */
	...
	/**
	 * The <tt>DSCaptureDevice</tt> which identifies the DirectShow video capture device this
	...
	/**
	 * The captured media data to become the value of {@link #data} as soon as the latter
	 * becomes is consumed. Thus prepares this <tt>DirectShowStream</tt> to provide the latest
	 * available frame and not wait for DirectShow to capture a new one.
	 */
	 ...
	 * <tt>DirectShowStream</tt> so that the call is not made in DirectShow and we can drop late
	...
	/**
	 * Initializes a new <tt>DirectShowStream</tt> instance which is to have its <tt>Format</tt>
	...
	/**
	 * Connects this <tt>SourceStream</tt> to the DirectShow video capture device identified by
	 * {@link #device}.
	 *
	 * @throws IOException
	 * 		if anything goes wrong while this <tt>SourceStream</tt> connects to the DirectShow
	 * 		video capture device identified by <tt>device</tt>
	 */
	...
	/**
	 * Disconnects this <tt>SourceStream</tt> from the DirectShow video capture device it has
	 * previously connected to during the execution of {@link #connect()}.
	 *
	 * @throws IOException
	 * 		if anything goes wrong while this <tt>SourceStream</tt> disconnects from the
	 * 		DirectShow video capture device it has previously connected to during the execution
	 * 		of <tt>connect()</tt>
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overrides the super implementation to enable setting the <tt>Format</tt> of this
	 * <tt>DirectShowStream</tt> after the <tt>DataSource</tt> which provides it has been
	 * connected.
	 */
	@Override
		...
		if (isSupportedFormat(format)) {
			if (device == null)
				...
				try {
					...
					/*
					 * Ignore the exception because the method is to report failures by returning
					 * null (which will be achieved outside the catch block).
					 */
				}
		...
		synchronized (dataSyncRoot) {
			...
			if (bufferFormat instanceof AVFrameFormat) {
				if (nativePixelFormat == DSFormat.MJPG) {
					...
					if (FFmpeg.avcodec_decode_video(avctx, avframe, data.getPtr(),
							data.getLength()) != -1) {
					...
					if (AVFrame.read(buffer, bufferFormat, data) < 0)
						data.free();
					/*
					 * XXX For the sake of safety, make sure that this instance does not reference
					 * the data instance as soon as it is set on the AVFrame.
					 */
					data = null;
				}
			}
				...
				/*
				 * TODO Copy the media from the native memory into the Java heap.
				 */
				data.free();
				data = null;
	 ...
	 * {@link #transferDataThread} so that the call is not made in DirectShow and we can drop late
	...
	private void runInTransferDataThread()
	{
		...
		FrameRateControl frameRateControl
				= (FrameRateControl) dataSource.getControl(FrameRateControl.class.getName());
		...
		while (Thread.currentThread().equals(transferDataThread)) {
			if (transferData) {
				...
				if (transferHandler != null) {
					/*
					 * Respect the frame rate specified through the FrameRateControl of the
					...
					if (frameRateControl != null) {
						...
						if ((transferDataTimeStamp != -1)
								&& ((frameRate = frameRateControl.getFrameRate()) > 0)) {
	 ...
	 *
	 * @param source
	 * 		pointer to the native <tt>DSCaptureDevice</tt> which is the source of the notification
	 * @param ptr
	...
	/**
	 * Sets the <tt>DSCaptureDevice</tt> of this instance which identifies the DirectShow video
	 * capture device this <tt>SourceStream</tt> is to capture data from.
	 *
	 * @param device
	 * 		a <tt>DSCaptureDevice</tt> which identifies the DirectShow video capture device this
	 * 		<tt>SourceStream</tt> is to capture data from
	 * @throws IOException
	 * 		if anything goes wrong while setting the specified <tt>device</tt> on this instance
	 */
	...
	/**
	 * Sets a specific <tt>Format</tt> on the <tt>DSCaptureDevice</tt> of this instance.
	 *
	 * @param format
	 * 		the <tt>Format</tt> to set on the <tt>DSCaptureDevice</tt> of this instance
	 * @throws IOException
	 * 		if setting the specified <tt>format</tt> on the <tt>DSCaptureDevice</tt> of this
	 * 		instance fails
	 */
			...
			throws IOException
	{
		if (format == null)
			...
			if (size == null)
				...
				int hresult = device.setFormat(new DSFormat(size.width, size.height,
	...
	/**
	 * Throws a new <tt>IOException</tt> the detail message of which describes a specific
	 * <tt>HRESULT</tt> value indicating a failure.
	 *
	 * @param hresult
	 * 		the <tt>HRESUlT</tt> to be described by the detail message of the new
	 * 		<tt>IOException</tt> to be thrown
	 * @throws IOException
	 */
