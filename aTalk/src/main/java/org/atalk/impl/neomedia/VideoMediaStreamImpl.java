...
import org.atalk.impl.neomedia.rtp.remotebitrateestimator.*;
		...
		implements VideoMediaStream
{
	 ...
	 *
	 * @param videoDS
	 * 		the video <tt>DataSource</tt> which is to have its supported formats examined and its
		...
		if (count == 1)
			...
			if ((selectedFormat != null) && (selectedFormat.getSize() == null)) {
				...
				selectedFormat = (VideoFormat) new VideoFormat(null, new Dimension(width, height),
						...
						Format.NOT_SPECIFIED).intersects(selectedFormat);
			}
		}
	...
	private final RemoteBitrateEstimatorWrapper remoteBitrateEstimator
			= new RemoteBitrateEstimatorWrapper(new RemoteBitrateObserver()
	{
		@Override
	 ...
	 *
	 * @param connector
	 * 		the <tt>StreamConnector</tt> the new instance is to use for sending and receiving video
	 * @param device
	 ...
	 *
	 * @param oldValue
	 * 		the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt> this instance used work
	 * 		with
	 * @param newValue
	 * 		the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt> this instance is to work
	 * 		with
	...
	public void movePartialDesktopStreaming(int x, int y)
	{
		...
		if (!DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING
				.equals(dev.getCaptureDeviceInfoLocatorProtocol())) {
			return;
		}
	 ...
	 * @param bitrate
	 */
	private void remoteBitrateEstimatorOnReceiveBitrateChanged(Collection<Long> ssrcs,
			long bitrate)
	{
	 ...
	 * 		parameters of advanced attributes that may affect quality control
	 */
