...
/**
 * Implements a <tt>CaptureDevice</tt> and a <tt>DataSource</tt> using DirectShow.
 *
...
{
	/**
	 * The map of DirectShow pixel formats to FFmpeg pixel formats which allows converting between
	 * the two.
	 */
	 ...
	 *
	 * @param dsPixFmt
	 * 		the DirectShow pixel format to get the matching FFmpeg pixel format of
			...
			if (device != null) {
				...
				for (DSFormat supportedFormat : supportedFormats) {
					logger.trace("width= " + supportedFormat.getWidth()
							...
							+ ", pixelFormat= " + supportedFormat.getPixelFormat());
				}
			}
		}
		return stream;
			...
			throws IOException
	{
		...
		try {
			...
			if (!connected) {
				/*
				 * The connect attempt has failed but it may have been successful up to the
				 * point of failure thus partially modifying the state. The disconnect procedure
				 * is prepared to deal with a partially modified state and will restore it to its
				  * pristine form.
				 */
				doDisconnect();
			}
	...
	private DSCaptureDevice getDevice()
	{
		...
		if (device == null) {
			...
			try {
				/*
				 * Find the device specified by the locator using matching by name.
				 */
	...
	protected Format setFormat(int streamIndex, Format oldValue, Format newValue)
	{
		...
		return DirectShowStream.isSupportedFormat(newValue)
				? newValue
