...
public class AudioSystemClipImpl extends AbstractSCAudioClip
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioSystemClipImpl</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 * <tt>AudioSystemClipImpl</tt> in order to ensure that they are played back long enough to be
	 * heard.
	 */
	...
	public AudioSystemClipImpl(String url, AudioNotifierService audioNotifier,
			AudioSystem audioSystem, boolean playback)
			throws IOException
	{
	...
	protected boolean runOnceInPlayThread()
	{
		...
		try {
			...
			if (success && (audioStreamFormat != null) && (audioStreamLength > 0)
				...
				long audioStreamDuration = (audioStreamFormat.computeDuration(
						audioStreamLength) + 999999) / 1000000;
