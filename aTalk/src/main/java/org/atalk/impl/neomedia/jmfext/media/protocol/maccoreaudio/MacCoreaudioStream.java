 ...
 */
package org.atalk.impl.neomedia.jmfext.media.protocol.maccoreaudio;
...
import org.atalk.impl.neomedia.device.MacCoreAudioDevice;
...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>MacCoreaudioStream</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 * <tt>MacCoreaudioStream</tt> in accord with the preferences of the user.
	 */
	...
	/**
	 * The buffer which stores the outgoing data before sending them to the RTP stack.
	 */
	private byte[] buffer = null;

	/**
	 * The number of bytes to read from a native MacCoreaudio stream in a single invocation. Based
	 * on {@link #framesPerBuffer}.
	 */
	private int bytesPerBuffer;

	/**
	 * The device identifier (the device UID, or if not available, the device name) of the
	 * MacCoreaudio device read through this <tt>PullBufferStream</tt>.
	 */
	...
	/**
	 * A list of already allocated and filled buffers, ready to be send throw the network.
	 */
	...
	/**
	 * A mutual eclusion used to avoid conflict when starting / stoping the stream for this stream;
	 */
	...
	private final UpdateAvailableDeviceListListener updateAvailableDeviceListListener = new UpdateAvailableDeviceListListener()
	{
		 ...
		 * MacCoreaudioStream) used before and after (if still available) the update.
		 */
	...
	/**
	 * Initializes a new <tt>MacCoreaudioStream</tt> instance which is to have its <tt>Format</tt>
	 ...
	 *        <tt>true</tt> to enable audio quality improvement for the new instance in accord with
	 *        the preferences of the user or <tt>false</tt> to completely disable audio quality
	 *        improvement
	 */
	public MacCoreaudioStream(DataSource dataSource, FormatControl formatControl,
		boolean audioQualityImprovement)
	{
		...
		// WeakReference.
		AudioSystem2 audioSystem = (AudioSystem2) AudioSystem
			.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_MACCOREAUDIO);
	...
	private void connect()
	{
		...
		int framesPerBuffer = (int) ((sampleRate * MacCoreAudioDevice.DEFAULT_MILLIS_PER_BUFFER) / (channels * 1000));
		...
		this.format = new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, channels,
	 ...
	 *
	 * @param buffer
	 *        The data captured from the input.
	 * @param bufferLength
	 *        The length of the data captured.
	 */
	public void readInput(byte[] buffer, int bufferLength)
	{
		...
		while (bufferLength > 0) {
			...
			System.arraycopy(buffer, nbCopied, this.buffer, nbBufferData, length);
	 ...
	 *
	 * @param deviceID
	 *        The ID of the device used to be read trough this MacCoreaudioStream. This String
	 *        contains the deviceUID, or if not available, the device name. If set to null, then
	 *        there was no device used before the update.
	 */
			...
			if (stream == 0 && deviceUID != null) {
				...
				try {
					stream = MacCoreAudioDevice.startStream(deviceUID, this,
						(float) format.getSampleRate(), format.getChannels(),
						format.getSampleSizeInBits(), false,
						format.getEndian() == AudioFormat.BIG_ENDIAN, false, true,
						(audioSystem == null) ? true : audioSystem.isEchoCancel());
				}
				finally {
					if (audioSystem != null)
