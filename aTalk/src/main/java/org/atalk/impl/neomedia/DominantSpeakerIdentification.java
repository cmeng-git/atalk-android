...
public class DominantSpeakerIdentification extends AbstractActiveSpeakerDetector
{
	 ...
	 * about changes in the values of the properties of this <tt>DominantSpeakerIdentification
	 * </tt>.
	 *
	 * @param listener
	...
	/**
	 * Retrieves a JSON representation of this instance for the purposes of the REST API of
	 * Videobridge.
	 * <p>
	 * By the way, the method name reflects the fact that the method handles an HTTP GET request.
	 * </p>
	 *
	 * @return a <tt>JSONObject</tt> which represents this instance of the purposes of the REST API
	 * of Videobridge
	 */
		...
		if (DEBUG) {
			synchronized (this) {
				...
				jsonObject.put("dominantSpeaker",
						(dominantSpeaker == -1) ? null : dominantSpeaker);
				...
				for (Speaker speaker : speakersCollection) {
					...
					// ssrc
					speakerJSONObject.put("ssrc", speaker.ssrc);
					// levels
	 ...
	 *
	 * @param property
	 * 		the name of the property of this <tt>DominantSpeakerIdentification</tt> which had its
	 * 		value changed
	 * @param oldValue
	 * 		the value of the property with the specified name before the change
	 * @param newValue
	 * 		the value of the property with the specified name after the change
	 */
	...
	/**
	 * Gets the synchronization source identifier (SSRC) of the dominant speaker in this multipoint
	 * conference.
	 *
	 * @return the synchronization source identifier (SSRC) of the dominant speaker in this
	 ...
	 * @return the <tt>Speaker</tt> in this multipoint conference identified by the specified
	 * <tt>ssrc</tt>
	 */
	 ...
	 * event, notifies the registered listeners that a new speaker is dominating the multipoint
	 * conference.
	 */
		...
		synchronized (this) {
			...
			if (speakerCount == 0) {
				...
				Speaker dominantSpeaker
						= (dominantSSRC == null) ? null : speakers.get(dominantSSRC);
				...
				for (Map.Entry<Long, Speaker> s : speakers.entrySet()) {
					...
					for (
							int interval = 0; interval < relativeSpeechActivities.length;
							++interval) {
	 ...
	 * integer to specify the time in milliseconds until the next execution of the
	 * <tt>DecisionMaker</tt>
	 */
	...
	private static class DecisionMaker implements Runnable
	{
		 ...
		 *
		 * @param algorithm
		 * 		the <tt>DominantSpeakerIdentification</tt> to be repeatedly run by the new
	...
	/**
	 * Represents a speaker in a multipoint conference identified by synchronization source
	 * identifier/SSRC.
	 *
	 * @author Lyubomir Marinov
	 */
	private static class Speaker
	{
		 ...
		 *
		 * @param ssrc
		 * 		the synchronization source identifier/SSRC of the new instance
		 */
		...
		/**
		 * Computes/evaluates the speech activity score of this <tt>Speaker</tt> for the immediate
		 * time-interval.
		 */
		...
		/**
		 * Computes/evaluates the speech activity score of this <tt>Speaker</tt> for the long
		 * time-interval.
		 */
		...
		/**
		 * Computes/evaluates the speech activity score of this <tt>Speaker</tt> for the medium
		 * time-interval.
		 */
		 ...
		 *
		 * @param interval
		 * 		<tt>0</tt> for the immediate time-interval, <tt>1</tt> for the medium
		 ...
		 *
		 * @param level
		 * 		the audio level which has been received or measured for this <tt>Speaker</tt>
		 */
		...
		/**
		 * Notifies this <tt>Speaker</tt> that a new audio level has been received or measured at a
		 * specific time.
		 *
		 * @param level
		 * 		the audio level which has been received or measured for this <tt>Speaker</tt>
		 * @param time
		 * 		the (local <tt>System</tt>) time in milliseconds at which the specified
		 ...
		 *
		 * @param level
		 * 		the audio level received or measured for this <tt>Speaker</tt>
		 */
