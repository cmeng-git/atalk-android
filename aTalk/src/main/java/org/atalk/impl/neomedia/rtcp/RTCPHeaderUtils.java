...
public class RTCPHeaderUtils
{
	 ...
	 *
	 * @param buf
	 * 		the byte buffer that contains the RTCP header.
	 * @param off
	 * 		the offset in the byte buffer where the RTCP header starts.
	 * @param len
	 ...
	 *
	 * @param baf
	 * 		the {@link ByteArrayBuffer} that contains the RTCP header.
	 * @return the unsigned RTCP packet type, or -1 in case of an error.
	 */
	 ...
	 * the RTCP header version and makes sure the buffer is at least 8 bytes long.
	 *
	 * @param buf
	 * 		the byte buffer that contains the RTCP header.
	 * @param off
	 ...
	 * 		the {@link ByteArrayBuffer} that contains the RTCP header.
	 * @param senderSSRC
	 * 		the sender SSRC to set.
	 * @return the number of bytes that were written to the byte buffer, or -1 in case of an error.
	 */
	...
	public static int getReportCount(ByteArrayBuffer baf)
	{
		...
		return getReportCount(baf.getBuffer(), baf.getOffset(), baf.getLength());
	}
