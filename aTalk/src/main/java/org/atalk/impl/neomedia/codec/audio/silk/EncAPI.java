...
public class EncAPI
{
	...
	static int SKP_Silk_SDK_Encode(Object encState, /* I/O: State */
		...
		int samplesIn_offset, int nSamplesIn, /* I: Number of samples in input vector */
		byte[] outData, /* O: Encoded output vector */
		int outData_offset, short[] nBytesOut /* I/O: Number of bytes in outData (input: Max bytes) */
	)
	{
		...
		if (((encControl.API_sampleRate != 8000) && (encControl.API_sampleRate != 12000)
			...
			&& (encControl.API_sampleRate != 32000) && (encControl.API_sampleRate != 44100) && (encControl.API_sampleRate != 48000))
		...
		if ((ret = ControlCodecFLP.SKP_Silk_control_encoder_FLP(psEnc, API_fs_Hz,
			max_internal_fs_kHz, PacketSize_ms, TargetRate_bps, PacketLoss_perc, UseInBandFEC,
			UseDTX, input_ms, Complexity)) != 0) {
			assert (false);
			return (ret);
		}
		/* Detect energy above 8 kHz */
		if (Math.min(API_fs_Hz, 1000 * max_internal_fs_kHz) == 24000
			&& psEnc.sCmn.sSWBdetect.SWB_detected == 0 && psEnc.sCmn.sSWBdetect.WB_detected == 0) {
		...
		while (true) {
			...
			if (psEnc.sCmn.inputBufIx >= psEnc.sCmn.frame_length) {
				...
				if (MaxBytesOut == 0) {
					...
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc, outData,
						outData_offset, MaxBytesOut_ptr, psEnc.sCmn.inputBuf, 0)) != 0) {
						assert (false);
					}
					...
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc, outData,
						outData_offset, nBytesOut, psEnc.sCmn.inputBuf, 0)) != 0) {
						assert (false);
					}
