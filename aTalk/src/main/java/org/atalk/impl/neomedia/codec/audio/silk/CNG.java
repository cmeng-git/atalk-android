...
public class CNG
{
	...
	static void SKP_Silk_CNG_exc(short residual[], /* O CNG residual signal Q0 */
		int residual_offset, int exc_buf_Q10[], /* I Random samples buffer Q10 */
	...
	)
	{
		...
		for (i = 0; i < length; i++) {
			...
			residual[residual_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
	 ...
	 *        Decoder state.
	 */
	static void SKP_Silk_CNG_Reset(SKP_Silk_decoder_state psDec /* I/O Decoder state */
	)
	{
	...
	)
	{
		...
		if (psDec.lossCnt == 0 && psDec.vadFlag == NO_VOICE_ACTIVITY) {
			...
			/* Smooth gains */
			for (i = 0; i < NB_SUBFR; i++) {
				psCNG.CNG_smth_Gain_Q16 += SKP_SMULWB(psDecCtrl.Gains_Q16[i]
