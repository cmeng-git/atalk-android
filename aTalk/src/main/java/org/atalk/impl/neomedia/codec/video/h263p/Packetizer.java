...
public class Packetizer extends AbstractPacketizer
{
	...
	/**
	 * Finds the index in <tt>byteStream</tt> at which a Picture Start code begins.
	 *
	 * @param byteStream
	 ...
	 * @return the index in <tt>byteStream</tt> at which the Picture Start code begins, otherwise,
	 * <tt>endIndex</tt>
	 */
	private static int findStartcode(byte[] byteStream, int beginIndex, int endIndex)
	{
		for (; beginIndex < (endIndex - 3); beginIndex++) {
			if ((byteStream[beginIndex] == 0) && (byteStream[beginIndex + 1] == 0)
	...
	/**
	 * The list of H263+ "Start code" video packets to be sent as payload in RTP packets.
	 */
	private final List<byte[]> videoPkts = new LinkedList<byte[]>();

	/**
	 * Initializes a new <tt>Packetizer</tt> instance which is to packetize H.263+ encoded data
	 * into RTP packets in accord with RFC 4529 "RTP Payload Format for ITU-T Rec. H.263 Video".
	 */
	public Packetizer()
	{
	...
	private Format[] getMatchingOutputFormats(Format inputFormat)
	{
		...
		return new Format[]{new VideoFormat(Constants.H263P_RTP, inputVideoFormat.getSize(),
	...
	/**
	 * Packetizes H.263+ encoded data so that it becomes ready to be sent as the payload of RTP
	 * packets.
	 *
	 * @param data
	 * 		the bytes which contain the H.263+ encoded data to be packetized
	 * @param offset
