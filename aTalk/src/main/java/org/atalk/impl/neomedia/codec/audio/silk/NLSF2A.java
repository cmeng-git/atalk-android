...
public class NLSF2A
{
	 ...
	 */
	static void SKP_Silk_NLSF2A_find_poly(int[] out, /* o intermediate polynomial, Q20 */
	...
	)
	{
		...
		for (k = 1; k < dd; k++) {
			...
			out[k + 1] = (out[k - 1] << 1)
				- (int) SigProcFIX.SKP_RSHIFT_ROUND64(SigProcFIX.SKP_SMULL(ftmp, out[k]), 20);

			for (n = k; n > 1; n--) {
				out[n] += out[n - 2]
					- (int) SigProcFIX.SKP_RSHIFT_ROUND64(SigProcFIX.SKP_SMULL(ftmp, out[n - 1]),
						20);
			}
			out[1] -= ftmp;
		}
	 ...
	 */
	static void SKP_Silk_NLSF2A(short[] a, /* o monic whitening filter coefficients in Q12, [d] */
		...
		for (i = 0; i < 10; i++) {
			...
			if (maxabs > SKP_int16_MAX) {
				...
				maxabs = SigProcFIX.SKP_min(maxabs, 98369); // ( SKP_int32_MAX / ( 65470 >> 2 ) ) +
															// SKP_int16_MAX =
															// 98369
				sc_Q16 = 65470 - (((65470 >> 2) * (maxabs - SKP_int16_MAX)) / ((maxabs * (idx + 1)) >> 2));
