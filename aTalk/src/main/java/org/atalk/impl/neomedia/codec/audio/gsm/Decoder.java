...
public class Decoder extends AbstractCodec
{
	...
	{
		super();
		this.inputFormats = new Format[] { new AudioFormat(AudioFormat.GSM, 8000, 8, 1,
			Format.NOT_SPECIFIED, AudioFormat.SIGNED, 264, Format.NOT_SPECIFIED, Format.byteArray) };
	}

	// TODO: move to base class?
	protected Format[] outputFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, 8000, 16,
		1, Format.NOT_SPECIFIED, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
		Format.byteArray) };
	...
	public Format[] getSupportedOutputFormats(Format input)
	{
		if (input == null)
			...
			final AudioFormat result = new AudioFormat(AudioFormat.LINEAR,
				inputCast.getSampleRate(), 16, 1, inputCast.getEndian(), AudioFormat.SIGNED, 16,
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer)
	{
		...
		System.arraycopy(inputBuffer.getData(), inputBuffer.getOffset(), inputContent, 0,
			inputContent.length);
		...
		if (outputBufferData == null
			|| outputBufferData.length < PCM_BYTES * innerBuffer.getLength() / GSM_BYTES) {
		...
		if (innerBuffer.getLength() < GSM_BYTES) {
			...
			GSMDecoderUtil.gsmDecode(bigEndian, (byte[]) innerBuffer.getData(),
				inputBuffer.getOffset(), innerBuffer.getLength(), outputBufferData);
