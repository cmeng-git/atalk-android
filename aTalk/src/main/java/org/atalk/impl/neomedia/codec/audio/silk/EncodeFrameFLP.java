...
public class EncodeFrameFLP
{
	 ...
	 * @return
	 */
	static int SKP_Silk_encode_frame_FLP(SKP_Silk_encoder_state_FLP psEnc, /* I/O Encoder state FLP */
		byte[] pCode, int pCode_offset, short[] pnBytesOut, /* I/O Number of payload bytes; */
		...
		int pIn_offset)
	{
		...
		if (Define.HIGH_PASS_INPUT) {
			...
			HPVariableCutoffFLP.SKP_Silk_HP_variable_cutoff_FLP(psEnc, sEncCtrl, pIn_HP, 0, pIn,
				pIn_offset);
		}
		else {
		...
		if (Define.SWITCH_TRANSITION_FILTERING != 0) {
			...
			LPVariableCutoff.SKP_Silk_LP_variable_cutoff(psEnc.sCmn.sLP, pIn_HP_LP, 0, pIn_HP, 0,
		...
		/*******************************************/
		SigProcFLP.SKP_short2float_array(x_frame, x_frame_offset + psEnc.sCmn.la_shape, pIn_HP_LP,
			0, psEnc.sCmn.frame_length);
		...
		/*****************************************/
		FindPitchLagsFLP.SKP_Silk_find_pitch_lags_FLP(psEnc, sEncCtrl, res_pitch, x_frame,
			x_frame_offset);

		/************************/
		/* Noise shape analysis */
		/************************/
		NoiseShapeAnalysisFLP.SKP_Silk_noise_shape_analysis_FLP(psEnc, sEncCtrl, res_pitch_frame,
			res_pitch_frame_offset, x_frame, x_frame_offset);
		...
		/* Encode Parameters */
		/****************************************/
		EncodeParameters.SKP_Silk_encode_parameters(psEnc.sCmn, sEncCtrl.sCmn, psEnc.sCmn.sRC,
			psEnc.sCmn.q);
		...
		/****************************************/
		/* Update input buffer */
		System.arraycopy(psEnc.x_buf, psEnc.x_buf_offset + psEnc.sCmn.frame_length, psEnc.x_buf,
			psEnc.x_buf_offset, psEnc.sCmn.frame_length + psEnc.sCmn.la_shape);
		...
		if (psEnc.sCmn.nFramesInPayloadBuf * Define.FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms) {
			...
			if (pnBytesOut[0] >= nBytes[0]) {
				...
				if (frame_terminator > Define.SKP_SILK_MORE_FRAMES
					...
					System.arraycopy(psEnc.sCmn.LBRR_buffer[LBRR_idx].payload, 0, pCode,
						pCode_offset + nBytes[0], psEnc.sCmn.LBRR_buffer[LBRR_idx].nBytes);
		...
		psEnc.BufferedInChannel_ms = SigProcFLP.SKP_LIMIT_float(psEnc.BufferedInChannel_ms, 0.0f,
			100.0f);
		psEnc.sCmn.nBytesInPayloadBuf = nBytes[0];

		if (psEnc.speech_activity > DefineFLP.WB_DETECT_ACTIVE_SPEECH_LEVEL_THRES) {
			psEnc.sCmn.sSWBdetect.ActiveSpeech_ms = SigProcFIX.SKP_ADD_POS_SAT32(
				psEnc.sCmn.sSWBdetect.ActiveSpeech_ms, Define.FRAME_LENGTH_MS);
		}
	 ...
	 *        Input signal.
	 */
	static void SKP_Silk_LBRR_encode_FLP(SKP_Silk_encoder_state_FLP psEnc, /* I/O Encoder state FLP */
	...
	)
	{
		...
		if (psEnc.sCmn.LBRR_enabled != 0) {
			...
			if (psEnc.sCmn.Complexity > 0 && psEnc.sCmn.TargetRate_bps > Rate_only_parameters) {
				if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
					...
					psEncCtrl.sCmn.GainsIndices[0] = SigProcFIX.SKP_LIMIT(
						psEncCtrl.sCmn.GainsIndices[0], 0, Define.N_LEVELS_QGAIN - 1);
				}
				...
				/* Noise shaping quantization */
				/*****************************************/
				WrappersFLP.SKP_Silk_NSQ_wrapper_FLP(psEnc, psEncCtrl, xfw, 0, psEnc.sCmn.q_LBRR,
					0, 1);
			}
			else {
			...
			/* Encode Parameters */
			/****************************************/
			EncodeParameters.SKP_Silk_encode_parameters(psEnc.sCmn, psEncCtrl.sCmn,
			...
			/* Finalize payload and copy to output */
			/****************************************/
			if (SKP_SMULBB(nFramesInPayloadBuf, Define.FRAME_LENGTH_MS) >= psEnc.sCmn.PacketSize_ms) {

				/* Check if FEC information should be added */
				frame_terminator = Define.SKP_SILK_LAST_FRAME;

				/* Add the frame termination info to stream */
				RangeCoder.SKP_Silk_range_encoder(psEnc.sCmn.sRC_LBRR, frame_terminator,
