...
import org.atalk.impl.neomedia.transform.csrc.*;
 ...
 * @author Emil Ivov
 */
public class AudioMediaStreamImpl extends MediaStreamImpl
		implements AudioMediaStream, PropertyChangeListener
{
	/**
	 ...
	 *
	 * @param listener
	 * 		the <tt>DTMFListener</tt> to register for notifications about the remote party
	 ...
	 *
	 * @param extensionID
	 * 		the ID assigned to <tt>rtpExtension</tt> for the lifetime of this stream.
	 * @param rtpExtension
			...
			if (ssrcEngine != null) {
				...
				if (ssrcDir.allowsSending()) {
					...
					if (deviceSession != null) {
						deviceSession.enableOutputSSRCAudioLevels(true,
								ssrcExtID == null ? -1 : ssrcExtID);
					}
				}
			}
	 ...
	 *
	 * @param audioLevels
	 * 		an array mapping CSRC IDs to audio levels in consecutive elements.
	 */
		...
		if (cfg != null) {
			...
			try {
				...
				logger.warn(bufferLengthStr
						+ " is not a valid receive buffer length/long value", nfe);
			}
		}
	 ...
	 * 		the new tone
	 * @param end
	 * 		<tt>true</tt> if the tone is to be ended or <tt>false</tt> to be started
	 */
	 ...
	 *
	 * @param rtpManager
	 * 		the <tt>RTPManager</tt> to register {@link #CUSTOM_CODEC_FORMATS} with
	...
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager)
	{
		...
		for (AudioFormat format : CUSTOM_CODEC_FORMATS) {
			...
			rtpManager.addFormat(format,
					MediaUtils.getRTPPayloadType(format.getEncoding(), format.getSampleRate()));
		}
	}
	 ...
	 *
	 * @param listener
	 * 		the <tt>DTMFListener</tt> to no longer be notified by this <tt>AudioMediaStream</tt>
	 ...
	 * 		if <tt>dtmfMethod</tt> is not one of {@link DTMFMethod#INBAND_DTMF},
		...
		switch (dtmfMethod) {
			...
			case RTP_DTMF:
				if (dtmfTransformEngine != null) {
					...
					if (t != null)
						dtmfTransformEngine.startSending(t, minimalToneDuration,
								maximalToneDuration, volume);
				}
				break;
	...
	public void stopSendingDTMF(DTMFMethod dtmfMethod)
	{
		switch (dtmfMethod) {
			case INBAND_DTMF:
				// The INBAND DTMF is sent by impulse of constant duration and does not need to
