...
public class TlsServerImpl extends DefaultTlsServer
{
	 ...
	 */
	private final CertificateRequest certificateRequest = new CertificateRequest(
			new short[]{ClientCertificateType.rsa_sign},
			null /* supportedSignatureAlgorithms */,
			null /* certificateAuthorities */);
	...
	/**
	 * Gets the <tt>SRTPProtectionProfile</tt> negotiated between this DTLS-SRTP server and its
	 * client.
	 *
			...
			throws IOException
	{
		...
		if (maxFragmentLengthOffered >= 0
			...
			TlsExtensionsUtils.addMaxFragmentLengthExtension(checkServerExtensions(),
		...
		if (TlsECCUtils.isECCCipherSuite(selectedCipherSuite)) {
			...
			TlsECCUtils.addSupportedPointFormatsExtension(checkServerExtensions(),
