...
/**
 * Logs all the packets that go in and out of a <tt>MediaStream</tt>. In order to use it, you can
 * setup logging like this:
 * <p>
 * <pre>
 ...
 * <p>
 * <pre>
 * mkfifo ~/.sip-communicator/log/atalk0.pcap
 * </pre>
 * <p>
 * Finally, you can launch Wireshark like this (assuming that you're using Bash):
 * <p>
 * <pre>
 * wireshark -k -i &lt;(cat ~/.sip-communicator/log/atalk0.pcap)
 * </pre>
 *
...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>DebugTransformEngine</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 *
	 */
	private final static String LOG_RTP_PAYLOAD_PNAME
	...
	/**
	 * The {@code PacketLoggingService} (to be) used by this instance. Cached for the sake of
	 * performance because fetching OSGi services is not inexpensive.
	 */
	 ...
	 * 		the <tt>MediaStream</tt> that owns this instance.
	 * @param pktLogging
	 * 		the {@code PacketLoggingService} to be used by the new instance
	 */
	...
	/**
	 * Creates and returns a new <tt>DebugTransformEngine</tt> if logging is enabled for
	 * <tt>PacketLoggingService.ProtocolName.ARBITRARY</tt>.
	 *
	 * @param mediaStream
	 * 		the <tt>MediaStream</tt> that will own the newly created <tt>DebugTransformEngine</tt>.
	 *
	...
	public static DebugTransformEngine createDebugTransformEngine(MediaStreamImpl mediaStream)
	{
		...
		if (pktLogging != null
				&& pktLogging.isLoggingEnabled(PacketLoggingService.ProtocolName.ARBITRARY)) {
	 ...
	 *
	 * @param pkt
	 * 		the {@code RawPacket} to log
	 * @param data
	 * 		{@code true} if {@code pkt} represents a data/RTP packet or {@code false} if
	 * 		{@code pkt} represents a control/RTCP packet
	 * @param sender
	 * 		{@code true} if {@code pkt} has originated from the local peer and is to be sent to
	 * 		the remote peer or {@code false} if {@code pkt} has originated from the remote peer
	 * 		and has been received by the local peer
	 * @return {@code pkt}
	 */
	private RawPacket transform(RawPacket pkt, boolean data, boolean sender)
	{
		...
		if (data) {
			...
			if (targetDataAddress == null) {
				logger.warn("Not logging a packet because the media stream target address" +
						" is null.");
				return pkt;
			}
		...
		// When the caller is a receiver, the situation is the exact opposite of when the caller
		// is a sender.
		if (!sender) {
	...
	/**
	 * Logs RTP packets that go in and out of the <tt>MediaStream</tt> that owns this instance.
	 */
			...
			extends SinglePacketTransformer
	{
		...
		{
			return DebugTransformEngine.this.transform(
					pkt,
					true, /* data */
					false /* sender */
			);
	...
	/**
	 * Logs RTCP packets that go in and out of the <tt>MediaStream</tt> that owns this instance.
	 */
