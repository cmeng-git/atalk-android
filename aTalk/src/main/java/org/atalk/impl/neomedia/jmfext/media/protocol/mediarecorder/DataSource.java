...
/**
 * Implements <tt>PushBufferDataSource</tt> and <tt>CaptureDevice</tt> using Android's
 * <tt>MediaRecorder</tt>.
 *
...
public class DataSource extends AbstractPushBufferCaptureDevice
{
	 ...
	 */
	private static final String LOCAL_SERVER_SOCKET_NAME = DataSource.class.getName()
		+ ".localServerSocket";
	 ...
	 * states: [t]he maximum size of a NAL unit encapsulated in any aggregation packet is 65535
	 * bytes.
	 */
		...
		throws IOException
	{
		if (this.mediaRecorder == null) {
			...
			try {
				camera = getCamera();
				if (camera == null) {
					throw new RuntimeException("Unable to select camera for locator: "
						+ getLocator());
				}

				// Adjust preview display orientation
				int rotation = CameraUtils.getCameraDisplayRotation(AndroidCamera
					.getCameraId(getLocator()));
	...
	/**
	 * Configures the camera nad media recorder to work with given <tt>videoFormat</tt>.
	 *
	 * @param camera
	 *        the camera to be configured.
	 * @param mediaRecorder
	 *        the media recorder to be configured.
	 * @param videoFormat
	...
	private void configure(Camera camera, MediaRecorder mediaRecorder, VideoFormat videoFormat)
	{
		 ...
		 * provided with. Anyway, MediaRecorder.setVideoSize(int,int) is not always supported so it
		 * may (or may not) turn out that Camera.Parameters.setPictureSize(int,int) saves the day in
		 * some cases.
		 */
	 ...
	 * @throws IOException
	 *         if we failed to retrieve the parameters.
	 */
	private H264Parameters obtainParameters(Camera camera, MediaRecorder mediaRecorder)
		throws IOException
	{
		final String sampleFile = aTalkApp.getGlobalContext().getCacheDir().getPath()
			+ "/atalk-test.mpeg4";
		...
		try {
			...
			if (mediaRecorderStopState != null) {
				...
				if (logger.isDebugEnabled()
					...
					logger.debug("Stopping and/or releasing MediaRecorder seemed to"
						+ " take a long time and we decided to give"
						+ " up; otherwise, we may have had to wait" + " forever.");
				}
			}
	...
	/**
	 * Gets a <tt>Camera</tt> instance which corresponds to the <tt>MediaLocator</tt> of this
	 * <tt>DataSource</tt>.
	 *
	 * @return a <tt>Camera</tt> instance which corresponds to the <tt>MediaLocator</tt> of this
	 *         <tt>DataSource</tt>
	 * @throws IOException
		...
		try {
			...
			while (true) {
				...
				if (type == FILE_TYPE_BOX_TYPE) {
					...
					if (size == 1) {
						size = readUnsignedInt64(inputStream) /* largesize */
						- 8;
					}
					if (size == 0) {
	...
	private void nalRead()
	{
		...
		/*
		 * Determine whether the access unit time stamp associated with the (current) NAL unit is to
		 * be changed.
		 */
		...
		/*
		 * Determine whether the Buffer flags associated with the (current) NAL unit are to be
		 * changed.
		 */
		...
		synchronized (this) {
			...
			synchronized (nalSyncRoot) {
				...
				if (bytes.length < nalLength)
					...
					/*
					 * Notify this DataSource that a NAL unit has just been read from the
	...
	private static void runInLocalServerSocketThread()
	{
		while (true) {
			...
			if (maxDataSourceKeySize < 1) {
				...
				localSocketAcceptedThread.setName(DataSource.class.getName()
	 ...
	 * specified <tt>mediaRecorder</tt> which may be slow and performing additional cleanup in the
	 * meantime.
	 *
	 * @param mediaRecorder
	...
	private void writeNAL()
	{
		...
		synchronized (getStreamSyncRoot()) {
			...
			stream = ((streams != null) && (streams.length != 0)) ? (MediaRecorderStream) streams[0]
				: null;
		}
			...
			throws IOException
		{
			...
			synchronized (dataSource.nalSyncRoot) {
				...
				if (nalLength > 0) {
					...
					System.arraycopy(dataSource.nal, 0, bytes, H264.NAL_PREFIX.length,
						nalLength);
			...
			if (bytes == null)
				...
				Arrays.fill(bytes, byteLength - FFmpeg.FF_INPUT_BUFFER_PADDING_SIZE, byteLength,
					(byte) 0);
