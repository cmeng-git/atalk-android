...
class Postfil extends Ld8k
{
	...
	/*
	 * File : POSTFIL.C Used for the floating point version of G.729 main body (not for G.729A)
	 */
	 ...
	 */
	int post(int t0, float[] signal_ptr, int signal_ptr_offset, float[] coeff, int coeff_offset,
		float[] sig_out, int sig_out_offset)
	{
		...
		Filter.syn_filt(apond1, 0, sig_ltp, sig_ltp_ptr, sig_ltp, sig_ltp_ptr, L_SUBFR, mem_stp, 0,
			1);
	 ...
	 * @return voicing decision 0 = uv, > 0 delay
	 */
	private int pst_ltp(int t0, float[] ptr_sig_in, int ptr_sig_in_offset, float[] ptr_sig_pst0,
		int ptr_sig_pst0_offset)
	{
		...
		search_del(t0, ptr_sig_in, ptr_sig_in_offset, _ltpdel, _phase, _num_gltp, _den_gltp, y_up,
			_off_yup);
		...
		if (num_gltp == 0.f) {
			...
			if (phase == 0) {
				...
				compute_ltp_l(ptr_sig_in, ptr_sig_in_offset, ltpdel, phase, ptr_sig_pst0,
					ptr_sig_pst0_offset, _num2_gltp, _den2_gltp);
			...
			filt_plt(ptr_sig_in, ptr_sig_in_offset, ptr_y_up, ptr_y_up_offset, ptr_sig_pst0,
				ptr_sig_pst0_offset, gain_plt);
		}
		return vo;
	}
	 ...
	 * @param off_yup
	 */
	private void search_del(int t0, float[] ptr_sig_in, int ptr_sig_in_offset, IntReference ltpdel,
		IntReference phase, FloatReference num_gltp, FloatReference den_gltp, float[] y_up,
	 ...
	 *        input : filter gain
	 */
	private void filt_plt(float[] s_in, int s_in_offset, float[] s_ltp, int s_ltp_offset,
		float[] s_out, int s_out_offset, float gain_plt)
	{
	...
	/**
	 * Compute delayed signal, num & den of gain for fractional delay with long interpolation filter
	 *
	 * @param s_in
	 ...
	 */
	private void compute_ltp_l(float[] s_in, int s_in_offset, int ltpdel, int phase, float[] y_up,
		int y_up_offset, FloatReference num, FloatReference den)
	{
	...
	/**
	 * Selects best of (gain1, gain2) with gain1 = num1 / den1 and gain2 = num2 / den2
	 *
	 * @param num1
	...
	/**
	 * Computes impulse response of A(gamma2) / A(gamma1). controls gain : computation of energy
	 * impulse response as SUMn (abs (h[n])) and computes parcor0
	 *
	 * @param apond2
	 ...
	 */
	private float calc_st_filt(float[] apond2, float[] apond1, float[] sig_ltp_ptr,
	...
	/**
	 * Tilt filtering with : (1 + mu z-1) * (1/1-|mu|). computes y[n] = (1/1-|mu|) (x[n]+mu*x[n-1])
	 *
	 * @param sig_in
	...
	/**
	 * Control of the subframe gain. gain[n] = AGC_FAC * gain[n-1] + (1 - AGC_FAC) g_in/g_out
	 *
	 * @param sig_in
	 ...
	 */
	private float scale_st(float[] sig_in, int sig_in_offset, float[] sig_out, int sig_out_offset,
		float gain_prec)
	{
