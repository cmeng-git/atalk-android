...
public class AudioNotifierServiceImpl implements AudioNotifierService, PropertyChangeListener
{
	...
	public SCAudioClip createAudio(String uri, boolean playback)
	{
		...
		synchronized (audiosSyncRoot) {
			...
			/*
			 * Make sure that the SCAudioClip will be reclaimed for reuse when it is no longer in
			 * use.
			 */
			if (audio != null) {
				if (audios == null)
	 ...
	 *
	 * @param mute
	 * 		when <tt>true</tt> disables the sound; otherwise, enables the sound.
	 */
	 ...
	 * <tt>SCAudioClip</tt>
	 * in question uses the playback or the notify audio device.
	 */
