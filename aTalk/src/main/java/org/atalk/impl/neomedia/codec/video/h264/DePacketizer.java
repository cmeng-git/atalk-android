...
import static org.atalk.impl.neomedia.codec.video.h264.H264.*;
...
public class DePacketizer extends AbstractCodec2
{
	...
	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>DePacketizer</tt> to control its key
	 * frame-related logic.
	 */
	 ...
	 * fragmentation unit, the value is equal to the <tt>nal_unit_type</tt> of the fragmented NAL
	 * unit.
	 */
	 ...
	 *
	 * @param in
	 * 		the payload of the RTP packet from which a FU-A fragment of a NAL unit is to be
	 * 		extracted
	 * @param inOffset
	 * 		the offset in <tt>in</tt> at which the payload begins
	 * @param inLength
	 * 		the length of the payload in <tt>in</tt> beginning at <tt>inOffset</tt>
	 * @param outBuffer
	 * 		the <tt>Buffer</tt> which is to receive the extracted FU-A fragment of a NAL unit
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer)
	{
		...
		if ((nal_unit_type >= 1) && (nal_unit_type <= 23)) {
			...
			ret = dePacketizeSingleNALUnitPacket(
					nal_unit_type, in, inOffset, inBuffer.getLength(), outBuffer);
		}
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat)
	{
		...
		if ((matchingOutputFormats != null)
			...
			Map<String, String> fmtps
					= ((ParameterizedVideoFormat) inputFormat).getFormatParameters();
				...
				if (!fmtps.isEmpty()) {
					for (int i = 0; i < matchingOutputFormats.length; i++) {
						matchingOutputFormats[i]
								= new ParameterizedVideoFormat(Constants.H264, fmtps);
					}
				}
			}
		}
	 ...
	 *
	 * @param urgent
	 * 		<tt>true</tt> if the caller has determined that the need
	 ...
	 * 		<tt>true</tt> if this <tt>DePacketizer</tt> is to request a key frame from the remote
	 * 		peer associated with {@link #keyFrameControl}
	 */
