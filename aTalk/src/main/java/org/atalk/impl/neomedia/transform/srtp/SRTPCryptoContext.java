...
public class SRTPCryptoContext extends BaseSRTPCryptoContext
{
	 ...
	 * whether protection against replay attacks is to be activated. The default value is
	 * <tt>true</tt>.
	 */
	...
	/**
	 * The indicator which determines whether {@link #s_l} has seen set i.e. appropriately
	 * initialized.
	 */
	private boolean seqNumSet = false;

	/**
	 * Constructs an empty SRTPCryptoContext using ssrc. The other parameters are set to default
	 * null value.
	 *
	 * @param sender
	 *        <tt>true</tt> if the new instance is to be used by an SRTP sender; <tt>false</tt> if
	 ...
	 * @param roc
	 *        the initial Roll-Over-Counter according to RFC 3711. These are the upper 32 bit of the
	 *        overall 48 bit SRTP packet index. Refer to chapter 3.2.1 of the RFC.
	 * @param keyDerivationRate
	 *        the key derivation rate defines when to recompute the SRTP session keys. Refer to
	 *        chapter 4.3.1 in the RFC.
	 * @param masterK
	 *        byte array holding the master key for this SRTP cryptographic context. Refer to
	 *        chapter 3.2.1 of the RFC about the role of the master key.
	 * @param masterS
	 *        byte array holding the master salt for this SRTP cryptographic context. It is used to
	 ...
	 * @param policy
	 *        SRTP policy for this SRTP cryptographic context, defined the encryption algorithm, the
	...
	public void processPacketAESCM(RawPacket pkt)
	{
		...
		cipherCtr.process(pkt.getBuffer(), pkt.getOffset() + payloadOffset, payloadLength,
			ivStore);
	}
	 ...
	 * @param guessedIndex
	 *        the SRTP index of the accepted SRTP packet calculated by <tt>guessIndex(int)</tt>
	 */
