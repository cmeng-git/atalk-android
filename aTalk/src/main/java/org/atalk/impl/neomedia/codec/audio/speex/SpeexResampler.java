...
{
	/**
	 * The list of <tt>Format</tt>s of audio data supported as input and output by
	...
	/**
	 * The pointer to the native <tt>SpeexResamplerState</tt> which is represented by this
	 * instance.
	 */
	...
	/**
	 * Resamples audio from a specific input <tt>Buffer</tt> into a specific output
	 * <tt>Buffer</tt>.
	 *
	 * @param inBuffer
	 * 		input <tt>Buffer</tt>
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer)
	{
		...
		if (inSampleRate == outSampleRate) {
			...
			/*
			 * XXX The method Speex.speex_resampler_process_interleaved_int will crash if in is
			 * null.
			 */
			if (inSampleCount == 0) {
				outSampleCount = 0;
				...
				/*
				 * Report how many bytes of inBuffer have been consumed in the sample rate
				 * conversion.
				 */
	 ...
	 *
	 * @param format
	 * 		the <tt>Format</tt> of the media data to be input for processing in this
	 * 		<tt>Codec</tt>
	 * @return the <tt>Format</tt> of the media data to be input for processing in this
	 * <tt>Codec</tt> if <tt>format</tt> is compatible with this <tt>Codec</tt>; otherwise,
	 * <tt>null</tt>
	...
	public Format setInputFormat(Format format)
	{
		...
		if (inFormat != null) {
			...
			if (outputFormat == null) {
				...
				/*
				 * Conversion between data types is only supported when not resampling but rather
				 * passing through.
				 */
