...
public class MacCoreaudioRenderer extends AbstractAudioRenderer<MacCoreaudioSystem>
{
	...
	/**
	 * A mutual exclusion used to avoid conflict when starting / stopping the stream for this
	 * renderer;
	 */
	private Object startStopMutex = new Object();

	/**
	 * The buffer which stores the incoming data before sending them to CoreAudio.
	 */
	...
	/**
	 * Locked when currently stopping the stream. Prevents deadlock between the CaoreAudio callback
	 * and the AudioDeviceStop function.
	 */
	...
	/**
	 * The list of JMF <tt>Format</tt>s of audio data which <tt>MacCoreaudioRenderer</tt> instances
	...
	/**
	 * The <tt>UpdateAvailableDeviceListListener</tt> which is to be notified before and after
	 * MacCoreaudio's native function <tt>UpdateAvailableDeviceList()</tt> is invoked. It will
	 * close
	...
	/**
	 * Initializes a new <tt>MacCoreaudioRenderer</tt> instance which is to either perform playback
	 * or sound a notification.
	 *
	 * @param playback
	...
	public MacCoreaudioRenderer(boolean playback)
	{
		super(AudioSystem.LOCATOR_PROTOCOL_MACCOREAUDIO,
				playback ? AudioSystem.DataFlow.PLAYBACK : AudioSystem.DataFlow.NOTIFY);

		// XXX We will add an UpdateAvailableDeviceListListener and will not
		// remove it because we will rely on MacCoreaudioSystem's use of WeakReference.
		if (audioSystem != null) {
			audioSystem.addUpdateAvailableDeviceListListener(updateAvailableDeviceListListener);
		}
	}
	...
	public Format[] getSupportedInputFormats()
	{
		if (supportedInputFormats == null) {
			...
			if (deviceUID == null) {
				...
				// them will take a lot of time. Besides, we currently support at most 2.
				int maxOutputChannels = Math.min(MacCoreAudioDevice.countOutputChannels(deviceUID),
						2);
				...
				for (Format supportedInputFormat : SUPPORTED_INPUT_FORMATS) {
					getSupportedInputFormats(supportedInputFormat, minOutputChannels,
	...
	private void getSupportedInputFormats(Format format, int minOutputChannels,
			int maxOutputChannels, List<Format> supportedInputFormats)
	{
		...
		float minRate
				= MacCoreAudioDevice.getMinimalNominalSampleRate(deviceUID, true, isEchoCancel);
		float maxRate = MacCoreAudioDevice.getMaximalNominalSampleRate(deviceUID, true,
				isEchoCancel);
	...
	/**
	 * Opens the MacCoreaudio device and output stream represented by this instance which are to be
	 * used to render audio.
	 *
	 * @throws ResourceUnavailableException
	 * 		if the MacCoreaudio device or output stream cannot be created or opened
	 */
	@Override
	public void open()
			throws ResourceUnavailableException
	 ...
	 * {@link AudioSystem#FEATURE_NOTIFY_AND_PLAYBACK_DEVICES} property of its associated
	 * <tt>AudioSystem</tt> has changed.
	 *
	 * @param ev
	 * 		a <tt>PropertyChangeEvent</tt> which specifies details about the change such as the
	...
	/**
	 * Renders the audio data contained in a specific <tt>Buffer</tt> onto the MacCoreaudio device
	 * represented by this <tt>Renderer</tt>.
	 *
	 * @param buffer
	 * 		the <tt>Buffer</tt> which contains the audio data to be rendered
	...
	public int process(Buffer buffer)
	{
		synchronized (startStopMutex) {
			if (stream != 0 && !isStopping) {
				...
				if (gainControl != null) {
					BasicVolumeControl.applyGain(gainControl, (byte[]) buffer.getData(),
				...
				int timeout = 2000;
				int maxNbBuffers = timeout / MacCoreAudioDevice.DEFAULT_MILLIS_PER_BUFFER;
				updateBufferLength(Math.min(nbBufferData + length, length * maxNbBuffers));

				if (nbBufferData + length > this.buffer.length)
					length = this.buffer.length - nbBufferData;

				// Copy the received data.
				System.arraycopy((byte[]) buffer.getData(), buffer.getOffset(), this.buffer,
						nbBufferData, length);
				nbBufferData += length;
			}
		}
	...
	/**
	 * Sets the <tt>MediaLocator</tt> which specifies the device index of the MacCoreaudio
	 * device to be used by this instance for rendering.
	 *
	 * @param locator
	 * 		a <tt>MediaLocator</tt> which specifies the device index of the MacCoreaudio device to
	 * 		be used by this instance for rendering
	 */
	@Override
	 ...
	 * with this <tt>MacCoreaudioRenderer</tt> will begin being rendered.
	 */
	public void start()
	{
		// Start the stream
		synchronized (startStopMutex) {
			if (stream == 0 && deviceUID != null) {
				...
				try {
					/*
					 * XXX A Renderer will participate in the acoustic echo cancellation if
					 * (acoustic echo cancellation is enabled, of course, and) the Renderer is not
					 * sounding a notification.
					 */
					boolean isEchoCancel = AudioSystem.DataFlow.PLAYBACK.equals(dataFlow)
							&& audioSystem.isEchoCancel();

					stream = MacCoreAudioDevice.startStream(deviceUID, this,
							(float) inputFormat.getSampleRate(), nbChannels,
							inputFormat.getSampleSizeInBits(), false,
							inputFormat.getEndian() == AudioFormat.BIG_ENDIAN, false, false,
							isEchoCancel);
				} finally {
					audioSystem.didOpenStream();
				}
	...
	public void stop()
	{
		...
		synchronized (startStopMutex) {
			if (stream != 0 && deviceUID != null && !isStopping) {
				...
				while (nbBufferData > 0 && (currentTime - startTime) < timeout) {
					try {
	...
	/**
	 * Writes the data received to the buffer give in arguments, which is provided by the CoreAudio
	 * library.
	 *
	 * @param buffer
	 * 		The buffer to fill in provided by the CoreAudio library.
	 * @param bufferLength
	 * 		The length of the buffer provided.
	 */
	public void writeOutput(byte[] buffer, int bufferLength)
	{
		// If the stop function has been called, then skip the synchronize which can lead to a
		// deadlock because the AudioDeviceStop native function waits for this callback to end.
		if (stopLock.tryLock()) {
			try {
				synchronized (startStopMutex) {
					...
					nbBufferData -= length;
					if (nbBufferData > 0) {
						System.arraycopy(this.buffer, length, this.buffer, 0, nbBufferData);
					}
	...
	/**
	 * Increases the buffer length if necessary: if the new length is greater than the current
	 * buffer length.
	 *
	 * @param newLength
	...
	private void updateBufferLength(int newLength)
	{
		synchronized (startStopMutex) {
			if (this.buffer == null) {
				...
				System.arraycopy(this.buffer, 0, newBuffer, 0, nbBufferData);
