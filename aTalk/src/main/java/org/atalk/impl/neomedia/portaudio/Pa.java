...
public final class Pa
{
	 ...
	 *
	 * @param deviceInfo
	 * 		the pointer to the <tt>PaDeviceInfo</tt> to get the human-readable name of
	 ...
	 *
	 * @param deviceInfo
	 * 		the pointer to the <tt>PaDeviceInfo</tt> instance to get the name of
	 ...
	 *
	 * @param deviceID
	 * 		a <tt>String</tt> identifying the PortAudio device to retrieve the index of. It is
	...
	public static int getDeviceIndex(String deviceID, int minInputChannels, int minOutputChannels)
	{
		if (deviceID != null) {
			...
			for (int deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
				...
				if (deviceID.equals(((deviceUID == null) || (deviceUID.length() == 0))
						 ? Pa.DeviceInfo_getName(deviceInfo) : deviceUID)) {
					/*
			...
			if (suggestedLatencyString != null) {
				try {
					...
					logger.error("Failed to parse configuration property "
							+ PROP_SUGGESTED_LATENCY + " value as a double", nfe);
				}
			}
		}
	 ...
	 * 		A structure that describes the input parameters used to open a stream.
	 * @param outputParameters
	 * 		A structure that describes the output parameters used to open a stream.
	 * @param sampleRate
	 ...
	 */
	public static native boolean IsFormatSupported(long inputParameters, long outputParameters,
			double sampleRate);
	 ...
	 * 		The number of frames passed to the stream callback function, or the preferred block
	 * 		granularity for a blocking read/write stream
	 * @param streamFlags
	 * 		Flags which modify the behavior of the streaming process.
	 * @param streamCallback
	 * 		A pointer to a client supplied function that is responsible for processing and filling
	 * 		input and output buffers. If <tt>null</tt>, the stream will be opened in 'blocking
	 * 		read/write' mode.
	 * @return pointer to the opened stream.
	 * @throws PortAudioException
	 */
	public static native long OpenStream(long inputParameters, long outputParameters,
			double sampleRate, long framesPerBuffer, long streamFlags,
	 ...
	 *
	 * @param stream
	 * 		the (input) PortAudio stream for which denoise is to be enabled or disabled
	 * @param denoise
	 * 		<tt>true</tt> if denoise is to be performed on the audio data provided by
	 ...
	 *
	 * @param stream
	 * 		the (input) PortAudio stream for which the number of milliseconds of echo to be
	 * 		canceled is to be set
	 * @param echoFilterLengthInMillis
	 * 		the number of milliseconds of echo to be canceled in the audio data provided by
	 * 		<tt>stream</tt>
	 */
	 ...
	 * 		the suggested latency in milliseconds: LATENCY_UNSPECIFIED - use default(default high
	 ...
	 */
	public static native long StreamParameters_new(int deviceIndex, int channelCount,
			long sampleFormat, double suggestedLatency);
	 ...
	 * {@link #WriteStream(long, byte[], long)} with one and the same buffer because the JNI access
	 ...
	 * @param offset
	 * 		the byte offset in <tt>buffer</tt> at which the samples to be written start
	 * @param frames
	 * 		the number of frames from <tt>buffer</tt> starting at <tt>offset</tt> are to be
	 * 		written with a single write
	 * @param numberOfWrites
	 * 		the number of writes each writing <tt>frames</tt> number of frames to be performed
	 * @throws PortAudioException
