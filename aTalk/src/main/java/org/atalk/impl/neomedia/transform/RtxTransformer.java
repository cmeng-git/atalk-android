...
public class RtxTransformer implements TransformEngine
{
	...
	public static final String DISABLE_NACK_TERMINATION_PNAME
			= "neomedia.rtcp.DISABLE_NACK_TERMINATION";
	 ...
	 *
	 * @param mediaStream
	 * 		the <tt>MediaStreamImpl</tt> for the transformer.
	 */
	 ...
	 *
	 * @param ssrc
	 * 		the SSRC of the RTX stream for the packet.
	 ...
	 *
	 * @param pkt
	 * 		the {@code RawPacket} that holds the RTP packet for
	 ...
	 * @param rtxPt
	 * 		the RTX payload type to use for the re-transmitted packet.
	 * @param after
	 * 		the {@code TransformEngine} in the chain of
	...
	public int sendPadding(long ssrc, int bytes)
	{
		...
		if (receiveRTPManager == null) {
			logger.warn("rtp_manager_not_found, stream_hash="
					+ mediaStream.hashCode() + " ssrc=" + ssrc);
			return bytes;
		}

		MediaStream receiveStream = receiveRTPManager.getMediaStream();
		if (receiveStream == null) {
			logger.warn("stream_not_found, stream_hash="
					+ mediaStream.hashCode() + " ssrc=" + ssrc);
			return bytes;
		}
