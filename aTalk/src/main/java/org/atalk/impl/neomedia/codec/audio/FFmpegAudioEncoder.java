...
public class FFmpegAudioEncoder extends AbstractFFmpegAudioCodec
{
	 ...
	 *
	 * @param codecID
	 *        the <tt>AVCodecID</tt> of the encoder to find
	 * @throws RuntimeException
	 *         if no encoder with the specified <tt>codecID</tt> is found by FFmpeg
	 */
	public static void assertFindAVCodec(int codecID)
	{
		if (FFmpeg.avcodec_find_encoder(codecID) == 0) {
			throw new RuntimeException("Could not find FFmpeg encoder: "
	...
	protected synchronized int doProcess(Buffer inBuf, Buffer outBuf)
	{
		...
		int outLen = FFmpeg
			.avcodec_encode_audio(avctx, out, outOff, out.length - outOff, in, inOff);
