...
public class PLC
{
	 ...
	 *        Decoder state.
	 */
	static void SKP_Silk_PLC_Reset(SKP_Silk_decoder_state psDec /* I/O Decoder state */
	)
	{
	 ...
	 * @param length
	 */
	static void SKP_Silk_PLC_update(SKP_Silk_decoder_state psDec, /* (I/O) Decoder state */
			...
			for (j = 0; j * psDec.subfr_length < psDecCtrl.pitchL[NB_SUBFR - 1]; j++) {
				...
				if (temp_LTP_Gain_Q14 > LTP_Gain_Q14) {
					...
					System.arraycopy(psDecCtrl.LTPCoef_Q14,
						SKP_SMULBB(NB_SUBFR - 1 - j, LTP_ORDER), psPLC.LTPCoef_Q14, 0, LTP_ORDER);
	 ...
	 *        Length of residual.
	 */
	static void SKP_Silk_PLC_conceal(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		SKP_Silk_decoder_control psDecCtrl, /* I/O Decoder control */
		short[] signal, /* O concealed signal */
		int signal_offset, int length /* I length of residual */
	)
	{
		...
		for (k = (NB_SUBFR >> 1); k < NB_SUBFR; k++) {
			for (i = 0; i < psDec.subfr_length; i++) {
				exc_buf_ptr[exc_buf_ptr_offset + i] = (short) (SKP_SMULWW(psDec.exc_Q10[i + k
					* psDec.subfr_length], psPLC.prevGain_Q16[k]) >> 10);
			}
		...
		if (psDec.lossCnt == 0) {
			...
			if (psDec.prev_sigtype == SIG_TYPE_UNVOICED) {
				...
				down_scale_Q30 = Math.max(((1 << 30) >> LOG2_INV_LPC_GAIN_LOW_THRES),
					down_scale_Q30);
		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			for (i = 0; i < psDec.subfr_length; i++) {
				...
				LPC_exc_Q10 = (SKP_SMULWB(rand_ptr[rand_ptr_offset + idx], rand_scale_Q14) << 2); /*
																								 * Random
																								 * noise
																								 * part
																								 */
				LPC_exc_Q10 = (LPC_exc_Q10 + SigProcFIX.SKP_RSHIFT_ROUND(LTP_pred_Q14, 4)); /*
																							 * Harmonic
																							 * part
																							 */
			...
			psPLC.pitchL_Q8 = Math.min(psPLC.pitchL_Q8,
				(SKP_SMULBB(MAX_PITCH_LAG_MS, psDec.fs_kHz) << 8));
			lag = SigProcFIX.SKP_RSHIFT_ROUND(psPLC.pitchL_Q8, 8);
		}
		...
		for (k = 0; k < NB_SUBFR; k++) {
			for (i = 0; i < psDec.subfr_length; i++) {
				...
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 2],
					A_Q12_tmp[1]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 3],
					A_Q12_tmp[2]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 4],
					A_Q12_tmp[3]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 5],
					A_Q12_tmp[4]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 6],
					A_Q12_tmp[5]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 7],
					A_Q12_tmp[6]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 8],
					A_Q12_tmp[7]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 9],
					A_Q12_tmp[8]);
				LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - 10],
					A_Q12_tmp[9]);

				for (j = 10; j < psDec.LPC_order; j++) {
					LPC_pred_Q10 = SKP_SMLAWB(LPC_pred_Q10, psDec.sLPC_Q14[MAX_LPC_ORDER + i - j
						- 1], A_Q12_tmp[j]);
				}
		...
		for (i = 0; i < psDec.frame_length; i++) {
			signal[signal_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(
				SKP_SMULWW(sig_Q10[i], psPLC.prevGain_Q16[NB_SUBFR - 1]), 10));
		}
	 ...
	 */
	static void SKP_Silk_PLC_glue_frames(SKP_Silk_decoder_state psDec, /* I/O decoder state */
		SKP_Silk_decoder_control psDecCtrl, /* I/O Decoder control */
	...
	)
	{
		...
		if (psDec.lossCnt != 0) {
			...
			if (psDec.sPLC.last_frame_lost != 0) {
				...
				SumSqrShift.SKP_Silk_sum_sqr_shift(energy_ptr, energy_shift_ptr, signal,
					signal_offset, length);
				energy = energy_ptr[0];
