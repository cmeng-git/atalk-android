 ...
 */
package org.atalk.impl.neomedia.codec.video.h263p;
...
/**
 * Depacketizes H.263+ RTP packets in in accord with RFC 4529 "RTP Payload Format for
 * ITU-T Rec.H.263 Video".
 *
...
public class DePacketizer extends AbstractCodec2
{
	...
	/**
	 * The indicator which determines whether incomplete buffer packets are output from the H.263+
	 * <tt>DePacketizer</tt> to the decoder.
	 */
	private static final boolean OUTPUT_INCOMPLETE_BUFFER = true;

	/**
	 * Initializes a new <tt>DePacketizer</tt> instance which is to depacketize H.263+ RTP packet.
	 */
	public DePacketizer()
	{
	 ...
	 * {@link AbstractCodec#opened} is <tt>false</tt>. All required input and/or output formats
	 * are assumed to have been set on this <tt>Codec</tt> before <tt>doOpen</tt> is called.
	 *
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer)
	{
		...
		if ((lastSequenceNumber != -1) && ((sequenceNumber - lastSequenceNumber) != 1)) {
			...
			/*
			 * maybe we lost a frame somewhere or the sequence number reach its maximum number
			 */
			if (logger.isTraceEnabled())
				logger.trace(
						"Dropped RTP packets upto sequenceNumber " + lastSequenceNumber + " and " +
								"continuing with sequenceNumber " + sequenceNumber);
		...
		System.arraycopy(in, inOffset + 2 + (vBit ? 1 : 0) + plen, out, outOffset
				+ (pBit ? 2 : 0), dataLength - (pBit ? 2 : 0));
		 ...
		 * RTP time stamp to allow an efficient playout buffer handling. Consequently, we have to
		  * output it as well.
		 */
	...
	/**
	 * Appends {@link #outputPaddingSize} number of bytes to <tt>out</tt> beginning at index
	 * <tt>outOffset</tt>. The specified <tt>out</tt> is expected to be large enough to
	 * accommodate the mentioned number of bytes.
	 *
	 * @param out
	 * 		the buffer in which <tt>outputPaddingSize</tt> number of bytes are to be written
	 * @param outOffset
	 * 		the index in <tt>outOffset</tt> at which the writing of <tt>outputPaddingSize</tt>
	 * 		number of bytes is to begin
	 */
	...
	/**
	 * Resets the states of this <tt>DePacketizer</tt> and a specific output <tt>Buffer</tt> so
	 * that they are ready to have this <tt>DePacketizer</tt> process input RTP payloads.
	 *
	 * @param outBuffer
