...
public class JavaEncoder extends AbstractCodec2 implements AdvancedAttributesAwareCodec
{
	...
	/**
	 * The previous input if it was less than the input frame size and which is to be prepended to
	 * the next input in order to form a complete input frame.
	 */
	private byte[] prevIn;
	...
	public JavaEncoder()
	{
		super("G.729 Encoder", AudioFormat.class, new AudioFormat[] { new AudioFormat(
			AudioFormat.G729_RTP, 8000, AudioFormat.NOT_SPECIFIED, 1) });

		inputFormats = new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1,
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer)
	{
		...
		byte[] output = validateByteArraySize(outBuffer, outBuffer.getOffset()
			+ OUTPUT_FRAMES_COUNT * OUTPUT_FRAME_SIZE_IN_BYTES, true);

		packetize(serial, output, outBuffer.getOffset() + OUTPUT_FRAME_SIZE_IN_BYTES
			* outFrameCount);
		...
		if (ret == BUFFER_PROCESSED_OK) {
			updateOutput(outBuffer, getOutputFormat(), outBuffer.getLength(), outBuffer.getOffset());
	...
	{
		Arrays.fill(outFrame, outFrameOffset, outFrameOffset + L_FRAME / 8, (byte) 0);
