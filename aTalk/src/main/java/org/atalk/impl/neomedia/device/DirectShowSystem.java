...
import org.atalk.impl.neomedia.jmfext.media.protocol.directshow.DSCaptureDevice;
import org.atalk.impl.neomedia.jmfext.media.protocol.directshow.DSFormat;
import org.atalk.impl.neomedia.jmfext.media.protocol.directshow.DSManager;
import org.atalk.impl.neomedia.jmfext.media.protocol.directshow.DataSource;
...
public class DirectShowSystem extends DeviceSystem
{
	 ...
	 *
	 * @throws Exception
	 *         if anything goes wrong while discovering and registering DirectShow capture defines
	 *         with JMF
	 */
		...
		throws Exception
	{
		...
		try {
			...
			for (int i = 0, count = (devices == null) ? 0 : devices.length; i < count; i++) {
				...
				CaptureDeviceInfo cdi = new CaptureDeviceInfo(name, new MediaLocator(
					LOCATOR_PROTOCOL + ':' + name), formatsArray);
