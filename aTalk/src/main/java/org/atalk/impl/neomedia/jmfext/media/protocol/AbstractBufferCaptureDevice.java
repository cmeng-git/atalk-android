 ...
 */
public abstract class AbstractBufferCaptureDevice<AbstractBufferStreamT
		extends AbstractBufferStream<?>>
		implements CaptureDevice, Controls
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractBufferCaptureDevice</tt> class and its instances
	 * for logging output.
	 */
	...
	/**
	 * The indicator which determines whether the transfer of media data from this
	 * <tt>DataSource</tt> has been started.
	 */
	 ...
	 *
	 * @throws IOException
	 * 		if anything goes wrong while opening the connection to the media source of this
	...
	protected FormatControl createFormatControl(final int streamIndex)
	{
		...
		{
			/**
			 * The <tt>Format</tt> of this <tt>FormatControl</tt> and, respectively, of the media
			 * data of its owner.
			 */
			 ...
			 * <tt>FormatControl</tt>.
			 *
			 * @return the <tt>Format</tt> of the media data of the owner of this
			 *         <tt>FormatControl</tt>
			 */
			 ...
			 * <tt>Format</tt> in which the owner of this <tt>FormatControl</tt> is to provide
			 * media data.
			 *
			 * @param format
			 ...
			 *         whether it was actually set; <tt>null</tt> if <tt>format</tt> is not
			...
			public Format setFormat(Format format)
			{
				...
				if (setFormat != null) {
					setFormat = AbstractBufferCaptureDevice.this.internalSetFormat(streamIndex,
							setFormat, format);
					if (setFormat != null)
	 ...
	 *
	 * @param streamIndex
	 * 		the zero-based index of the <tt>AbstractBufferStream</tt> in the list of streams of
	...
	/**
	 * Provides the default implementation of <tt>AbstractBufferCaptureDevice</tt> for
	 * {@link #getFormat(int, Format)}.
	 *
	 * @param streamIndex
	 * 		the zero-based index of the <tt>AbstractBufferStream</tt> the <tt>Format</tt> of which
	 * 		is to be retrieved
	 * @param oldValue
	 * 		the last-known <tt>Format</tt> for the <tt>AbstractBufferStream</tt> at the specified
	 * 		<tt>streamIndex</tt>
	 ...
	 *
	 * @param streamIndex
	 * 		the zero-based index of the <tt>AbstractBufferStream</tt> for which the specified
	 ...
	 *
	 * @param captureDevice
	 * 		the <tt>CaptureDevice</tt> to gets the <tt>CaptureDeviceInfo</tt> of
	 * @return the <tt>CaptureDeviceInfo</tt> of the specified <tt>CaptureDevice</tt> as registered
	 * in JMF
	 */
	 ...
	 *
	 * @param <SourceStreamT>
	 * 		the type of <tt>SourceStream</tt> which is to be the element type of the returned
	 * 		array
	 * @param clz
	 * 		the <tt>Class</tt> of <tt>SourceStream</tt> which is to be the element type of the
	 * 		returned array
	 ...
	 * the
	 * time of the attempt to set its <tt>Format</tt>. Allows extenders to override the default
	 ...
	 * the <tt>Format</tt> did not success and any last-known <tt>Format</tt> is to be left
	 * in effect
	 */
