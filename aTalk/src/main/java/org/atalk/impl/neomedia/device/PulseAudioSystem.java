...
public class PulseAudioSystem extends AudioSystem
{
	 ...
	 * automatic, default PulseAudio device to be used in the absence of a specification of a
	 * PulseAudio device.
	 */
	 ...
	 * @param b
	 *        <tt>true</tt> to pause or <tt>false</tt> to resume the playback of the specified
	 *        <tt>stream</tt>
	 */
	...
	public Renderer createRenderer(boolean playback)
	{
		...
		PulseAudioRenderer renderer = new PulseAudioRenderer(playback ? MEDIA_ROLE_PHONE
			: MEDIA_ROLE_EVENT);
	 ...
	 *
	 * @param sampleRate
	 *        the sample rate at which the new PulseAudio stream is to input or output
	 * @param channels
	 *        the number of channels of the audio to be input or output by the new PulseAudio stream
	 * @param mediaName
	 *        the human-readable name of the new PulseAudio stream
	 * @param mediaRole
	 *        the PulseAudio logic role of the new stream
		...
		throws Exception
	{
		...
		if (!captureDeviceFormats.isEmpty()) {
			captureDevices.add(
				0,
				new CaptureDeviceInfo2(NULL_DEV_CAPTURE_DEVICE_INFO_NAME, new MediaLocator(
					LOCATOR_PROTOCOL + ":"), captureDeviceFormats
					.toArray(new Format[captureDeviceFormats.size()]), null, null, null));
		}
	 ...
	 *
	 * @param proplist
	 *        the <tt>pa_proplist</tt> which is to be populated with <tt>pa_context</tt>-related
	 *        properties such as the application name and version
	 */
	 ...
	 *
	 * @param waitForAccept
	 *        <tt>true</tt> to not return before the signal is accepted by a
	 ...
	 *
	 * @param context
	 *        the connection context for asynchronous communication with the PulseAudio server which
	 *        is reporting the <tt>sinkInfo</tt>
	 * @param sinkInfo
	 *        the information about the sink being reported
	 * @param deviceList
	 *        the list of <tt>CaptureDeviceInfo</tt>s which reperesents existing devices and into
	 *        which the callback is represent the <tt>sinkInfo</tt>
	 * @param formatList
	 *        the list of <tt>Format</tt>s supported by the various <tt>CaptureDeviceInfo</tt>s in
	 ...
	 * @param sourceInfo
	 *        the information about the source being reported
	 * @param deviceList
	 *        the list of <tt>CaptureDeviceInfo</tt>s which reperesents existing devices and into
	 ...
	 *         if a PulseAudio event loop thread exists and is associated with this
	 ...
	 *
	 * @param context
	 *        the PulseAudio context to wait for
	 * @param stateToWaitForm
	 *        the PulseAudio state of the specified <tt>context</tt> to wait for
	 ...
	 *
	 * @param stream
	 *        the PulseAudio stream to wait for
	 * @param stateToWaitForm
	 *        the PulseAudio state of the specified <tt>stream</tt> to wait for
