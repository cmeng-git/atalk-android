...
import org.atalk.impl.neomedia.recording.*;
 ...
 */
public class MediaServiceImpl extends PropertyChangeNotifier
		implements MediaService
{
	/**
	 * The <tt>Logger</tt> used by the <tt>MediaServiceImpl</tt> class and its instances for
	 * logging output.
	 */
	...
	/**
	 * The prefix of the property names the values of which specify the dynamic payload type
	 * preferences.
	 */
	 ...
	 * <tt>MediaDevice</tt>s are available. Explicitly defined in order to reduce unnecessary
	 * allocations.
	 */
	...
	/**
	 * The indicator which determines whether the loading of the JMF/FMJ <tt>Registry</tt> is
	 * disabled.
	 */
	 ...
	 * for the purposes of working with the <tt>MediaStream</tt>s created by this
	 * <tt>MediaService</tt>.
	 */
	 ...
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt> to be used by the new instance for capture and playback of
	 * 		media
	 ...
	 * <tt>AudioMediaStreamImpl</tt> or <tt>VideoMediaStreamImpl</tt> in accord with
	 * <tt>mediaType</tt>
	 */
	 ...
	 *
	 * @param connector
	 * 		the <tt>StreamConnector</tt> that the new <tt>MediaStream</tt> instance is to use for
	 * 		sending and receiving media
	 * @param device
	 * 		the <tt>MediaDevice</tt> that the new <tt>MediaStream</tt> instance is to use for both
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> of the new <tt>MediaStream</tt> instance to be initialized. If
	 ...
	 * @param connector
	 * 		the <tt>StreamConnector</tt> to be used by the new instance if non-<tt>null</tt>
	 * @param device
	 * 		the <tt>MediaDevice</tt> to be used by the instance if non- <tt>null</tt>
	 * @param srtpControl
	 * 		the <tt>SrtpControl</tt> to be used by the new instance if non- <tt>null</tt>
	 * @return a new <tt>MediaStream</tt> instance
	 */
	 ...
	 * returned <tt>MediaDevice</tt> is the same as the <tt>MediaType</tt> of the specified
	 * <tt>device</tt>.
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt> which is to be used by the returned <tt>MediaDevice</tt> to
	 ...
	 *
	 * @param mediaType
	 * 		a <tt>MediaType</tt> value indicating the type of media to be handled by the
	 * 		<tt>MediaDevice</tt> to be obtained
	 * @param useCase
	 * 		the <tt>MediaUseCase</tt> to obtain the <tt>MediaDevice</tt> list for
		...
		if (captureDeviceInfo != null) {
			for (MediaDevice device : getDevices(mediaType, useCase)) {
				if ((device instanceof MediaDeviceImpl)
						...
						((MediaDeviceImpl) device).getCaptureDeviceInfo())) {
	...
	/**
	 * Gets the <tt>CaptureDevice</tt> user choices such as the default audio and video capture
	 * devices.
	 *
	 * @return the <tt>CaptureDevice</tt> user choices such as the default audio and video capture
	 * devices.
	 */
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> to obtain the <tt>MediaDevice</tt> list for
	 * @param useCase
	 * 		the <tt>MediaUseCase</tt> to obtain the <tt>MediaDevice</tt> list for
	 ...
	 *
	 * @return the current encoding configuration -- the instance that contains the global
	 * settings.
	 */
	 ...
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt> which is used for media capture and playback by the call to
	 * 		be recorded
		...
		if (dynamicPayloadTypePreferences == null) {
			...
			if (cfg != null) {
				...
				for (String propertyName : propertyNames) {
					...
					try {
						dynamicPayloadTypePreference = Byte.parseByte(
					...
					if (exception != null) {
						logger.warn("Ignoring dynamic payload type preference"
								+ " which could not be parsed: " + propertyName, exception);
						continue;
					}
					...
					if ((source != null) && (source.length() != 0)) {
						try {
							...
							if (json.containsKey(MediaFormatImpl.FORMAT_PARAMETERS_PNAME)) {
								JSONObject jsonFmtps = (JSONObject) json.get(
							...
							MediaFormat mediaFormat = MediaUtils.getMediaFormat(encoding,
									clockRate, fmtps);
							...
							logger.warn("Ignoring dynamic payload type preference"
									+ " which could not be parsed: " + source, jsone);
						}
					}
				}
			}
	 ...
	 */
	@Override
	public Object getVideoPreviewComponent(MediaDevice device, int preferredWidth,
			int preferredHeight)
	{
		ResourceManagementService resources = LibJitsi.getResourceManagementService();
		String noPreviewText = (resources == null)
				? "" : resources.getI18NString("media.configform.NO_PREVIEW");
		...
		try {
			CaptureDeviceInfo captureDeviceInfo;
			if ((device != null) && ((captureDeviceInfo = ((MediaDeviceImpl) device)
	 ...
	 * 		the player.
	 */
	private static void showPreview(final JComponent previewContainer, final Component preview,
			final Player player)
	{
	...
	public Point getOriginForDesktopStreamingDevice(MediaDevice mediaDevice)
	{
		...
		int index = Integer.parseInt(
				((split != null) && (split.length > 1)) ? split[0] : remainder);
	 ...
	 *
	 * @param listener
	 * 		the new <tt>Recorder.Listener</tt> to be added to the list of listeners interested in
	 ...
	 *
	 * @param listener
	 * 		the existing <tt>Listener</tt> to be removed from the list of listeners interested in
	 * 		notifications from <tt>Recorder</tt>s
	 */
	 ...
	 *
	 * @param event
	 * 		a <tt>PropertyChangeEvent</tt> which specifies the name of the property which had its
	 ...
	 * 		the <tt>MediaServiceImpl</tt> instance which has caused the need to perform the
	...
	private static void setupFMJ()
	{
		...
		jmfRegistryDisableLoad
				= "true".equalsIgnoreCase(System.getProperty(JMF_REGISTRY_DISABLE_LOAD));
		...
		if (scHomeDirLocation != null) {
			...
			if (scHomeDirName != null) {
				...
				if (System.getProperty(jmfRegistryFilename) == null) {
					System.setProperty(jmfRegistryFilename,
		...
		if (cfg != null) {
			...
			for (String prop : cfg.getPropertyNamesByPrefix(
					"neomedia.adaptive_jitter_buffer", true)) {
	 ...
	 *
	 * @param mediaType
	 * 		the <tt>MediaType</tt> to be determined whether the support for it is enabled
	 * @return <tt>true</tt> if the support for the specified <tt>mediaType</tt> is enabled;
	 * otherwise, <tt>false</tt>
	 */
	public static boolean isMediaTypeSupportEnabled(MediaType mediaType)
	{
		...
		ConfigurationService cfg = LibJitsi.getConfigurationService();
		return ((cfg == null) || !cfg.getBoolean(propertyName, false))
