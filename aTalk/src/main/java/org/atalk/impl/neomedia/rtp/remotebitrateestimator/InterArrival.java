...
class InterArrival
{
	 ...
	 * most {@code timestampGroupLengthTicks} older than the first timestamp in that group.
	 *
	...
	private boolean belongsToBurst(long arrivalTimeMs, long timestamp)
	{
		...
		long arrivalTimeDeltaMs = arrivalTimeMs - currentTimestampGroup.completeTimeMs;
	...
	/**
	 * Returns {@code true} if a delta was computed, or {@code false} if the current group is still
	 ...
	 * @param deltas
	 * 		{@code timestampDelta} is the computed timestamp delta, {@code arrivalTimeDeltaMs} is
	...
	public boolean computeDeltas(long timestamp, long arrivalTimeMs, int packetSize,
			long[] deltas, long systemTimeMs)
	{
		if (deltas == null)
		...
		if (currentTimestampGroup.isFirstPacket()) {
			...
			if (prevTimestampGroup.completeTimeMs >= 0) {
				/* long timestampDelta */
				deltas[0] = TimestampUtils.subtractAsUnsignedInt32(
						currentTimestampGroup.timestamp,
						prevTimestampGroup.timestamp);

				long arrivalTimeDeltaMs
						= deltas[1]
						= currentTimestampGroup.completeTimeMs - prevTimestampGroup.completeTimeMs;
			...
			currentTimestampGroup.timestamp
					= latestTimestamp(currentTimestampGroup.timestamp, timestamp);
		}
	...
	/**
	 * Returns {@code true} if the last packet was the end of the current batch and the packet with
	 * {@code timestamp} is the first of a new batch.
	 *
	 * @param arrivalTimeMs
	 * @param timestamp
	 * @return
	...
	private static class TimestampGroup
	{
		 ...
		 *
		 * @param source
		 * 		the {@code TimestampGroup} the values of the fields of which are to be assigned to
		 * 		the respective fields of this {@code TimestampGroup}
		 */
