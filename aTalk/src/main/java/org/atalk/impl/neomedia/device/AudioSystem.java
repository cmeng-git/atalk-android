...
public abstract class AudioSystem extends DeviceSystem
{
	...
	public static AudioSystem[] getAudioSystems()
	{
		...
		return (audioSystems == null)
				? null : audioSystems.toArray(new AudioSystem[audioSystems.size()]);
	}
	 ...
	 * {@inheritDoc}
	 *
	 * Delegates to {@link #createRenderer(boolean)} with the value of the <tt>playback</tt>
		...
		throws IOException
	{
		...
		try {
			...
			if (url == null)
				url = new URL(uri);
			audioStream = org.atalk.android.util.javax.sound.sampled.AudioSystem
	 ...
	 *
	 * @param dataFlow
	 *        the flow of the media data supported by the <tt>CaptureDeviceInfo2</tt> to be returned
	 * @param locator
	 *        the <tt>MediaLocator</tt> of the <tt>CaptureDeviceInfo2</tt> to be returned
	 ...
	 *
	 * @param dataFlow
	 *        the data flow of the devices to retrieve: capture, notify or playback
	 ...
	 *        the <tt>InputStream</tt> providing audio media to determine the FMJ format of
	 ...
	 *        the (base) <tt>AudioSystem</tt>-specific property name of which the associated (full)
	...
	{
		return DeviceConfiguration.PROP_AUDIO_SYSTEM + "." + getLocatorProtocol() + "."
			+ basePropertyName;
	}
	 ...
	 *
	 * @param dataFlow
	 *        the data flow of the selected device to retrieve: capture, notify or playback.
	 ...
	 *        <tt>true</tt> if automatic gain control (AGC) is to be performed for captured audio;
	 *        otherwise, <tt>false</tt>
	 */
	 ...
	 *
	 * @param denoise
	 *        <tt>true</tt> if noise suppression is to be performed for captured audio; otherwise,
	 *        <tt>false</tt>
	 */
	 ...
	 *
	 * @param dataFlow
	 *        the data flow of the device to set: capture, notify or playback
	 * @param device
	 *        The selected active device.
	 * @param save
	 *        Flag set to true in order to save this choice in the configuration. False otherwise.
	 */
