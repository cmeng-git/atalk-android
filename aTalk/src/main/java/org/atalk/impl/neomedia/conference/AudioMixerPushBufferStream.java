 ...
 */
class AudioMixerPushBufferStream extends ControlsAdapter
		implements PushBufferStream
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMixerPushBufferStream</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 *
	 * @param audioMixer
	 * 		the <tt>AudioMixer</tt> which creates this instance and for which it is to output data
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> in which the new instance is to output data
	 */
	 ...
	 *
	 * @param outStream
	 * 		the <tt>AudioMixingPushBufferStream</tt> to add to the collection of such streams to
	 * 		which this instance is to push the data for audio mixing it reads from its input
	 * 		<tt>SourceStream</tt>s
	 * @throws IOException
	 * 		if <tt>outStream</tt> was the first <tt>AudioMixingPushBufferStream</tt> and the
	 ...
	 *
	 * @param inStreamDesc
	 * 		an <tt>InStreamDesc</tt> which describes the input stream and its originating
	 ...
	 *
	 * @param buffer
	 * 		the <tt>Buffer</tt> in which the audio samples read from the input
	 * 		<tt>SourceStream</tt>s are to be returned to the caller
	 * @throws IOException
	 * 		if any of the input <tt>SourceStream</tt>s throws such an exception while reading
	 ...
	 *
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> in which the audio samples read from the
	 * 		<tt>PullBufferStream</tt>s are to be converted before being returned
	 * @param outSampleCount
	 * 		the maximum number of audio samples to be read from each of the
	 * 		<tt>PullBufferStream</tt>s but the very <tt>PullBufferStream</tt>s may not honor the
	 * 		request
	 * @param inSampleDesc
	 * 		an <tt>InStreamDesc</tt> which specifies the input streams to be read and the
	 ...
	 * @throws IOException
	 * 		if anything goes wrong while reading the specified input streams
	 */
	private int readInPullBufferStreams(AudioFormat outFormat, int outSampleCount,
			InSampleDesc inSampleDesc)
			throws IOException
	{
	 ...
	 *
	 * @param inStreamDesc
	 * 		an <tt>InStreamDesc</tt> which specifies the input <tt>PushBufferStream</tt> to read
	 * 		from
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> to which the samples read from <tt>inStream</tt> are to be
	 * 		converted before being returned
	 * @param sampleCount
	 * 		the maximum number of samples which the read operation should attempt to read from
	 * 		<tt>inStream</tt> but the very <tt>inStream</tt> may not honor the request
	 * @param outBuffer
	 * 		the <tt>Buffer</tt> into which the array of <tt>int</tt> audio samples read from the
	 * 		specified <tt>inStream</tt> is to be written
	 * @throws IOException
	 * 		if anything wrong happens while reading <tt>inStream</tt>
	 * @throws UnsupportedFormatException
	 * 		if converting the samples read from <tt>inStream</tt> to <tt>outFormat</tt> fails
	 */
			...
			throws IOException, UnsupportedFormatException
	{
		...
		if ((inChannels != outChannels) && (inChannels != Format.NOT_SPECIFIED)
			...
			logger.error("Read inFormat with channels " + inChannels
					+ " while expected outFormat channels is " + outChannels);
		...
		if (inSampleRate != outSampleRate) {
			logger.warn("Read inFormat with sampleRate " + inSampleRate
		...
		if (inData == null) {
			...
			if (logger.isTraceEnabled() && (inSampleSizeInBits != outSampleSizeInBits)) {
				logger.trace("Read inFormat with sampleSizeInBits " + inSampleSizeInBits
	 ...
	 * @param inSampleDesc
	 * 		an <tt>InSampleDesc</tt> which specifies the input streams to be read and the
	 * 		collection of audio samples in which the read audio samples are to be returned
	 * @return the maximum number of audio samples actually read from the input
	 * <tt>PushBufferStream</tt>s of this instance
	 * @throws IOException
	 * 		if anything wrong happens while reading the specified input streams
	 * @throws UnsupportedFormatException
	 * 		if any of the input streams provides media in a format different than
	 * 		<tt>outFormat</tt>
	 */
	 ...
	 *
	 * @param outStream
	 * 		the <tt>AudioMixingPushBufferStream</tt> to remove from the collection of such streams
	 * 		to which this instance pushes the data for audio mixing it reads from its input
	 * 		<tt>SourceStream</tt>s
	 * @throws IOException
	 * 		if <tt>outStream</tt> was the last <tt>AudioMixingPushBufferStream</tt> and the
	 ...
	 *
	 * @param outStream
	 * 		the <tt>AudioMixingPushBufferStream</tt> to push the specified set of audio samples to
	 * @param inSampleDesc
	 * 		the set of audio samples to be pushed to <tt>outStream</tt> for audio mixing
	 * @param maxInSampleCount
	 * 		the maximum number of audio samples available in <tt>inSamples</tt>
	 */
	 ...
	 *
	 * @param inStreams
	 * 		the <tt>SourceStream</tt>s (in the form of <tt>InStreamDesc</tt>) from which this
		...
		if (valueIsChanged) {
			...
			for (InStreamDesc inStreamDesc : newValue) {
				...
				if (!(inStream instanceof CachingPushBufferStream)) {
					...
					if (logger.isTraceEnabled())
						logger.trace("Created CachingPushBufferStream with hashCode "
			...
			if (logger.isTraceEnabled()) {
				...
				if (difference > 0)
					logger.trace("Added " + difference + " inStream(s) and the total is "
							+ newValueLength);
				else if (difference < 0)
					logger.trace("Removed " + difference + " inStream(s) and the total is "
							+ newValueLength);
			}
		}
	}
	 ...
	 *
	 * @param inStreams
	 * 		the input <tt>SourceStream</tt>s to which the specified <tt>BufferTransferHandler</tt>
	 * 		is to be set
	 * @param transferHandler
	 ...
	 *
	 * @param buffer
	 * 		the cached <tt>Buffer</tt> instance to be used during the execution of the method in
