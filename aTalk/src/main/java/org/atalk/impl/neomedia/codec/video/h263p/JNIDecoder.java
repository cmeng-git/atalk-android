...
public class JNIDecoder extends AbstractCodec
{
	...
	/**
	 * Initializes a new <tt>JNIDecoder</tt> instance which is to decode H.263+ encoded data into
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat)
	{
		...
		return new Format[]{new AVFrameFormat(inputVideoFormat.getSize(),
				ensureFrameRate(inputVideoFormat.getFrameRate()), FFmpeg.PIX_FMT_YUV420P)};
	}
			...
			throws ResourceUnavailableException
	{
		...
		/*
		 * from ffmpeg -formats output: "For example, the h263 decoder corresponds to the h263 and
		  * h263p encoders". That's why we use CODEC_ID_H263 for the decoder side (instead of
		  * CODEC_ID_H263P).
		 */
	...
	/**
	 * Decodes H.263+ media data read from a specific input <tt>Buffer</tt> into a specific output
	 * <tt>Buffer</tt>.
	 *
	 * @param in
	 * 		input <tt>Buffer</tt>
	 ...
	 *
	 * @param format
	 * 		the <tt>Format</tt> of the media data to be input for processing in this <tt>Codec</tt>
