...
{
	/**
	 * The Java equivalent of the DirectShow <tt>ISampleGrabberCB</tt> interface as it is utilized
	 * by <tt>DSCaptureDevice</tt>.
	 */
	public interface ISampleGrabberCB
	{
		 ...
		 *
		 * @param source
		 * 		pointer to the native <tt>DSCaptureDevice</tt> which is the source of the
		 * 		notification
		 * @param ptr
		 * 		a pointer to the captured/grabbed video frame i.e. to the data of the DirectShow
		 * 		<tt>IMediaSample</tt>
		 * @param length
		 * 		the length in bytes of the valid data pointed to by <tt>ptr</tt>
		 */
		void SampleCB(long source, long ptr, int length);
	}

	/**
	 * Empty array with <tt>DSFormat</tt> element type. Explicitly defined in order to avoid
	 ...
	 * <p>
	 * This pointer is hold and will be released by <tt>DSManager</tt> singleton.
	 */
	 ...
	 *
	 * @param ptr
	 * 		a pointer to a native <tt>DSCaptureDevice</tt> to disconnect from
	 */
	 ...
	 * @param format
	 * 		format to set
	 * @return an <tt>HRESULT</tt> value indicating whether the specified <tt>format</tt> was
	 * successfully set or describing a failure
	 */
