...
public class AVFrameFormat extends VideoFormat
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AVFrameFormat</tt> class and its instances to print out debugging
	 * information.
	 */
	 ...
	 *        the <tt>DeviceSystem</tt>-specific colorspace to be represented by the new instance
	 */
	public AVFrameFormat(Dimension size, float frameRate, int pixFmt, int deviceSystemPixFmt) {
		...
		if ((pixFmt == NOT_SPECIFIED) && (deviceSystemPixFmt != NOT_SPECIFIED)) {
			logger.warn("Specifying a device system-specific pixel format 0x" + Long.toHexString(deviceSystemPixFmt & 0xffffffffL)
				+ " without a matching FFmpeg pixel format may" + " eventually lead to a failure.", new Throwable());
		}
	 ...
	 *
	 * @param obj
	 *        the <tt>Object</tt> to be determined whether it represents a value that is equal to the value represented
	 *        by this instance
