...
public class AudioMixer
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMixer</tt> class and its instances for logging
	 * output.
	 */
	 ...
	 *
	 * @param dataSource
	 * 		the <tt>DataSource</tt> for which the <tt>Format</tt> in which it provides stream data
	 * 		is to be determined
	 ...
	 *
	 * @param stream
	 * 		the <tt>SourceStream</tt> for which the <tt>Format</tt> in which it provides data is
	 * 		to be determined
	 ...
	 * 		a new <tt>DataSource</tt> to input audio to this instance
	 * @param outDataSource
	 * 		the <tt>AudioMixingPushBufferDataSource</tt> to not include the audio contributions of
	 ...
	 * @param inDataSource
	 * 		the <tt>DataSource</tt> which is the cause for <tt>dataSource</tt> to exist in this
	 * 		<tt>AudioMixer</tt>
	 * @throws IOException
	 * 		if anything wrong happens while connecting to <tt>dataSource</tt>
	 */
	 ...
	 *
	 * @param inDataSource
	 * 		the <tt>InDataSourceDesc</tt> of the input <tt>DataSource</tt> which has finished its
	 * 		connecting procedure
	 * @throws IOException
	 * 		if anything wrong happens while including <tt>inDataSource</tt> into the mix
	 */
	 ...
	 *
	 * @param inStream
	 * 		the input <tt>SourceStream</tt> to be described by the new instance
	 * @param inDataSourceDesc
	 * 		the input <tt>DataSource</tt> given by its <tt>InDataSourceDesc</tt> to be described
	 * 		by the new instance
	 ...
	 *
	 * @param inDataSourceDesc
	 * 		the <tt>InDataSourceDesc</tt> describing the input <tt>DataSource</tt> to be
	 * 		transcoded into the specified output <tt>Format</tt> and to receive the transcoding
	 * 		<tt>DataSource</tt>
	 * @param outFormat
	 * 		the <tt>Format</tt> in which the tracks of the input <tt>DataSource</tt> are to be
	 * 		transcoded
	 ...
	 *
	 * @param inStream
	 * 		the <tt>SourceStream</tt> to locate an <tt>InStreamDesc</tt> for in
	 * 		<tt>existingInStreamDescs</tt>
	 * @param existingInStreamDescs
	 * 		the list of existing <tt>InStreamDesc</tt>s in which an <tt>InStreamDesc</tt> for
	 ...
	 * 		<tt>AudioFormat</tt>
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> in which the collected <tt>SourceStream</tt>s are to produce
	 * 		data
	 * @param existingInStreams
	 * 		the <tt>InStreamDesc</tt> instances which already exist and which are used to avoid
	 * 		creating multiple <tt>InStreamDesc</tt>s for input <tt>SourceStream</tt>s which
	 * 		already have ones
	 * @param inStreams
	 * 		the <tt>List</tt> of <tt>InStreamDesc</tt> in which the discovered
	 ...
	 *
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> in which the retrieved <tt>SourceStream</tt>s are to produce
	 * 		data
	 * @param existingInStreams
	 * 		the <tt>SourceStream</tt>s which are already known to this <tt>AudioMixer</tt>
	 ...
	 * @throws IOException
	 * 		if anything wrong goes while retrieving the input <tt>SourceStream</tt>s from the
	 ...
	 *
	 * @param stream
	 * 		the <tt>PushBufferStream</tt> to read media from and known to belong to the specified
	 * 		<tt>DataSOurce</tt>
	 * @param buffer
	 * 		the output <tt>Buffer</tt> in which the media read from the specified <tt>stream</tt>
	 * 		is to be written so that it gets returned to the caller
	 * @param dataSource
	 * 		the <tt>DataSource</tt> from which <tt>stream</tt> originated
	 * @throws IOException
	 * 		if anything wrong happens while reading from the specified <tt>stream</tt>
	 */
	 ...
	 *
	 * @param outFormat
	 * 		the <tt>AudioFormat</tt> in which the input <tt>DataSource</tt>s of this
				...
				if (formatControl != null) {
					...
					if ((inFormat == null) || !matches(inFormat, outFormat)) {
						...
						if (setFormat == null)
							...
							logger.warn("Failed to change format of inDataSource from "
									+ setFormat + " to " + outFormat);
	 ...
	 * 		deadlock risks
	 * @throws IOException
	 * 		if any of the input <tt>DataSource</tt>s of this <tt>AudioMixer</tt> throws such an
