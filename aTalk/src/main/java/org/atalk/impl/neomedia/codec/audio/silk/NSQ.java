...
public class NSQ
{
	 ...
	 */
	static void SKP_Silk_noise_shape_quantizer(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		...
		int b_Q14_offset, final short AR_shp_Q13[], /* I Noise shaping AR coefs */
	...
	)
	{
		...
		for (i = 0; i < length; i++) {
			...
			// SKP_assert( ( (SKP_int64)AR_shp_Q13 & 3 ) == 0 ); /* check that array starts at
			...
			xq[xq_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(
	...
	)
	{
		...
		scale_length = scale_length - SKP_SMULBB(NB_SUBFR - (subfr + 1), length); /*
																				 ...
																				 * old in next frame
																				 */
		scale_length = SigProcFIX.SKP_max_int(scale_length, lag + LTP_ORDER); /*
																			 * make sure to scale
