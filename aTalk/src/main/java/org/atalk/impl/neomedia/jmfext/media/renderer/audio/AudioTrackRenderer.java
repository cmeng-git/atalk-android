...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioTrackRenderer</tt> class and its instances for
	 * logging output.
	 */
	...
	private static final int ABSTRACT_VOLUME_CONTROL_PERCENT_RANGE
			= (BasicVolumeControl.MAX_VOLUME_PERCENT - BasicVolumeControl.MIN_VOLUME_PERCENT)
			/ 100;
	 ...
	 * lowering the volume using software gain control. The opposite happens for the maximum
	 * volume.
	...
	/**
	 * The <tt>AudioTrack</tt> which implements the output device represented by this
	 * <tt>Renderer</tt> and renders to it.
	 */
	private AudioTrack audioTrack;

	/**
	 * The length in bytes of media data to be written into {@link #audioTrack} via a single
	 * call to
	...
	/**
	 * The value of {@link GainControl#getLevel()} of {@link #gainControl} which has been
	 * applied to
	...
	/**
	 * The zero-based index in {@link #latency} at which the beginning of (actual/valid) media data
	 * is contained.
	 */
	 ...
	 * Initializes a new <tt>AudioTrackRenderer</tt> instance.
	 *
	 * @param enableGainControl
	 * 		<tt>true</tt> to enable controlling the volume/gain of the rendered media; otherwise,
	 * 		<tt>false</tt>
	 */
	...
	public Format[] getSupportedInputFormats()
	{
		if (supportedInputFormats == null) {
			double[] supportedInputSampleRates = new double[1 + Constants.AUDIO_SAMPLE_RATES
					.length];
	 ...
	 * 		if any of the required resources cannot be acquired
	 * @see PlugIn#open()
	 */
			...
			throws ResourceUnavailableException
	{
		if (audioTrack == null) {
			...
			int bytesPerMillisecond = (int) Math.round((sampleRate / 1000) * channels
					* (sampleSizeInBits / 8));

			audioTrackWriteLengthInBytes = 20 /* milliseconds */ * bytesPerMillisecond;

			/*
			 * Give the AudioTrack a large enough buffer size in bytes in case it remedies
			 * cracking.
			 */
			...
			audioTrack = new AudioTrack(getStreamType(), (int) sampleRate, channelConfig,
					audioFormat, Math.max(audioTrackBufferSizeInBytes,
			...
			if (USE_SOFTWARE_GAIN) {
				/*
				 * Set the volume of the audioTrack to the maximum value because there is volume
				 * control via neomedia.
				 */
				...
				if (setStereoVolume != AudioTrack.SUCCESS) {
					logger.warn("AudioTrack.setStereoVolume(float, float) failed"
							+ " with return value " + setStereoVolume);
				}
			}
			else {
				/*
	 ...
	 * specific {@link Buffer} and renders it to the output device represented by this
	 * <tt>Renderer</tt>.
	 *
	 * @param buffer
	 * 		the <tt>Buffer</tt> containing the media data to be processed and rendered to the
				...
				synchronized (this) {
					if (audioTrack == null) {
							...
							if (USE_SOFTWARE_GAIN) {
								...
								if (gainControlLevelAppliedToAudioTrack != gainControlLevel) {
									...
									if (gainControlLevelAppliedToAudioTrack
										...
										setStereoVolume = audioTrack.setStereoVolume(
									...
									if ((setStereoVolume != AudioTrack.SUCCESS)
											|| (volume > maxVolume)) {
										BasicVolumeControl.applyGain(gainControl, bytes, offset,
												length);
									}
								}
							}
						...
						if (latency == null)
							...
							/*
							 * If there is no free room in the latency buffer, wait for it to be
							 * freed.
							 */
							if (latency.length - latencyLength <= 0) {
								...
								if (freeTail > 0) {
									...
									System.arraycopy(bytes, offset, latency, latencyTail,
											tailToWrite);
								...
								if (toWrite > 0) {
									...
									if (freeHead > 0) {
										...
										System.arraycopy(bytes, offset + written, latency, 0,
												headToWrite);
	...
	/**
	 * Runs in {@link #latencyThread}. Reads from {@link #latency} and writes into
	 * {@link #audioTrack}.
	 */
		...
		try {
			...
			while (true) {
				synchronized (this) {
					...
					if (written < 0) {
						throw new RuntimeException("android.media.AudioTrack"
								+ "#write(byte[], int, int)");
					}
					else if (written > 0) {
						latencyHead += written;
	...
	{
		return new Object[0]; // To change body of implemented methods use File | Settings | File
		// Templates.
	}
}
