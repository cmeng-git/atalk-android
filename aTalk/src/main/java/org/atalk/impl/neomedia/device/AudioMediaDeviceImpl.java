...
public class AudioMediaDeviceImpl extends MediaDeviceImpl
{
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMediaDeviceImpl</tt> class and its instances for
	 * logging output.
	 */
	 ...
	 * 		the <tt>CaptureDeviceInfo</tt> of the <tt>CaptureDevice</tt> to which the new instance
	 ...
	 *
	 * @param captureDevice
	 * 		an exclusive <tt>CaptureDevice</tt> for which sharing is to be enabled
						...
						if (!supportedFormatToSet.matches(format)) {
							...
							if ((channels != Format.NOT_SPECIFIED)
								...
								if (supportedFormatToSet.matches(formatToSet))
									formatControl.setFormat(supportedFormatToSet
