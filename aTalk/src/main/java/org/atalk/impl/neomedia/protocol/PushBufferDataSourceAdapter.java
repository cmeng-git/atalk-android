...
public class PushBufferDataSourceAdapter extends PushBufferDataSourceDelegate<PullBufferDataSource>
{
	...
	private static class PushBufferStreamAdapter implements PushBufferStream
	{
		 ...
		 *
		 * @param stream
		 * 		the <tt>PullBufferStream</tt> the new instance is to implement
		 ...
		 *
		 * @param controlType
		 * 		a <tt>String</tt> value which specifies the type of the control of the wrapped
		 ...
		 *
		 * @param buffer
		 * 		a <tt>Buffer</tt> in which media data is to be written by this
		 * 		<tt>PushBufferDataSource</tt>
		 * @throws IOException
		 * 		if anything wrong happens while reading media data from this
		 ...
		 * 		the <tt>BufferTransferHandler</tt> through which <tt>PushBufferStream</tt> is to
					...
					while (streamReadThread != null) {
						try {
							...
							logger.info(getClass().getSimpleName()
									...
									+ " PullBufferStream read thread to stop.", iex);
							interrupted = true;
						}
					}
					if (interrupted)
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>PushBufferDataSourceAdapter</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 *
	 * @param dataSource
	 * 		the <tt>PullBufferDataSource</tt> the new instance is to implement
	 ...
	 *
	 * @param stream
	 * 		the <tt>PullBufferStream</tt> adapted by a <tt>PushBufferStreamAdapter</tt> that is to
	 ...
	 *
	 * @throws IOException
	 * 		if anything wrong happens while starting the wrapped <tt>PullBufferDataSource</tt> or
	 * 		the pushing from the <tt>PushBufferStreamAdapter</tt>s which wrap the
	 ...
	 *
	 * @throws IOException
	 * 		if anything wrong happens while stopping the wrapped <tt>PullBufferDataSource</tt> or
	 * 		the pushing from the <tt>PushBufferStreamAdapter</tt>s which wrap the
	 * 		<tt>PullBufferStream</tt>s of the <tt>PullBufferDataSource</tt> wrapped by this
	 * 		instance
	 */
