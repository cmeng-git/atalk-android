...
/**
 * An Android implementation of the {@link ResourceManagementService}.<br/>
 * <br/>
 * Strings - requests are redirected to the strings defined in "strings.xml" file, but in case
 * the string is not found it will try to look for strings defined in default string resources.<br/>
 * <br/>
 * Dots in keys are replaced with "_", as they can not be used for string names in "strings.xml".
 * For example the string for key "service.gui.CLOSE" should be declared as:<br/>
 ...
 * size resolving mechanism. The mapping must be specified in file {@link #IMAGE_PATH_RESOURCE}
 * .properties. <br/>
 * Sample entries:<br/>
...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>AndroidResourceServiceImpl</tt> class and its instances
	 * for logging output.
	 */
	private static final Logger logger = Logger.getLogger(AndroidResourceServiceImpl.class);

	/**
	 * Path to the .properties file containing image path's translations to android drawable
	 * resources
	 */
	 ...
	 *
	 * @param strKey
	 * 		the color text identifier that has to be resolved
	 ...
	 *
	 * @param typeName
	 * 		the type name (color, drawable, raw, string ...)
	 * @param key
	 * 		the resource name
	 ...
	 *
	 * @param key
	 * 		the name of string resource as defined in "strings.xml"
	 ...
	 *
	 * @param key
	 * 		the name of sound, for busy.wav it will be just busy
	 ...
	 *
	 * @param path
	 * 		the path, for which we're looking for a sound URL
	 ...
	 *
	 * @param file
	 * 		the zip file from which we prepare a skin
