...
/**
 * The dialog that displays certificate details. It allows user to mark the certificate as "always trusted". The dialog
 * details are created dynamically, by appending table rows. That's because it's certificate implementation dependent.
 * Parent <tt>Activity</tt> must implement <tt>CertInfoDialogListener</tt>.
 *
 * @author Pawel Domas
 */
public class CertificateInfoDialog extends OSGiDialogFragment
{
    /**
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState)
    {
        ...
        if (cert instanceof X509Certificate) {
            ...
            if (algorithm.equals("RSA")) {
                ...
                String keyStr = getString(R.string.service_gui_CERT_INFO_KEY_BYTES_PRINT,
                        String.valueOf(key.getModulus().toByteArray().length - 1), key.getModulus().toString(16));
                appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_PUB_KEY), keyStr);
                appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_EXP),
                        String.valueOf(key.getPublicExponent()));
                String keySizeStr = getString(R.string.service_gui_CERT_INFO_KEY_BITS_PRINT,
            ...
            // Signature
            String signatureStr = getString(R.string.service_gui_CERT_INFO_KEY_BYTES_PRINT,
                    String.valueOf(x509.getSignature().length), getHex(x509.getSignature()));
        ...
        CompoundButton alwaysTrustBtn = content.findViewById(R.id.alwaysTrust);
    ...
    /**
     * Appends next row to given <tt>table</tt>. The row will contain one or two columns of text specified in
     * <tt>texts</tt> variable.
     *
     ...
     */
    static public CertificateInfoDialog createFragment(long requestId)
    {
        CertificateInfoDialog dialog = new CertificateInfoDialog();
    ...
    static public interface CertInfoDialogListener
    {
         ...
         *
         * @param continueAnyway <tt>true</tt> if continue anyway button was pressed, <tt>false</tt> means that
         * the dialog was discarded or cancel button was pressed.
         */
