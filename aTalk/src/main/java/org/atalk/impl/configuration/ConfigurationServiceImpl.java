...
{
	/**
	 * The <tt>Logger</tt> used by this <tt>ConfigurationServiceImpl</tt> instance for logging
	 * output.
	 */
	...
	/**
	 * Name of the file containing overrides (possibly set by the developer) for any of the
	 * default properties.
	 */
	private static final String DEFAULT_OVERRIDES_PROPS_FILE_NAME
			= "atalk-default-overrides.properties";
	 ...
	 * properties in this file will be impossible to override and attempts to do so will simply
	 * be ignored.
	 *
	...
	public ConfigurationServiceImpl()
	{
		 ...
		 * and we want to make sure that we have given the  chance to this
		 * ConfigurationServiceImpl to set these properties before FileAccessServiceImpl looks
		 * for them.
		 */
	 ...
	 * @param property
	 * 		the object that we'd like to be come the new value of the property.
	 ...
	 * @param isSystem
	 * 		specifies whether or not the property being is a System property and should be
			...
			throws ConfigPropertyVetoException
	{
		...
		for (Map.Entry<String, Object> property : properties.entrySet()) {
			...
			if (changeEventDispatcher.hasPropertyChangeListeners(propertyName))
				changeEventDispatcher.firePropertyChange(propertyName, oldValues.get(propertyName),
						property.getValue());
		}
	}
	 ...
	 *
	 * @param propertyName
	 * 		the name of the property which is to be set to a specific value
	 * @param property
	 * 		the value to be assigned to the property with the specified name
	 * @param isSystem
	 * 		<tt>true</tt> if the property with the specified name is to be set as a system
...
//			logger.warn("Found empty or null property value for: " + propertyName);
	...
	public List<String> getAllPropertyNames(String name)
	{
		...
		for (String key : store.getPropertyNames(name)) {
	 ...
	 *
	 * @param prefix
	 * 		a String containing the prefix (the non dotted non-caps part of a property name) that
	 * 		we're looking for.
	 * @param exactPrefixMatch
	 * 		a boolean indicating whether the returned property names should all have a prefix that
	 ...
	 *
	 * @param file
	 * 		a reference to the configuration file where properties should be stored.
	 * @throws IOException
	...
	{
		/*
		 * Choose the format of the configuration file so with the slow and fast XML format when
		 * necessary.
		 */
		...
		if (configurationFile == null) {
			...
			String extension = (extensionBeginIndex > -1)
					? name.substring(extensionBeginIndex) : null;
			...
			if (".properties".equalsIgnoreCase(extension)) {
				...
				File newConfigurationFile = new File(configurationFile.getParentFile(),
						((extensionBeginIndex > -1)
								? name.substring(0, extensionBeginIndex) : name) + ".properties");
				 ...
				 * .properties file is there to signal that we have to use it instead of the .xml
				 * file.
				 */
	...
	{
		// first let's check whether we already have the name of the directory set as a
	 ...
	 *
	 * @param extension
	 * 		the extension of the file name of the configuration file. The specified extension may
	 * 		not be taken into account if the the configuration file name is forced through a
	 * 		system property.
	 * @param create
	 * 		<tt>true</tt> to create the configuration file with the determined file name if it
			...
			throws IOException
	{
		...
		// Return an empty file if there wasn't any in the jar null check report from John J.
		// Barton - IBM
		if (in == null) {
			if (create) {
				configDir.mkdirs();
	 ...
	 *
	 * @param inputStream
	 * 		the <code>InputStream</code> the contents of which is to be output in the specified
	 * 		<code>File</code>
	 * @param outputFile
	 * 		the <code>File</code> to write the contents of the specified <code>InputStream</code>
	 * 		into
	 * @throws IOException
	 */
	 ...
	 * 		the name of the property that is being queried.
	 * @param defaultValue
	 * 		the value to be returned if the specified property name is not associated with a value
	 * 		in this
	 ...
	 *
	 * @param propertyName
	 * 		the name of the property to get the value of as a signed decimal integer
	 * @param defaultValue
	 * 		the value to be returned if parsing the value of the specified property name as a
	 ...
	 *
	 * @param propertyName
	 * 		the name of the property to get the value of as a signed decimal long integer
	 * @param defaultValue
	 * 		the value to be returned if parsing the value of the specified property name as a
	...
	public long getLong(String propertyName, long defaultValue)
	{
		...
		if ((stringValue != null) && (stringValue.length() > 0)) {
			try {
				...
				logger.error(propertyName + " does not appear to be a longinteger. "
						+ "Defaulting to " + defaultValue + ".", ex);
			}
		}
		return longValue;
	 ...
	 *
	 * @param clazz
	 * 		the {@link ConfigurationStore} that this configuration service instance instance has
	 * 		to use.
	 * @throws IOException
	...
	private void loadDefaultProperties(String fileName)
	{
		try {
			...
			// now get those properties and place them into the mutable and immutable properties
			// maps.
			for (Map.Entry<Object, Object> entry : fileProps.entrySet()) {
				...
				if (name.startsWith("*")) {
					...
					// in case this is an override, make sure we remove previous definitions of
					// this property
