...
import org.atalk.service.version.*;
...
public class UpdateServiceImpl implements UpdateService
{
	 ...
	 * Checks for updates.
	 *
	 * @param notifyAboutNewestVersion
	 * 		<tt>true</tt> if the user is to be notified if they have the newest version already;
	...
	public void checkForUpdates(boolean notifyAboutNewestVersion)
	{
		...
		ResourceManagementService R = ServiceUtils.getService(UpdateActivator.bundleContext,
		...
		if (!isLatest && (downloadLink != null)) {
			...
			if (previousDownloads.size() > 0) {
				...
				if (lastJobStatus == DownloadManager.STATUS_SUCCESSFUL) {
					...
					AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
							R.getI18NString("plugin.updatechecker.DIALOG_IN_PROGRESS_TITLE"),
							R.getI18NString("plugin.updatechecker.DIALOG_IN_PROGRESS"));
					return;
				}
			}
			...
			AndroidUtils.showAlertConfirmDialog(aTalkApp.getGlobalContext(),
					...
					R.getI18NString("plugin.updatechecker.DIALOG_MESSAGE",
							new String[]{latestVersion, Integer.toString(latestVersionCode),
									R.getI18NString("app.name")}),
			...
			AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
					...
					R.getI18NString("plugin.updatechecker.DIALOG_NOUPDATE",
							new String[]{currentVersion, Integer.toString(currentVersionCode)}));
		}
	}
	 ...
	 *
	 * @param apkDownloadId
	 * 		download id of the apk to install.
	 */
	 ...
	 *
	 * @param id
	 * 		download identifier which status will be returned.
	...
	{
		if (store == null) {
			store = aTalkApp.getGlobalContext().getSharedPreferences("store",
