...
/**
 * The fragment shares common parts for all protocols settings. It handles security and encoding
 * preferences.
 *
 * @author Pawel Domas
		...
		implements SharedPreferences.OnSharedPreferenceChangeListener
{
	 ...
	 *
	 * @param preferencesResourceId
	 * 		the ID of preferences xml file for current protocol
	 */
	 ...
	 * <tt>pref_cat_video_encoding</tt> must be included in preferences xml to trigger encodings
	 * activities.
	 *
	 * @return impl fragments should return <tt>EncodingsRegistrationUtil</tt> if encodings are
	 * supported.
	 */
	...
	/**
	 * Returns <tt>true</tt> if preference views have been initialized with values from the
	 * registration object.
	 *
	 * @return <tt>true</tt> if preference views have been initialized with values from the
	 * registration object.
	 */
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		// Loads preference Views. They will be initialized with values loaded into
		// SharedPreferences in loadAccount
		addPreferencesFromResource(preferencesResourceId);
	...
	/**
	 * Load the <tt>account</tt> and it's encoding and security parts if they exist
	 *
	 * @param account
	 * 		the {@link AccountID} that will be edited
	 */
	...
	{
		// Audio,video and security are optional and should be present in settings XML to be
		// handled
	...
	AccountRegistrationWizard findRegistrationService(String protocolName)
	{
		...
		try {
			...
			accountWizardRefs = context.getServiceReferences(
			...
			for (ServiceReference accountWizardRef : accountWizardRefs) {
				AccountRegistrationWizard wizard = (AccountRegistrationWizard)
						context.getService(accountWizardRef);
	 ...
	 *
	 * @param summaryMapper
	 * 		the {@link SummaryMapper} managed by this {@link AccountPreferenceFragment} that can
	 ...
	 *
	 * @param mediaType
	 * 		indicates if AUDIO or VIDEO encodings will be edited
	 */
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		if (requestCode == EDIT_ENCODINGS && resultCode == Activity.RESULT_OK) {
			Boolean hasChanges = data.getBooleanExtra(EncodingActivity.EXTRA_KEY_HAS_CHANGES,
					false);
			if (!hasChanges)
				return;
			...
			Boolean hasChanges
					= data.getBooleanExtra(SecurityActivity.EXTR_KEY_HAS_CHANGES, false);
