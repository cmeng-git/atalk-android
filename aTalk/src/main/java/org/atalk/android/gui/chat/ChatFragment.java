		...
		implements ChatSessionManager.CurrentChatListener, FileTransferStatusListener, CryptoModeChangeListener
{
	...
	private void initListViewListeners()
	{
		...
		chatListView.setOnItemLongClickListener(new ListView.OnItemLongClickListener()
		{
			...
			{
				Toast.makeText(getContext(), R.string.chat_message_long_press_hint,
	 ...
	 *
	 * @param isSelected
	 * 		<tt>true</tt> if the fragment is now the primary selected page.
		...
		public boolean onActionItemClicked(ActionMode mode, MenuItem item)
		{
			...
			switch (item.getItemId()) {
				...
				case R.id.copy_to_clipboard:
					...
					ClipboardManager cmgr = (ClipboardManager) getActivity()
			...
			ContactPresenceStatusListener, ChatStateNotificationsListener
	{
		 ...
		 * 		the message to add.
		 * @param update
		 * 		if set to <tt>true</tt> will notify the UI about update immediately.
		 */
		...
		private void addMessageImpl(ChatMessage newMessage, boolean update)
		{
			...
			ChatMessage lastMsg = (lastMsgIdx != -1)
					? chatListAdapter.getMessage(lastMsgIdx) : null;
		...
		/**
		 * Finds index of the message that will handle <tt>newMessage</tt> merging process
		 * (usually just the last one). If the
		 * <tt>newMessage</tt> is a correction message, then the last message of the same type
		 * will be returned.
		 *
		 * @param newMessage
				...
				case FILE_TRANSFER_MESSAGE_VIEW:
					...
					switch (msgType) {
						case ChatMessage.MESSAGE_FILE_TRANSFER_RECEIVE:
							...
							ReceiveFileConversation filexferR
									= (ReceiveFileConversation) getFileXfer(position);
							if (filexferR == null) {
						...
						case ChatMessage.MESSAGE_FILE_TRANSFER_SEND:
							...
							viewTemp = filexferS.SendFileConversationForm(inflater,
									messageViewHolder, parent, position, init);
							break;
						...
						case ChatMessage.MESSAGE_FILE_TRANSFER_HISTORY:
							...
							FileHistoryConversation filexferH
									= FileHistoryConversation.newInstance(currentChatFragment, fileRecord, message.getChatMessage());
				...
				default:
					...
					if (message != null) {
						if (messageViewHolder.viewType == INCOMING_MESSAGE_VIEW
							...
							if (messageViewHolder.viewType == INCOMING_MESSAGE_VIEW)
								messageViewHolder.jidView.setText(message.getChatMessage()
		...
		private View inflateViewForType(int viewType, MessageViewHolder messageViewHolder,
				ViewGroup parent)
		{
			messageViewHolder.viewType = viewType;
			LayoutInflater inflater = getActivity().getLayoutInflater();
			...
			if (viewType == INCOMING_MESSAGE_VIEW) {
				...
				messageViewHolder.jidView = (TextView) convertView
						.findViewById(R.id.incomingJidView);
				...
				messageViewHolder.chatStateView = (ImageView) convertView
						.findViewById(R.id.chatStateImageView);
				messageViewHolder.showMapButton = (Button) convertView
...
//			}
		...
		private void updateStatusAndAvatarView(MessageViewHolder viewHolder, String jabberID)
		{
			...
			if (viewHolder.viewType == INCOMING_MESSAGE_VIEW) {
				...
				if (descriptor instanceof MetaContact) {
					...
					if (jabberID != null) {
						ProtocolProviderService provider = chatPanel.getChatSession()
								.getCurrentChatTransport().getProtocolProvider();
						OperationSetPersistentPresenceJabberImpl presenceOpSet
								= (OperationSetPersistentPresenceJabberImpl) provider
								.getOperationSet(OperationSetPersistentPresence.class);
						Contact contact = presenceOpSet.findContactByID(
								XmppStringUtils.parseBareJid(jabberID));
						...
						if (contact != null) {
							...
							status = MetaContactRenderer.getCachedAvatarFromBytes(
									StatusUtil.getContactStatusIcon(pStatus));
						}
					}
				}
			}
		...
		public void chatStateNotificationReceived(ChatStateNotificationEvent evt)
		{
			...
			ChatStateNotificationHandler.handleChatStateNotificationReceived(evt,
					ChatFragment.this);
		}
		...
		class MessageDisplay implements View.OnClickListener, View.OnLongClickListener
		{
			 ...
			 *
			 * @param msg
			 * 		the <tt>ChatMessage</tt> that will be displayed by this instance.
			 */
			...
			public void onClick(View view)
			{
				...
				intent.putExtra(SplitStreetViewPanoramaAndMapActivity.MARKER_POSITION_KEY,
						mLatLng);
			...
			public boolean onLongClick(View v)
			{
				...
				if (!xLatLng.isEmpty()) {
					mSVP_Started = true;
					Intent intent = new Intent(mActivity, SplitStreetViewPanoramaAndMapActivity
							.class);
					intent.putExtra(SplitStreetViewPanoramaAndMapActivity.MARKER_POSITION_KEY,
							mLatLng);
	 ...
	 *
	 * @param chatState
	 * 		the chat state that should be represented in the view
	 */
	public void setChatState(final ChatState chatState)
	{
		...
		mActivity.runOnUiThread(new Runnable()
		{
			...
			{
				if (chatState != null) {
					TextView chatStateTextView
							= (TextView) chatStateView.findViewById(R.id.chatStateTextView);
					ImageView chatStateImgView
							= (ImageView) chatStateView.findViewById(R.id.chatStateImageView);
					...
					switch (chatState) {
						case composing:
							...
							chatStateTextView.setText(chatPanel.getShortDisplayName()
									+ " " + aTalkApp.getResString(
									R.string.service_gui_CONTACT_COMPOSING));
							break;
						case paused:
	 ...
	 *
	 * @param id
	 * 		the identifier of the file transfer to add
	 * @param fileTransfer
	 * 		the descriptor of the file transfer
	 */
	 ...
	 *
	 * @param fileTransfer
	 * 		the identifier of the file transfer to remove
	 */
	 ...
	 *
	 * @param event
	 * 		the file transfer status change event the notified us for the change
	 */
	public void statusChanged(FileTransferStatusChangeEvent event)
	{
		...
		if (chatListAdapter != null) {
			...
			if ((newStatus == FileTransferStatusChangeEvent.CANCELED) && (chatListAdapter
					.getXferStatus(msgPos) == FileTransferStatusChangeEvent.PREPARING)) {
				String msg = AndroidGUIActivator.getResources()
						.getI18NString("service.gui.FILE_TRANSFER_CANCELED");
				// chatPanel.sendMessage(msg);
				try {
					chatPanel.getChatSession().getCurrentChatTransport()
							.sendInstantMessage(msg, ChatMessage.ENCODE_PLAIN);
				}
				catch (Exception e) {
					e.printStackTrace();
				}
	...
	public class SendFile extends AsyncTask<Void, Void, Exception>
	{
		...
		public void onPreExecute()
		{
			...
			if (file.length() > maxFileLength) {
				chatPanel.addMessage(currentChatTransport.getDisplayName(), new Date(),
						Chat.ERROR_MESSAGE, AndroidGUIActivator.getResources()
		...
		protected void onPostExecute(Exception ex)
		{
			if (ex != null) {
				...
				if (ex instanceof IllegalStateException) {
					...
					chatPanel.addMessage(currentChatTransport.getDisplayName(), new Date(),
							Chat.ERROR_MESSAGE, AndroidGUIActivator.getResources()
									.getI18NString("service.gui.FILE_DELIVERY_ERROR",
	 ...
	 *
	 * @param chatType
	 * 		Change chat fragment view background color based on chatType
	 */
...
//		new Exception().printStackTrace();
		...
		if (mChatType == chatType)
			return;
		// logger.warn("Change ChatType: " + mChatType + "=>" + chatType);
