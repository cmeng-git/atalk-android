...
public class ProtocolContactSourceServiceImpl implements ContactSourceService
{
	...
	/**
	 * The <tt>List</tt> of <tt>ProtocolContactQuery</tt> instances which have been started and
	 * haven't stopped yet.
	 */
	 ...
	 * @param opSetClass
	 * 		the <tt>OperationSet</tt> class that is supported by source contacts
	 */
	...
	{
		return AndroidGUIActivator.getResources().getI18NString("service.gui.CONTACTS") + " "
	...
	private class ProtocolCQuery extends AsyncContactQuery<ProtocolContactSourceServiceImpl>
	{
		...
		public void run()
		{
			Iterator<MetaContact> contactListIter
					= metaContactListService.findAllMetaContactsForProvider(protocolProvider);
		 ...
		 *
		 * @param metaContact
		 * 		the metaContact, which child protocol contacts we'll be adding to the result
		 */
