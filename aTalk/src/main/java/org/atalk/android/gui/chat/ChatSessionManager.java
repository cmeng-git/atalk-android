...
public class ChatSessionManager
{
	 ...
	 *
	 * @param chatPanel
	 * 		the <tt>ChatPanel</tt> corresponding to the active chat
	 ...
	 *
	 * @param chatPanel
	 * 		the <tt>ChatPanel</tt> corresponding to the active chat to remove
	 */
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> corresponding to the <tt>ChatPanel</tt> we're looking for
	 * @return the <tt>ChatPanel</tt> corresponding to the given chat identifier
	 */
	 ...
	 *
	 * @param chatId
	 * 		the identifier of the current chat session
	 */
	public synchronized static void setCurrentChatId(String chatId)
	{
		...
		if (chatId != null) {
			...
			if (currChat != null) {
				...
				logger.debug("Current chat with: " + currChat.getChatSession()
						.getCurrentChatTransport().getDisplayName());
			}
	 ...
	 *
	 * @param l
	 * 		the listener to add to current chat listeners list.
	 */
	 ...
	 *
	 * @param l
	 * 		the listener to remove from current chat listeners list.
	 */
	 ...
	 *
	 * @param chatLinkClickedListener
	 * 		the <tt>ChatLinkClickedListener</tt> to add.
	 */
	 ...
	 *
	 * @param chatLinkClickedListener
	 * 		the <tt>ChatLinkClickedListener</tt> to remove.
	 */
	 ...
	 *
	 * @param descriptor
	 * 		the contact we want to start new chat with.
	 ...
	 * 		protocol provider for which all chat sessions will be removed.
	 */
	 ...
	 */
	// public synchronized static ChatPanel findChatForContact(Contact contact,
	 ...
	 *
	 * @param chatId
	 * 		 A string identifier that uniquely represents the caller in the containing chat
	 * 		 session database
	 * @param chatMode
	 * 		can have one of the value as shown in below code
	 ...
	 * optionally creates it if it does not exist yet. Must be executed on the event
	 * dispatch thread.
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>ChatRoomWrapper</tt> to get the corresponding <tt>ChatPanel</tt> of
	 * @param create
	 * 		<tt>true</tt> to create a new <tt>ChatPanel</tt> for the specified
	 * 		<tt>ChatRoomWrapper</tt> if no such <tt>ChatPanel</tt>
	 * 		exists already; otherwise, <tt>false</tt>
	 * @return the <tt>ChatPanel</tt> corresponding to the specified <tt>ChatRoomWrapper</tt> or
	 * <tt>null</tt> if no such <tt>ChatPanel</tt>
	 ...
	 * and optionally creates it if it does not exist yet. Must be executed on the event
	 * dispatch thread.
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>AdHocChatRoomWrapper</tt> to get the corresponding <tt>ChatPanel</tt> of
	 * @param create
	 * 		<tt>true</tt> to create a new <tt>ChatPanel</tt> for the specified
	 * 		<tt>AdHocChatRoomWrapper</tt> if no such <tt>ChatPanel</tt>
	 * 		exists already; otherwise, <tt>false</tt>
	 ...
	 *
	 * @param chatRoom
	 * 		the <tt>ChatRoom</tt> to get the corresponding <tt>ChatPanel</tt> of
	 * @param create
	 * 		<tt>true</tt> to create a <tt>ChatPanel</tt> corresponding to the specified
	 * 		<tt>ChatRoom</tt> if such <tt>ChatPanel</tt> does not exist yet
	 * @param escapedMessageID
	 * 		the message ID of the message that should be excluded from the history when the last
	 * 		one is loaded in the chat
	 * @return the <tt>ChatPanel</tt> corresponding to the specified <tt>ChatRoom</tt>;
	 * <tt>null</tt> if there is no such <tt>ChatPanel</tt>
	 * and <tt>create</tt> is <tt>false</tt>
	 */
	private static ChatPanel getMultiChatInternal(ChatRoom chatRoom, boolean create, String
			escapedMessageID)
	{
		synchronized (chatSyncRoot) {
			ChatRoomWrapper chatRoomWrapper = AndroidGUIActivator.getMUCService()
	 ...
	 * @param create
	 * 		<tt>true</tt> to create a <tt>ChatPanel</tt> corresponding to the specified
	 * 		<tt>ChatRoom</tt> if such <tt>ChatPanel</tt> does
	 * 		not exist yet
	 * @param escapedMessageID
	 * 		the message ID of the message that should be excluded from the history when the last
	 * 		one is loaded in the chat
	 ...
	 *
	 * @param adHocChatRoom
	 * 		the <tt>AdHocChatRoom</tt> to get the corresponding <tt>ChatPanel</tt> of
	 * @param create
	 * 		<tt>true</tt> to create a <tt>ChatPanel</tt> corresponding to the specified
	 * 		<tt>AdHocChatRoom</tt> if such <tt>ChatPanel</tt>
	 * 		does not exist yet
	 * @param escapedMessageID
	 * 		the message ID of the message that should be excluded from the history when the last
	 * 		one is loaded in the chat
	 ...
	 */
	private static ChatPanel getMultiChatInternal(AdHocChatRoom adHocChatRoom, boolean create,
			String escapedMessageID)
	{
		synchronized (chatSyncRoot) {
			...
			AdHocChatRoomWrapper chatRoomWrapper
					= chatRoomList.findChatRoomWrapperFromAdHocChatRoom(adHocChatRoom);
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> to get the default <tt>Contact</tt> of
	...
	private static Contact getDefaultContact(MetaContact metaContact)
	{
		...
		if ((defaultContact.getPresenceStatus().getStatus() < 1)
			...
			while (protoContacts.hasNext()) {
				...
				OperationSetBasicInstantMessaging protoContactIM = protoContactProvider
	...
	/**
	 * Creates a <tt>ChatPanel</tt> for the given contact and saves it in the list of created
	 * <tt>ChatPanel</tt>s.
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> to create a <tt>ChatPanel</tt> for
	 * 		<p>
	 * 		// @param protocolContact
	 * 		the <tt>Contact</tt> (respectively its <tt>ChatTransport</tt>) to be selected in the
	 * 		newly created <tt>ChatPanel</tt>;
	 ...
	 * 		the <tt>ContactResource</tt>, to be selected in the newly created <tt>ChatPanel</tt>
	...
	private static ChatPanel createChat(MetaContact metaContact)
	{
		...
		MetaContactChatSession chatSession = new MetaContactChatSession(chatPanel,
	 ...
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>ChatRoom</tt>, for which the chat will be created
	 * @return The <code>ChatPanel</code> newly created.
	 */
	 ...
	 *
	 * @param chatRoomWrapper
	 * 		the <tt>AdHocChatRoom</tt>, for which the chat will be created
	 * @return The <code>ChatPanel</code> newly created.
	 */
	 ...
	 * @return The <code>ChatPanel</code> newly created.
	 */
	private static ChatPanel createChat(AdHocChatRoomWrapper chatRoomWrapper, String
			escapedMessageID)
	{
		ChatPanel chatPanel = new ChatPanel(null);
	 ...
	 * @param descriptor
	 * 		the chat descriptor.
	 * @return the <tt>ChatPanel</tt> corresponding to the given chat descriptor if any;
	 ...
	 * <tt>Chat</tt> has been closed.
	 *
	 * @param chat
	 * 		the <tt>Chat</tt> which has been closed and which the <tt>ChatListener</tt>s
	 ...
	 *
	 * @param chat
	 * 		the <tt>Chat</tt> which has been created and which the <tt>ChatListener</tt>s
	 * 		registered with this instance are to be notified about
	 */
	...
	private class CreateChatRoomWrapperRunner extends AbstractChatPanelCreateRunnable
	{
		 ...
		 *
		 * @param chatRoomWrapper
		 * 		the <tt>ChatRoomWrapper</tt> to use for creating a panel.
		 */
	...
	private class CreateChatRoomRunner extends AbstractChatPanelCreateRunnable
	{
		 ...
		 *
		 * @param chatRoom
		 * 		the <tt>ChatRoom</tt> used to create the corresponding <tt>ChatPanel</tt>.
		 */
	...
	private class CreateAdHocChatRoomRunner extends AbstractChatPanelCreateRunnable
	{
		 ...
		 *
		 * @param adHocChatRoom
		 * 		the <tt>AdHocChatRoom</tt> used to create the corresponding <tt>ChatPanel</tt>.
		 */
