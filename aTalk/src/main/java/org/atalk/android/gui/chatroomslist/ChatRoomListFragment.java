...
//		textView.setHintTextColor(ContextCompat.getColor(getContext(), R.color.white));
	 ...
	 *
	 * @param menu
	 * 		the menu to inflate when long press on chatRoom item.
	 * @param inflater
	 * 		the menu inflater.
	 * @param group
	 * 		clicked group index.
	 ...
	 */
	private void createChatRoomCtxMenu(ContextMenu menu, MenuInflater inflater,
			int group, int child)
	{
	...
	private void createGroupCtxMenu(ContextMenu menu, MenuInflater inflater, int group)
	{
		...
		menu.setHeaderTitle(mClickedGroup.getProtocolProvider().getAccountID()
				.getAccountUniqueID());
	}
...
//	}
	...
	{
		BaseChatRoomListAdapter adapter
				= (BaseChatRoomListAdapter) listView.getExpandableListAdapter();
	 ...
	 *
	 * @param query
	 * 		the query string that will be used for filtering contacts.
	 */
