...
public class MetaContactChatSession extends ChatSession
	 ...
	 *
	 * @param chatPanel
	 * 		the renderer, which gives the connection with the UI.
	 * @param metaContact
	 * 		the meta contact corresponding to the session and the protocol contact.
	 * @param protocolContact
	 * 		the protocol contact to be used as transport.
	 * @param contactResource
	 * 		the specific resource to be used as transport
	 */
	...
	public String getChatName() {
		...
		return AndroidGUIActivator.getResources().getI18NString("service.gui.UNKNOWN");
	}
	...
	public Collection<Object> getHistory(int count) {
		...
		return metaHistory.findLast(chatHistoryFilter, metaContact,
	 ...
	 *
	 * @param protocolContact
	 * 		the <tt>Contact</tt> which is to be selected into this instance as the current
	 * 		i.e. its <tt>ChatTransport</tt> is to be selected as <tt>currentChatTransport</tt>
	 * @param contactResource
	 * 		the <tt>ContactResource</tt>, which is to be selected into this instance
	 ...
	 *
	 * @param evt
	 * 		the <tt>ProtoContactEvent</tt> that contains information about
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> to search for
	...
	private ChatContact<?> findChatContactByMetaContact(MetaContact metaContact) {
		for (ChatContact<?> chatContact : chatParticipants) {
			...
			if (chatSourceContact instanceof MetaContact) {
				...
				MetaContact parentMetaContact = AndroidGUIActivator.getContactListService()
	 ...
	 *
	 * @param contact
	 * 		the <tt>Contact</tt>, which transports to add
	 * @param resourceName
	 * 		the resource to be pre-selected
	 */
	 ...
	 *
	 * @param resource
	 * 		the <tt>ContactResource</tt>, which corresponding transport we're looking for
