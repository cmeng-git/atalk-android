...
public class SourceUIContact extends UIContactImpl
{
	 ...
	 *
	 * @param contact
	 * 		the <tt>SourceContact</tt>, on which this abstraction is based
	 ...
	 *
	 * @param isSelected
	 * 		indicates if the contact is currently selected in the contact list component
	 * @param width
	...
	public List<UIContactDetail> getContactDetails()
	{
		...
		for (ContactDetail detail : sourceContact.getContactDetails()) {
			resultList.add(new SourceContactDetail(detail,
					getInternationalizedLabel(detail.getCategory()),
					getInternationalizedLabels(detail.getSubCategories()),
					null, sourceContact));
		}
		return resultList;
	}
			...
			Class<? extends OperationSet> opSetClass)
	{
		...
		PhoneNumberI18nService phoneNumberService
				= AndroidGUIActivator.getPhoneNumberI18nService();
	 ...
	 *
	 * @param contactNode
	 * 		the corresponding <tt>ContactNode</tt>
	 */
	@Override
	...
	protected static class SourceContactDetail extends UIContactDetailImpl
	{
		 ...
		 *
		 * @param detail
		 * 		the underlying <tt>ContactDetail</tt>
		 * @param category
		 * 		detail category string
		 * @param subCategories
		 * 		the detail list of sub-categories
		 * @param opSetClass
		 * 		the <tt>OperationSet</tt> class for the preferred protocol provider
		 * @param sourceContact
		 * 		the source contact
		 */
		public SourceContactDetail(ContactDetail detail, String category,
				...
				Class<? extends OperationSet> opSetClass, SourceContact sourceContact)
		{
			...
			if (contactSource instanceof PrefixedContactSourceService) {
				String prefix
						= ((PrefixedContactSourceService) contactSource).getPhoneNumberPrefix();
	...
	public ExtendedTooltip getToolTip()
	{
		...
		try {
			List<ContactDetail> details
					= sourceContact.getContactDetails(ContactDetail.Category.Phone);
			if (details != null && details.size() > 0)
				addDetailsToToolTip(details, AndroidGUIActivator.getResources()
						.getI18NString("service.gui.PHONES"), tip);

			details = sourceContact.getContactDetails(ContactDetail.Category.Email);
			if (details != null && details.size() > 0)
				addDetailsToToolTip(details, AndroidGUIActivator.getResources()
						.getI18NString("service.gui.EMAILS"), tip);

			details = sourceContact.getContactDetails(ContactDetail.Category.InstantMessaging);
			if (details != null && details.size() > 0)
				addDetailsToToolTip(details, AndroidGUIActivator.getResources()
						.getI18NString("service.gui.INSTANT_MESSAGINGS"), tip);
		}
			...
			addDetailsToToolTip(allDetails, AndroidGUIActivator.getResources()
					.getI18NString("service.gui.CALL_WITH"), tip);
		}
		return tip;
	}

	private void addDetailsToToolTip(List<ContactDetail> details, String i18nString,
			ExtendedTooltip tip)
	{
		// TODO Auto-generated method stub
	 ...
	 *
	 * @param category
	 * 		the <tt>ContactDetail.SubCategory</tt>, for which we would like to obtain an
	 ...
	 *
	 * @param subCategories
	 * 		an Iterator over a list of <tt>ContactDetail.SubCategory</tt>s
	 ...
	 *
	 * @param initActions
	 * 		if <tt>true</tt> the actions will be reloaded.
