		...
		ChatRoomPropertyChangeListener, ChatRoomConferencePublishedListener
{
	 ...
	 *
	 * @param chatPanel
	 * 		the renderer to be used for communication with the UI.
	 * @param chatRoomWrapper
	 * 		the chat room corresponding to this conference session.
	 */
	 ...
	 * 		if no configuration form is available for the chat room.
	 */
	...
	public Collection<Object> getHistoryBeforeDate(Date date, int count)
	{
		...
		return metaHistory.findLastMessagesBefore(chatHistoryFilter, chatRoomWrapper.getChatRoom(),
				date, count);
	}
	 ...
	 *
	 * @param evt
	 * 		the <tt>ChatRoomMemberPresenceChangeEvent</tt> that notified us
	 */
			...
			public void run()
			{
				...
				if (eventType.equals(ChatRoomMemberPresenceChangeEvent.MEMBER_JOINED)) {
					...
					if (!evt.isReasonUserList()) {
						statusMessage = AndroidGUIActivator.getResources().getI18NString(
	 ...
	 *
	 * @param evt
	 * 		the event containing information about the property change
	 */
	 ...
	 *
	 * @param evt
	 * 		the event received, which contains the <tt>ChatRoom</tt>, <tt>ChatRoomMember</tt> and
	...
	public void conferencePublished(final ChatRoomConferencePublishedEvent evt)
	{
		(new Activity()).runOnUiThread(new Runnable()
		{
			public void run()
			{
				...
				if (evt.getType() == ChatRoomConferencePublishedEvent
						.CONFERENCE_DESCRIPTION_SENT) {
					// sessionRenderer.chatConferenceDescriptionSent(cd);
				}
				else if (evt.getType() == ChatRoomConferencePublishedEvent
	 ...
	 *
	 * @param chatRoom
	 * 		the chat room where the conference is announced.
	 * @param chatRoomMember
	 * 		the chat room member who announced the conference.
	 * @param cd
	 * 		the <tt>ConferenceDescription</tt> instance which represents the conference.
	 */
			...
			ConferenceDescription cd, int activeConferencesCount)
	{
		...
		for (ChatContact<?> chatContact : chatParticipants) {
			if (chatContact.getDescriptor().equals(chatRoomMember)) {
				/*
				 * TODO: we want more things to happen, e.g. the ConferenceDescription being added
				  * to a list in the GUI
