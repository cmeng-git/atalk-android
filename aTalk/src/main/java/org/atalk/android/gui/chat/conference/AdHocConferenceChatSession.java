 ...
 */
public class AdHocConferenceChatSession extends ChatSession implements
		AdHocChatRoomParticipantPresenceListener
{
	 ...
	 *
	 * @param sessionRenderer
	 * 		the renderer to be used for communication with the UI.
	 * @param chatRoomWrapper
	 * 		the ad-hoc chat room corresponding to this conference session.
	 */
	public AdHocConferenceChatSession(ChatPanel sessionRenderer,
			AdHocChatRoomWrapper chatRoomWrapper)
	{
		this.sessionRenderer = sessionRenderer;
		this.chatRoomWrapper = chatRoomWrapper;

		this.currentChatTransport = new AdHocConferenceChatTransport(this,
	...
	public void participantPresenceChanged(AdHocChatRoomParticipantPresenceChangeEvent evt)
	{
		...
		if (eventType.equals(AdHocChatRoomParticipantPresenceChangeEvent.CONTACT_JOINED)) {
			...
			if (!evt.isReasonUserList()) {
				statusMessage = AndroidGUIActivator.getResources()
						.getI18NString("service.gui.CHAT_ROOM_USER_JOINED",
			...
			if (eventType.equals(AdHocChatRoomParticipantPresenceChangeEvent.CONTACT_LEFT)) {
				statusMessage = AndroidGUIActivator.getResources()
						.getI18NString("service.gui.CHAT_ROOM_USER_LEFT",
								new String[]{sourceChatRoom.getName()});
			}
			else if (eventType.equals(AdHocChatRoomParticipantPresenceChangeEvent.CONTACT_QUIT)) {
				statusMessage = AndroidGUIActivator.getResources()
						.getI18NString("service.gui.CHAT_ROOM_USER_QUIT",
								new String[]{sourceChatRoom.getName()});
			}
