...
public class EntityListHelper
{
	 ...
	 *
	 * @param desc
	 * 		the contact or chatRoom to be removed from the list.
	 */
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				try {
					...
					AndroidUtils.showAlertDialog(ctx,
							ctx.getString(R.string.service_gui_REMOVE_CONTACT), ex.getMessage());
				}
			}
		}).start();
	}
	...
	public static void removeMetaContactGroup(final MetaContactGroup group)
	{
		...
		DialogActivity.showConfirmDialog(ctx, ctx.getString(R.string.service_gui_REMOVE), message,
	 ...
	 *
	 * @param group
	 * 		the group to remove from the contact list.
	 */
		...
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				try {
					...
					AndroidUtils.showAlertDialog(ctx,
							ctx.getString(R.string.service_gui_REMOVE_GROUP), ex.getMessage());
				}
			}
		}).start();
	}
	 ...
	 * 		the context
	 * @param desc
	 * 		descriptor either MetaContact or ChatRoomWrapper
	 * @param msgUUIDs
	 * 		list of message UID to be deleted. null to delete all for the specified desc
	 */
	public static void eraseEntityChatHistory(final Context caller, final Object desc,
			final List<String> msgUUIDs)
	{
		Context ctx = aTalkApp.getGlobalContext();
		String entityJid;
		...
		String title = ctx.getString(R.string.service_gui_HISTORY_CONTACT, entityJid);
		String message = ctx.getString(R.string.service_gui_HISTORY_REMOVE_PER_CONTACT_WARNING,
				entityJid);

		DialogActivity.showConfirmDialog(ctx, title, message,
				ctx.getString(R.string.service_gui_PURGE),
				new DialogActivity.DialogListener()
				{
					@Override
					public boolean onConfirmClicked(DialogActivity dialog)
