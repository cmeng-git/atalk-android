...
import org.atalk.android.plugin.audioservice.*;
 ...
 */
public class ChatController implements View.OnClickListener, View.OnLongClickListener,
		View.OnTouchListener, TextWatcher
{
	...
	public void onShow()
	{
		if (!isAttached) {
			...
			// Gets the send audio button and hooks on click action
	...
	public void onItemClick(AdapterView<?> adapter, View view, int position, long id)
	{
		...
		if (cPos != chatListAdapter.getCount() - 1) {
			for (int i = cPos + 1; i < chatListAdapter.getCount(); i++) {
				if (chatListAdapter.getItemViewType(i)
						== ChatFragment.ChatListAdapter.OUTGOING_MESSAGE_VIEW) {
					cancelCorrection();
					return;
				}
			}
	...
	public void editText(AdapterView adapter, ChatMessage chatMessage, int position)
	{
		...
		if (uidToCorrect != null && content != null) {
			...
			InputMethodManager inputMethodManager = (InputMethodManager) parent.getSystemService(
		...
		switch (v.getId()) {
			case R.id.audioRecordButton:
				...
				if (!mChatTransport.getStatus().isOnline()) {
					Toast.makeText(parent, R.string.chat_noaudio_buddyOffline,
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver()
	{
		...
		{
			if (isRecording
					&& intent.getAction().equals(AudioBgService.ACTION_SMI)) {
			...
			else if (intent.getAction().equals(AudioBgService.ACTION_AUDIO_RECORD)) {
	...
	/**
	 * Updates visibility state of cancel correction button and toggles bg color of the
	 * message edit field.
	 */
