...
public interface ChatMessage
{
	...
	String MSG_BODY = "msgBody";	// message
	String ENC_TYPE = "encType";	// indicator only for ENCRYPTION_* content_type???
	...
	/* Chat message body Encryption Type */
	int ENCODE_PLAIN = 0;	// text/plain (UTF-8)
	int ENCODE_HTML = 1;	// text/html

	int ENCRYPTION_NONE = 10;
	int ENCRYPTION_OMEMO = 11;
	int ENCRYPTION_OTR = 12;
	int ENCRYPTION_PGP = 13;

	int ENCRYPTION_DECRYPTED = 20;
	int ENCRYPTION_DECRYPTION_FAILED = 21;
	...
	/**
	 * Returns the encode/encryption of the content type (e.g. "text", "text/html", etc.).
	 *
	 * @return the encType
	 */
	int getEncType();
	 ...
	 * 
	 * @param consecutiveMessage
	 *        the next message to merge with <tt>this</tt> instance(it must be consecutive in terms of
