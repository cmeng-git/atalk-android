...
public class FileHistoryConversation extends FileTransferConversation
{
	...
	public static FileHistoryConversation newInstance(ChatFragment cPanel,
			FileRecord fileRecord, ChatMessage msg)
	{
	...
	public View FileHistoryConversationForm(LayoutInflater inflater,
			MessageViewHolder msgViewHolder, ViewGroup container, boolean init)
	{
		...
		messageViewHolder.titleLabel.setText(date.toString() + ":\n" + titleString);
		messageViewHolder.fileLabel.setText(getFileLabel(fileRecord.getFile()));

		if (bgAlert) {
			messageViewHolder.titleLabel.setTextColor(AndroidGUIActivator.getResources()
					.getColor("red"));
		}
		return convertView;
	}
		...
		if (dir.equals(FileRecord.IN)) {
			switch (status) {
				case FileRecord.COMPLETED:
					statusMsg = aTalkApp.getResString(
							R.string.xFile_FILE_RECEIVE_COMPLETED, entityJid);
					break;
				case FileRecord.CANCELED:
					statusMsg = aTalkApp.getResString(
							R.string.service_gui_FILE_TRANSFER_CANCELED);
					break;
				case FileRecord.FAILED:
					statusMsg = aTalkApp.getResString(
							R.string.xFile_FILE_RECEIVE_FAILED, entityJid);
					break;
				...
				case FileRecord.ACTIVE:
					statusMsg = aTalkApp.getResString(
							R.string.service_gui_FILE_TRANSFER_ACTIVE);
					break;
			}
		}
		else {
			switch (status) {
				case FileRecord.COMPLETED:
					statusMsg = aTalkApp.getResString(
							R.string.xFile_FILE_SEND_COMPLETED, entityJid);
					break;
				case FileRecord.CANCELED:
				...
				case FileRecord.FAILED:
					statusMsg = aTalkApp.getResString(
							R.string.xFile_FILE_UNABLE_TO_SEND, entityJid);
					break;
				case FileRecord.REFUSED:
					statusMsg = aTalkApp.getResString(
							R.string.xFile_FILE_SEND_REFUSED, entityJid);
					break;
				case FileRecord.ACTIVE:
					statusMsg = aTalkApp.getResString(
							R.string.service_gui_FILE_TRANSFER_ACTIVE);
