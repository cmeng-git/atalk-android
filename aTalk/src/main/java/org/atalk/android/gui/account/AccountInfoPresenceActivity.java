		...
		CalendarDatePickerDialogFragment.OnDateSetListener
{
	...
	private static final CalendarDay DEFAULT_START_DATE
			= new CalendarDay(1900, Calendar.JANUARY, 1);
	 ...
	 */
	private final Map<Class<? extends GenericDetail>, EditText> detailToTextField
			= new HashMap<>();
	...
	protected void onCreate(android.os.Bundle savedInstanceState)
	{
		...
		if (accountInfoOpSet != null) {
			...
			if (!isRegistered) {
				...
				Toast.makeText(this, R.string.plugin_accountinfo_NO_REGISTERED_MESSAGE,
	...
	private void initSummaryPanel()
	{
		...
		if (accountInfoOpSet.isDetailClassSupported(AboutMeDetail.class)) {
			...
			filterArray[0] = new InputFilter.LengthFilter(Integer
					.valueOf(getString(R.string.plugin_accountinfo_ABOUT_ME_MAX_CHARACTERS)));
	 ...
	 *
	 * @param detail
	 * 		the loaded detail for extraction.
	 */
	...
	public boolean onOptionsItemSelected(MenuItem item)
	{
		...
		if (id == R.id.remove) {
			...
			Intent preferences = AccountPreferencesActivity.getIntent(this,
	 ...
	 *
	 * @param avatarView
	 * 		the {@link View} that has been clicked
	 */
	 ...
	 *
	 * @param requestCode
	 * 		the request code {@link #SELECT_IMAGE}
	 * @param resultCode
	 * 		the result code
	 * @param data
	 * 		the source {@link Intent} that returns the result
	 */
	...
	private void showAvatarChangeError()
	{
		AndroidUtils.showAlertDialog(this, getString(R.string.service_gui_ERROR),
				getString(R.string.service_gui_AVATAR_SET_ERROR, mAccount.getAccountName()));
	}
		...
		new Thread(new Runnable()
		{
			public void run()
			{
				try {
					...
					GlobalStatusService globalStatus = ServiceUtils.getService(
							AndroidGUIActivator.bundleContext, GlobalStatusService.class);
	 ...
	 *
	 * @param eventObject
	 * 		the instance that has been changed
			...
			if ((selectedStatus.getStatus() == PresenceStatus.OFFLINE)
				...
				if (progressDialog != null) {
					progressDialog.setMessage(
							getString(R.string.plugin_accountinfo_DISCARD_CHANGE));
				}
			}
	...
	private class StatusListAdapter extends CollectionAdapter<PresenceStatus>
	{
		 ...
		 *
		 * @param objects
		 * 		{@link Iterator} for a set of {@link PresenceStatus}
		 */
		...
		@Override
		protected View getView(boolean isDropDown, PresenceStatus item, ViewGroup parent,
				LayoutInflater inflater)
		{
			// Retrieve views
			View statusItemView
					= inflater.inflate(R.layout.account_presence_status_row, parent, false);
			TextView statusNameView
					= (TextView) statusItemView.findViewById(R.id.presenceStatusNameView);
			ImageView statusIconView
					= (ImageView) statusItemView.findViewById(R.id.presenceStatusIconView);
		 ...
		 *
		 * @param status
		 * 		the {@link PresenceStatus} for which the position is returned
