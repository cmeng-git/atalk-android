...
/**
 * <tt>BroadcastReceiver</tt> that listens for {@link #CALL_CTRL_ACTION} action and performs few basic operations(mute,
 * hangup...) on the call.<br/>
 * Target call must be specified by ID passed as extra argument under {@link #EXTRA_CALL_ID} key. The IDs are managed by
 * {@link CallManager}.<br/>
...
public class CallControl extends BroadcastReceiver
{
	...
	/**
	 * Creates new <tt>Intent</tt> for given call <tt>action</tt> value that will be performed on the call identified by
	 * <tt>callID</tt>.
	 *
	 * @param callID
	 *        target call ID managed by {@link CallManager}.
	 * @param action
	 *        the action value that will be used.
	 *
	 * @return new <tt>Intent</tt> for given call <tt>action</tt> value that will be performed on the call identified by
	 *         <tt>callID</tt>.
	 */
