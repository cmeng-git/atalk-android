...
public class AndroidImageUtil
{
	 ...
	 *
	 * @param imageBlob
	 * 		array of bytes with raw image data
	 ...
	 *
	 * @param imageBlob
	 * 		the array of bytes containing raw image data
	 ...
	 *
	 * @param options
	 * 		the <tt>Options</tt> object that contains image <tt>outWidth</tt> and
	 * 		<tt>outHeight</tt>.
	 * @param reqWidth
	 * 		requested width.
	 * @param reqHeight
	...
	/**
	 * Decodes <tt>Bitmap</tt> identified by given <tt>resId</tt> scaled to requested width and
	 * height.
	 *
	 * @param res
	 * 		the <tt>Resources</tt> object.
	 * @param resId
	 * 		bitmap resource id.
	 * @param reqWidth
	 * 		requested width.
	 ...
	 * height.
	 */
	public static Bitmap scaledBitmapFromResource(Resources res, int resId, int reqWidth,
			int reqHeight)
	{
		// First decode with inJustDecodeBounds=true to check dimensions
		final BitmapFactory.Options options = new BitmapFactory.Options();
	...
	/**
	 * Reads <tt>Bitmap</tt> from given <tt>uri</tt> using <tt>ContentResolver</tt>. Output image
	 * is scaled to given
	 * <tt>reqWidth</tt> and <tt>reqHeight</tt>. Output size is not guaranteed to match exact
	 * given values, because only
	 * powers of 2 are used as scale factor. Algorithm tries to scale image down as long as the
	 * output size stays larger
	 * than requested value.
	 *
	 * @param ctx
	 * 		the context used to create <tt>ContentResolver</tt>.
	 * @param uri
	 * 		the <tt>Uri</tt> that points to the image.
	 * @param reqWidth
	 * 		requested width.
	 * @param reqHeight
	 * 		requested height.
	 ...
	 * and down sampled as
	 * close as possible to match requested width and height.
	 * @throws IOException
	 */
	public static Bitmap scaledBitmapFromContentUri(Context ctx, Uri uri, int reqWidth,
			int reqHeight)
			throws IOException
	{
	...
	/**
	 * Encodes given <tt>Bitmap</tt> to array of bytes using given compression <tt>quality</tt> in
	 * PNG format.
	 *
	 * @param bmp
	 * 		the bitmap to encode.
	 * @param quality
	 * 		encoding quality in range 0-100.
	 ...
	 *
	 * @param bitmap
	 * 		the bitmap that will have it's corners rounded.
	 * @param factor
	 * 		factor used to calculate corners radius based on width and height of the image.
	...
	{
		Bitmap bmp
				= getRoundedCornerBitmap(scaledBitmapFromBytes(imageBytes, width, height), 0.1f);
		return new BitmapDrawable(aTalkApp.getAppResources(), bmp);
	}
}
