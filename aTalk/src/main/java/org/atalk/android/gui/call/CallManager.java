...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>CallManager</tt> class and its instances for logging
	 * output.
	 */
	...
	/**
	 * A table mapping protocol <tt>Call</tt> objects to the GUI dialogs that are currently used
	 * to display them.
	 */
	 ...
	 * 		the call to answer
	 * @param isVideoCall
	 * 		the incoming call type (audio/video?)
	 */
	 ...
	 *
	 * @param conference
	 * 		the <tt>CallConference</tt> whose participating <tt>Call</tt>s are to be hanged up
	 */
	...
	{
		new CreateCallThread(protocolProvider, null, null, uiContact, contact, null, null, false
				/* audio-only */).run();
	}
	...
	public static void createVideoCall(ProtocolProviderService protocolProvider, String contact,
			UIContactImpl uiContact)
	{
		new CreateCallThread(protocolProvider, null, null, uiContact, contact, null, null,
				true /* video */).start();
	}
	 ...
	 * @param enable
	 * 		<tt>true</tt> to enable the local video; otherwise, <tt>false</tt>
	 */
	 ...
	 *
	 * @param call
	 * 		the <tt>Call</tt>, for which we would to check if the desktop sharing is currently
	 * 		enabled
	 ...
	 * 		the string to call
	 * @param c
	 * 		the component, which indicates where should be shown the "call via" menu if needed
	 */
	 ...
	 * 		the component, which indicates where should be shown the "call via" menu if needed
	 * @param l
	 * 		listener that is notified when the call interface has been started after call was
	 * 		created
	 */
		...
		if (telephonyProviders.size() == 1) {
			...
			if (c == null) {
				...
				for (ProtocolProviderService telephonyProvider : telephonyProviders) {
					try {
						OperationSetPresence presenceOpSet = telephonyProvider
								.getOperationSet(OperationSetPresence.class);

						if ((presenceOpSet != null)
								&& (presenceOpSet.findContactByID(callString) != null)) {
			...
			new ErrorDialog(null, resources.getI18NString("service.gui.WARNING"),
					resources.getI18NString(
							"service.gui.NO_ONLINE_TELEPHONY_ACCOUNT")).showDialog();
		}
	}
	 ...
	 * 		the list of contacts to invite
	 * @param conference
	 * 		the telephony conference to invite the specified <tt>callees</tt> into
	 */
	public static void inviteToConferenceCall(Map<ProtocolProviderService, List<String>> callees,
	 ...
	 *
	 * @param callees
	 * 		the list of participants/callees to invite to a newly-created conference <tt>Call</tt>
	 */
	 ...
	 *
	 * @param callProvider
	 * 		the <tt>ProtocolProviderService</tt> to use for creating the call
	 * @param callees
	 * 		the list of participants/callees to invite to the newly-created video bridge
	 * 		conference <tt>Call</tt>
	 */
	public static void createJitsiVideobridgeConfCall(ProtocolProviderService callProvider,
			String[] callees)
	{
	 ...
	 *
	 * @param peer
	 * 		the <tt>CallPeer</tt> to transfer
	 * @param target
	 * 		the <tt>CallPeer</tt> target to transfer to
	 */
	public static void transferCall(CallPeer peer, CallPeer target)
	{
		OperationSetAdvancedTelephony<?> telephony = peer.getCall().getProtocolProvider()
	 ...
	 *
	 * @param call
	 * 		the call to check for video streaming
	 ...
	 *
	 * @param conference
	 * 		the conference we check for video streaming
	 * @return <tt>true</tt> if we have video streams to show in this interface; otherwise,
	 * <tt>false</tt>
	 */
	 ...
	 *
	 * @param device
	 * 		the <tt>MediaDevice</tt>, which audio formats we're looking for
	 * @param protocolProvider
	 * 		the provider to check.
	 * @return list of supported/enabled audio formats or empty list otherwise.
	 */
	private static List<MediaFormat> getAudioFormats(MediaDevice device,
			ProtocolProviderService protocolProvider)
	{
	...
	private static class CreateCallThread extends Thread
	{
		 ...
		 *
		 * @param protocolProvider
		 * 		the protocol provider through which the call is going.
		 * @param contact
		 * 		the contact to call
		 * @param contactResource
		 * 		the specific <tt>ContactResource</tt> we're calling
		 * @param video
		 * 		indicates if this is a video call
		 */
		 ...
		 * 		indicates if this is a video call
		 */
		public CreateCallThread(ProtocolProviderService protocolProvider, String contact,
				boolean video)
		{
		 ...
		 *
		 * @param protocolProvider
		 * 		the <tt>ProtocolProviderService</tt> which is to perform the establishment of the
		 * 		new <tt>Call</tt>.
		 * @param conferenceDescription
		 * 		the description of the conference to call.
		 * @param chatRoom
		 * 		the chat room associated with the call.
		 */
		 ...
		 * 		the specific contact resource to call
		 * @param uiContact
		 * 		the ui contact we're calling
		 * @param stringContact
		 * 		the string to call
		 * @param video
		 * 		<tt>true</tt> if this instance is to create a new video (as opposed to audio-only)
		 * 		<tt>Call</tt>
		 * @param conferenceDescription
		 * 		the description of a conference to call
		 * @param chatRoom
		 * 		the chat room associated with the call.
		 */
		public CreateCallThread(ProtocolProviderService protocolProvider, Contact contact,
		...
		public void run()
		{
			if (!video) {
				...
				if (!dev.getDirection().allowsSending())
					errMsg = AndroidGUIActivator.getResources()
							.getI18NString("service.gui.CALL_NO_AUDIO_DEVICE");
				else if (formats.isEmpty()) {
					errMsg = AndroidGUIActivator.getResources()
							.getI18NString("service.gui.CALL_NO_AUDIO_CODEC");
				}

				if (errMsg != null) {
					AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
							AndroidGUIActivator.getResources().getI18NString("service.gui.CALL"),
							errMsg + "  " + AndroidGUIActivator.getResources().getI18NString(
									"service.gui.CALL_NO_DEVICE_CODEC_H", new String[]{""})
					);
					return;
				}
			}
			...
			try {
				if (conferenceDescription != null) {
					...
					if (video) {
						...
						internalCall(protocolProvider, contact, stringContact, contactResource,
								uiContact);
					}
				}
			}
				...
				String message = AndroidGUIActivator.getResources()
						.getI18NString("service.gui.CREATE_CALL_FAILED");

				if (t.getMessage() != null)
					message += " " + t.getMessage();

//				new ErrorDialog(null, AndroidGUIActivator.getResources()
//						.getI18NString("service.gui.ERROR"), message, t).showDialog();
				AndroidUtils.showAlertDialog(aTalkApp.getGlobalContext(),
						AndroidGUIActivator.getResources().getI18NString("service.gui.ERROR"),
						message
				);
			}
		}
	 ...
	 *
	 * @param protocolProvider
	 * 		the <tt>ProtocolProviderService</tt> through which to make the call
	 * @param contact
	 * 		the <tt>Contact</tt> to call
	 * @param uiContact
	 * 		the <tt>UIContactImpl</tt> we're calling
	 * @param stringContact
	 * 		the contact string to call
	 * @throws OperationFailedException
	 * 		thrown if the call operation fails
	 * @throws ParseException
	 * 		thrown if the contact string is malformatted
	 */
	...
	{
		OperationSetVideoTelephony telephony
				= protocolProvider.getOperationSet(OperationSetVideoTelephony.class);
			...
			throws OperationFailedException, ParseException
	{
		...
		if (resourceTelephony != null && contactResource != null) {
			if (contact != null)
				...
				createdCall = resourceTelephony.createCall(stringContact,
	 ...
	 *
	 * @param call
	 * 		the <tt>Call</tt>, which corresponding <tt>MetaContact</tt> we're looking for
	 ...
	 *
	 * @param uiContact
	 * 		the <tt>UIContact</tt> corresponding to the call
	 * @param call
	 * 		the <tt>Call</tt> corresponding to the <tt>MetaContact</tt>
	 */
	...
	private static class CreateDesktopSharingThread extends Thread
	{
		 ...
		 *
		 * @param protocolProvider
		 * 		protocol provider through which we share our desktop
		 * @param contact
		 * 		the contact to share the desktop with
		 * @param uiContact
		 * 		the <tt>UIContact</tt>, which initiated the desktop sharing session
		 * @param mediaDevice
		 * 		the media device corresponding to the screen we would like to share
		 */
				...
				if (isVideoCall) {
					...
					try {
						...
						logger.error("Could not answer " + peer
								+ " with video because of the following exception: " + ofe);
					}
				}
				else {
					OperationSetBasicTelephony<?> telephony
	...
	private static class InviteToConferenceCallThread extends Thread
	{
		 ...
		 *
		 * @param callees
		 * 		the addresses of the callees to be invited into a telephony conference. The
		 ...
		 * @param call
		 * 		the <tt>Call</tt> to invite the specified <tt>callees</tt> into. If <tt>null</tt>,
		 ...
		 */
		public InviteToConferenceCallThread(Map<ProtocolProviderService, List<String>> callees,
				Call call)
		{
	...
	private static class InviteToConferenceBridgeThread extends Thread
	{
		...
		public InviteToConferenceBridgeThread(ProtocolProviderService callProvider,
				String[] callees, Call call)
		{
		...
		public void run()
		{
			OperationSetVideoBridge opSetVideoBridge
					= callProvider.getOperationSet(OperationSetVideoBridge.class);
	...
	private static class HangupCallThread extends Thread
	{
		 ...
		 *
		 * @param call
		 * 		the <tt>Call</tt> whose associated <tt>CallPeer</tt>s are to be hanged up
		 */
		 ...
		 *
		 * @param conference
		 * 		the <tt>CallConference</tt> whose participating <tt>Call</tt>s re to be hanged up
		 */
		...
		public void run()
		{
			...
			for (CallPeer peer : peers) {
				OperationSetBasicTelephony<?> basicTelephony = peer.getProtocolProvider()
						.getOperationSet(OperationSetBasicTelephony.class);
	...
	private static class EnableLocalVideoThread extends Thread
	{
		 ...
		 * @param enable
		 * 		allow to have LocalVideo streaming if true
		 */
	...
	private static class PutOnHoldCallPeerThread extends Thread
	{
		...
		public void run()
		{
			OperationSetBasicTelephony<?> telephony = callPeer.getProtocolProvider()
					.getOperationSet(OperationSetBasicTelephony.class);
			try {
				if (isOnHold)
	...
	private static class MergeExistingCalls extends Thread
	{
		 ...
		 *
		 * @param conference
		 * 		the telephony conference in which the specified <tt>Call</tt>s are to be merged
		 * @param calls
		 * 		the <tt>Call</tt>s to be merged into the specified telephony conference
		 */
		 ...
		 *
		 * @param call
		 * 		the <tt>Call</tt> which is to have its <tt>CallPeer</tt>s put off hold
		 */
		private void putOffHold(Call call)
		{
			...
			while (peers.hasNext()) {
				...
				if (callPeer instanceof MediaAwareCallPeer) {
					putOffHold = ((MediaAwareCallPeer<?, ?, ?>) callPeer).getMediaHandler()
							.isLocallyOnHold();
				}
				if (putOffHold) {
					try {
	 ...
	 *
	 * @param callees
	 * 		the list of contact addresses or phone numbers to be normalized
	 */
