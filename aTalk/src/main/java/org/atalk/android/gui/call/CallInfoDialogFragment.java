...
public class CallInfoDialogFragment extends OSGiDialogFragment
{
	 ...
	 *
	 * @param callKey
	 *        the key string that identifies active call in {@link CallManager}.
	 *
	...
	private void updateIceSection(CallPeerMediaHandler<?> callPeerMediaHandler)
	{
		...
		if (isTotalHarvestTime) {
            ...
            setTextViewValue(getView(), R.id.totalHarvestTime,
		...
		for (int i = 0; i < harvesterLabels.length; ++i) {
			...
			if (visible) {
                ...
                setTextViewValue(getView(), harvesterValues[i],
                        getString(R.string.service_gui_callinfo_HARVESTING_DATA, harvestingTime, callPeerMediaHandler.getNbHarvesting()));
            }
		}
	}
	 ...
	 * @param mediaStream
	 *        the <tt>MediaStream</tt> that gives us access to audio/video info.
	 * @param mediaType
	 *        The media type used to determine which stream of the media handler must returns it encryption method.
	 */
	 ...
	 *
	 * @param callPeerMediaHandler
	 *        The <tt>CallPeerMadiaHandler</tt> containing the AUDIO/VIDEO stream.
	 * @param mediaType
	 *        The media type used to determine which stream of the media handler will be used.
	 */
	private void updateAudioVideoInfo(CallPeerMediaHandler<?> callPeerMediaHandler, MediaType mediaType)
	{
		...
		if (mediaType == MediaType.VIDEO) {
			...
			if (downloadVideoSize != null || uploadVideoSize != null) {
				...
				setTextViewValue(container, R.id.videoSize,
					DOWN_ARROW + " " + this.videoSizeToString(downloadVideoSize) + " " + UP_ARROW + " " + this.videoSizeToString(uploadVideoSize));
			}
		}
		...
		// Discarded total.
		String discardedTotalStr = String.valueOf(mediaStreamStats.getNbDiscarded()) + " (" + mediaStreamStats.getNbDiscardedLate() + " late, "
		...
		// Jitter buffer delay.
		String jitterDelayStr = "~" + mediaStreamStats.getJitterBufferDelayMs() + "ms; currently in queue: " + mediaStreamStats.getPacketQueueCountPackets()
	 ...
	 *
	 * @param videoSize
	 *        The video size Dimension, containing the width and the height of the video.
	 *
