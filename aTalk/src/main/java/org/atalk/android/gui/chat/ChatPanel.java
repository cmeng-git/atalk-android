...
import org.atalk.android.util.javax.swing.event.*;
...
public class ChatPanel implements Chat, MessageListener
{
	 ...
	 *
	 * @param metaContact
	 * 		the <tt>MetaContact</tt> we're chatting with
	 */
	 ...
	 *
	 * @param chatType
	 * 		selected chatType e.g. MSGTYPE_NORMAL
	 **/
	 ...
	 *
	 * @param correctionUID
	 * 		the UID of recently corrected message.
	 */
	 ...
	 *
	 * @param msgListener
	 * 		the <tt>ChatSessionListener</tt> to add
	 */
	 ...
	 *
	 * @param msgListener
	 * 		the <tt>ChatSessionListener</tt> to remove
	 */
	 ...
	 *
	 * @param l
	 * 		the <tt>ChatStateNotificationsListener</tt> to add
	 */
	public void addChatStateListener(ChatStateNotificationsListener l)
	{
		...
		while (protoContacts.hasNext()) {
			...
			OperationSetChatStateNotifications chatStateOpSet = protoContact.getProtocolProvider()
					.getOperationSet(OperationSetChatStateNotifications.class);
	 ...
	 *
	 * @param l
	 * 		the <tt>ChatStateNotificationsListener</tt> to remove
	 */
	 ...
	 *
	 * @param l
	 * 		the <tt>ContactPresenceStatusListener</tt> to add
	 */
	 ...
	 *
	 * @param l
	 * 		the <tt>ContactPresenceStatusListener</tt> to remove
	 */
	 ...
	 */
	private List<ChatMessage> mergeMsgLists(List<ChatMessage> list1, List<ChatMessage> list2,
			int msgLimit)
	{
		if (msgLimit == -1)
	 ...
	 *
	 * @param isVisible
	 * 		tells if the chat will be made visible or not.
	 */
	@Override
	 ...
	 *
	 * @param message
	 * 		the text that would be set to the chat write area
	 */
	@Override
	 ...
	 *
	 * @param chatFocusListener
	 * 		the <tt>ChatFocusListener</tt> to add
	 */
	@Override
	 ...
	 *
	 * @param chatMenuListener
	 * 		the <tt>ChatMenuListener</tt> to add
	 */
	 ...
	 *
	 * @param caretListener
	 * 		the <tt>CaretListener</tt> to add
	 */
	@Override
	 ...
	 *
	 * @param caretListener
	 * 		the <tt>CaretListener</tt> to remove
	 */
	 ...
	 *
	 * @param documentListener
	 * 		the <tt>DocumentListener</tt> to remove
	 */
	 ...
	 * 		the message text
	 * @param encType
	 * 		the content encryption type
	 */
	@Override
	public void addMessage(String contactName, Date date, String messageType, String message,
			int encType)
	{
	 ...
	 * 		the time at which the message is sent or received
	 * @param messageType
	 * 		the type of the message. One of MESSAGE_OUT or MESSAGE_IN
	 * @param message
	 * 		the message text
	 * @param encType
	 * 		the content encryption type
			...
			Date date)
	{
		...
		ChatMessageImpl chatMsg = new ChatMessageImpl(senderName, date, msgType, message,
				encType, opSet, request);
	 ...
	 * 		callback that is notified when a link was clicked.
	 */
	@Override
	 ...
	 *
	 * @param chatTransport
	 * 		the <tt>chatTransport</tt> to update
	 */
	 ...
	 *
	 * @param chatContact
	 * 		the chat contact of the conference to update
	 * @param statusMessage
	 ...
	 *
	 * @param inviteChatTransport
	 * 		the chat transport to use to send the invite
	 * @param chatContacts
	 * 		the contacts to invite
	 * @param reason
	 * 		the reason of the invitation
	 */
	public void inviteContacts(ChatTransport inviteChatTransport, Collection<String> chatContacts,
			String reason)
	{
		...
		if (mChatSession instanceof MetaContactChatSession) {
			ConferenceChatManager conferenceChatManager = AndroidGUIActivator.getUIService()
		...
		// We're already in a conference chat.
			...
			for (String contactAddress : chatContacts) {
				try {
					mCurrentChatTransport
							.inviteChatContact(JidCreate.entityBareFrom(contactAddress), reason);
				}
