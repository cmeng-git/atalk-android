...
public class PublicIPResolver {
    ...
    public static ByteBuffer createSTUNChangeRequest() {
        ...
        System.arraycopy(CHANGE_REQUEST_NO_CHANGE, 0, data, header.length,
                CHANGE_REQUEST_NO_CHANGE.length);
        ...
        if (data.length >= 8) {
            ...
            if (family == 1) {
                ...
                final StringBuilder ip = new StringBuilder()
                        ...
                        .append(".")
                        .append(fourthOctet);
                return new Header(new InetSocketAddress(ip.toString(), port), type,
                        lengthValue + 4);
            }
        }
        ...
        for (int t = 0; t < 3; t++) {
            try {
                final SelDatagramChannel channel = SelDatagramChannel.open(null,
                        new InetSocketAddress(System.getProperty("os.name") != null
                                && System.getProperty("os.name").toLowerCase().contains("win")
                                ? LocalIPResolver.getLocalIP() : "0.0.0.0", lport));
    ...
    public static InetSocketAddress getPublicAddress(final SelDatagramChannel channel,
        ...
        try {
            channel.setDatagramListener(new DatagramListener() {
                public void datagramReceived(SelDatagramChannel channel, ByteBuffer buffer,
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (i % 2 == 0) {
                    channel.send(createSTUNChangeRequest(),
                            new InetSocketAddress(stunServer, port));
                }
            }
            return null;
        } catch (IOException e) {
