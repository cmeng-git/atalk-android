...
public class SmackServiceNode implements ConnectionListener, StanzaListener
{
	...
	static {
		ProviderManager.addIQProvider(JingleChannelIQ.NAME, JingleChannelIQ.NAMESPACE,
				new JingleNodesProvider());
		ProviderManager.addIQProvider(JingleTrackerIQ.NAME, JingleTrackerIQ.NAMESPACE,
				new JingleTrackerProvider());
	}
	...
	public static JingleChannelIQ getChannel(final XMPPTCPConnection xmppConnection,
			final Jid serviceNode)
	{
		...
		StanzaCollector collector
				= xmppConnection.createStanzaCollector(new StanzaIdFilter(iq.getStanzaId()));
		try {
		...
		try {
			result = (JingleChannelIQ) collector.nextResult(
	...
	public static JingleTrackerIQ getServices(final XMPPTCPConnection xmppConnection,
			final Jid serviceNode)
	{
		if (xmppConnection == null || !xmppConnection.isConnected()) {
			return null;
			...
			final String protocol, final boolean searchBuddies)
	{
		return searchServices(new ConcurrentHashMap<Jid, Jid>(), xmppConnection, maxEntries,
				maxDepth, maxSearchNodes, protocol, searchBuddies,
				new MappedNodes());
	}
	...
	{
		final Map<Jid, TrackerEntry> relayEntries
				= Collections.synchronizedMap(new LinkedHashMap<Jid, TrackerEntry>());
		final Map<Jid, TrackerEntry> trackerEntries
				= Collections.synchronizedMap(new LinkedHashMap<Jid, TrackerEntry>());
