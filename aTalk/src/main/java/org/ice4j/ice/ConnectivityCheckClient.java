...
class ConnectivityCheckClient implements ResponseCollector
{
     ...
     * @param scheduledExecutorService the <tt>ScheduledExecutorService</tt> to execute clients tasks
     ...
     * check client transaction or <tt>null</tt> if sending the check has failed for some reason.
     */
            ...
            int maxRetransmissions)
    {
        ...
        //reflexive candidate if one is discovered as a consequence of this check.
        PriorityAttribute priority = AttributeFactory.createPriorityAttribute(
                localCandidate.computePriorityForType(CandidateType.PEER_REFLEXIVE_CANDIDATE));
        ...
        MessageIntegrityAttribute msgIntegrity
                = AttributeFactory.createMessageIntegrityAttribute(localUserName);
        ...
        logger.debug("start check for " + candidatePair.toShortString() + " tid " + tran);
        try {
        ...
        if (checkList.allChecksCompleted()) {
            ...
            if (!stream.validListContainsAllComponents()) {
                ...
                if (!checkListCompletionCheckers.containsKey(streamName)) {
                    ...
                    Runnable checkLickCompletedChecker = () -> {
    ...
    private void processSuccessResponse(StunResponseEvent ev)
    {
        ...
        if (!response.containsAttribute(Attribute.XOR_MAPPED_ADDRESS)) {
            ...
            logger.info("Pair failed (no XOR-MAPPED-ADDRESS): "
                    + checkedPair.toShortString() + ". Local ufrag" + parentAgent.getLocalUfrag());
            checkedPair.setStateFailed();
            return; //malformed error response
        }
