 ...
 * the media stream as a whole to work. For media streams based on RTP, there
 * are two components per media stream - one for RTP, and one for RTCP.
 *
 * @author Emil Ivov
...
public class Component implements PropertyChangeListener
{
    ...
    /**
     * A <tt>Comparator</tt> that we use for sorting <tt>Candidate</tt>s by their priority.
     */
     ...
     * {@link MultiplexingDatagramSocket} for the convenience of users of the library.
     * This is the instance which should be used by applications for reading/writing application data.
     */
    private final MultiplexingDatagramSocket socket;

    /**
     * A wrapper around {@link #socket}, kept only to help preserve the old
     * API (see {@link LocalCandidate#getIceSocketWrapper()} and {@link CandidatePair#getIceSocketWrapper()})
     */
    @Deprecated
    ...
    /**
     * Add a local candidate to this component. The method should only be accessed and
    ...
    public boolean addLocalCandidate(LocalCandidate candidate)
    {
        ...
        synchronized (localCandidates) {
            ...
            if (!CandidateType.HOST_CANDIDATE.equals(candidate.getType())) {
                logger.info("Add transport candidate: " + candidate
                        + ";\nredundantCandidate: " + redundantCandidate);
            }
    ...
    /**
     * Returns the number of local host candidates currently registered in this <tt>Component</tt>.
     *
    ...
    /**
     * Returns the number of all local candidates currently registered in this <tt>Component</tt>.
     *
    ...
    /**
     * Adds a remote <tt>Candidate</tt>s to this media-stream <tt>Component</tt>.
     *
    ...
    public void addUpdateRemoteCandidates(RemoteCandidate candidate)
    {
        ...
        synchronized (remoteCandidates) {
            existingCandidates = new LinkedList<>(remoteCandidates);
        }
                ...
                for (RemoteCandidate remoteCnd : remoteUpdateCandidates) {
                    if (localCnd.canReach(remoteCnd)
                        ...
                        if (localCnd.getTransport() == Transport.TCP &&
                            localCnd.getIceSocketWrapper().getTCPSocket().isConnected()) {
                            if (!localCnd.getIceSocketWrapper().getTCPSocket().
                                getRemoteSocketAddress().equals(remoteCnd.getTransportAddress())) {
                                continue;
                            }
                        }
                        */
            ...
            synchronized (streamCheckList) {
                for (CandidatePair pair : checkList) {
                    ...
                    if (existingPair != null) {
                        logger.info("existing Pair updated: " + existingPair.toShortString() +
                        ...
                        logger.info("new Pair added: " + pair.toShortString()
                                + ". Local ufrag " + parentStream.getParentAgent().getLocalUfrag());
                    }
                }
            }
        }
    ...
    /**
     * Returns the number of all remote candidates currently registered in this <tt>Component</tt>.
     *
    ...
    /**
     * Returns a reference to the <tt>IceMediaStream</tt> that this <tt>Component</tt> belongs to.
     *
    ...
    /**
     * Returns the ID of this <tt>Component</tt>. For RTP/RTCP flows this would be <tt>1</tt> for RTP and 2 for <tt>RTCP</tt>.
     *
    ...
    /**
     * Returns a short <tt>String</tt> representation of this <tt>Component</tt>.
     *
    ...
    protected void prioritizeCandidates()
    {
        synchronized (localCandidates) {
            ...
            Collections.addAll(localCandidates, candidates);
        }
    }

    /**
     * Eliminates redundant candidates, removing them from the specified <tt>component</tt>.
     ...
     * and these would not be considered redundant.
     *
     * Frequently, a server reflexive candidate and a host candidate will be redundant when
     * the agent is not behind a NAT; via cand.getBase().equals(redundantCand.getBase()
     * The agent SHOULD eliminate the redundant candidate with the lower priority which
        ...
        synchronized (localCandidates) {
            for (LocalCandidate redundantCand : localCandidates) {
                if ((cand != redundantCand)
                        && cand.getTransportAddress().equals(redundantCand.getTransportAddress())
    ...
    /**
     * Returns the <tt>Candidate</tt> that has been selected as the default for this <tt>Component</tt>
     * or <tt>null</tt> if no such <tt>Candidate</tt> has been selected yet. A candidate is said to be
    ...
    /**
     * Selects a <tt>Candidate</tt> that should be considered as the default for this <tt>Component</tt>.
     * A candidate is said to be default if it would be the target of media from a non-ICE peer;
     * <p>
     * The ICE specification RECOMMENDED that default candidates be chosen
    ...
    protected void free()
    {
        synchronized (localCandidates) {
             ...
             */
            CandidateType[] candidateTypes = new CandidateType[]{
    ...
    /**
     * Returns the local <tt>LocalCandidate</tt> with the specified <tt>localAddress</tt>
    ...
    /**
     * Returns the remote <tt>Candidate</tt> with the specified <tt>remoteAddress</tt>
    ...
    /**
     * @return the single socket for this {@link Component} which should be used for reading
     * and writing data, if the component socket is enabled  and {@code null} if it is not enabled.
     * </p>
    ...
    public void propertyChange(PropertyChangeEvent event)
    {
        ...
        if (!equals(pair.getParentComponent())) {
            ...
            // multiple components. Make sure that we only handle events for this component.
            return;
        }
